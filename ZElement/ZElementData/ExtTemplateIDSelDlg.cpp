...
// 	}
	unsigned long ulmaxID = g_ExtBaseIDMan.GetMaxID();
	AString path;
 	m_listExt.InsertString(0,"(无)");
	int index = 1;
	for(int i = 1; i<= ulmaxID; ++i)
	{
		if ((path = g_ExtBaseIDMan.GetPathByID(i)) != "")
		{
			CString ID2Path;

			ID2Path.Format("[%d]%s",i,path);
			if(m_nExtID !=0 )
			{
				if(i ==m_nExtID) sel = index;
			}
			m_listExt.InsertString(index, ID2Path);
...
void CALLBACK SEL_EXT_FUNCS::OnSetValue(const AVariant& var)
{
	m_var = var;
	int id = int(m_var);
	if(id==-1) m_strText = "(错误)";
	else if(id==0) m_strText = "(无)";
	else 
	{
