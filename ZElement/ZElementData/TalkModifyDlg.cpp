...
void CTalkModifyDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_OVERSEAS_TALK_ID, m_ulOverseasTalkID);
	//}}AFX_DATA_MAP
}
...
BOOL CTalkModifyDlg::OnInitDialog() 
{
	...
	//添加与海外ID预留相关的控制代码，使得Talk ID输入框根据需要置灰 Added 2011-04-20
	GetDlgItem( IDC_OVERSEAS_TALK_ID_LABEL )->EnableWindow( g_bOverseasEnable );
	GetDlgItem( IDC_OVERSEAS_TALK_ID )->EnableWindow( g_bOverseasEnable );
	//Added end.

	if (init_talk)
	{
		UpdateData(false);
		//目前，由于Data编辑器和Task编辑器共用了这个对话框的类，而Task编辑器的对话内容写入到了
		//任务模板中，但Data编辑器则需要Talk ID，因此，对于任务中的对话编辑来说，并不需要Talk ID
		//而目前，只有在Task中才使用了init_talk，因此，根据这个变量来判断海外ID的版本中是否需要输入框
		GetDlgItem( IDC_OVERSEAS_TALK_ID_LABEL )->EnableWindow( FALSE );
		GetDlgItem( IDC_OVERSEAS_TALK_ID )->EnableWindow( FALSE );
	}
...
void CTalkModifyDlg::OnOK()
{
	...
	if (m_dwID == 0)
	{
		...
		while(1)
		{	
			//添加与海外ID相关的代码，如果是海外ID预留，则直接通过输入ID而不走程序自动生成ID Added 2011-04-20
			if ( g_bOverseasEnable )
			{
				//生成新的模板名称
				m_strTalkName.Format( "Talk_%d", m_ulOverseasTalkID + k );
				CString strPath("TalkData\\");
				strPath = strPath + m_strTalkName + ".talk";
				m_strPathName = strPath;
				m_dwID = g_TalkIDMan.CreateIDExt( m_ulOverseasTalkID, AString(m_strPathName) );
			}
			else
			{
