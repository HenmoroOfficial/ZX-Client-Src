...
{
	for( int i = 0; i < m_listChar.size(); ++i)
	{
...
CVectorString* CVectorString::Disorder()
{
	...
	for( int i = 0; i < n; ++i)
	{
		...
		for( int j = 0; j < len; ++j)
		{
			...
			{
				VECTOR_PATH* pNewPath = ProcessData(pOldPath->m_pPtData,pOldPath->m_nPtNum);
...
VECTOR_PATH* CVectorString::ProcessData(char *pOldData, int& len)
{
	VECTOR_PATH *pNewPath = new VECTOR_PATH;;
	char cTempArray[1024*100];
	int nNewLen = 0;
	for( int i = 0; i < len; ++i)
	{
		int x = pOldData[i] >> 4 & 0xf;
		int y = pOldData[i] & 0xf;
		...
		char ch = (char)x<<4 | (char)y;
		cTempArray[nNewLen++] = ch;
		
		if(rand() % 3 == 0) cTempArray[nNewLen++] = ch;
	}

	if(nNewLen > 0)
	{
		pNewPath->m_nPtNum = nNewLen;
		pNewPath->m_pPtData = new char[nNewLen];
		memcpy(pNewPath->m_pPtData,cTempArray,nNewLen);
	}else
	{
		pNewPath->m_nPtNum = 0;
		pNewPath->m_pPtData = NULL;
	}
