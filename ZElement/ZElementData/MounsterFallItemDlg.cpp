...
#include "Global.h" //Added 2011-08-24.
#include "Render.h"
...
BEGIN_MESSAGE_MAP(CMounsterFallItemDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_IMPORT_TXTFILE, OnImportTxtfile)
	ON_BN_CLICKED(IDC_EXPORT_TXTFILE, OnExportTxtfile)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
*@param: void
*@return: void
*@usage: 根据用户指定的文件路径，导入相应的数据配置文件，并更新当前掉落表的数据信息 Added 2011-08-23.
**/
void CMounsterFallItemDlg::OnImportTxtfile() 
{
	//获得文件整体路径和文件名
	CCommonFileName FileDlg(g_Render.GetA3DEngine(), AFILE_NOALLOWMULFILES, "");
...
/*	CFileDialog FileDlg(TRUE);
	...
	for( i = 0; i < 8; ++i)
	{
		...
		CString strFormat;
		strFormat.Format( "掉落%d个物品概率: ", i );
		strFormat += "%f";
		sscanf(line, strFormat, &fValue);
		m_pProperty->SetPropVal(i,fValue);
	}
	...
	for( i = 0; i < 256; ++i)
	{
		...
		int id = 0;
		CString strFormat;
		strFormat.Format( "掉落物品%d: ", i );
		strFormat += "%d";
		sscanf(line, strFormat, &id);
		m_pProperty->SetPropVal(index++,id);//id
		//
		m_pListIDSel[i]->SetValue(AVariant(id));
		...
		strFormat.Format( "物品%d掉落概率: ", i );
		strFormat += "%f";
		sscanf(line, strFormat, &fProbability);
		m_pProperty->SetPropVal(index++,fProbability);//per
	}
	if(m_pList && m_pProperty) m_pList->AttachDataObject(m_pProperty);
...
*@param: void
*@return: void
*@usage: 根据用户指定的文件路径，导出当前的掉落表数据信息到相应的配置文件中 Added 2011-08-23.
**/
void CMounsterFallItemDlg::OnExportTxtfile() 
{
	//弹出保存对话框，使得用户选择保存路径
	BROWSEINFO Info;
	...
	//获得完整路径
	strFileName += "\\";
	strFileName += m_pItem->GetItemName();
	strFileName += ".txt";
	...
	for( i = 0; i < 8; ++i)
	{
		float fValue = m_pProperty->GetPropVal(i);
		fprintf(fp, "掉落%d个物品概率: %f", i, fValue);
		fprintf(fp, "\r\n");
	}

	for( i = 0; i < 256; ++i)
	{
		int id = m_pProperty->GetPropVal(index++);//id
		float fProbability = m_pProperty->GetPropVal(index++);//per
		fprintf(fp, "掉落物品%d: %d", i, id);
		fprintf(fp, "\r\n");
		
		fprintf(fp, "物品%d掉落概率: %f", i, fProbability);
		fprintf(fp, "\r\n");
	}	
