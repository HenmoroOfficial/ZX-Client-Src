...
protected:
	MAIN_TYPE* FindMainType( int id);
	void ReplaceSubTextName( int id, const char *szName);
	void ReplaceMainTextName( int id, const char *szName);
	bool ObjIsEqual(TREASURE_ITEM_VERSION11 *pObj1, TREASURE_ITEM_VERSION11 *pObj2);
	void UpdateMenu(CMenu *pMenu);
	int  GetChildItemsCount(HTREEITEM hParent);

	HTREEITEM CopyItem(HTREEITEM hItem, HTREEITEM htiNewParent, HTREEITEM htiAfter);
	void EnumTree(HTREEITEM hParent, abase::vector<TREASURE_ITEM_VERSION11*>* pList);
	void EnumNode(HTREEITEM hParent,HTREEITEM hNewParent);
	...
	afx_msg void OnMenuAddmain();
	afx_msg void OnMenuDelmain();
	afx_msg void OnTimer(UINT nIDEvent);
	afx_msg void OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult);
	afx_msg void OnMouseMove(UINT nFlags, CPoint point);
	afx_msg void OnLButtonDown(UINT nFlags, CPoint point);
	afx_msg void OnLButtonUp(UINT nFlags, CPoint point);
	afx_msg void OnMenuDoffTreasure();
