...
const char*	g_szVersion = "3.1.0.7"; 

unsigned long g_ulVersionCurrent = ELEMENTDATA_VERSION;
unsigned long g_ulVersion = ELEMENTDATA_VERSION;
unsigned long g_ulVersionTime = 0;		// edit_version.ini上次修改日期返回FILETIME中的高位，判断文件是否在分版本后修改过
//unsigned long g_ulVersionTime = 0;	// edit_version.ini上次修改日期返回FILETIME中的底位，如果出问题调试用
...
int TemplVersionState(AString tmpl)
{
	AString path(tmpl);
	...
	return pIDMan->VersionState(tmpl);
}
