...
const AVariant& BaseDataOrg::GetItemDefValue(int nIndex) const
{
	return m_ItemArray[nIndex]->m_defVal;
}
...
int BaseDataOrg::Load(const char* szPath)
{
	...
	for (int i = 0; i < nItemNum; i++)
	{
		...
		AVariant defValue;
		defValue.Load(&file);
		if( defValue.GetType() == AVariant::AVT_INVALIDTYPE )
