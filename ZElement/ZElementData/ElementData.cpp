...
BEGIN_MESSAGE_MAP(CElementDataApp, CWinApp)
	...
	ON_COMMAND(ID_UPDATE_LOG, OnUpdateLog)
	//}}AFX_MSG_MAP
...
static const char* _format_org		= "%x=%s";
...
/*	//显示对话框，请求用户是否启用海外预留ID版本 Added 2011-04-19.
	if ( IDYES == AfxMessageBox( "是否启用海外ID预留版本？", MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2 ) )
	{
		g_bOverseasEnable = true;
	}
	else//不在编辑器运行过程中支持海外和国内版本的切换，而是在程序启动时通过读取VSS版本标示确定Canceled 2011-05-19
*/	{
		g_bOverseasEnable = false;
	}
	//Added end.
	...
	char szPath[256];
	char szVersionName[MAX_PATH];
	sprintf(szPath,"%sedit_version.ini",g_szWorkDir);	
	
	if (g_ulVersion==g_ulVersionCurrent)
		GetPrivateProfileString("Editor Version", "OldVersionName", "Old Version", szVersionName, MAX_PATH, szPath);
	else
		GetPrivateProfileString("Editor Version", "NewVersionName", "New Version", szVersionName, MAX_PATH, szPath);

	CString titleWithVersion;
	titleWithVersion.Format("%s-%s", titile, szVersionName);

	//Modified end.
	pMainFrame->SetWindowText(titleWithVersion);
	pMainFrame->SetTitle(titleWithVersion);
...
BOOL RefreshBaseDataSettings()
{
	...
	if (!fp) 
	{
		...
		if(IDYES == AfxMessageBox(err_msg,MB_ICONQUESTION|MB_YESNO))
		{
			...
			err_msg = g_VSS.GetLastCommandResult();
			if(err_msg.GetLength() > 1024) err_msg = err_msg.Right(1023);
			g_Log.Log("+++++++++++++++++++++++++++++++++%s\r\n",err_msg);
	...
	while (ReadLine(fp, line) && strlen(line) != 0)
	{
		...
		if (strstr(line, "="))
		{
			sscanf(line, _format_org, &nVersion, path);
			_org_versions[AString(path)] = nVersion;
		}
		else
			sscanf(line, "%s", path);
		while (true)
		{
			CString strOrg;
			strOrg.Format("%s\\%d.org", path, nLevel + 1);
			if (GetFileAttributes(strOrg) == (DWORD)-1)
				break;
			nLevel++;
		}
		_org_paths[AString(path)] = nLevel;
	}
	fclose(fp);
	...
	g_PathIDMan.Release();
	if( g_PathIDMan.Load("BaseData\\PathData.dat") !=0)
	{
		AfxMessageBox("打开PathData.dat文件失败", MB_ICONSTOP);
		return FALSE;
	}
...
void CElementDataApp::OnUpdateLog()
{
	UpdateLogDlg logDlg;
	logDlg.UpdateLog();
	logDlg.DoModal();
...
void CElementDataApp::CloseAllDoc()
{
	// TODO: Add your command handler code here
	...
	if(bModifed)
	{
		if(AfxMessageBox("你是否要保存所有修改过的问文档？",MB_YESNO)==IDYES)
			OnSaveAllDoc(); 
	}
...
BOOL CElementDataApp::PreTranslateMessage(MSG* pMsg) 
{
	...
	if (m_hAccel != NULL)
	{
		if (TranslateAccelerator(this->m_pMainWnd->m_hWnd, m_hAccel, pMsg))
			return TRUE;
	}
...
bool CElementDataApp::GetProjectName()
{
	...
	//在VSS取得文件s_d_project.ini文件之前，先强行设置该文件为只读 Added 2011-05-23
	SetFileAttributes( "s_d_project.ini", FILE_ATTRIBUTE_READONLY );
	g_VSS.GetFile("s_d_project.ini");
	SetFileAttributes( "edit_version.ini", FILE_ATTRIBUTE_READONLY );
	g_VSS.GetFile("edit_version.ini");

	// 每次版本合并需要修改这个为上次分版本时间
	g_ulVersionTime = g_VSS.GetTime(13, 7, 17, 14, 26);

	// 获取需要的*.org或者其它文件，每次修改完模板修改这部分需要重新获取的文件
	static const CString filesToGet[] = {
		
		"BaseData\\Config\\染色剂初始配置表\\1.org",
		"BaseData\\Config\\染色剂初始配置表\\0.org",
		"BaseData\\EnumTypes.type",
		"BaseData\\OrgData.cfg", 
		"",
	};
	int i=0;
	int iSize = sizeof(filesToGet)/sizeof(CString);
	while(i<iSize)
	{
		if (strlen(filesToGet[i])==0)
			break;
		int index = filesToGet[i].ReverseFind('\\');
		if (index>0)
			g_VSS.SetWorkFolder(str+filesToGet[i].Left(index));
		else
			g_VSS.SetWorkFolder(str);
		if (filesToGet[i].Find('.')>=0)
			g_VSS.GetFile(filesToGet[i]);
		else
		{
			CreateDirectory(filesToGet[i],NULL);
		}
		i++;
	}
	...
	sprintf(szPath,"%sedit_version.ini",g_szWorkDir);
	g_ulVersionCurrent = GetPrivateProfileInt("Editor Version", "DataSeparatorVersion", 0, szPath);
	
	if (g_ulVersion<g_ulVersionCurrent)
	{
		AfxMessageBox("编辑器版本过期，请更新编辑器!");
		return false;
		//::ExitProcess(0);
	}

	
	//添加与海外版本相关的控制信息，以VSS服务器的标示信息来表示石否为海外版本。Added 2011-05-19
	int	nOverseasTag = -1;//0表示国内版本，1表示海外版本
	nOverseasTag = GetPrivateProfileInt( "Data Project", "Overseas", nOverseasTag, szPath );
	if ( g_bLinkVss ) //在VSS连接的情况下，检查VSS中的Overseas表示变量
	{
		if ( 0 == nOverseasTag )
		{
			g_bOverseasEnable = false;
		}
		else if( 1 == nOverseasTag )
		{
			g_bOverseasEnable = true;
		}
	}
