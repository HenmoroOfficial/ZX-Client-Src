...
bool CELBitmap::LockRect(const ARectI& rc, LOCKINFO* pLockInfo)
{
	...
	//	return false;
	ARectI realRect(rc);
	a_Clamp(realRect.left,0,m_iWidth);
	a_Clamp(realRect.right,0,m_iWidth);
	a_Clamp(realRect.top,0,m_iHeight);
	a_Clamp(realRect.bottom,0,m_iHeight);

	pLockInfo->iWidth	= realRect.Width();
	pLockInfo->iHeight	= realRect.Height();
	pLockInfo->iPitch	= m_iPitch;
	pLockInfo->pData	= m_pBmpData + realRect.top * m_iPitch + realRect.left * m_iBPP / 8;
