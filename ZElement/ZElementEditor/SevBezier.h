...
public:
	...
	void SetRatio( float fRatio ){ m_fFov = fRatio; }
	void SetSpeed( float fSpeed ){ m_fFov = fSpeed; }
...
public:
	...
	bool IsPathmapAvail(){ return m_bPassmapAvail; }
...
protected:
	...
	int				m_nNumPoint;
	int				m_nNumSeg;
	bool			m_bPassmapAvail;	//  是否使用通过图.如果为TRUE表示正常使用通过图，否则不使用通过图完全沿路径走
