...
void CTerrainModifySmoothPanel::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_CHECK_ABS_HEIGHT, m_checkAbsHeight);
}
...
BEGIN_MESSAGE_MAP(CTerrainModifySmoothPanel, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_ABS_HEIGHT, OnBnClickedCheckAbsHeight)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CTerrainModifySmoothPanel::OnInitDialog() 
{
	...
	if(pOperation)
	{
		...
		m_checkAbsHeight.SetCheck((pOperation)->GetIsAbsHeight());
	}
...
void CTerrainModifySmoothPanel::OnBnClickedCheckAbsHeight()
{
	// TODO: Add your control notification handler code here
	CTerrainSmoothOperation *pOperation = (CTerrainSmoothOperation *)g_OperationContainer.GetOperation(OPERATION_TERRAIN_SMOOTH);
	if(pOperation)
	{
		CButton *pButton = (CButton*)GetDlgItem(IDC_CHECK_ABS_HEIGHT);
		if(pOperation->GetIsAbsHeight())
			pOperation->AbsHeight(false);
		else pOperation->AbsHeight(true);
	}
}
