...
void CSceneObjectsList::FreshObjectList()
{
	...
	if(pMap)
	{
		...
		TVINSERTSTRUCT tvBuffRegion;
		TVINSERTSTRUCT tvGuildBuilding;
		...
		tvBuffRegion.hParent = NULL;
		tvBuffRegion.hInsertAfter = NULL;
		tvBuffRegion.item.mask = TVIF_TEXT;
		tvBuffRegion.item.pszText = _T("Buff区域");
		HTREEITEM hBuffRegion = pCtrl->InsertItem(&tvBuffRegion);

		tvGuildBuilding.hParent = NULL;
		tvGuildBuilding.hInsertAfter = NULL;
		tvGuildBuilding.item.mask = TVIF_TEXT;
		tvGuildBuilding.item.pszText = _T("帮派地块");
		HTREEITEM hGuildBuilding = pCtrl->InsertItem(&tvGuildBuilding);
		...
		pCtrl->SetItemImage(hBuffRegion,1,0);
		pCtrl->SetItemImage(hGuildBuilding,1,0);
		...
		while(pos)
		{
			...
			}else if(ptemp->GetObjectType()==CSceneObject::SO_TYPE_DYNAMIC)
			{
				...
				pCtrl->SetItemImage(hNew,2,2);
			}else if(ptemp->GetObjectType()==CSceneObject::SO_TYPE_BUFF_REGION)
			{
				HTREEITEM hNew = pCtrl->InsertItem(ptemp->GetObjectName(), hBuffRegion, TVI_SORT);
				pCtrl->SetItemImage(hNew,2,2);
			}else if(ptemp->GetObjectType()==CSceneObject::SO_TYPE_GUILD_BUILDING)
			{
				HTREEITEM hNew = pCtrl->InsertItem(ptemp->GetObjectName(), hGuildBuilding, TVI_SORT);
				pCtrl->SetItemImage(hNew,2,2);
			}
		}
	}
