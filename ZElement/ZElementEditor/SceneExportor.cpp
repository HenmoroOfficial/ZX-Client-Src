...
bool CSceneExportor::ExportSceneData()
{
	...
	//  Export brightness data for engine
	sprintf(szFile, "%s\\%s.env", m_strDestDir, m_strName);
	FILE *pfBrightness = fopen(szFile,"wb+");
	if(pfBrightness)
	{
		__GLOBALDATA dat = AUX_GetGlobalData();
		DWORD dwVersion = 1;
		fwrite(&dwVersion,sizeof(dwVersion),1,pfBrightness);
		fwrite(&dat.fBrightness, sizeof(float), 1, pfBrightness);
		fwrite(&dat.fBrightness_n, sizeof(float), 1, pfBrightness);
		fclose(pfBrightness);
	}
	...
	//	Create block share data file
	sprintf(szFile, "%s\\%s.cbezier", m_strDestDir, m_strName);
	if (!m_CtsBezierFile.Open(szFile, AFILE_CREATENEW))
	{
		AUX_ProgressLog("CSceneExportor::ExportSceneData, Failed to create cutscene bezier file %s", szFile);
		return false;
	}
...
bool CSceneExportor::ExportProject(int iProject)
{
	...
	DWORD dwWrite;
#ifdef _ANGELICA22
	//  save post effect in bsd-file.
	m_dwProjPostEftPos = m_BlkShareFile.GetPos();
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.FLAGS_ENABLE, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_ENABLE_FLAGS), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_DOF, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_DOF_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_BLOOM_DAY, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_BLOOM_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_FULLGLOW_DAY, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_FULLGLOW_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_BLOOM_NIGHT, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_BLOOM_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_FULLGLOW_NIGHT, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_FULLGLOW_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_UNITED_WARP, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_UNITED_WARP_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_UNITED_WARP_EX1.bEnableMask, sizeof(bool), &dwWrite);
	m_BlkShareFile.WriteString(Map.m_ExpPostEffectParam.PARAM_UNITED_WARP_EX1.strMaskTextureName);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_CC_DAY, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_CC_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_CC_NIGHT, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_CC_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_CC_MANUAL1, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_CC_MANUAL_PARAM), &dwWrite);
	m_BlkShareFile.Write(&Map.m_ExpPostEffectParam.PARAM_CC_MANUAL2, sizeof(EDITOR_DATA_POST_EFFECT_PARAM::EDITOR_DATA_POST_EFFECT_CC_MANUAL_PARAM), &dwWrite);
	m_BlkShareFile.WriteString(Map.m_ExpPostEffectParam.PARAM_CC_EX1_DAY.szTex1);
	m_BlkShareFile.WriteString(Map.m_ExpPostEffectParam.PARAM_CC_EX1_DAY.szTex2);
	m_BlkShareFile.WriteString(Map.m_ExpPostEffectParam.PARAM_CC_EX1_NIGHT.szTex1);
	m_BlkShareFile.WriteString(Map.m_ExpPostEffectParam.PARAM_CC_EX1_NIGHT.szTex2);
#endif
	...
	while (pos)
	{
		...
		A3DAABB aabb = pWater->GetAABB();
		ARectF rc;
		rc.left = aabb.Mins.x;
		rc.right = aabb.Maxs.x;
		rc.top = aabb.Mins.z;
		rc.bottom = aabb.Maxs.z;
		waterRectList.push_back(rc);
	}
	int ws = waterRectList.size();
	ws = ws - 1;
	for( int i = 0; i < ws; ++i)
	{
		ARectF rc1 = waterRectList[i];
		ARectF rc2 = waterRectList[i+1];

		if(rc1.left >= rc2.right) continue;
		if(rc1.right <= rc2.left) continue;
		if(rc1.top >= rc2.bottom) continue;
		if(rc1.bottom <= rc2.top) continue;

		CString strEMSG;
		strEMSG.Format("地图 %s 中有水块互相交迭，请修改水块位置后重新输出!",Map.GetMapName());
		AfxMessageBox(strEMSG,MB_ICONSTOP);
		Map.Release();
		return false;
	}
	...
	if (m_bProjOnmts)
	{
		...
		sprintf(szTempPath, "%s\\%s\\lightmap", m_strBuildingDir, m_strCurProjName);
		AUX_CreateDirectory(szTempPath);
	}
...
bool CSceneExportor::ExportProjectBlock(CExpElementMap* pMap, int iWdRow, int iWdCol, int iRow, int iCol)
{
	...
	BlockData.iPostEftOffset = m_dwProjPostEftPos;
...
void CSceneExportor::EmitBlockGrasses(CExpElementMap* pMap, const ARectF& rcBlock)
{
	...
	{
#ifdef _ANGELICA22
		A3DGrassType* pGrassType = pGrassland->GetGrassType(i);
#else
		CELGrassType* pGrassType = pGrassland->GetGrassType(i);
#endif // #ifdef _ANGELICA22
		...
		{
#ifdef _ANGELICA22
			A3DGrassBitsMap* pBitsMap = pGrassType->GetGrassBitsMapByIndex(j);
#else
			CELGrassBitsMap* pBitsMap = pGrassType->GetGrassBitsMapByIndex(j);
#endif // #ifdef _ANGELICA22
...
#ifdef _ANGELICA22
				A3DWaterArea2* pArea = pTerrainWater->GetWaterAreaByID(pWater->GetObjectID());
#else
	...
	while (pos)
	{
		...
		if (pObject->IsIntersectionWithArea(vMins, vMaxs))
		{
			...
			else
			{
				...
				//  Copy lightmap to export direction
				//if(g_bGenerateLightmap)
				{
					//g_Log.Log("id:%d scene:%s dl:%s nl:%s", pObject->GetObjectID(), pObject->GetModelPath(), pBuilding->GetA3DLitModel()->GetLightMapName(), pBuilding->GetA3DLitModel()->GetNightLightMapName());
					if(pBuilding->GetA3DLitModel()->GetLightMapName())
					{
						CString tile;
						strTemp = pBuilding->GetA3DLitModel()->GetLightMapName();
						int ps = strTemp.ReverseFind('\\');
						if(ps != -1)
						{
							tile = strTemp.Right(strTemp.GetLength() - ps - 1);
							strFile.Format("%s\\%s\\lightmap\\%s", m_strBuildingDir, m_strCurProjName,tile);
							CopyFile(strTemp,strFile,false);
							strFile.Format("LitModels\\%s\\%s\\lightmap\\%s",m_InitData.szName,m_strCurProjName,tile);
							pBuilding->GetA3DLitModel()->SetLightMap(strFile);
						}
					}
				}

				{
					if(pBuilding->GetA3DLitModel()->GetNightLightMapName())
					{
						CString tile;
						strTemp = pBuilding->GetA3DLitModel()->GetNightLightMapName();
						/*int iFind = strTemp.Find("1RenderD");
						if (iFind>=0)
						{
							strTemp[iFind+strlen("1Render")] = 'N';
						}*/
						int ps = strTemp.ReverseFind('\\');
						if(ps != -1)
						{
							...
							CopyFile(strTemp,strFile,false);
							strFile.Format("LitModels\\%s\\%s\\lightmap\\%s",m_InitData.szName,m_strCurProjName,tile);
							pBuilding->GetA3DLitModel()->SetNightLightMap(strFile);
						}
					}
				}
				...
				g_Log.Log("export building %d", pObject->GetObjectID());
#endif
				//	Save lit model file
				pBuilding->ExportBrushBuilding(&File);
				File.Close();

				ECBSDFILEORNAMENT13 Data;
				Data.vPos[0]	= v.x;
				Data.vPos[1]	= v.y;
				Data.vPos[2]	= v.z;
				...
				Data.bOptimize	= pObject->GetOptimizeFlag();
	...
	while (pos)
	{
		...
		if (pArea->IsIntersectionWithArea(vMins, vMaxs))
		{
			...
			else
			{
				...
				Data.dwFogColor		= Prop.m_dwFogColor;	
				...
				Data.dwFogColor_n	= Prop.m_dwFogColorNight;	
				...
				Data.tmBorn.iYear		= Prop.m_StartTime.nYear;
				Data.tmBorn.iMouth		= Prop.m_StartTime.nMouth;
				Data.tmBorn.iDay		= Prop.m_StartTime.nDay;
				Data.tmBorn.iWeek		= Prop.m_StartTime.nWeek;
				Data.tmBorn.iHour		= Prop.m_StartTime.nHours;
				Data.tmBorn.iMinute	= Prop.m_StartTime.nMinutes;

				Data.tmDie.iYear		= Prop.m_EndTime.nYear;
				Data.tmDie.iMouth		= Prop.m_EndTime.nMouth;
				Data.tmDie.iDay			= Prop.m_EndTime.nDay;
				Data.tmDie.iWeek		= Prop.m_EndTime.nWeek;
				Data.tmDie.iHour		= Prop.m_EndTime.nHours;
				Data.tmDie.iMinute		= Prop.m_EndTime.nMinutes;

				Data.fCapRotSpeed		= Prop.m_fCapRot/180.0f * A3D_PI;
				Data.fCapTileSpeed		= Prop.m_fCapTile;

				Data.nPriority			= Prop.m_nPriority;

				Data.fGlbFogHeightStart = Prop.m_fFogHeightStart;
				Data.fGlbFogHeightEnd = Prop.m_fFogHeightEnd;
				Data.fGlbFogDensityEnd = Prop.m_fFogDensityEnd;
				Data.fGlbFogDensityStart = Prop.m_fFogColorDensityStart;
				Data.fGlbFogHeightStartNight = Prop.m_fFogHeightStartNight;
				Data.fGlbFogHeightEndNight = Prop.m_fFogHeightEndNight;
				Data.fGlbFogDensityStartNight = Prop.m_fFogDensityStartNight;
				Data.fGlbFogDensityEndNight = Prop.m_fFogDensityEndNight;

				Data.dwFogColor2 = Prop.m_dwFogColor2;
				Data.fFogColorDensityStart = Prop.m_fFogColorDensityStart;
				Data.fFogColorDensityEnd = Prop.m_fFogColorDensityEnd;
				Data.fFogHorzeDensity = Prop.m_fFogHorzeDensity;
				Data.fFogHorzePower = Prop.m_fFogHorzePower;

				Data.dwFogColor2Night = Prop.m_dwFogColor2Night;
				Data.fFogColorDensityStartNight = Prop.m_fFogColorDensityStartNight;
				Data.fFogColorDensityEndNight = Prop.m_fFogColorDensityEndNight;
				Data.fFogHorzeDensityNight = Prop.m_fFogHorzeDensityNight;
				Data.fFogHorzePowerNight = Prop.m_fFogHorzePowerNight;

				Data.dwAmbient2 =pMap->GetLightingParams().dwAmbient2;
				Data.dwAmbient2_n = pMap->GetNightLightingParams().dwAmbient2;
				...
				//	Save skybox files
				m_BlkShareFile.Write(&Prop.m_nBoxAreaType, sizeof(int), &dwWrite);

				m_BlkShareFile.WriteString(Prop.m_strSkyTop);
				m_BlkShareFile.WriteString(Prop.m_strSkyBottom);
				m_BlkShareFile.WriteString(Prop.m_strSkyLeft);
				m_BlkShareFile.WriteString(Prop.m_strSkyRight);
				m_BlkShareFile.WriteString(Prop.m_strSkyFront);
				m_BlkShareFile.WriteString(Prop.m_strSkyBack);

				m_BlkShareFile.WriteString(Prop.m_strSkyNightTop);
				m_BlkShareFile.WriteString(Prop.m_strSkyNightBottom);
				m_BlkShareFile.WriteString(Prop.m_strSkyNightLeft);
				m_BlkShareFile.WriteString(Prop.m_strSkyNightRight);
				m_BlkShareFile.WriteString(Prop.m_strSkyNightFront);
				m_BlkShareFile.WriteString(Prop.m_strSkyNightBack);

				m_BlkShareFile.WriteString(Prop.m_strColorMappingTex);
			}
...
void CSceneExportor::EmitBlockBeziers(CExpElementMap* pMap, const ARectF& rcBlock)
{
	...
	while (pos)
	{
		...
		if (pBezier->IsIntersectionWithArea(vMins, vMaxs))
		{
			...
			//	This bezier curve has been exported ?
			...
			else
			{
				...
				if(pBezier->GetBezierType() != 0)
					pBezier->ExportBezier(&m_CtsBezierFile, vOffset);				
				else
