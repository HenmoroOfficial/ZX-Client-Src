...
{
	int nRadius = (int)g_Configs.iBrushRadius;
	...
	if(pMap)
	{
		...
		int NumX = static_cast<int>((updateRect.right - updateRect.left)/m_fTileSize);
		int NumZ = static_cast<int>((updateRect.top - updateRect.bottom)/m_fTileSize);
		...
		for(i=0; i<NumZ; i++)
		{
			...
			for(int n=0; n<NumX; n++)
			{
				...
				if (m_bAbsHeight || nCnt>0)
				{
					float weight = m_nSmoothFactory*dwTime*0.001f;

					if( GetAsyncKeyState(VK_F10) & 0x8000 )
						g_Log.Log("Test: ( %.2f - %.2f ) * %.2f",avgHeight,pHeightMap->pHeightData[i0],weight);

					if(weight < 0.0f) weight = 0.0f;
					if(weight > 1.0f) weight = 1.0f;
					pHeightMap->pHeightData[i0] += (avgHeight - pHeightMap->pHeightData[i0])*weight;
				}
				startX +=m_fTileSize;
			}
			startZ +=m_fTileSize;
