...
BOOL CResManDlg::OnInitDialog() 
{
	...
	m_listRes.InsertColumn(2, "原始名字", LVCFMT_LEFT, 500);
...
void CResManDlg::UpdateList()
{
	...
	for(int i = 0; i < nResCount; i++)
	{
		...
		m_listRes.SetItemText(nItem,1,pItem->szOrgName);
		nItem ++;
	}
}
...
void CResManDlg::OnButtonAddRes() 
{
	...
	if(FileDia.DoModal()==IDOK)
	{
		...
		while(pos)
		{
			...
			if(strFileName.Find("auto_sorts")==-1) 
			{
				MessageBox("路径中必须带有 “auto_sorts” 字样！");
...
void CResManDlg::OnButtonDelRes() 
{
	...
	else
	{
		...
		while (pos)
		{
			...
			CString temp = m_listRes.GetItemText(nItem,0);
			msg +=temp;
			msg +="\n";
		}
...
void CResManDlg::OnButtonReplaceRes() 
{
	...
	else
	{
		while (pos)
		{
			int nItem = m_listRes.GetNextSelectedItem(pos);
			CString sel = m_listRes.GetItemText(nItem,0);
			CString org = m_listRes.GetItemText(nItem,1);
			CString strPath,strName,strName2;
			ElementResMan::GetPathAndName(org,strPath,strName);
			char szFilter[256];
			SHORT s = GetKeyState(VK_RCONTROL);
			if(s >= 0) sprintf(szFilter, "Resorce Files (%s)|%s||",strName,strName);
			else sprintf(szFilter, "Resorce Files (*.mox)|*.mox||");
			DWORD dwFlags = OFN_FILEMUSTEXIST | OFN_EXPLORER | OFN_NOCHANGEDIR;
			CFileDialog	FileDia(TRUE, "*.*", NULL, dwFlags, szFilter, NULL);
			
			if(FileDia.DoModal()==IDOK)
			{
				CString strSrc = FileDia.GetPathName();
				ElementResMan::GetPathAndName(strSrc,strPath,strName2);
				strName2.MakeLower(); strName.MakeLower();
				if(strName2 == strName) ReplaceRes(ElementResMan::RES_TYPE_BUILDING,sel,strSrc);
				else MessageBox("名字不同的文件不能替换！");
			}
		}
		UpdateList();
		AUX_GetMainFrame()->GetToolTabWnd()->UpdateModelList();
...
void CResManDlg::ReplaceRes(int nResType, CString src,CString dest)
{
	if(!AUX_CheckOutResTable())
	{
	...
	CString strFileName = dest;
	strFileName.MakeLower();
	bool bResult = mResMan.ReplaceRes(nResType,src,strFileName);
	if(bResult)
	{
		mResMan.Save();
		if(!AUX_CheckInResTable())
