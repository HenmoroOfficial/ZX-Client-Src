...
bool CAIGenExportor::ExportControllers()
{
	...
	abase::hash_map<int,int> listCtrls;
	for (i=0; i < iNumCtrl; i++)
	{
		CONTROLLER_OBJECT src;
		g_ControllerObjectMan.GetObjByIndex(i, src);
		
		abase::hash_map<int, int>::iterator it = listCtrls.find(src.nControllerID);
		if(it != listCtrls.end())
		{
			CString msg;
			msg.Format("怪物控制器的控制ID重复！控制器ID=%d,控制ID=%d",src.id,src.nControllerID);
			AfxMessageBox(msg);
			g_Log.Log(msg);
		}


		NPCGENFILECTRL13 dst;
		dst.id					= src.id;
		...
		for(int j=0;j<dst.GetTimeSegNum();j++)
		{
			...
			time.iYear = src.aTimeSeg[2*j].nYear;
			time.iMouth = src.aTimeSeg[2*j].nMouth;
			time.iWeek = src.aTimeSeg[2*j].nWeek;
			time.iDay = src.aTimeSeg[2*j].nDay;
			time.iHours = src.aTimeSeg[2*j].nHours;
			time.iMinutes = src.aTimeSeg[2*j].nMinutes;
			
			dst.aTimeSeg.push_back(time);

			time.iYear = src.aTimeSeg[2*j+1].nYear;
			time.iMouth = src.aTimeSeg[2*j+1].nMouth;
			time.iWeek = src.aTimeSeg[2*j+1].nWeek;
			time.iDay = src.aTimeSeg[2*j+1].nDay;
			time.iHours = src.aTimeSeg[2*j+1].nHours;
			time.iMinutes = src.aTimeSeg[2*j+1].nMinutes;
		...
		listCtrls[dst.iControllerID] = dst.id;
		m_aExpCtrls.Add(dst);
	}
...
bool CAIGenExportor::ExportProject(int iProject)
{
	...
	dst.Format("%s\\map\\%s.hmap",m_InitData.szExpDir,Map.GetMapName(), strProject);
	FILE*  fhMap;
	if (fhMap = fopen(dst, "wb+"))
	{
		CTerrain::HEIGHTMAP* pHMap = Map.GetTerrain()->GetHeightMap();
		fwrite(pHMap->pHeightData, sizeof (float), pHMap->iHeight*pHMap->iWidth, fhMap);
		fclose(fhMap);
	}
	else
		AUX_ProgressLog("CAIGenExportor::ExportProject, Failed to open %s", dst);

	src.Format("%s%s\\%s\\%s.parmap", g_szWorkDir, g_szEditMapDir, strProject, strProject);
	...
	for(i = 0; i < 3; ++i)
	{
		src.Format("%s%s\\%s\\%s-%d.rmap", g_szWorkDir, g_szEditMapDir, strProject, strProject,i);
		dst.Format("%s\\movemap",m_InitData.szExpDir,Map.GetMapName());
		if(!AUX_CopyFile(dst,src)) 
			AUX_ProgressLog("CAIGenExportor::ExportProject, Failed to copy %s", src);
		
		src.Format("%s%s\\%s\\%s-%d.dhmap", g_szWorkDir, g_szEditMapDir, strProject, strProject,i);
		dst.Format("%s\\movemap",m_InitData.szExpDir,Map.GetMapName());
		if(!AUX_CopyFile(dst,src)) 
			AUX_ProgressLog("CAIGenExportor::ExportProject, Failed to copy %s", src);

		src.Format("%s%s\\%s\\%s-%d.prmap", g_szWorkDir, g_szEditMapDir, strProject, strProject,i);
		dst.Format("%s\\movemap",m_InitData.szExpDir,Map.GetMapName());
		if(!AUX_CopyFile(dst,src)) 
			AUX_ProgressLog("CAIGenExportor::ExportProject, Failed to copy %s", src);
		
		src.Format("%s%s\\%s\\%s-%d.pdhmap", g_szWorkDir, g_szEditMapDir, strProject, strProject,i);
		dst.Format("%s\\movemap",m_InitData.szExpDir,Map.GetMapName());
		if(!AUX_CopyFile(dst,src)) 
			AUX_ProgressLog("CAIGenExportor::ExportProject, Failed to copy %s", src);
	}
	...
	/*
	dst.Format("%s\\movemap\\%s.bht",m_InitData.szExpDir,Map.GetMapName());
	
	if(NULL != Map.GetBrushTree()->Check(rs))
	{
		AUX_ProgressLog("CAIGenExportor::ExportProject,faild to export brush tree data!\r\n%s",rs);
		goto Error;
	}
	Map.GetBrushTree()->Export(dst);
	*/
	...
	if (!ExportGuildBuildings(&Map))
	{
		AUX_ProgressLog("CAIGenExportor::ExportProject, Project %d failed to call ExportGuildBuildings() !",iProject);
		goto Error;
	}
...
bool CAIGenExportor::ExportProjectAIGens(CExpElementMap* pMap)
{
	...
	while (pos)
	{
		...
		if (pSceneObject->GetObjectType() == CSceneObject::SO_TYPE_AIAREA)
		{
			...
			if (!IsExpCopy(pAIGen->GetCopyFlags()) && pAIGen->GetCopyFlags() != -1)
				continue;

			AI_DATA Data = pAIGen->GetProperty();

			NPCAREA* pna = new NPCAREA;
			pna->iFirstGen = m_aExpNPCGens.GetSize();

			pna->Area.iNumGen		= (int)pAIGen->GetNPCNum();
			pna->Area.iNPCType		= CNPCGenMan::NPC_MONSTER;
			pna->Area.iGroupType	= Data.nMonsterType;
			pna->Area.bInitGen		= Data.bGenMounsterInit;
			pna->Area.cReviveType	= Data.iReviveType;
			pna->Area.bValidOnce	= Data.bValidAtOneTime;
			pna->Area.dwGenID		= Data.dwGenID;
			pna->Area.idCtrl		= Data.nControllerId;
			pna->Area.iLifeTime		= Data.nMounsterLife;
			pna->Area.iMaxNum		= Data.nMounsterNum;
			pna->Area.dwExportID	= pAIGen->GetExportID();
			pna->Area.iPhase		= 0;

			v = pAIGen->GetPos();
			pna->Area.vPos[0] = v.x + fSceneOffX;
			pna->Area.vPos[1] = v.y;
			pna->Area.vPos[2] = v.z + fSceneOffZ;
			...
			pna->Area.vExts[0] = Data.fEdgeLen;
			pna->Area.vExts[1] = Data.fEdgeHei;
			pna->Area.vExts[2] = Data.fEdgeWth;
		#ifdef _DEBUG
			g_Log.Log("%s",pSceneObject->GetObjectName());
			g_Log.Log("Pos(%4.2f,%4.2f,%4.2f)",pna->Area.vPos[0],pna->Area.vPos[1],pna->Area.vPos[2]);
			g_Log.Log("Exts(%4.2f,%4.2f,%4.2f)",pna->Area.vExts[0],pna->Area.vExts[1],pna->Area.vExts[2]);
		#endif
			...
			for (int i=0; i < pna->Area.iNumGen; i++)
			{
				...
				NPCDst.dwRefresh		= pSrc->dwRefurbish;
				...
				if (pBezier)
				{
					NPCDst.iPathID		= pBezier->GetGlobalID();
			...
			//	Emit attaching area
			if (pAIGen->IsExtraArea())
			{
				pna->Area.iAttachNum = -1;
			}
			else
			{
				int i, iNumAttach = pAIGen->GetExtraAreaNum();

				for (i=0; i < iNumAttach; i++)
				{
					DWORD id = pAIGen->GetExtraAreaID(i);
					CSceneAIGenerator* pTempObj = (CSceneAIGenerator*)pManager->FindAiAreaObject(id);
					if (pTempObj && (pTempObj->GetCopyFlags() == pAIGen->GetCopyFlags() || pTempObj->GetCopyFlags()==-1))
						pna->aAttachs.Add(pTempObj->GetExportID());
				}

				pna->Area.iAttachNum = pna->aAttachs.GetSize();
			}

			m_aExpNPCAreas.Add(pna);
		}
		else if (pSceneObject->GetObjectType() == CSceneObject::SO_TYPE_FIXED_NPC)
		{
			...
			pna->Area.iNumGen		= 1;
			// 历史原因
			int iNPCType = NPCData.cNpcInfo;
			if ( iNPCType==0 || iNPCType==1 )
			{
				iNPCType = 1-iNPCType;
			}
			pna->Area.iNPCType		= iNPCType; //bIsMounster ? CNPCGenMan::NPC_MONSTER : CNPCGenMan::NPC_SERVER;
			pna->Area.iGroupType	= 0;
			pna->Area.bInitGen		= NPCData.bGenMounsterInit;
			pna->Area.cReviveType	= NPCData.iReviveType;
			pna->Area.bValidOnce	= NPCData.bValidAtOneTime;
			pna->Area.dwGenID		= NPCData.dwGenID;
			pna->Area.idCtrl		= NPCData.nControllerId;
			pna->Area.iLifeTime		= NPCData.nMounsterLife;
			pna->Area.iMaxNum		= NPCData.nMounsterNum;
			pna->Area.dwExportID	= pAIGen->GetExportID();

			if( pna->Area.iNPCType==1 && pna->Area.cReviveType==2)   // 普通npc不能激活重生
			{
				CString msg;
				msg.Format("npc:%s 类型错误.\r\n,普通npc重生类型不能为2!",NPCData.strNpcName);
				AfxMessageBox(msg);
				return false;
			}
		...
		//	pna->Area.vExts[0] = NPCData.fRadius;
		//	pna->Area.vExts[1] = 0.0f;
		//	pna->Area.vExts[2] = NPCData.fRadius;
			pna->Area.vExts[0] = 0.0f;
			pna->Area.vExts[1] = 0.0f;
			pna->Area.vExts[2] = 0.0f;
			pna->Area.iPhase = NPCData.sPhase;

			NPCGENFILEAIGEN NPCDst;

			NPCDst.dwID				= NPCData.dwID;
			NPCDst.dwRefresh		= NPCData.dwRefurbish;
			NPCDst.dwNum			= 1;
			...
			else
			{
				...
				{
					DWORD id = pAIGen->GetExtraAreaID(i);
					CSceneFixedNpcGenerator* pTempObj = (CSceneFixedNpcGenerator*)pManager->FindNpcObject(id);
					if (pTempObj &&  (pTempObj->GetCopyFlags() == pAIGen->GetCopyFlags() || pTempObj->GetCopyFlags()==-1))
						pna->aAttachs.Add(pTempObj->GetExportID());
				}
		...
		else if (pSceneObject->GetObjectType() == CSceneObject::SO_TYPE_GATHER)
		{
			...
			if (!IsExpCopy(pGather->GetCopyFlags()) && pGather->GetCopyFlags() != -1)
				continue;

			GATHER_DATA Prop = pGather->GetProperty();

			RESAREA* pra = new RESAREA;
			pra->Area.fExtX			= Prop.fEdgeLen;
			pra->Area.fExtZ			= Prop.fEdgeWth;
			pra->Area.iNumRes		= pGather->GetGatherNum();
			pra->Area.bInitGen		= Prop.bGenGatherInit;
			pra->Area.bAutoRevive	= Prop.bGatherRenascenceAuto;
			pra->Area.bValidOnce	= Prop.bValidAtOneTime;
			pra->Area.dwGenID		= Prop.dwGenID;
			pra->Area.idCtrl		= Prop.nControllerId;
			pra->Area.iMaxNum		= Prop.nGatherNum;
			pra->Area.dwExportID	= pGather->GetExportID();

			v = pGather->GetPos();
			pra->Area.vPos[0] = v.x + fSceneOffX;
			pra->Area.vPos[1] = v.y;
			pra->Area.vPos[2] = v.z + fSceneOffZ;
			pra->Area.iPhase = Prop.sPhase;
			...
			vAxis.Normalize();
			a3d_CompressDir(vAxis, pra->Area.dir[0], pra->Area.dir[1]);
			pra->Area.rad = _RadianToChar(fAngle);

			pra->iFirstRes = m_aExpRes.GetSize();

			for (int i=0; i < pra->Area.iNumRes; i++)
			{
			...
			//	Emit attaching area
			if (pGather->IsExtraArea())
			{
				pra->Area.iAttachNum = -1;
			}
			else
			{
				int i, iNumAttach = pGather->GetExtraAreaNum();

				for (i=0; i < iNumAttach; i++)
				{
					DWORD id = pGather->GetExtraAreaID(i);
					CSceneGatherObject* pTempObj = (CSceneGatherObject*)pManager->FindGatherObject(id);
					if (pTempObj && (pTempObj->GetCopyFlags() == pGather->GetCopyFlags() || pTempObj->GetCopyFlags()==-1))
						pra->aAttachs.Add(pTempObj->GetExportID());
				}

				pra->Area.iAttachNum = pra->aAttachs.GetSize();
			}

			m_aExpResAreas.Add(pra);
		}
		else if (pSceneObject->GetObjectType() == CSceneObject::SO_TYPE_DYNAMIC)
		{
			...
			if(!IsExpCopy(pObject->GetCopyFlags()) && pObject->GetCopyFlags()!=-1) continue;
			NPCGENFILEDYNOBJ14 obj;
			obj.dwDynObjID		= pObject->GetDynamicObjectID();
			obj.idController	= pObject->GetControllerID();
			obj.iPhase			= pObject->GetPhase();
...
bool CAIGenExportor::ExportProjectPrecincts(CExpElementMap* pMap)
{
	...
	while (pos)
	{
		...
		if(!IsExpCopy(pPrecinct->GetCopyFlags()) && pPrecinct->GetCopyFlags() != -1) continue;
		...
		{
			AUX_ProgressLog("CAIGenExportor::ExportProjectPrecincts, 该区域相关联的曲线自相交,导出数据无效，请查看日志文件Logs\\ElementEditor.log更改后重新导出!", pPrecinct->GetObjectName());
		...
		if (Prop.dwPrecinctID)
		{
			...
			if(m_fpPCTF_s) fwrite(&Prop.nFeishengCnt, sizeof (Prop.nFeishengCnt), 1, m_fpPCTF_s);
			if(m_fpPCTF_s) fwrite(&Prop.nAllowLevelMax, sizeof (Prop.nAllowLevelMax), 1, m_fpPCTF_s);
			if(m_fpPCTF_s) fwrite(&Prop.nAllowLevelMin, sizeof (Prop.nAllowLevelMin), 1, m_fpPCTF_s);
			if(m_fpPCTF_s) fwrite(&Prop.nKickCity, sizeof (Prop.nKickCity), 1, m_fpPCTF_s);
			if(m_fpPCTF_s) fwrite(Prop.vKickCityPoint.m, sizeof (Prop.vKickCityPoint.m), 1, m_fpPCTF_s);
		...
		if(m_fpPCTF_c) fwprintf(m_fpPCTF_c, L"%d  %d  %d  %d  %d  %d  %d  %d  %d  %d\n", 
			Prop.dwPrecinctID, 
			iNumPoint, 
			pPrecinct->GetNpcNum(), 
			Prop.nPriority, 
		...
		if(m_fpPCTF_c) fwprintf(m_fpPCTF_c, L"%d  %d  %d  %d\n", 
			Prop.nFeishengCnt,
			Prop.nAllowLevelMax,
			Prop.nAllowLevelMin,
			Prop.nKickCity);

		if(m_fpPCTF_c) fwprintf(m_fpPCTF_c, L"%f, %f, %f\n", Prop.vKickCityPoint.x, Prop.vKickCityPoint.y, Prop.vKickCityPoint.z);
...
bool CAIGenExportor::ExportProjectRegions(CExpElementMap* pMap)
{
	...
	while (pos)
	{
		...
		if(!IsExpCopy(pRegion->GetCopyFlags()) && pRegion->GetCopyFlags()!=-1) continue;
		CEditerBezier* pBezier = pManager->FindBezier(pRegion->GetBezierID());
		if (!pBezier || pBezier->m_list_points.GetCount() < 3)
		{
			AUX_ProgressLog("CAIGenExportor::ExportProjectRegions, region %s isn't polygon area", pRegion->GetObjectName());
			continue;
		}

		if (pBezier->IntersectWithSelf())
		{
			AUX_ProgressLog("CAIGenExportor::ExportProjectRegions, 该区域相关联的曲线自相交,导出数据无效，请查看日志文件Logs\\ElementEditor.log更改后重新导出!");
			g_Log.Log("输出错误: 曲线自相交! 地图： %s, 对象：%s", pMap->GetMapName(), pBezier->GetObjectName());
			continue;
		}
	...
	// export buff region data (server version only)
	pos = pManager->m_listSceneObject.GetHeadPosition();
	while (pos)
	{
		CSceneObject* pSceneObject = pManager->m_listSceneObject.GetNext(pos);
		if (pSceneObject->GetObjectType() != CSceneObject::SO_TYPE_BUFF_REGION)
			continue;
		
		CSceneBuffRegion* pRegion = (CSceneBuffRegion*)pSceneObject;
		if(!IsExpCopy(pRegion->GetCopyFlags()) && pRegion->GetCopyFlags()!=-1) continue;
		CEditerBezier* pBezier = pManager->FindBezier(pRegion->GetProperty().nPathID);
		if (!pBezier || pBezier->m_list_points.GetCount() < 3)
		{
			AUX_ProgressLog("CAIGenExportor::ExportProjectRegions, buff region %s isn't polygon area", pRegion->GetObjectName());
			continue;
		}

		if (pBezier->IntersectWithSelf())
		{
			AUX_ProgressLog("CAIGenExportor::ExportProjectRegions, 该buff区域相关联的曲线自相交,导出数据无效，请查看日志文件Logs\\ElementEditor.log更改后重新导出!");
			g_Log.Log("输出错误: 曲线自相交! 地图： %s, 对象：%s", pMap->GetMapName(), pBezier->GetObjectName());
			continue;
		}

		//	------------ Save buff region data (server version)
		int iDataType = 2;
		if(m_fpRgn_s) fwrite(&iDataType, sizeof (iDataType), 1, m_fpRgn_s);

		if(m_fpRgn_s) fwrite(&pRegion->GetProperty().nRegionTemplID, sizeof (pRegion->GetProperty().nRegionTemplID), 1, m_fpRgn_s);
		if(m_fpRgn_s) fwrite(&pRegion->GetProperty().nMonsterID, sizeof (pRegion->GetProperty().nMonsterID), 1, m_fpRgn_s);
...
bool CAIGenExportor::ExportTransportBoxes(CExpElementMap* pMap)
{
	...
	while (pos)
	{
		...
		if(!IsExpCopy(pTransBox->GetCopyFlags()) && pTransBox->GetCopyFlags()!=-1) continue;
		const INSTANCE_DATA& Prop = pTransBox->GetProperty();

		CONTROLLER_OBJECT ctrlObj;
		ctrlObj.nControllerID = -1;
		g_ControllerObjectMan.GetObjByID(Prop.nControllerId,ctrlObj);

		/*
		if(ctrlObj.nControllerID == -1)
		{
			g_Log.Log("未找到合适的控制器 id=%d !",Prop.nControllerId);
			continue;
		}*/
		int nControllerID = ctrlObj.nControllerID;
		...
		if(m_fpRgn_s) fwrite(&Prop.nLine, 1, sizeof (Prop.nLine), m_fpRgn_s);
		if(m_fpRgn_s) fwrite(&nControllerID, 1, sizeof (nControllerID), m_fpRgn_s);
		...
		if(m_fpRgn_c) fwprintf(m_fpRgn_c, L"%d  %d  %d  %d  %d\n", Prop.nWorldId, Prop.nSelfWorldId, Prop.nLimitLevel,Prop.nLine,nControllerID);
...
bool CAIGenExportor::ExportGuildBuildings(CExpElementMap* pMap)
{
	float fSceneOffX = -m_fWorldWid * 0.5f + m_iProjColCnt * m_fProjTrnSize + m_fProjTrnSize * 0.5f;
	float fSceneOffZ = m_fWorldLen * 0.5f - m_iProjRowCnt * m_fProjTrnSize - m_fProjTrnSize * 0.5f;
	...
	{
		CSceneObject* pSceneObject = pManager->m_listSceneObject.GetNext(pos);
		if (pSceneObject->GetObjectType() != CSceneObject::SO_TYPE_GUILD_BUILDING)
			continue;
		
		CSceneGuildBuilding* pGuildBuilding = (CSceneGuildBuilding*)pSceneObject;
		if(!IsExpCopy(pGuildBuilding->GetCopyFlags()) && pGuildBuilding->GetCopyFlags()!=-1) continue;
		const EC_GUILDBUILDING_PROPERTY& Prop = pGuildBuilding->GetProperty();

		vPos = pGuildBuilding->GetPos();
		vDir = pGuildBuilding->GetRotateMatrix().GetRow(2);
		vUp = pGuildBuilding->GetRotateMatrix().GetRow(1);

		if(m_fpRgn_c) fwprintf(m_fpRgn_c, L"[GuildBuilding]\n");
		if(m_fpRgn_c) fwprintf(m_fpRgn_c, L"%d  %d  %d\n", Prop.iTmplId, Prop.iWidth, Prop.iHeight);
		if(m_fpRgn_c) fwprintf(m_fpRgn_c, L"%f, %f, %f\n", vPos.x, vPos.y, vPos.z);
...
bool CAIGenExportor::EndExportAIGenFile()
{
	if (m_fpAIGen==NULL)
		return true;
	...
	m_FileHeader.iNumNPCCtrl	= m_aExpCtrls.GetSize();
	...
	for (i=0; i < m_aExpNPCAreas.GetSize(); i++)
	{
		const NPCAREA* pna = m_aExpNPCAreas[i];
		fwrite(&pna->Area, 1, sizeof (pna->Area), m_fpAIGen);

		for (j=0; j < pna->Area.iNumGen; j++)
		{
			const NPCGENFILEAIGEN& ngen = m_aExpNPCGens[pna->iFirstGen+j];
			fwrite(&ngen, 1, sizeof (ngen), m_fpAIGen);
		}

		//	Write attaching area
		if (pna->aAttachs.GetSize())
			fwrite(pna->aAttachs.GetData(), 1, sizeof (int) * pna->aAttachs.GetSize(), m_fpAIGen);
	}

	//	Write resources data
	for (i=0; i < m_aExpResAreas.GetSize(); i++)
	{
		const RESAREA* pra = m_aExpResAreas[i];
		fwrite(&pra->Area, 1, sizeof (pra->Area), m_fpAIGen);

		for (j=0; j < pra->Area.iNumRes; j++)
		{
			const NPCGENFILERES& res = m_aExpRes[pra->iFirstRes+j];
			fwrite(&res, 1, sizeof (res), m_fpAIGen);
		}

		//	Write attaching area
		if (pra->aAttachs.GetSize())
			fwrite(pra->aAttachs.GetData(), 1, sizeof (int) * pra->aAttachs.GetSize(), m_fpAIGen);
	}

	//	Write dynamic object data
	for (i=0; i < m_aExpDynObjs.GetSize(); i++)
	{
		const NPCGENFILEDYNOBJ14& obj = m_aExpDynObjs[i];
	...
	{
		NPCGENFILECTRL13& ctrl = m_aExpCtrls[i];
		fwrite(&ctrl, 1, sizeof (NPCGENFILECTRL11), m_fpAIGen);

		fwrite(&ctrl.nTimeSegNum, 1, sizeof(int), m_fpAIGen);
		for(j=0;j<ctrl.GetTimeSegNum(); j++)
		{
			fwrite(&ctrl.aTimeSeg[2*j], 1, sizeof(NPCCTRLTIME), m_fpAIGen);
			fwrite(&ctrl.aTimeSeg[2*j+1], 1, sizeof(NPCCTRLTIME), m_fpAIGen);
		}
	}
