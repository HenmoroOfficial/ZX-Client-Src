		...
		while( pos )
		{
			...
			if(pManager->m_coordinateDirection.OnLButtonDown(x,y,dwFlags))
			{
				...
				if(m_nDirectFlag!=0)
				{
					...
					CPoint pt(x,y);
					float dot[3];
					ScreenTraceAxis(ptemp->GetPos(),pt,&m_vXY,&m_vXZ,&m_vYZ,dot);
...
void SceneObjectMultiSelectOperation::ScreenTraceAxis(A3DVECTOR3 &pos, CPoint pt, A3DVECTOR3 *pXY, A3DVECTOR3 *pXZ, A3DVECTOR3 *pYZ, float dot[3])
{
