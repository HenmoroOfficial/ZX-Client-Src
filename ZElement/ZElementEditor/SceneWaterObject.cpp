...
CSceneWaterObject::CSceneWaterObject(CSceneObjectManager* pSceneMan)
{
	...
	m_dwWaterColor = A3DCOLORRGB(255,255,255);
	//m_pTestData = NULL;
	BuildProperty();
}

#ifdef _ANGELICA22
CSceneWaterObject::CSceneWaterObject(const CSceneWaterObject& rhs)
: CSceneObject(rhs)
{
	m_nObjectType = SO_TYPE_WATER;

	m_pSceneMan = rhs.m_pSceneMan;
	m_nAreaX = rhs.m_nAreaX;
	m_nAreaZ = rhs.m_nAreaZ;
	m_nGridSize = rhs.m_nGridSize;
	m_fWaveSize = rhs.m_fWaveSize;
	m_dwColor = rhs.m_dwColor;
	m_nStartPointX = rhs.m_nStartPointX;
	m_nStartPointZ = rhs.m_nStartPointZ;
	m_nProfileWidth = rhs.m_nProfileWidth;
	m_nProfileHeight = rhs.m_nProfileHeight;
	m_bMinorWaterFlag = rhs.m_bMinorWaterFlag;
	m_fPixelValue = rhs.m_fPixelValue;
	//m_btWaterData;
	//m_listWaterProfile;
	m_bCamUnderWater = rhs.m_bCamUnderWater;
	m_dwWaterColor = rhs.m_dwWaterColor;
...
void CSceneWaterObject::BuildProperty()
{
	if(m_pProperty)
	{
		...
		m_pProperty->DynAddProperty(AVariant(m_nGridSize), "格子尺寸");
		m_pProperty->DynAddProperty(AVariant(m_fWaveSize), "水波尺寸");
		m_pProperty->DynAddProperty(AVariant(m_bMinorWaterFlag), "是否为次级水");
...
void CSceneWaterObject::DrawGrid(const A3DVECTOR3& vMin, const A3DVECTOR3& vMax)
{
...
void CSceneWaterObject::DrawLine(const WGRID& old, const WGRID& cur,DWORD clr)
{
...
void CSceneWaterObject::DrawLine2(const A3DVECTOR3& v1,const A3DVECTOR3& v2, DWORD clr)
{
	CElementMap *pMap = m_pSceneMan->GetMap();
	if(pMap)
	{
...
void CSceneWaterObject::PostRender(A3DViewport* pA3DViewport)
{
	if(m_bSelected)
	{
		DrawCenter();
	...
	if(bGet)
	{
		...
		if(pMap)
		{
			...
			if(pObj != NULL && pObj != this)
			{
				...
				m_pProperty->SetPropVal(4,m_nGridSize);
				m_pProperty->SetPropVal(5,m_fWaveSize);
				m_pProperty->SetPropVal(6,m_bMinorWaterFlag);
...
void CSceneWaterObject::SetWaterSize(int nWidth,int nHeight,int nGridSize)
{
	m_nAreaX = nWidth;
	m_nAreaZ = nHeight;
...
void CSceneWaterObject::BuildRenderWater()
{
	...
	if(pMap)
	{
...
#ifdef _ANGELICA22
		A3DWaterArea2 *pArea = pWater->GetWaterAreaByID(m_nObjectID);
#else
		A3DWaterArea *pArea = pWater->GetWaterAreaByID(m_nObjectID);
#endif // #ifdef _ANGELICA22
		if(pArea==NULL) return;
		pArea->SetMinorWaterFlag(m_bMinorWaterFlag);
		...
		//pArea->SetWaterColor(m_dwColor);//无用了
...
bool CSceneWaterObject::CreateRenderWater()
{
	...
	{
		CTerrainWater *pWater = pMap->GetTerrainWater();
		pWater->AddWaterArea(m_nObjectID,m_nAreaX,m_nAreaZ,m_nGridSize,m_vPos,m_vPos.y);
	}
...
bool CSceneWaterObject::PlaneIntersectionWithLine(const A3DVECTOR3& vStart,const A3DVECTOR3& vVec,const A3DVECTOR3& point,const A3DVECTOR3& normal,A3DVECTOR3 &out)
{
...
bool CSceneWaterObject::IsEdge(const WGRID& grid)
{
	CElementMap *pMap = m_pSceneMan->GetMap();
	CTerrain::HEIGHTMAP* pHeightMap = pMap->GetTerrain()->GetHeightMap();
	POINT ptSet[4] = { {0,0}, {1,0}, {1,-1}, {0,-1} };
...
bool CSceneWaterObject::IsOrder(const WGRID& oldGrid, const WGRID& w1, const WGRID& w2)
{
	WGRID delta1;
	WGRID delta2;
...
bool CSceneWaterObject::IsIntersectionWithWater(const A3DVECTOR3& pt1,const A3DVECTOR3& pt2)
{
...
CSceneObject* CSceneWaterObject::CopyObject()
{
	...
	if(pMap)
	{
		...
		pNewObject->SetWaterSize(m_nAreaX,m_nAreaZ,m_nGridSize);
		int WaterNum = 0;
...
{
	A3DAABB box;
	box.Mins = A3DVECTOR3(m_vPos.x - m_nAreaX * m_nGridSize/2,m_vPos.y,m_vPos.z - m_nAreaZ * m_nGridSize/2);
	box.Maxs = A3DVECTOR3(m_vPos.x + m_nAreaX * m_nGridSize/2,m_vPos.y,m_vPos.z + m_nAreaZ * m_nGridSize/2);
	return box;
}
