...
BOOL CNpcPropertyDlg::OnInitDialog() 
{
	...
	while (dwid)
	{
		...
		if (DataType == DT_MONSTER_ESSENCE)
		{
			...
			pData = (MONSTER_ESSENCE*)m_pDataMan->get_data_ptr(dwid,ID_SPACE_ESSENCE,DataType);
	...
	{
		CString str;
		//str.Format("%s->%d",m_npcData.strNpcName,m_npcData.dwID);
		...
		str.Format("%d",m_npcData.dwRefurbish);
...
void CNpcPropertyDlg::OnOK()
{
	...
	{	
		...
		m_npcData.dwRefurbish = (DWORD)atof(str);
