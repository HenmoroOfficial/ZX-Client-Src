...
void CSceneSpeciallyObject::PostRender(A3DViewport* pA3DViewport)
{
	DrawCenter();
...
void  CSceneSpeciallyObject::UpdateGfx()
{
	...
	{
		AString msg;
		msg.Format("CSceneSpeciallyObject::UpdateGfx(), faild to load %s in %s",m_strGfxPathName,m_strName);
		g_Log.Log(msg);
...
int CSceneSpeciallyObject::GetTriangleNum()
{
	if(m_pGfxEx)
	{
		UINT nTriangle = 0;
		int nEle  = m_pGfxEx->GetElementCount();
		for( int i = 0; i < nEle; i++)
		{
			A3DGFXElement *pEl = m_pGfxEx->GetElement(i);
			if(pEl==NULL) continue;
			A3DSkinModel *pModel = pEl->GetSkinModel();
			if(pModel==NULL) continue;
			int snum = pModel->GetSkinNum();
			for( int j = 0; j < snum; j++)
			{
				A3DSkin *pSkin = pModel->GetA3DSkin(j);
