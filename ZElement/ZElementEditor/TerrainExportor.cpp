...
bool CTerrainExportor::DoExport()
{
	...
	//这儿是特殊情况，只针对SDS这种提前把光算好了的游戏，如果没算好的，执行这个是多余的。
...
bool CTerrainExportor::SetLightMapProject(int iPos, int r, int c,bool bIsNight)
{
	...
	{
		if(bIsNight) pMap = (CExpElementMap*)m_InitData.pLMNightGen->GetCurrentMap();
		else pMap = (CExpElementMap*)m_InitData.pLMGen->GetCurrentMap();
	...
	else m_InitData.pLMGen->SetMap(iPos, pMap);
...
CExpElementMap * CElMapCache::GetMap(CString strProjName)
{
	for(int i = 0; i < 9; i++)
	{
...
void CElMapCache::AddProject(CString *strProjName)
{
...
CExpElementMap* CElMapCache::AddStartMap(CString strProjName)
{ 
	int i;
	for( i = 0; i < 9; i++)
	{
