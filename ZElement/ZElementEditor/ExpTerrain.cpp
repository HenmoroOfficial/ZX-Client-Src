...
#ifdef _ANGELICA22
bool CExpTerrain::ExpScene_LoadHeighMap(const TERRAINFILEDATA& Data)
#else
...
#ifdef _ANGELICA22
	if(!read_file_to_buffer(strFullFile,m_HeightMap.pHeightData,Data.iHMWidth*Data.iHMHeight*sizeof(float)))
	{
		g_Log.Log("Error,Failed to load height map,read %s file wrong!",strFullFile);
		return false;
	}
#else
...
#else
//	Create terrain mesh
bool CExpTerrain::CreateTerrainMesh()
{
