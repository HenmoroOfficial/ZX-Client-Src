...
bool CELCloudSceneRender::Init(A3DDevice *pA3DDevice,const char* strTextureFile)
{
	...
	WORD* pIndex;
	m_pStream->LockIndexBuffer(0,6*m_nStreamSprites*sizeof(WORD),(BYTE**)&pIndex,D3DLOCK_DISCARD);
...
void CELCloudSceneRender::ReFillStream_Line()
{
	...
	A3DLVERTEX* pVertex;
	if( !m_pStreamLine->LockVertexBuffer(0,nVertices*sizeof(A3DLVERTEX),(BYTE**)&pVertex,D3DLOCK_DISCARD))
		return ;
	
	WORD* pIndex;
	if( !m_pStreamLine->LockIndexBuffer(0,nIndex*sizeof(WORD),(BYTE**)&pIndex,D3DLOCK_DISCARD))
		return ;
