	...
	if(pMap)
	{
		...
		{//不需要在地图上绘制矩形
			...
			if(pNewAI)
			{
				...
				pNewAI->SetExtraArea(m_AI.IsExtraArea());
				//for undo
...
void CAIGeneratorOperation::DrawRect(A3DViewport *pViewport)
{
	...
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap && data.dwAreaType == 0) vCenter.y = pMap->GetTerrain()->GetPosHeight(vCenter) + 0.1f;
	...
	if(m_bDrawRect)
	{
		...
		if(pMap && len>0.0f && wdt>0.0f)
		{
			...
			if(pNew)
			{
				...
				pNew->SetExtraArea(m_Gather.IsExtraArea());
				//for undo
				CUndoSceneObjectAddAction *pUndo = new CUndoSceneObjectAddAction(pMap->GetSceneObjectMan());
				pUndo->GetSeletedListData();
				g_UndoMan.AddUndoAction(pUndo);
...
void CFixedNpcGeneratorOperation::TransObstructData()
{
	...
	for(int h = 0; h < OBS_MAP_WIDTH; h ++)
	{
		for( int w = 0; w < OBS_MAP_WIDTH; w ++)
		{
			int idx = h*OBS_MAP_WIDTH*4 + w*4;
			
			int idh= pMap->GetPassMapGen()->GetDeltaHeightMap()[h * OBS_MAP_WIDTH + w];
			
			if(pData[h * OBS_MAP_WIDTH + w]==0)
			{
				m_pData[idx + 0] = 255;
				m_pData[idx + 1] = 0;
				m_pData[idx + 2] = 0;
				m_pData[idx + 3] = 0;
			}else if(pData[h * OBS_MAP_WIDTH + w]==1)
			{
				if(idh)
				{
					m_pData[idx + 0] = 255;
...
bool CFixedNpcGeneratorOperation::OnLButtonDown(int x, int y, DWORD dwFlags)
{
	...
	if(pMap)
	{
		...
		if(pNewNpc)
		{
			...
			pNewNpc->SetExtraArea(m_Npc.IsExtraArea());
...
///////////////////////////////////////////////////////////////////////////////////////
//class CBuffRegionOperation

CBuffRegionOperation::CBuffRegionOperation()
{
}

CBuffRegionOperation::~CBuffRegionOperation()
{
	m_BuffRegion.Release();
}

bool CBuffRegionOperation::OnLButtonDown(int x, int y, DWORD dwFlags)
{	
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap)
	{
		...
		if(!pSManager->GetTracePos(vStart,vPos,TraceRt.vPoint)) return true;
		AUX_GetMainFrame()->GetToolTabWnd()->UpdatePropertyData(true);//读属性数据
		m_BuffRegion.UpdateProperty(true);
		CSceneBuffRegion* pNew = pSManager->CreateBuffRegion(m_BuffRegion.GetObjectName());
		if(pNew)
		{
			BUFF_REGION_DATA data = m_BuffRegion.GetProperty();
			pNew->Translate(TraceRt.vPoint);
			pNew->SetProperty(data);
			pNew->SetObjectID(pSManager->GenerateObjectID());
...
bool CBuffRegionOperation::OnLButtonUp(int x, int y, DWORD dwFlags)
{
	return true;
}

bool CBuffRegionOperation::OnMouseMove(int x, int y, DWORD dwFlags)
{
	return true;
}

void CBuffRegionOperation::CreateBuffRegion()
{
	int Num = 0;
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap)
	{
		...
		m_BuffRegion.SetObjectName(name);
		BUFF_REGION_DATA data;
		init_buff_region_data(data);
		m_BuffRegion.SetProperty(data);
		m_BuffRegion.UpdateProperty(false);
		AUX_GetMainFrame()->GetToolTabWnd()->ShowPropertyPannel(m_BuffRegion.m_pProperty);
	}
}

void CBuffRegionOperation::Render(A3DViewport* pA3DViewport)
{
}
