...
bool CLightMapGen::Init(const LIGHTINGPARAMS& Params,bool bOnlyRenderShadow)
{
	...
	A3DCOLORVALUE col2 = m_Params.dwAmbient2;
	col2 *= 0.5f;
	m_Params.dwAmbient2 = col2.ToRGBAColor();
...
bool CLightMapGen::CreateColorMap(int iMapSize)
{
	...
	if (!m_aColors || m_iMapSize != iMapSize)
	{
		...
		if (!(m_aColors = (A3DCOLORVALUE*)malloc(iSize)))
		{
...
bool CLightMapGen::DoGenerate(bool bNight,bool bFast)
{
	...
	if( !hwLightMapGen.Init(g_Render.GetA3DDevice(), (CExpTerrain *)pTerrain, pTerrainWater, pSceneObjectManager, 2048, m_Params.iMapSize, 1,m_bOnlyRenderShadow,bFast) )
	{
	...
	if( bCanHWLightGen )
	{
		...
		hwLightMapGen.SetWorldInfo(m_Params.dwAmbient,m_Params.dwAmbient2, m_Params.vSunDir, a3d_ColorRGBAToColorValue(m_Params.dwSunCol), m_Params.fSunPower, vecCenter, aabbWorld);
...
{
	A3DCOLORVALUE* aTempColMap = (A3DCOLORVALUE*)malloc(m_iMapSize * m_iMapSize * sizeof (A3DCOLORVALUE));
	if (!aTempColMap)
	{
