...
public:
	...
	void SelectedPoint(int ptid );
	int GetSelectedPoint() { return m_nCurPoint; }
	...
	CEditerBezierSegment* GetTailSeg();
	void ReleaseA3DObjects();
	void SetMap(int map);
	int  GetMap();

	CEditerBezier();
	CEditerBezier(int map, A3DDevice * pA3DDevice, float ptradius = 0.1f, float segradius = 0.05f, float anchorradius = 0.05f);
	...
	CEditerBezierPoint* GetPoint( DWORD id );
	...
	void               SetGlobalID( int id){ m_nGlobalID = id; }
	//判断区域是否跟该区域相交
	bool        IsIntersectionWithArea(const A3DVECTOR3& vMin, const A3DVECTOR3& vMax);
	void        ReCalculateLength();
	float       GetLength(){ return m_fLenght; }
	int         GetLoopType(){ return m_nLoopType; }
	int			GetBezierType() { return m_nBezierType; }
