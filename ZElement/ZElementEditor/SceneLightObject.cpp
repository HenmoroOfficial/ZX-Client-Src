...
void Init_Light_Data(LIGHTDATA &data)
{
    ...
    data.m_fAttenuation0 = 1.0f;
    data.m_fAttenuation1 = 4.0f;
    data.m_fAttenuation2 = 0.0f;
	data.m_bUsed2_2IlluminationEquation = false;
    data.m_fTheta = 30.0f;
    data.m_fPhi = 60.0f;	
	data.m_fColorScale = 5.0f;
	data.m_vDir = A3DVECTOR3(0.0f,0.0f,1.0f);
	data.m_nEffectType = 2;
	data.m_nEffectTypeRange = 0;
	data.m_bShadow = true;
}
...
CSceneLightObject::CSceneLightObject()
{
	...
	light_effect_type_range.AddElement("影响所有对象", 0);
	light_effect_type_range.AddElement("只影响动态对象", 1);
	light_effect_type_range.AddElement("只影响静态对象", 2);
...
void CSceneLightObject::SetObjectName(AString name)
{
	m_strName = name;
}
...
void CSceneLightObject::PostRender(A3DViewport* pA3DViewport)
{
	Draw();
...
void CSceneLightObject::BuildLightProperty()
{
	if(m_pProperty)
	{
...
#ifndef _ANGELICA22
		m_pProperty->DynAddProperty(AVariant(m_Data.m_bUsed2_2IlluminationEquation), "是否引擎2.2光照方程"
			,NULL,NULL,WAY_READONLY);
#else
		m_pProperty->DynAddProperty(AVariant(m_Data.m_bUsed2_2IlluminationEquation), "是否引擎2.2光照方程");
#endif
		...
		m_pProperty->DynAddProperty(AVariant(m_Data.m_nEffectTypeRange),"影响对象类型",&light_effect_type_range);
		m_pProperty->DynAddProperty(AVariant(m_Data.m_bShadow),"是否产生阴影",&light_effect_type_range);
	...
	if(bGet)
	{
		...
		if(pMap)
		{
			...
			if(pObj != NULL && pObj != this)
			{
				...
				m_pProperty->SetPropVal(2, m_Data.m_bUsed2_2IlluminationEquation );
				m_pProperty->SetPropVal(3,m_vPos);
				m_pProperty->SetPropVal(4,m_Data.m_vDir);
				m_pProperty->SetPropVal(5,m_Data.m_dwColor);
				m_pProperty->SetPropVal(6,m_Data.m_fColorScale);
				m_pProperty->SetPropVal(7,m_Data.m_fRange);
				m_pProperty->SetPropVal(8,m_Data.m_dwAbient);
				m_pProperty->SetPropVal(9,m_Data.m_fFalloff);
				m_pProperty->SetPropVal(10,m_Data.m_fAttenuation0);
				m_pProperty->SetPropVal(11,m_Data.m_fAttenuation1);
				m_pProperty->SetPropVal(12,m_Data.m_fAttenuation2);
				m_pProperty->SetPropVal(13,m_Data.m_fTheta);
				m_pProperty->SetPropVal(14,m_Data.m_fPhi);
				m_pProperty->SetPropVal(15,m_Data.m_nEffectType);
				m_pProperty->SetPropVal(16,m_Data.m_nEffectTypeRange);
				m_pProperty->SetPropVal(17,m_Data.m_bShadow);
		...
		m_Data.m_bUsed2_2IlluminationEquation = m_pProperty->GetPropVal(2);
		m_vPos	= m_pProperty->GetPropVal(3);
		m_Data.m_vDir  = m_pProperty->GetPropVal(4);
		
		m_Data.m_dwColor = m_pProperty->GetPropVal(5);
		m_Data.m_fColorScale = m_pProperty->GetPropVal(6);
		m_Data.m_fRange = m_pProperty->GetPropVal(7);
	
		m_Data.m_dwAbient = m_pProperty->GetPropVal(8);
		m_Data.m_fFalloff = m_pProperty->GetPropVal(9);
		m_Data.m_fAttenuation0 = m_pProperty->GetPropVal(10);
		m_Data.m_fAttenuation1 = m_pProperty->GetPropVal(11);
		m_Data.m_fAttenuation2 = m_pProperty->GetPropVal(12);
		m_Data.m_fTheta = m_pProperty->GetPropVal(13);
		m_Data.m_fPhi = m_pProperty->GetPropVal(14);
		m_Data.m_nEffectType = m_pProperty->GetPropVal(15);
		m_Data.m_nEffectTypeRange = m_pProperty->GetPropVal(16);
		m_Data.m_bShadow = m_pProperty->GetPropVal(17);
...
bool CSceneLightObject::Load(CELArchive& ar,DWORD dwVersion,int iLoadFlags)
{
	...
	}else if(dwVersion>0xd && dwVersion <= 0x10)
	{
		...
		m_matTrans.Translate(m_vPos.x,m_vPos.y,m_vPos.z);
		m_matAbs = m_matScale * m_matRotate * m_matTrans;
	}else if(dwVersion > 0x10 && dwVersion <= 0x49 )
	{
		A3DVECTOR3 vr,vu,vl;
		m_strName = AString(ar.ReadString());
		if(!ar.Read(&m_nObjectID,sizeof(int)))
	...
	else if(dwVersion > 0x49 )
	{
		...
		//加载新的变量，是否应用新的光照
		if(!ar.Read(&m_Data.m_bUsed2_2IlluminationEquation,sizeof(bool)))
			return false;
		...
		if (dwVersion > 0x4e)
		{
			if(!ar.Read(&m_Data.m_nEffectTypeRange,sizeof(int)))
				return false;

			if(!ar.Read(&m_Data.m_bShadow,sizeof(bool)))
				return false;
		}
...
bool CSceneLightObject::Save(CELArchive& ar,DWORD dwVersion)
{
	if(dwVersion>4)
	{
		...
		if(!ar.Write(&m_Data.m_nType,sizeof(int)))
			return false;
		//保存新的变量，是否应用新的光照
		if(!ar.Write(&m_Data.m_bUsed2_2IlluminationEquation,sizeof(bool)))
			return false;
		...
		if(!ar.Write(&m_Data.m_nEffectTypeRange,sizeof(int)))
			return false;
		if(!ar.Write(&m_Data.m_bShadow,sizeof(bool)))
			return false;
