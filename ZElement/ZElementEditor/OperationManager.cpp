...
void OperationContainer::CreateOperation()
{
	...
	CObstructOperation3 *pObstruct3 = new CObstructOperation3();
	CBrushTreeOperation *pBrushTree = new CBrushTreeOperation();
	CBuffRegionOperation* pBuffRegion = new CBuffRegionOperation();
	CTerrainColorMapOperation* pColorMap = new CTerrainColorMapOperation();
	CTerrainColorSmoothOperation* pColorSmooth = new CTerrainColorSmoothOperation();
	CGuildBuildingOperation* pGuildBuilding = new CGuildBuildingOperation();
	...
	if(pObstruct3) m_nOperationArray[OPERATION_TERRAIN_OBSTRUCT3] = pObstruct3;
	if(pBrushTree) m_nOperationArray[OPERATION_BRUSH_TREE] = pBrushTree;
	if(pBuffRegion) m_nOperationArray[OPERATION_SCENE_BUFF_REGION_ADD] = pBuffRegion;
	if(pColorMap) m_nOperationArray[OPERATION_TERRAIN_COLORMAP] = pColorMap;
	if(pColorSmooth) m_nOperationArray[OPERATION_TERRAIN_COLOR_SMOOTH] = pColorSmooth;
	if(pGuildBuilding) m_nOperationArray[OPERATION_SCENE_GUILDBUILDING] = pGuildBuilding;
}
