...
bool CLightProcessor::Init(A3DDevice * pA3DDevice, CExpTerrain * pTerrain, CTerrainWater * pTerrainWater, CSceneObjectManager * pSceneObjectManager, int nShadowMapSize, int nLightMapSize, int nLightMapDivide, bool bOnlyRenderShadow, bool bFast)
{
	...
	{
		g_Log.Log("CLightProcessor::Init(), failed to load shadow_map_vs_dx9.txt");
	}
	...
	{
		g_Log.Log("CLightProcessor::Init(), failed to load light_map_ps.txt");
	}
	...
	{
		g_Log.Log("CLightProcessor::Init(), failed to load light_map_vs.txt");
		return false;
	}
...
void CLightProcessor::LitTerrain20(bool bNight)
{
...
void CLightProcessor::LitTerrain22(bool bNight)
{
...
bool CLightProcessor::GenerateLightMaps(A3DCOLORVALUE * pColors, bool bNight, LPRENDERFORSHADOW pfnRenderForShadow, LPVOID pArg)
{
	...
	D3DXSaveTextureToFile("shadowmap1.bmp", D3DXIFF_BMP, m_pLightShadowMapTarget->GetTargetTexture(), NULL);
...
/*
	m_pA3DDevice->SetSourceAlpha(A3DBLEND_ONE);//A3DBLEND_ZERO);
	m_pA3DDevice->SetDestAlpha(A3DBLEND_ZERO);//A3DBLEND_INVSRCCOLOR);
	m_pA3DDevice->DrawIndexedPrimitive(A3DPT_TRIANGLELIST, 0, (m_nGridX + 3) * (m_nGridZ + 3),
		0, (m_nGridX + 2) * (m_nGridZ + 2) * 2);
	D3DXSaveTextureToFile("shadowmap2_1_0.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);

	m_pLightMapViewport->ClearDevice();
	m_pA3DDevice->SetSourceAlpha(A3DBLEND_SRCALPHA);//A3DBLEND_ZERO);
	m_pA3DDevice->SetDestAlpha(A3DBLEND_ZERO);//A3DBLEND_INVSRCCOLOR);
	m_pA3DDevice->DrawIndexedPrimitive(A3DPT_TRIANGLELIST, 0, (m_nGridX + 3) * (m_nGridZ + 3),
		0, (m_nGridX + 2) * (m_nGridZ + 2) * 2);
	D3DXSaveTextureToFile("shadowmap2_2_0.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);

	m_pLightMapViewport->ClearDevice();
	m_pA3DDevice->SetSourceAlpha(A3DBLEND_ZERO);//A3DBLEND_ZERO);
	m_pA3DDevice->SetDestAlpha(A3DBLEND_SRCCOLOR);//A3DBLEND_INVSRCCOLOR);
	m_pA3DDevice->DrawIndexedPrimitive(A3DPT_TRIANGLELIST, 0, (m_nGridX + 3) * (m_nGridZ + 3),
		0, (m_nGridX + 2) * (m_nGridZ + 2) * 2);
	D3DXSaveTextureToFile("shadowmap2_0_1.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);
*/
	m_pLightMapViewport->ClearDevice();
	...
	m_pA3DDevice->DrawIndexedPrimitive(A3DPT_TRIANGLELIST, 0, (m_nGridX + 3) * (m_nGridZ + 3),
		0, (m_nGridX + 2) * (m_nGridZ + 2) * 2);
	D3DXSaveTextureToFile("shadowmap2.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);
/*
	...
	*/
	...
	D3DXSaveTextureToFile("shadowmap3.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);
	m_pLightSun->TurnOff();
	...
	*/
	...
	LitTerrain22(bNight);
#endif
	D3DXSaveTextureToFile("shadowmap4.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);
	m_pLightSun->TurnOn();
	...
	D3DXSaveTextureToFile("shadowmap5.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);
#else
	pLightMapSurface = m_pLightMapTarget->GetTargetSurface();
	hval = m_pA3DDevice->GetD3DDevice()->CopyRects(pLightMapSurface, NULL, 0, pTempDXSurface, NULL);
#endif // #ifdef _ANGELICA22
	...
	// output a test file
	D3DXSaveTextureToFile("shadowmap6.bmp", D3DXIFF_BMP, m_pLightMapTarget->GetTargetTexture(), NULL);
...
void CLightProcessor::LitStaticModel20( bool bNight, A3DLitMesh *	pMesh,A3DVERTEX& vert, APtrList<CSceneObject *>& listLight)
{
...
void CLightProcessor::LitStaticModel22(bool bNight, A3DLitMesh *	pMesh,A3DVERTEX& vert, APtrList<CSceneObject *>& listLight)
{
...
bool CLightProcessor::LitStaticModelInScene(bool bNight, A3DViewport* pOldView)
{
...
#endif // #ifdef _ANGELICA22
	D3DXSaveTextureToFile("litmodel_depth.bmp", D3DXIFF_BMP, m_pLightShadowMapTarget->GetDepthTexture(), NULL);
	D3DXSaveTextureToFile("litmodel_tex.bmp", D3DXIFF_BMP, m_pLightShadowMapTarget->GetTargetTexture(), NULL);
	...
	hval = m_pA3DDevice->GetD3DDevice()->CopyRects(pLightMapSurface, NULL, 0, pTempDXSurface, NULL);
#endif // #ifdef _ANGELICA22
	D3DXSaveTextureToFile("litmodel_target2.bmp", D3DXIFF_BMP, m_pLightMapTargetForModel->GetTargetTexture(), NULL);
	if( hval != D3D_OK )
	{
		g_Log.Log("CLightProcessor::GenerateTerrainLightMaps(), copy rects failed!");
#ifdef _ANGELICA22
...
void CLightProcessor::LitWater20( bool bNight, A3DVERTEX& vert, APtrList<CSceneObject *>& listLight)
{
	if( m_bSoftware )
	{
		m_pA3DDevice->ClearVertexShader();
...
void CLightProcessor::LitWater22( bool bNight, A3DVERTEX& vert, APtrList<CSceneObject *>& listLight )
{
	m_pLightVertexShader->Appear();
	m_pLightRenderShader->Appear();
...
bool CLightProcessor::IsModelInPoly(CStaticModelObject* pModel)
{
	for( size_t i = 0; i < m_listScalePoly.size(); ++i)
	{
...
bool CLightProcessor::GetPointScale(A3DVECTOR3& v, float& fScale1,float& fScale2,DWORD& clr)
{
	POINT_FLOAT pt;
	pt.x = v.x;
	pt.y = v.z;
...
void CLightProcessor::BuildScalePolyList()
{
...
void CLightProcessor::ScalePolyColor(unsigned char* pColor, int nMapW)
{
	//APtrList<CSceneObject *>* pListObjects = m_pSceneObjectManager->GetSortObjectList(CSceneObject::SO_TYPE_DUMMY);
	//ALISTPOSITION pos = pListObjects->GetHeadPosition();
	//
