...
BOOL CToolTabWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
	...
	CFont* pDefaultFont = CFont::FromHandle( hfont );
	LOGFONT fontinfo;
	pDefaultFont->GetLogFont( &fontinfo );
	//fontinfo.
	m_Font.CreatePointFont( 96, fontinfo.lfFaceName );
...
void CToolTabWnd::SetColorMapPanelCurColor(DWORD dwColor)
{
	CTerrainModifyColorMapPanel* pPanel = dynamic_cast<CTerrainModifyColorMapPanel*>(m_TerrainPage.GetPanel(CTerrainPage::PANEL_MODIFYCOLORMAP)->GetContentWnd());
	if(!pPanel)
		return;
	pPanel->SetColor(dwColor);
}
...
void CToolTabWnd::OnMove(int x, int y) 
{
	CWnd::OnMove(x, y);
...
void CToolTabWnd::OnShowWindow(BOOL bShow, UINT nStatus) 
{
