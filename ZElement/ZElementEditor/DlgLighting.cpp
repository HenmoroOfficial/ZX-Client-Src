	...
	: CDialog(CDlgLighting::IDD, pParent)
{
    ...
    m_NightParams.fAmbientPower = InitData.fNightAmbientPower;
    ...
    m_Ambient2Wnd.SetNotifyWindow(this);
...
void CDlgLighting::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_EDIT_AMBIENT2, m_editAmbient2);
    ...
    DDX_Control(pDX, IDC_EDIT_MAX_LIGHT, m_editMaxLight);
    DDX_Control(pDX, IDC_STATIC_SUNCOLOR, m_SunColWnd);
    DDX_Control(pDX, IDC_STATIC_AMBIENT, m_AmbientWnd);
    DDX_Control(pDX, IDC_STATIC_AMBIENT2, m_Ambient2Wnd);
    ...
    //}}AFX_DATA_MAP
    DDX_Control(pDX, IDC_EDIT_AMBIENTPOWER, m_editAmbientPower);
}
...
BEGIN_MESSAGE_MAP(CDlgLighting, CDialog)
    ...
    ON_STN_CLICKED(IDC_STATIC_AMBIENT, OnStnClickedStaticAmbient)
    ON_EN_CHANGE(IDC_EDIT_SUNPOWER2, OnEnChangeEditSunpower2)
    ON_EN_CHANGE(IDC_EDIT_AMBIENT2, OnEnChangeEditAmbient2)
    ON_EN_CHANGE(IDC_EDIT_AMBIENTPOWER, OnEnChangeEditAmbientpower)
	//}}AFX_MSG_MAP
...
BOOL CDlgLighting::OnInitDialog() 
{
    ...
    AUX_FlatEditBox(&m_editMaxLight);
    AUX_FlatEditBox(&m_editAmbientPower);
    ...
    r = A3DCOLOR_GETRED(m_Params.dwAmbient2);
    g = A3DCOLOR_GETGREEN(m_Params.dwAmbient2);
    b = A3DCOLOR_GETBLUE(m_Params.dwAmbient2);
    m_Ambient2Wnd.SetColor(RGB(r, g, b));
    m_Ambient2Wnd.Redraw();
    m_Ambient2Wnd.EnableWindow(TRUE);
    ...
    str.Format("%x",m_Params.dwAmbient2);
    m_editAmbient2.SetWindowText(str);
    ...
    str.Format("%4.2f",g_GlobalData.fBrightness);
    m_editMaxLight.SetWindowText(str);
    str.Format("%4.2f",m_Params.fAmbientPower);
    m_editAmbientPower.SetWindowText(str);
    ...
    fMaxLight = g_GlobalData.fBrightness;
    fNightMaxLight = g_GlobalData.fBrightness_n;
	UpdateData(FALSE);
	m_bInited = true;
...
void CDlgLighting::UpdateControl()
{
    ...
    {
        r = A3DCOLOR_GETRED(m_NightParams.dwAmbient2);
        g = A3DCOLOR_GETGREEN(m_NightParams.dwAmbient2);
        b = A3DCOLOR_GETBLUE(m_NightParams.dwAmbient2);
    }
    m_Ambient2Wnd.SetColor(RGB(r, g, b));
    m_Ambient2Wnd.Redraw();
    m_Ambient2Wnd.EnableWindow(TRUE);
	...
	}else
	{
		...
		m_editAmbient.SetWindowText(str);
        str.Format("%x",m_NightParams.dwAmbient2);
        m_editAmbient2.SetWindowText(str);
		str.Format("%x",m_NightParams.dwSunCol);
		m_editSunColor.SetWindowText(str);
		str.Format("%4.2f",m_NightParams.fSunPower);
		m_editSunPower.SetWindowText(str);
        str.Format("%4.2f",fNightMaxLight);
        m_editMaxLight.SetWindowText(str);
        str.Format("%4.2f",m_NightParams.fAmbientPower);
        m_editAmbientPower.SetWindowText(str);
...
//		dwAmbient		= m_Params.dwAmbient;
        ...
        CString str;
        m_editMaxLight.GetWindowText(str);
        fMaxLight = (float)atof(str);
	}else
	{
		iSunDir		= m_SunDirSlider.GetPos();
		iSunHeight	= m_SunHeiSlider.GetPos();
		...
		int iSize = m_MapSizeCombo.GetCurSel();
		m_NightParams.iMapSize = 256 << iSize;
...
void CDlgLighting::OnOK() 
{
    ...
    g_GlobalData.fBrightness = fMaxLight;
    g_GlobalData.fBrightness_n = fNightMaxLight;
...
HRESULT CDlgLighting::OnColorChange(WPARAM wParam, LPARAM lParam)
{
	...
	else if (wParam == (WPARAM)m_AmbientWnd.m_hWnd)	//	Ambient color
	{
    ...
    else if (wParam == (WPARAM)m_Ambient2Wnd.m_hWnd)	//	Ambient color
    {
        DWORD dwCol = m_Ambient2Wnd.GetColor();
        r = GetRValue(dwCol);
        g = GetGValue(dwCol);
        b = GetBValue(dwCol);
        ...
        if(m_bIsDay)
        {
            m_Params.dwAmbient2 = A3DCOLORRGBA(r, g, b, 255);
            //	Update lighting view
            //m_LightingView.SetAmbient2(m_Params.dwAmbient2);
            //m_LightingView.InvalidateRect(NULL);
            CString str;
            str.Format("%x",m_Params.dwAmbient2);
            m_editAmbient2.SetWindowText(str);
        ...
        {
            m_NightParams.dwAmbient2 = A3DCOLORRGBA(r, g, b, 255);
            //	Update lighting view
            //m_LightingView.SetAmbient2(m_NightParams.dwAmbient2);
            //m_LightingView.InvalidateRect(NULL);
            CString str;
            str.Format("%x",m_NightParams.dwAmbient2);
            m_editAmbient2.SetWindowText(str);
...
void CDlgLighting::OnButtonExport() 
{
	char szFilter[] = "光照信息(*.litinfo)|*.litinfo||";
	DWORD dwFlags = OFN_OVERWRITEPROMPT | OFN_EXPLORER | OFN_NOCHANGEDIR;
	CFileDialog	FileDia(TRUE, ".litinfo", NULL, dwFlags, szFilter, NULL);
	
	if (FileDia.DoModal() == IDCANCEL)
		return;
	SaveItemData();
	if (!ExportLitInfo(FileDia.GetPathName(), m_Params, m_NightParams))
	{
		MessageBox("无法导出文件");
		return;
	}
...
void CDlgLighting::OnButtonImport() 
{
	char szFilter[] = "光照信息(*.litinfo)|*.litinfo||";
	...
	if (!ImportLitInfo(FileDia.GetPathName(), m_Params, m_NightParams))
	{
		MessageBox("无法导入文件");
		return;
	}
...
bool CDlgLighting::ExportLitInfo(const char* szFileName, const LIGHTINGPARAMS& paramDay, const LIGHTINGPARAMS& paramNight)
{
	...
	fprintf(fp, "Dir.y=%g\n", paramDay.vSunDir.y);
	fprintf(fp, "Dir.z=%g\n", paramDay.vSunDir.z);
	fprintf(fp, "Color=%d\n", (int)paramDay.dwSunCol);
	fprintf(fp, "Ambient=%d\n", (int)paramDay.dwAmbient);
	fprintf(fp, "Ambient2=%d\n", (int)paramDay.dwAmbient2);
	fprintf(fp, "AmbientPower=%d\n", (int)paramDay.fAmbientPower);
	fprintf(fp, "Power=%g\n", paramDay.fSunPower);

	fprintf(fp, "[Night]");
	fprintf(fp, "Dir.x=%g\n", paramNight.vSunDir.x);
	fprintf(fp, "Dir.y=%g\n", paramNight.vSunDir.y);
	fprintf(fp, "Dir.z=%g\n", paramNight.vSunDir.z);
	fprintf(fp, "Color=%d\n", (int)paramNight.dwSunCol);
	fprintf(fp, "Ambient=%d\n", (int)paramNight.dwAmbient);
	fprintf(fp, "Ambient2=%d\n", (int)paramNight.dwAmbient2);
	fprintf(fp, "AmbientPower=%d\n", (int)paramNight.fAmbientPower);
	fprintf(fp, "Power=%g\n", paramNight.fSunPower);
	fclose(fp);
	return true;
}

bool CDlgLighting::ImportLitInfo(const char* szFileName, LIGHTINGPARAMS& paramDay, LIGHTINGPARAMS& paramNight)
{
	AIniFile IniFile;
	if (!IniFile.Open(szFileName))
	{
		return false;
	}
	paramDay.vSunDir.x = IniFile.GetValueAsFloat("Day", "Dir.x", paramDay.vSunDir.x);
	paramDay.vSunDir.y = IniFile.GetValueAsFloat("Day", "Dir.y", paramDay.vSunDir.y);
	paramDay.vSunDir.z = IniFile.GetValueAsFloat("Day", "Dir.z", paramDay.vSunDir.z);
	paramDay.dwSunCol = (DWORD) IniFile.GetValueAsInt("Day", "Color", paramDay.dwSunCol);
	paramDay.dwAmbient = (DWORD) IniFile.GetValueAsInt("Day", "Ambient", paramDay.dwAmbient);
	paramDay.dwAmbient2 = (DWORD) IniFile.GetValueAsInt("Day", "Ambient2", paramDay.dwAmbient2);
	paramDay.fAmbientPower = (DWORD) IniFile.GetValueAsInt("Day", "AmbientPower", paramDay.fAmbientPower);
	paramDay.fSunPower = IniFile.GetValueAsFloat("Day", "Power", paramDay.fSunPower);
	
	paramNight.vSunDir.x = IniFile.GetValueAsFloat("Night", "Dir.x", paramNight.vSunDir.x);
	paramNight.vSunDir.y = IniFile.GetValueAsFloat("Night", "Dir.y", paramNight.vSunDir.y);
	paramNight.vSunDir.z = IniFile.GetValueAsFloat("Night", "Dir.z", paramNight.vSunDir.z);
	paramNight.dwSunCol = (DWORD) IniFile.GetValueAsInt("Night", "Color", paramNight.dwSunCol);
	paramNight.dwAmbient = (DWORD) IniFile.GetValueAsInt("Night", "Ambient", paramNight.dwAmbient);
	paramNight.dwAmbient2 = (DWORD) IniFile.GetValueAsInt("Night", "Ambient2", paramNight.dwAmbient2);
	paramNight.fAmbientPower = (DWORD) IniFile.GetValueAsInt("Night", "AmbientPower", paramNight.fAmbientPower);
	paramNight.fSunPower = IniFile.GetValueAsFloat("Night", "Power", paramNight.fSunPower);
	
	return true;
}


void CDlgLighting::OnStnClickedStaticAmbient()
{
    // TODO: Add your control notification handler code here
}

void CDlgLighting::OnEnChangeEditSunpower2()
{
    // TODO:  If this is a RICHEDIT control, the control will not
    // send this notification unless you override the CDialog::OnInitDialog()
    // function and call CRichEditCtrl().SetEventMask()
...
void CDlgLighting::OnEnChangeEditAmbient2()
{
    // TODO:  If this is a RICHEDIT control, the control will not
    // send this notification unless you override the CDialog::OnInitDialog()
    // function and call CRichEditCtrl().SetEventMask()
...
void CDlgLighting::OnEnChangeEditAmbientpower()
{
    ...
    UpdateData(true);
    float power; 
    m_editAmbientPower.GetWindowText(str);
    power = (float)atof(str);
    if(power>100.0f)
    {
        power = 100;
    ...
    if(m_bIsDay)
    {
        m_Params.fAmbientPower = power;
        //g_Configs.fSunPower = power; 
    }else
    {
        m_NightParams.fAmbientPower = power;
        //g_Configs.fNightSunPower = power; 
    }
}
