...
CObstructOperation3::CObstructOperation3()
{
	m_pData = 0;
	m_bStartPaint = false;
	m_bEndAddLine = false;
...
bool CObstructOperation3::OnLButtonDown(int x, int y, DWORD dwFlags)
{
	   ...
	   m_nOperationType == OBSTRUCT_RESET) )
	{
		...
		if(m_nOperationType == OBSTRUCT_YELLOW)
		{
			...
			}else
			{
				...
				pMap->GetPlayerAirPassMapGen()->Transf2DTo3D(mapPos,vPixelCenter);
				mapPos.h = vecHitPos.y - pTerrain->GetPosHeight(vPixelCenter);
			}
	
			if(mapPos.u >= 0 && mapPos.u < 1024 && mapPos.v >= 0 && mapPos.v < 1024)
...
bool CObstructOperation3::OnLButtonUp(int x,int y, DWORD dwFlags)
{
	m_bStartPaint = false;
	/*
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
...
bool CObstructOperation3::OnMouseMove(int x,int y,DWORD dwFlags)
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap &&
	   (m_nOperationType == OBSTRUCT_RED ||
...
void CObstructOperation3::UpdateObsData()
{
...
void CObstructOperation3::Render(A3DViewport* pA3DViewport)
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	...
	*/
	...
	if(pMap)
	{
		if(m_nOperationType == OBSTRUCT_RED || m_nOperationType == OBSTRUCT_GREEN || OBSTRUCT_RESET)
...
void CObstructOperation3::Init(int type)
{
	//Release();
...
void CObstructOperation3::SaveObsToFile(CString strPathName)
{
...
void CObstructOperation3::DrawGrid(A3DVECTOR3 v1, A3DVECTOR3 v2, A3DVECTOR3 v3, A3DVECTOR3 v4,int nSel)
{
	DWORD clr;
...
void CObstructOperation3::DrawLine(A3DVECTOR3 v1,A3DVECTOR3 v2, DWORD clr)
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap)
	{
...
void CObstructOperation3::DrawObstruct()
{
	float fWidth = 1.0f;
	int   nShowTitle = 20;
...
void CObstructOperation3::Release()
{
...
void CObstructOperation3::TransObstructToBitmap()
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap==NULL) return;
	UCHAR *pData = pMap->GetPlayerAirPassMapGen()->GetPassMap();	
	...
	for(int h = 0; h < OBS_MAP_WIDTH; h ++)
	{
		for( int w = 0; w < OBS_MAP_WIDTH; w ++)
		{
			...
			//int idh= pMap->GetPlayerAirPassMapGen()->GetDeltaHeightMap()[h * OBS_MAP_WIDTH + w];
