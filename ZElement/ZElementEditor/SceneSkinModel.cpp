	...
	if(bGet)
	{
		...
		if(pMap)
		{
			...
			if(pObj != NULL && pObj != this)
			{
				...
				m_pProperty->SetPropVal(3,m_nObjectID);

				if(m_bInitedEx) m_pProperty->SetPropVal(4,m_Property.dwActionID);
		...
		if(m_bInitedEx)
		{
			m_Property.dwActionID = m_pProperty->GetPropVal(4);
...
bool CSceneSkinModel::LoadSkinModel(AString strPathName)
{
	...
	{
		g_Log.Log("CSceneSkinModel::LoadSkinModel(), Failed load %s", strSkin);
		//m_pECModel->Release();
		//delete m_pECModel;
		//m_pECModel = NULL;
		//return false;
...
void	CSceneSkinModel::PostRender(A3DViewport* pA3DViewport)
{
	if(m_pECModel==NULL) return;
		DrawCenter();
	CBox3D renderBox;
...
void CSceneSkinModel::SetDirAndUp(const A3DVECTOR3& vDir, const A3DVECTOR3& vUp)
{
...
int	CSceneSkinModel::GetTriangleNum()
{
	if(m_pECModel)
	{
