...
public:
...
#ifdef _ANGELICA22
	CSceneWaterObject(const CSceneWaterObject& rhs);
	...
	void SetWaterGridSize(int nGridSize){ m_nGridSize = nGridSize; };
	...
	void GetProperty(int &nAreaX, int &nAreaZ,INT &nGridSize, float &fWaveSize,DWORD &dwColor)
	{
		nAreaX = m_nAreaX;
		nAreaZ = m_nAreaZ;
		nGridSize = m_nGridSize;
		fWaveSize = m_fWaveSize;
		dwColor = m_dwColor;
	};

	void SetProperty(int nAreaX, int nAreaZ,int nGridSize, float fWaveSize,DWORD dwColor)
	{
		m_nAreaX = nAreaX;
		m_nAreaZ = nAreaZ;
	...
	bool GetMinorWaterFlag(){ return m_bMinorWaterFlag; }
	
	void SetMinorWaterFlag(bool bMinor){ m_bMinorWaterFlag = bMinor; }
private:
	...
	void DrawGrid(const A3DVECTOR3& vMin, const A3DVECTOR3& vMax);//绘制一个XZ平面方块，调试用
	...
	bool IsOrder(const WGRID& oldGrid, const WGRID& w1, const WGRID& w2);

	bool IsEdge(const WGRID& grid);

	void DrawProfile();//绘制轮廓线，调试用
	
	void DrawLine(const WGRID& old, const WGRID& cur,DWORD clr);

	//for debug
	void DrawLine2(const A3DVECTOR3& v1,const A3DVECTOR3& v2, DWORD clr);
	...
	bool PlaneIntersectionWithLine(const A3DVECTOR3& vStart, const A3DVECTOR3& vEnd, const A3DVECTOR3& point, const A3DVECTOR3& normal,A3DVECTOR3 &out);
	
	bool GetNextPoint(int lastDir, int cx,int cy, int& nx, int& ny, int& newDir
									  ,BYTE *pData,BYTE *pFlag);

	bool IsIntersectionWithWater(const A3DVECTOR3& pt1,const A3DVECTOR3& pt2);
