...
public:
	enum
	{
		...
		SO_TYPE_DUMMY,
		SO_TYPE_AIAREA,
		SO_TYPE_SPECIALLY,		// 特效
		SO_TYPE_ECMODEL,		// 动画模型
		SO_TYPE_CRITTER_GROUP,	// 生物群
		SO_TYPE_FIXED_NPC,
		SO_TYPE_PRECINCT,
		SO_TYPE_GATHER,
		SO_TYPE_RANGE,
		...
		SO_TYPE_DYNAMIC,
		SO_TYPE_BUFF_REGION,
		SO_TYPE_MIRROR_PLANE,
		SO_TYPE_GUILD_BUILDING,	// 帮派基地地块
		SO_TYPE_NUM
	};
...
public:
	...
	virtual void				ReInit(); // 在拷贝构造或者赋值后调用，将指针字段初始化为NULL或者新对象。赋值前还应调用Release
	...
	virtual void                SetDirAndUp(const A3DVECTOR3& vDir, const A3DVECTOR3& vUp)
								{
									m_matRotate = a3d_TransformMatrix(vDir,vUp,0);
	...
	virtual int					GetMeshNum() { return 0; }
