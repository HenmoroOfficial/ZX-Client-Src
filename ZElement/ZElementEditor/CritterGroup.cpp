...
void CCritterGroup::PostRender(A3DViewport* pA3DViewport)
{
	DrawCenter();
	if(m_pGroup)
	{
	...
	if(bGet)
	{
		...
		if(pMap)
		{
			...
			if(pObj != NULL && pObj != this)
			{
				...
				m_pProperty->SetPropVal(9,m_nObjectID);
...
void CCritterGroup::Translate(const A3DVECTOR3& vDelta)
{
	m_vPos += vDelta;
...
void CCritterGroup::SetPos(const A3DVECTOR3& vPos)
{
