...
bool CSevBezier::Load(FILE* pFile)
{
	...
	if (dwVersion >=4 )
	{
		if(0==fread(&m_bPassmapAvail,sizeof(bool),1,pFile)) return false;
	}

	if( dwVersion >= 5)
	{
		if(0==fread(&m_iLoopType,sizeof(int),1,pFile)) return false;
		if(0==fread(&m_iBezierType,sizeof(int),1,pFile)) return false;
	}

	int ptNum,segNum,i;
	A3DVECTOR3 vAnchorHead,vAnchorTail;
	int headIndex,TailIndex;
	float lenght,fFov,fSpeed,fRatio;
	...
	for(i = 0; i < ptNum; i++)
	{
		...
		if (dwVersion >= 5)
		{
			if(0==fread(&fSpeed,sizeof(float),1,pFile)) goto FAIL;
			if(0==fread(&fRatio,sizeof(float),1,pFile)) goto FAIL;
			if(0==fread(&fFov,sizeof(float),1,pFile)) goto FAIL;
		}else
		{
			fSpeed = 1.0f;
			fRatio = 1.333f;
