...
// (0x0-0x38)场景对象文件版本号			
// 4b->4c: PrecinctObject changed
// 4d  SceneBoxArea
// 4e  CStaticModelObject
// 4f  CSceneLightObject
// 50  相位功能CSceneFixedNpcGenerator、CSceneGatherObject
// 51  相位功能SceneObjectManager
// 52  区域增加小地图功能ScenePrecientObject
// 53  帮派基地小地图保存数据增加第几套数据保存
#define  FS_VERSION 0x53
#define  LIT_MODEL_VERSION 0x8//(0x0-0x7)
...
public:
	...
	bool AABBTraceStaticModel(BrushTraceInfo * pInfo);
	void RebuildBrushTree();
	void ReloadHullToBrushTree();
	...
	CEditerBezier* FindBezierByGlobalID(int iGlobal);
	...
	virtual void			PostRenderObjects(A3DViewport* pA3DViewport);
	...
	bool IsShowObjectSort( int iType );
	//按对象类型创建对象
	CSceneObject* CreateObject(const AString& strName, int iType);
	//创建一个灯光
	...
	CSceneBuffRegion*		CreateBuffRegion(const AString& name);
	CSceneGuildBuilding*	CreateGuildBuilding(const AString& name);
	...
	CSceneObject *          FindAiAreaObject( int id );
	CSceneObject *          FindNpcObject( int id );
	CSceneObject *          FindGatherObject( int id );
	
	//判断一个静态模型对象是否已经放入到场景
	bool                    IsBuildingObjectInScene(const AString& name, const AString& szModelPath);
	...
	bool					RayTrace(const A3DVECTOR3& vStart,const A3DVECTOR3& vEnd,bool bAdd = true);
	...
	//取得草地接口
	A3DGrassLand *			GetElGrassLand(){ return m_pGrassLand; }
#else
	A3DSky*					GetSky(){ return m_pA3DSky; };
	void					SetSky(A3DSky* pSky){ m_pA3DSky = pSky; };
	...
	APtrList<CSceneObject *>* GetSceneObjectSelectedList() { return &m_listSelectedObject; }
	...
	Icon3DRender* GetIconRender() const { return m_pItemCenterRender; }
...
protected:
	...
	*/
	...
	APtrList<CSceneObject *> m_listSortObjects[CSceneObject::SO_TYPE_NUM];
	...
	A3DGrassLand * m_pGrassLand;		//场景中的草地接口
#else
	...
	int            m_nObjectIDFlag;
//	int			   m_nOverseaObjectIDFlag;
	DWORD          m_dwVssFileVersion;

	Icon3DRender*	m_pItemCenterRender;
};
