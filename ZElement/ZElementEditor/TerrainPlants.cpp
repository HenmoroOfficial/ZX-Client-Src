...
PPLANT CTerrainPlants::AddPlant(const AString& name,int nID,int nType)
{
	...
	if(GetPlant(name)==NULL)
	{
		...
		case PLANT_GRASS:
			...
			int nMaskGridNum = pTerrain == 0 ? 16 : (pMap->GetTerrain()->GetHeightMap()->iWidth - 1)/g_Configs.iTExpMaskTile;
			nMaskGridNum = nMaskGridNum*nMaskGridNum;
			if(nMaskGridNum<=0) 
			{
				delete pPlant;
				pPlant = NULL;
...
void CTerrainPlants::DeletePlant(const AString& name)
{
