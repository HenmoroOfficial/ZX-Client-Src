...
void Init_BoxArea_Data(BOX_AREA_PROPERTY &data)
{
	...
	data.m_dwFogColor = A3DCOLORRGB(255,255,255);

	data.m_fFogStartNight = 50.0f;
	data.m_fFogEndNight = 500.0f;
	data.m_fFogDeepnessNight = 0.5f;
	data.m_dwFogColorNight = A3DCOLORRGB(255,255,255);

	data.m_dwFogColor2 = A3DCOLORRGB(255,255,255);
	data.m_fFogHeightStart       = 0;
	data.m_fFogHeightEnd         = 300;
	data.m_fFogDensityStart      = 2;
	data.m_fFogDensityEnd        = 0.25f;
	data.m_fFogColorDensityStart    = 1.0f;
	data.m_fFogColorDensityEnd      = 1.0f;
	data.m_fFogHorzeDensity         = 1.0f;
	data.m_fFogHorzePower           = 1.0f;

	data.m_dwFogColor2Night = A3DCOLORRGB(255,255,255);
	data.m_fFogHeightStartNight  = 0;
	data.m_fFogHeightEndNight    = 300;
	data.m_fFogDensityStartNight = 2;;
	data.m_fFogDensityEndNight   = 0.25f;
	data.m_fFogColorDensityStartNight   = 1.0f;
	data.m_fFogColorDensityEndNight     = 1.0f;
	data.m_fFogHorzeDensityNight        = 1.0f;
	data.m_fFogHorzePowerNight          = 1.0f;
	...
	// sky box textures
	data.m_strSkyTop	= "Top.bmp";
	data.m_strSkyBottom	= "Bottom.bmp";
	data.m_strSkyLeft	= "Left.bmp";
	data.m_strSkyRight	= "Right.bmp";
	data.m_strSkyFront	= "Front.bmp";
	data.m_strSkyBack	= "Back.bmp";

	data.m_strSkyNightTop	= "NightTop.bmp";
	data.m_strSkyNightBottom	= "NightBottom.bmp";
	data.m_strSkyNightLeft	= "NightLeft.bmp";
	data.m_strSkyNightRight	= "NightRight.bmp";
	data.m_strSkyNightFront	= "NightFront.bmp";
	data.m_strSkyNightBack	= "NightBack.bmp";
	...
	data.m_StartTime.nYear = -1;
	data.m_StartTime.nMouth = -1;
	data.m_StartTime.nWeek = -1;
	data.m_StartTime.nDay = -1;
	data.m_StartTime.nHours = -1;
	data.m_StartTime.nMinutes = -1;

	data.m_EndTime.nYear = -1;
	data.m_EndTime.nMouth = -1;
	data.m_EndTime.nWeek = -1;
	data.m_EndTime.nDay = -1;
	data.m_EndTime.nHours = -1;
	data.m_EndTime.nMinutes = -1;

	data.m_fCapTile		= 8.0f;
	data.m_fCapRot		= 0.0f;

	data.m_dwSunShineColor = 0xFFFFFFFF;
	data.m_fBloomThredhold = 0.8f;
	data.m_dwSunShineColorNight = 0xFFFFFFFF;
	data.m_fBloomThredholdNight = 0.8f;
	//
	data.m_nPriority = 0;
}
...
CSceneBoxArea::CSceneBoxArea()
{
	...
	m_pStartTimeFunc = new START_TIME_SET_FUNCS;
	m_pEndTimeFunc = new END_TIME_SET_FUNCS;
...
CSceneBoxArea::CSceneBoxArea(bool bDefault)
{
	...
	//默认为当前数据套
	m_nCopy = g_Configs.nShowCopyNum;
	Init_BoxArea_Data(m_PropertyData);
	m_vPos = A3DVECTOR3(0.0f);
//	m_pA3DSky = NULL;
	m_bIsInside = false;
...
bool CSceneBoxArea::IsPriority()
{
	return IsTimeValid(m_PropertyData.m_StartTime) && IsTimeValid(m_PropertyData.m_EndTime);
}

bool CSceneBoxArea::IsInActiveTime()
{
	SYSTEMTIME st;
	GetLocalTime(&st);

	//	Check born time
	if (IsTimeValid(m_PropertyData.m_StartTime))
	{
		int d = CompareTime(m_PropertyData.m_StartTime, st);
		if (d > 0)
			return false;
	}

	if (!IsTimeValid(m_PropertyData.m_EndTime))
		return true;	//	Don't set die time

	//	Check die time
	int d = CompareTime(m_PropertyData.m_EndTime, st);
	return d >= 0 ? true : false;
	
}

bool CSceneBoxArea::IsTimeValid(TIME_CTR& _time)
{
	if( _time.nDay == -1 && 
		_time.nHours == -1 && 
		_time.nMinutes == -1 && 
		_time.nMouth == -1 &&
		_time.nWeek == -1 &&
		_time.nYear == -1) return false;
	
	return true;
}

int CSceneBoxArea::CompareTime(TIME_CTR& t1, SYSTEMTIME& t2)
{
	int d1, d2, d3;

	d1 = t1.nYear - t2.wYear;
	d2 = t1.nMouth - t2.wMonth;

	if (t1.nWeek >= 0)
		d3 = t1.nWeek - t2.wDayOfWeek;
	else
		d3 = t1.nDay - t2.wDay;
	...
	//	Check hour/minute
	d1 = t1.nHours - t2.wHour;
	d2 = t1.nMinutes - t2.wMinute;
...
void CSceneBoxArea::OnCamEntry(bool bForceUpdate)//摄象机进入区域事件
{
	...
	if(pMap)
	{
		...
		pMap->m_FogColor.SetTarget(ColorRGBAToColorValue(m_PropertyData.m_dwFogColor), fInvTime);
		...
		pMap->m_FogColorNight.SetTarget(ColorRGBAToColorValue(m_PropertyData.m_dwFogColorNight), fInvTime);
		...
		//改变天空
		A3DSky* pTempSky = pManager->GetSky();
		if(pTempSky)
		{
			pTempSky->Release();
			delete pTempSky;
		...
		if(m_PropertyData.m_nBoxAreaType == 0)
		{
			...
			if(pSky)
			{
				...
				{
					g_Log.Log("CSceneBoxArea::OnCamEntry,Failed to create sphere sky !");
					delete pSky;
					pManager->SetSky(NULL);
				}
				else
					pManager->SetSky(pSky);
			}
			...
			if(g_fNightFactor == 1.0f) 
				pSky->TransSky(m_PropertyData.m_strSkyNight0,m_PropertyData.m_strSkyNight1,m_PropertyData.m_strSkyNight2,0);
			else
				pSky->TransSky(m_PropertyData.m_strSky0,m_PropertyData.m_strSky1,m_PropertyData.m_strSky2,0);
 			m_bTransDone = false;
		}
#ifdef _ANGELICA22
#else
		else
		{
			...
			if(pSky)
			{
				if (!pSky->Init(g_Render.GetA3DDevice(), NULL, 
					g_szWorkDir + AString("Textures\\sky\\") + m_PropertyData.m_strSkyRight,
					g_szWorkDir + AString("Textures\\sky\\") + m_PropertyData.m_strSkyLeft,
					g_szWorkDir + AString("Textures\\sky\\") + m_PropertyData.m_strSkyTop,
					g_szWorkDir + AString("Textures\\sky\\") + m_PropertyData.m_strSkyBottom,
					g_szWorkDir + AString("Textures\\sky\\") + m_PropertyData.m_strSkyFront,
					g_szWorkDir + AString("Textures\\sky\\") + m_PropertyData.m_strSkyBack))
				{
					g_Log.Log("CSceneBoxArea::OnCamEntry,Failed to create box sky !");
					delete pSky;
					pManager->SetSky(NULL);
				}
				else
...
#ifdef _ANGELICA22
		//g_Render.SetColorMappingTex(m_PropertyData.m_strColorMappingTex);
#endif
	}

	CFogSettingDlg::UpdateDialog();
}
...
void CSceneBoxArea::PostRender(A3DViewport* pA3DViewport)
{
	...
	if(bGet)
	{
		...
		if(pMap)
		{
			...
			if(pObj != NULL && pObj != this)
			{
				...
				m_pProperty->SetPropVal(11,m_PropertyData.m_dwFogColor);
				...
				m_pProperty->SetPropVal(15,m_PropertyData.m_dwFogColorNight);
				...
				m_pProperty->SetPropVal(48,m_nCopy);

				m_pProperty->SetPropVal(51, m_PropertyData.m_strSkyTop);
				m_pProperty->SetPropVal(52, m_PropertyData.m_strSkyBottom);
				m_pProperty->SetPropVal(53, m_PropertyData.m_strSkyLeft);
				m_pProperty->SetPropVal(54, m_PropertyData.m_strSkyRight);
				m_pProperty->SetPropVal(55, m_PropertyData.m_strSkyFront);
				m_pProperty->SetPropVal(56, m_PropertyData.m_strSkyBack);


				m_pProperty->SetPropVal(57, m_PropertyData.m_strSkyNightTop);
				m_pProperty->SetPropVal(58, m_PropertyData.m_strSkyNightBottom);
				m_pProperty->SetPropVal(59, m_PropertyData.m_strSkyNightLeft);
				m_pProperty->SetPropVal(60, m_PropertyData.m_strSkyNightRight);
				m_pProperty->SetPropVal(61, m_PropertyData.m_strSkyNightFront);
				m_pProperty->SetPropVal(62, m_PropertyData.m_strSkyNightBack);

				m_pProperty->SetPropVal(63, m_PropertyData.m_nBoxAreaType);

				m_pProperty->SetPropVal(64,m_nObjectID);

				m_pProperty->SetPropVal(65,m_PropertyData.m_fCapTile);
				m_pProperty->SetPropVal(66,m_PropertyData.m_fCapRot);

				m_pProperty->SetPropVal(67,m_PropertyData.m_nPriority);
				m_pProperty->SetPropVal(68,m_PropertyData.m_strColorMappingTex);

				m_pProperty->SetPropVal(69,m_PropertyData.m_fFogHeightStart);
				m_pProperty->SetPropVal(70,m_PropertyData.m_fFogHeightEnd);
				m_pProperty->SetPropVal(71,m_PropertyData.m_fFogDeepness);
				m_pProperty->SetPropVal(72,m_PropertyData.m_fFogDensityStart);
				m_pProperty->SetPropVal(73,m_PropertyData.m_fFogDensityEnd);
				m_pProperty->SetPropVal(74,m_PropertyData.m_fFogColorDensityStart);
				m_pProperty->SetPropVal(75,m_PropertyData.m_fFogColorDensityEnd);
				m_pProperty->SetPropVal(76,m_PropertyData.m_dwFogColor);
				m_pProperty->SetPropVal(77,m_PropertyData.m_dwFogColor2);
				m_pProperty->SetPropVal(78,m_PropertyData.m_fFogHorzeDensity);
				m_pProperty->SetPropVal(79,m_PropertyData.m_fFogHorzePower);

				m_pProperty->SetPropVal(80,m_PropertyData.m_fFogHeightStartNight);
				m_pProperty->SetPropVal(81,m_PropertyData.m_fFogHeightEndNight);
				m_pProperty->SetPropVal(82,m_PropertyData.m_fFogDeepnessNight);
				m_pProperty->SetPropVal(83,m_PropertyData.m_fFogDensityStartNight);
				m_pProperty->SetPropVal(84,m_PropertyData.m_fFogDensityEndNight);
				m_pProperty->SetPropVal(85,m_PropertyData.m_fFogColorDensityStartNight);
				m_pProperty->SetPropVal(86,m_PropertyData.m_fFogColorDensityEndNight);
				m_pProperty->SetPropVal(87,m_PropertyData.m_dwFogColorNight);
				m_pProperty->SetPropVal(88,m_PropertyData.m_dwFogColor2Night);
				m_pProperty->SetPropVal(89,m_PropertyData.m_fFogHorzeDensityNight);
				m_pProperty->SetPropVal(90,m_PropertyData.m_fFogHorzePowerNight);

				m_pProperty->SetPropVal(91,m_PropertyData.m_dwSunShineColor);
				m_pProperty->SetPropVal(92,m_PropertyData.m_fBloomThredhold);
				m_pProperty->SetPropVal(93,m_PropertyData.m_dwSunShineColorNight);
				m_pProperty->SetPropVal(94,m_PropertyData.m_fBloomThredholdNight);


				m_pNatureDataSetFunc->OnSetValue(strText);
				m_pLightingSelFunc->OnSetValue(strText);
				m_pStartTimeFunc->OnSetValue("");
				m_pEndTimeFunc->OnSetValue("");
		...
		m_PropertyData.m_dwFogColor = m_pProperty->GetPropVal(11);
		...
		m_PropertyData.m_dwFogColorNight = m_pProperty->GetPropVal(15);
		...
		m_nCopy  = m_pProperty->GetPropVal(48);


		m_PropertyData.m_strSkyTop = ACString(m_pProperty->GetPropVal(51));
		m_PropertyData.m_strSkyBottom = ACString(m_pProperty->GetPropVal(52));
		m_PropertyData.m_strSkyLeft = ACString(m_pProperty->GetPropVal(53));
		m_PropertyData.m_strSkyRight = ACString(m_pProperty->GetPropVal(54));
		m_PropertyData.m_strSkyFront = ACString(m_pProperty->GetPropVal(55));
		m_PropertyData.m_strSkyBack = ACString(m_pProperty->GetPropVal(56));

		m_PropertyData.m_strSkyNightTop = ACString(m_pProperty->GetPropVal(57));
		m_PropertyData.m_strSkyNightBottom = ACString(m_pProperty->GetPropVal(58));
		m_PropertyData.m_strSkyNightLeft = ACString(m_pProperty->GetPropVal(59));
		m_PropertyData.m_strSkyNightRight = ACString(m_pProperty->GetPropVal(60));
		m_PropertyData.m_strSkyNightFront = ACString(m_pProperty->GetPropVal(61));
		m_PropertyData.m_strSkyNightBack = ACString(m_pProperty->GetPropVal(62));

		m_PropertyData.m_nBoxAreaType = m_pProperty->GetPropVal(63);

		m_PropertyData.m_fCapTile = m_pProperty->GetPropVal(65);
		m_PropertyData.m_fCapRot = m_pProperty->GetPropVal(66);

		m_PropertyData.m_nPriority = m_pProperty->GetPropVal(67);
		m_PropertyData.m_strColorMappingTex = AString(m_pProperty->GetPropVal(68));
		//g_Render.SetColorMappingTex(m_PropertyData.m_strColorMappingTex);

		m_PropertyData.m_fFogHeightStart = m_pProperty->GetPropVal(69);
		m_PropertyData.m_fFogHeightEnd = m_pProperty->GetPropVal(70);
		m_PropertyData.m_fFogDeepness = m_pProperty->GetPropVal(71);
		m_PropertyData.m_fFogDensityStart = m_pProperty->GetPropVal(72);
		m_PropertyData.m_fFogDensityEnd = m_pProperty->GetPropVal(73);
		m_PropertyData.m_fFogColorDensityStart = m_pProperty->GetPropVal(74);
		m_PropertyData.m_fFogColorDensityEnd = m_pProperty->GetPropVal(75);
		m_PropertyData.m_dwFogColor = m_pProperty->GetPropVal(76);
		m_PropertyData.m_dwFogColor2 = m_pProperty->GetPropVal(77);
		m_PropertyData.m_fFogHorzeDensity = m_pProperty->GetPropVal(78);
		m_PropertyData.m_fFogHorzePower = m_pProperty->GetPropVal(79);

		m_PropertyData.m_fFogHeightStartNight = m_pProperty->GetPropVal(80);
		m_PropertyData.m_fFogHeightEndNight = m_pProperty->GetPropVal(81);
		m_PropertyData.m_fFogDeepnessNight = m_pProperty->GetPropVal(82);
		m_PropertyData.m_fFogDensityStartNight = m_pProperty->GetPropVal(83);
		m_PropertyData.m_fFogDensityEndNight = m_pProperty->GetPropVal(84);
		m_PropertyData.m_fFogColorDensityStartNight = m_pProperty->GetPropVal(85);
		m_PropertyData.m_fFogColorDensityEndNight = m_pProperty->GetPropVal(86);
		m_PropertyData.m_dwFogColorNight = m_pProperty->GetPropVal(87);
		m_PropertyData.m_dwFogColor2Night = m_pProperty->GetPropVal(88);
		m_PropertyData.m_fFogHorzeDensityNight = m_pProperty->GetPropVal(89);
		m_PropertyData.m_fFogHorzePowerNight = m_pProperty->GetPropVal(90);

		m_PropertyData.m_dwSunShineColor = m_pProperty->GetPropVal(91);
		m_PropertyData.m_fBloomThredhold = m_pProperty->GetPropVal(92);
		m_PropertyData.m_dwSunShineColorNight = m_pProperty->GetPropVal(93);
		m_PropertyData.m_fBloomThredholdNight = m_pProperty->GetPropVal(94);
...
void CSceneBoxArea::BuildAreaProperty()
{
	if(m_pProperty)
	{
		...
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwFogColor), "(白天)雾颜色",NULL,NULL,WAY_COLOR);
		...
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwFogColorNight), "(夜晚)雾颜色",NULL,NULL,WAY_COLOR);
		...
		m_pProperty->DynAddProperty(AVariant(strText),"区域开始时间",(ASet*)m_pStartTimeFunc, NULL, WAY_CUSTOM);
		m_pProperty->DynAddProperty(AVariant(strText),"区域结束时间",(ASet*)m_pEndTimeFunc, NULL, WAY_CUSTOM);

		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyTop), "(白天)天空盒纹理(顶)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyBottom), "(白天)天空盒纹理(底)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyLeft), "(白天)天空盒纹理(左)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyRight), "(白天)天空盒纹理(右)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyFront), "(白天)天空盒纹理(前)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyBack), "(白天)天空盒纹理(后)",NULL,NULL,WAY_FILENAME,"Textures\\sky");

		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyNightTop), "(夜晚)天空盒纹理(顶)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyNightBottom), "(夜晚)天空盒纹理(底)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyNightLeft), "(夜晚)天空盒纹理(左)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyNightRight), "(夜晚)天空盒纹理(右)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyNightFront), "(夜晚)天空盒纹理(前)",NULL,NULL,WAY_FILENAME,"Textures\\sky");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strSkyNightBack), "(夜晚)天空盒纹理(后)",NULL,NULL,WAY_FILENAME,"Textures\\sky");

		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_nBoxAreaType), "天空盒类型");
		m_pProperty->DynAddProperty(AVariant(m_nObjectID), "场景对象ID（海外版专用）", NULL,NULL,WAY_READONLY);

		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fCapTile), "天空纹理重复次数");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fCapRot), "天空纹理旋转速度");


		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_nPriority), "优先级");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_strColorMappingTex), "颜色映射纹理",NULL,NULL,WAY_FILENAME,"Textures");

		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHeightStart), "(白天)垂直雾起始高度");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHeightEnd), "(白天)垂直雾结束高度");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogDeepness), "(白天)垂直雾浓度");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogDensityStart), "(白天)水平雾浓度(上半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogDensityEnd), "(白天)水平雾浓度(下半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogColorDensityStart), "(白天)颜色亮度(上半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogColorDensityEnd), "(白天)颜色亮度(下半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwFogColor), "(白天)水平雾颜色(上半)",NULL,NULL,WAY_COLOR);
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwFogColor2), "(白天)水平雾颜色(下半)",NULL,NULL,WAY_COLOR);
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHorzeDensity), "(白天)水平雾系数");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHorzePower), "(白天)水平雾过渡");

		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHeightStartNight), "(夜晚)垂直雾起始高度");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHeightEndNight), "(夜晚)垂直雾结束高度");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogDeepnessNight), "(夜晚)垂直雾浓度");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogDensityStartNight), "(夜晚)水平雾浓度(上半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogDensityEndNight), "(夜晚)水平雾浓度(下半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogColorDensityStartNight), "(夜晚)颜色亮度(上半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogColorDensityEndNight), "(夜晚)颜色亮度(下半)");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwFogColorNight), "(夜晚)水平雾颜色(上半)",NULL,NULL,WAY_COLOR);
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwFogColor2Night), "(夜晚)水平雾颜色(下半)",NULL,NULL,WAY_COLOR);
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHorzeDensityNight), "(夜晚)水平雾系数");
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fFogHorzePowerNight), "(夜晚)水平雾过渡");

		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwSunShineColor), "(白天)阳光散射颜色",NULL,NULL,WAY_COLOR);
#ifdef _ANGELICA22
		static ASliderRangeTemplate<float> s_BloomRange(0, 1, 0.1f);
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fBloomThredhold), "(白天)泛光阈值", NULL, &s_BloomRange,WAY_SLIDER);
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_dwSunShineColorNight), "(夜晚)阳光散射颜色",NULL,NULL,WAY_COLOR);
		m_pProperty->DynAddProperty(AVariant(m_PropertyData.m_fBloomThredholdNight), "(夜晚)泛光阈值", NULL, &s_BloomRange,WAY_SLIDER);
#endif
	}
}
...
//光线跟踪程序
bool CSceneBoxArea::RayTrace(const A3DVECTOR3& vStart,const A3DVECTOR3& vEnd)
{
	//首先投影8个顶点到2D屏幕空间
...
bool CSceneBoxArea::IsInside(const A3DVECTOR3& vPoint)
{
...
bool CSceneBoxArea::DotWithPlane(const A3DVECTOR3& testPoint,const A3DVECTOR3& point1, const A3DVECTOR3& point2, const A3DVECTOR3& point3)
{
	A3DVECTOR3 Normal;
	//计算法线
...
void CSceneBoxArea::UpdateMapParams(CElementMap * pMap, float fInvTime)
{
	pMap->m_FogColor.SetTarget(ColorRGBAToColorValue(m_PropertyData.m_dwFogColor), fInvTime);
	pMap->m_FogStart.SetTarget(m_PropertyData.m_fFogStart, fInvTime);
	pMap->m_FogEnd.SetTarget(m_PropertyData.m_fFogEnd, fInvTime);
	...
	pMap->m_FogHeightStart.SetTarget(m_PropertyData.m_fFogHeightStart, fInvTime);
	pMap->m_FogHeightEnd.SetTarget(m_PropertyData.m_fFogHeightEnd, fInvTime);
	pMap->m_FogDensityStart.SetTarget(m_PropertyData.m_fFogDensityStart, fInvTime);
	pMap->m_FogDensityEnd.SetTarget(m_PropertyData.m_fFogDensityEnd, fInvTime);
	pMap->m_FogColorDensityStart.SetTarget(m_PropertyData.m_fFogColorDensityStart, fInvTime);
	pMap->m_FogColorDensityEnd.SetTarget(m_PropertyData.m_fFogColorDensityEnd, fInvTime);
	pMap->m_FogHorzeDensity.SetTarget(m_PropertyData.m_fFogHorzeDensity, fInvTime);
	pMap->m_FogHorzePower.SetTarget(m_PropertyData.m_fFogHorzePower, fInvTime);
	pMap->m_FogColor2.SetTarget(m_PropertyData.m_dwFogColor2, fInvTime);

	pMap->m_FogHeightStartNight.SetTarget(m_PropertyData.m_fFogHeightStartNight, fInvTime);
	pMap->m_FogHeightEndNight.SetTarget(m_PropertyData.m_fFogHeightEndNight, fInvTime);
	pMap->m_FogDensityStartNight.SetTarget(m_PropertyData.m_fFogDensityStartNight, fInvTime);
	pMap->m_FogDensityEndNight.SetTarget(m_PropertyData.m_fFogDensityEndNight, fInvTime);
	pMap->m_FogColorDensityStartNight.SetTarget(m_PropertyData.m_fFogColorDensityStartNight, fInvTime);
	pMap->m_FogColorDensityEndNight.SetTarget(m_PropertyData.m_fFogColorDensityEndNight, fInvTime);
	pMap->m_FogHorzeDensityNight.SetTarget(m_PropertyData.m_fFogHorzeDensityNight, fInvTime);
	pMap->m_FogHorzePowerNight.SetTarget(m_PropertyData.m_fFogHorzePowerNight, fInvTime);
	pMap->m_FogColor2Night.SetTarget(m_PropertyData.m_dwFogColor2Night, fInvTime);
	...
	pMap->m_BloomThredhold.SetTarget(m_PropertyData.m_fBloomThredhold, fInvTime * 2);
	pMap->m_BloomThredholdNight.SetTarget(m_PropertyData.m_fBloomThredholdNight, fInvTime * 2);
}
...
{ 
	...
	if(dwVersion == 2)
	{
		...
		if(!ar.Read(&m_PropertyData.m_dwAmbient,sizeof(DWORD)))
			return false;
		if(!ar.Read(&m_PropertyData.m_dwFogColor,sizeof(DWORD)))
			return false;
	...
	}else if(dwVersion>0xe && dwVersion<=0xf)
	{
		...
		if(!ar.Read(&m_PropertyData.m_fFogDeepnessNight,sizeof(float)))
			return false;
		if(!ar.Read(&m_PropertyData.m_dwFogColorNight,sizeof(DWORD)))
			return false;
	...
	}if( dwVersion == 0x28 )
	{
		...
		m_matAbs = m_matScale * m_matRotate * m_matTrans;
		ReBuildArea();
	}if( dwVersion > 0x28 && dwVersion <= 0x42)
	{
		m_strName = AString(ar.ReadString());
		if(!ar.Read(&m_nObjectID,sizeof(int)))
			return false;
	...
	if( dwVersion > 0x42)
	{
		...
		if(!ar.Read(&m_PropertyData.m_StartTime,sizeof(TIME_CTR)))
			return false;
		if(!ar.Read(&m_PropertyData.m_EndTime,sizeof(TIME_CTR)))
			return false;
		...
		if(dwVersion > 0x48)
		{
			if(!ar.Read(&m_PropertyData.m_nBoxAreaType,sizeof(int)))
				return false;
			
			m_PropertyData.m_strSkyTop = AString(ar.ReadString());
			m_PropertyData.m_strSkyBottom = AString(ar.ReadString());
			m_PropertyData.m_strSkyLeft = AString(ar.ReadString());
			m_PropertyData.m_strSkyRight = AString(ar.ReadString());
			m_PropertyData.m_strSkyFront = AString(ar.ReadString());
			m_PropertyData.m_strSkyBack = AString(ar.ReadString());

			m_PropertyData.m_strSkyNightTop = AString(ar.ReadString());
			m_PropertyData.m_strSkyNightBottom = AString(ar.ReadString());
			m_PropertyData.m_strSkyNightLeft = AString(ar.ReadString());
			m_PropertyData.m_strSkyNightRight = AString(ar.ReadString());
			m_PropertyData.m_strSkyNightFront = AString(ar.ReadString());
			m_PropertyData.m_strSkyNightBack = AString(ar.ReadString());
		}

		if (dwVersion >= 0x4d)
		{
			if(!ar.Read(&m_PropertyData.m_fCapTile,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fCapRot,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_nPriority,sizeof(int)))
				return false;

			m_PropertyData.m_strColorMappingTex = AString(ar.ReadString());

			if(!ar.Read(&m_PropertyData.m_fFogHeightStart,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogHeightEnd,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogDensityStart,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogDensityEnd,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogColorDensityStart,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogColorDensityEnd,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogHorzeDensity,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogHorzePower,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_dwFogColor2,sizeof(DWORD)))
				return false;

			if(!ar.Read(&m_PropertyData.m_fFogHeightStartNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogHeightEndNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogDensityStartNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogDensityEndNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogColorDensityStartNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogColorDensityEndNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogHorzeDensityNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_fFogHorzePowerNight,sizeof(float)))
				return false;
			if(!ar.Read(&m_PropertyData.m_dwFogColor2Night,sizeof(DWORD)))
				return false;
		}
...
{ 
	...
	{//2.0版本开放
		...
		if(!ar.Write(&m_PropertyData.m_dwFogColor,sizeof(DWORD)))
			return false;
	...
	{//3.0版本开放
		...
		if(!ar.Write(&m_PropertyData.m_fFogDeepnessNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_dwFogColorNight,sizeof(DWORD)))
			return false;
		//Nature
		...
		if(!ar.Write(&m_PropertyData.m_StartTime,sizeof(TIME_CTR)))
			return false;
		if(!ar.Write(&m_PropertyData.m_EndTime,sizeof(TIME_CTR)))
			return false;
		...
		// sky box area type
		if(!ar.Write(&m_PropertyData.m_nBoxAreaType,sizeof(int)))
			return false;
		
		ar.WriteString(m_PropertyData.m_strSkyTop);
		ar.WriteString(m_PropertyData.m_strSkyBottom);
		ar.WriteString(m_PropertyData.m_strSkyLeft);
		ar.WriteString(m_PropertyData.m_strSkyRight);
		ar.WriteString(m_PropertyData.m_strSkyFront);
		ar.WriteString(m_PropertyData.m_strSkyBack);

		ar.WriteString(m_PropertyData.m_strSkyNightTop);
		ar.WriteString(m_PropertyData.m_strSkyNightBottom);
		ar.WriteString(m_PropertyData.m_strSkyNightLeft);
		ar.WriteString(m_PropertyData.m_strSkyNightRight);
		ar.WriteString(m_PropertyData.m_strSkyNightFront);
		ar.WriteString(m_PropertyData.m_strSkyNightBack);

		if(!ar.Write(&m_PropertyData.m_fCapTile,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fCapRot,sizeof(float)))
			return false;

		if(!ar.Write(&m_PropertyData.m_nPriority,sizeof(int)))
			return false;

		ar.WriteString(m_PropertyData.m_strColorMappingTex);

		if(!ar.Write(&m_PropertyData.m_fFogHeightStart,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogHeightEnd,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogDensityStart,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogDensityEnd,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogColorDensityStart, sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogColorDensityEnd,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogHorzeDensity,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogHorzePower,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_dwFogColor2,sizeof(DWORD)))
			return false;

		if(!ar.Write(&m_PropertyData.m_fFogHeightStartNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogHeightEndNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogDensityStartNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogDensityEndNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogColorDensityStartNight, sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogColorDensityEndNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogHorzeDensityNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_fFogHorzePowerNight,sizeof(float)))
			return false;
		if(!ar.Write(&m_PropertyData.m_dwFogColor2Night,sizeof(DWORD)))
			return false;
...
void CSceneBoxArea::Release()
{
	...
	if(m_pStartTimeFunc) delete m_pStartTimeFunc;
	if(m_pEndTimeFunc) delete m_pEndTimeFunc;
...
BOOL CALLBACK START_TIME_SET_FUNCS::OnActivate(void)
{
	m_strText = "...";
	CSceneBoxArea* pObj = NULL;
	CString strID;
	...
	if(pMap)
	{
		...
		if(pManager->m_listSelectedObject.GetCount()==1)
		{
			...
			if(pObj->GetObjectType()==CSceneObject::SO_TYPE_AREA)
			{
				...
				CImportTimeDlg dlg;
				dlg.m_Time = pro.m_StartTime;
				if(IDOK==dlg.DoModal())
				{
					pro.m_StartTime = dlg.m_Time;
					pObj->SetProperty(pro);
					m_strText = "...";
				}
			}
...
LPCTSTR CALLBACK START_TIME_SET_FUNCS::OnGetShowString(void) const
{
	return m_strText;
}

AVariant CALLBACK START_TIME_SET_FUNCS::OnGetValue(void) const
{
	return m_var;
}

void CALLBACK START_TIME_SET_FUNCS::OnSetValue(const AVariant& var)
{
	m_strText = "...";
}


BOOL CALLBACK END_TIME_SET_FUNCS::OnActivate(void)
{
	m_strText = "...";
	CSceneBoxArea* pObj = NULL;
	CString strID;
	...
	if(pMap)
	{
		...
		if(pManager->m_listSelectedObject.GetCount()==1)
		{
			...
			if(pObj->GetObjectType()==CSceneObject::SO_TYPE_AREA)
			{
				...
				CImportTimeDlg dlg;
				dlg.m_Time = pro.m_EndTime;
				if(IDOK==dlg.DoModal())
				{
					pro.m_EndTime = dlg.m_Time;
					pObj->SetProperty(pro);
					m_strText = "...";
				}
			}
...
LPCTSTR CALLBACK END_TIME_SET_FUNCS::OnGetShowString(void) const
{
	return m_strText;
}

AVariant CALLBACK END_TIME_SET_FUNCS::OnGetValue(void) const
{
	return m_var;
}

void CALLBACK END_TIME_SET_FUNCS::OnSetValue(const AVariant& var)
{
	m_strText = "...";
}
