...
void CNpcSelDlg::FreshList()
{
	...
	if(m_pSceneAIGenerator)
	{
		...
		for(int i = 0; i < num; i++)
		{
			...
			m_listAI.AddString(pProperty->strNpcName);
		}
	}
}
...
AString CNpcSelDlg::GetNpcName(DWORD id)
{
	...
	if(pData) strName = WC2AS((const wchar_t*)pData->name);
	return  strName;
}
...
void CNpcSelDlg::OnModifyAi() 
{
	...
	NPC_PROPERTY* pro = m_pSceneAIGenerator->GetNPCProperty(AString(name));
