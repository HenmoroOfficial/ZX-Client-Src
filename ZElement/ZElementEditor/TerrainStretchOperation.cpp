...
void CTerrainStretchOperation::UpdateTerrainHeight(int delta)
{
	...
	if(2==m_nStretchType || 3 == m_nStretchType) 
	{
...
bool CTerrainStretchOperation::OnWTPacket(int x, int y, DWORD button, DWORD param)
{
	...
	if(pMap)
	{
		...
		if(pTR)
		{
			...
			if(HIWORD(button)==TBN_DOWN)
			{
				if(m_bHeightAdd) UpdateTerrainHeight(int(-(param/10.0f)));
				else UpdateTerrainHeight(int(param/10.0f));
				return false;
			}
		}
	}
