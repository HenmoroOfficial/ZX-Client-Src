	...
	if(pMap)
	{
		...
		if(pSManager) 
		{
			...
			if(pModel)
			{
				...
				pModel->SetShadow(m_Model.IsShadow());
#endif
...
///////////////////////////////////////////////////////////////////////////////////////
//class CGuildBuildingOperation

CGuildBuildingOperation::CGuildBuildingOperation()
{
}

CGuildBuildingOperation::~CGuildBuildingOperation()
{
	m_GuildBuilding.Release();
}

bool CGuildBuildingOperation::OnLButtonDown(int x, int y, DWORD dwFlags)
{	
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap)
	{
		...
		if(!pSManager->GetTracePos(vStart,vPos,TraceRt.vPoint)) return true;
		AUX_GetMainFrame()->GetToolTabWnd()->UpdatePropertyData(true);//读属性数据
		m_GuildBuilding.UpdateProperty(true);
		CSceneGuildBuilding* pNew = pSManager->CreateGuildBuilding(m_GuildBuilding.GetObjectName());
		if(pNew)
		{
			EC_GUILDBUILDING_PROPERTY data = m_GuildBuilding.GetProperty();
...
bool CGuildBuildingOperation::OnLButtonUp(int x, int y, DWORD dwFlags)
{
	return true;
}

bool CGuildBuildingOperation::OnMouseMove(int x, int y, DWORD dwFlags)
{
	return true;
}

void CGuildBuildingOperation::CreateGuildBuilding()
{
	int Num = 0;
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap)
	{
		...
		while(1)
		{
			name.Format("GuildBuilding_%d",Num);
			if(pSManager->FindSceneObject(name)!=NULL)
			{
				Num++;
			}else 
		...
		m_GuildBuilding.SetObjectName(name);
		EC_GUILDBUILDING_PROPERTY data;
		memset(&data, 0, sizeof(EC_GUILDBUILDING_PROPERTY));
		m_GuildBuilding.SetProperty(data);
		m_GuildBuilding.UpdateProperty(false);
		AUX_GetMainFrame()->GetToolTabWnd()->ShowPropertyPannel(m_GuildBuilding.m_pProperty);
	}
}

void CGuildBuildingOperation::Render(A3DViewport* pA3DViewport)
{
}
