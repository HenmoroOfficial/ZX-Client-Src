...
void CObstructOperation::DrawObstruct()
{
	...
	if(pMap)
	{
		...
		float fFraction = 1.0f;
		// we do the exact RayTrace, that is we trace the mesh of the building!
		bool bTraceBuilding = pMan->RayTraceStaticModel(vCameraPos,99999.0f*vCameraDir,vecHitPos,&fFraction,vecNormal,pTraceCHData);
		pTerrain->RayTrace(vCameraPos, 99999.0f*vCameraDir, 1.0f, &rt);
		A3DVECTOR3 cPos=rt.vHitPos;
		A3DVECTOR3 temp = vecHitPos;
		temp = temp - vCameraPos;
		float dis = temp.MagnitudeH();
		temp = cPos - vCameraPos;
		if(dis < temp.MagnitudeH()) cPos = vecHitPos; 
				...
				for( int j = -nShowTitle; j < nShowTitle; j++)
				{
					...
					if( v>=0 && v < nTotal)
					{
						...
						if(true)
						{
							...
							FIX16 idh= pMap->GetPassMapGen()->GetDeltaHeightMap()[col* OBS_MAP_WIDTH + row];
							//*********************************
							// revised by wenfeng, 06-1-13
							if(idh)
							{
							...
							DrawGrid(leftup,rightup,rightbottom,leftbottom, pData[col* OBS_MAP_WIDTH + row], idh);
						}
					}
				}
			}
