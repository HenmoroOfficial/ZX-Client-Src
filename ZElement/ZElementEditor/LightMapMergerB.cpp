...
//	Do export
bool CLightMapMergerB::DoExport(CLightMapMerger* pMerger, int iLMType)
{
	m_pm = pMerger;
	m_iLMType = iLMType;
...
bool CLightMapMergerB::ExportProject(int iProject)
{
	...
	CString strFile;
	strFile.Format(szLMSrcPath[m_iLMType], g_szWorkDir, g_szEditMapDir, strProject, strProject);
	...
	else
	{
		...
		{
			AUX_ProgressLog("CLightMapMergerB::ExportProject, project (%d) %s file identity or version error", iProject, m_iLMType == 2 ? "colormap" : "lightmap");
			bHasLightmap = false;
			goto DoExport;
		}
...
DoExport:
	...
	else
	{
		if (!bHasLightmap)
		{
			...
			AUX_ProgressLog("CLightMapMergerB::ExportProject, couldn't open lightmap type: %d", m_iLMType);
			return false;
		}
	}
	...
	int iPixelBit = 3;
	if(m_iLMType == LM_MANUAL) iPixelBit = 4;
	if (!bHasLightmap || !CLightMapMerger::ReadLightmapData(m_pLMBmp, pLMFile, iNumMaskArea, m_iMaskLMSize,iPixelBit))
...
bool CLightMapMergerB::GenProjectLightmap(int iProject)
{
	...
	sprintf(szFile, szLMDesPath[m_iLMType], m_pm->m_strDestDir, m_pm->m_strName, *m_pm->m_aProjNames[iProject]);
	...
	for (r=0; r < m_pm->m_InitData.iProjMaskRow; r++)
	{
		...
		for (c=0; c < m_pm->m_InitData.iProjMaskCol; c++, iMaskArea++)
		{
			...
			/*//	test code ...
			char szTemp[256];
			sprintf(szTemp, "d:\\%d.dds", iMaskArea);
