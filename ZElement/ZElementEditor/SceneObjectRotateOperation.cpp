...
bool CSceneObjectRotateOperationEx::OnMouseMove(int x, int y, DWORD dwFlags)
{
	...
	if(pMap)
	{
		...
		while( pos )
		{
			...
			case  ROTATE_X: 
				_Delta(ptemp,deltaY,vCenter,ROTATE_X);
				break;
			case  ROTATE_Y: 
				_Delta(ptemp,deltaY,vCenter,ROTATE_Y);
				break;
			case  ROTATE_Z: 
				_Delta(ptemp,deltaY,vCenter,ROTATE_Z);
				break;
...
bool CSceneObjectRotateOperationEx::OnLButtonUp(int x, int y, DWORD dwFlags)
{
	...
	AUX_GetMainFrame()->SetXValue(0);
	AUX_GetMainFrame()->SetYValue(0);
	AUX_GetMainFrame()->SetZValue(0);
	return true;
}
...
void _Delta(CSceneObject* pTemp, float fDelta, A3DVECTOR3 vCenter, int nAxis)
{
