...
struct PRECINCT_DATA
{
	...
	// added 2012-12-17 by ice
	int		nFeishengCnt;	// 地图限制人物飞升次数
	int		nAllowLevelMax;	// 高于此等级传出地图
	int		nAllowLevelMin;	// 低于此等级传出地图
	int		nKickCity;		// 等级不符合时限制回城地图
	A3DVECTOR3 vKickCityPoint;	// 等级不符合时限制回城坐标
...
public:
	...
	void			SetProperty(const PRECINCT_DATA & dat) { m_Data = dat; };
