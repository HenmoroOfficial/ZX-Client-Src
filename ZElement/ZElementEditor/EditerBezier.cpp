...
CEditerBezier::CEditerBezier()
{
	...
	m_float_pointradius = 0.2f;
	m_float_segradius	= 0.2f;
	m_float_anchorsize	= 0.2f;
	...
	loop_type.AddElement("非循环",0);
	loop_type.AddElement("首尾循环",1);
	loop_type.AddElement("原路返回",2);

	bezier_type2.AddElement("一般路径",0);
	bezier_type2.AddElement("摄像机路径",1);
	bezier_type2.AddElement("角色路径",2);
...
CEditerBezierSegment* CEditerBezier::GetTailSeg()
{
	return (CEditerBezierSegment*)m_list_segments.GetTail();
}
...
{
	//if(!IsShowCopy(m_nCopy)) return;	
	CSceneObject::Render(pViewport);

}

void CEditerBezier::PostRender(A3DViewport* pA3DViewport)
{
	ASSERT(pA3DViewport);
	if(!m_bTestStart)
	{
		...
		while(pos)
		{
			...
			if(m_bIsBezier) pSeg->Render(pA3DViewport, m_bSelected);
			else pSeg->Render(pA3DViewport, false);
		}
	...
	if(bGet)
	{
		...
		if(pMap)
		{
			...
			if(pObj != NULL && pObj != this)
			{
				...
				m_pProperty->SetPropVal(8,m_nLoopType);
				m_pProperty->SetPropVal(9,m_nBezierType);
				m_pProperty->SetPropVal(10,m_bPassmapAvail);
				//m_pProperty->SetPropVal(11,m_nCopy);
				m_pProperty->SetPropVal(11,m_nObjectID);
		...
		m_nLoopType = m_pProperty->GetPropVal(8);
		m_nBezierType = m_pProperty->GetPropVal(9);
		m_bPassmapAvail = m_pProperty->GetPropVal(10);
...
void CEditerBezier::BuildBezierProperty()
{
	if(m_pProperty)
	{
		...
		m_pProperty->DynAddProperty(AVariant(m_nGlobalID), "曲线ID",NULL,NULL,WAY_READONLY);
		...
		m_pProperty->DynAddProperty(AVariant((int)m_nLoopType), "循环类型", &loop_type);
		m_pProperty->DynAddProperty(AVariant((int)m_nBezierType), "曲线类型", &bezier_type2);
		m_pProperty->DynAddProperty(AVariant(m_bPassmapAvail), "通过图是否有效");
		m_pProperty->DynAddProperty(AVariant(m_nObjectID), "场景对象ID（海外版专用）", NULL,NULL,WAY_READONLY);
		//m_pProperty->DynAddProperty(AVariant((int)m_nCopy), "第几套数据");
...
void CEditerBezier::SelectedPoint(int ptid )
{
	if(ptid >= 0 && ptid < m_list_points.GetCount())
		m_nCurPoint = ptid;
}
...
//	pNewObject->BuildBezierProperty();
	...
	DWORD color = A3DCOLORRGB(r,g,b);
	pNewObject->SetProperty(color);
	...
	if(pMap)
	{
		...
		A3DVECTOR3 position = m_vPos;
		position.y += 10.0f;
		pNewObject->SetPos(position);
		...
		if(g_Configs.iIsOverseaVersion == 1)
		{
			...
			pNewObject->SetObjectID(dlgID.m_nID);
			pNewObject->SetGlobalID(dlgID.m_nGlobalID);
		}
		else
		{
			pNewObject->m_nObjectID = pSManager->GenerateObjectID();

			int id = GenerateGlobalID();
			if(id==-1) 
			{
				delete pNewObject;
				return NULL;
			...
			pNewObject->SetGlobalID(id);
		}
		pNewObject->m_bIsBezier = m_bIsBezier;
		pNewObject->m_fLenght = m_fLenght;
		pNewObject->m_nLoopType = m_nLoopType;
		pNewObject->m_nBezierType = m_nBezierType;
		pNewObject->m_bPassmapAvail = m_bPassmapAvail;
...
bool CEditerBezier::Load(CELArchive& ar,DWORD dwVersion,int iLoadFlags) 
{
	if(dwVersion < 0x46)
	{
		...
		m_float_pointradius = 0.1f;
		m_float_anchorsize  = 0.05f;
		m_float_segradius   = 0.05f;
		//读点的数据
		int Num;
		int ID;
		A3DVECTOR3 temp;
	...
	}else if( dwVersion < 0x48)
	{
		...
		if(!ar.Read(&m_bIsBezier,sizeof(bool)))
			return false;
		if(!ar.Read(&m_bPassmapAvail,sizeof(bool)))
			return false;

		m_float_pointradius = 0.2f;
		m_float_anchorsize  = 0.1f;
		m_float_segradius   = 0.1f;
		//读点的数据
		int Num;
		int ID;
		A3DVECTOR3 temp;
	...
	else
	{
		...
		if(!ar.Read(&m_nCopy,sizeof(int)))
			return false;
		if(!ar.Read(&m_nLoopType,sizeof(int)))
			return false;
		if(!ar.Read(&m_nBezierType,sizeof(int)))
			return false;
		if(!ar.Read(&m_bPassmapAvail,sizeof(bool)))
			return false;
		...
		for(k=0; k<Num; k++)
		{
			...
			if(!ar.Read(&temp,sizeof(A3DVECTOR3)))
				return false;
			if(!ar.Read(&dir,sizeof(A3DVECTOR3)))
				return false;
			if(!ar.Read(&fspeed,sizeof(float)))
				return false;
			if(!ar.Read(&ffov,sizeof(float)))
				return false;
			if(!ar.Read(&fRatio,sizeof(float)))
				return false;
			//if(!ar.Read(&fRadius,sizeof(float)))
			//	return false;
			
			CEditerBezierPoint *pNewPt = new CEditerBezierPoint();
			pNewPt->Init(m_ptr_a3ddevice, A3DVECTOR3(temp.x, temp.y, temp.z), A3DCOLORRGBA(255,0,0,255), m_float_pointradius);
			pNewPt->SetID(ID);
			pNewPt->SetDirection(dir);
			pNewPt->SetSpeed( fspeed );
			pNewPt->SetFov(ffov);
			pNewPt->SetRatio(fRatio);
			//pNewPt->SetSpotRadius(fRadius);
	...
	if(dwVersion <= 0x40) 
	{
		m_nGlobalID = GenerateGlobalID();
		if(m_nGlobalID==-1) 
		{
			g_Log.Log("Generate global id for bezier faild!");
			return false;
		}
	}
...
void CEditerBezier::ConvertToObstruct()
{
	...
	//CTerrainObstruct *pObstruct = pSManager->GetTerrainObstruct();
	//pObstruct->AddBezier(this,false);
...
bool CEditerBezier::Save(CELArchive& ar,DWORD dwVersion) 
{
	...
	{//2.0版本开放
		...
		//ar.WriteString(m_strController);
		...
		if(!ar.Write(&m_nCopy,sizeof(int)))
			return false;
		if(!ar.Write(&m_nLoopType,sizeof(int)))
			return false;
		if(!ar.Write(&m_nBezierType,sizeof(int)))
			return false;
		if(!ar.Write(&m_bPassmapAvail,sizeof(bool)))
			return false;
		...
		while(pos)
		{
			...
			if(!ar.Write(&temp,sizeof(A3DVECTOR3)))
				return false;
			float fData = ptemp->GetSpeed();
			if(!ar.Write(&fData,sizeof(float)))
				return false;
			fData = ptemp->GetFov();
			if(!ar.Write(&fData,sizeof(float)))
				return false;
			fData = ptemp->GetRatio();
			if(!ar.Write(&fData,sizeof(float)))
				return false;
			//fData = ptemp->GetSpotRadius();
			//if(!ar.Write(&fData,sizeof(float)))
			//	return false;
...
CEditerBezierPoint* CEditerBezier::GetPoint( DWORD id )
{
	POSITION pos = m_list_points.GetHeadPosition();
	while(pos)
	{
		CEditerBezierPoint *ptemp = (CEditerBezierPoint *)m_list_points.GetNext(pos);
		if(ptemp->GetID()==id) return ptemp;
	}
...
bool CEditerBezier::ExportBezier(AFile*pFile ,const A3DVECTOR3& vOffset)
{
	...
	//if(!pFile->Write(&m_bPassmapAvail,sizeof(bool),&dwRr)) return false;
	if(!pFile->Write(&m_nLoopType,sizeof(int),&dwRr)) return false;
	if(!pFile->Write(&m_nBezierType,sizeof(int),&dwRr)) return false;
	
	//pFile->WriteString(m_strController);
	...
	while(pos)
	{
		...
		if(!pFile->Write(&temp,sizeof(A3DVECTOR3),&dwRr))
			return false;
		float fa = ptemp->GetSpeed();
		if(!pFile->Write(&fa,sizeof(float),&dwRr))
			return false;
		fa = ptemp->GetRatio();
		if(!pFile->Write(&fa,sizeof(float),&dwRr))
			return false;
...
//	if(0==fwrite(&m_bPassmapAvail,sizeof(bool),1,pFile)) return false;
	if(0==fwrite(&m_nLoopType,sizeof(int),1,pFile)) return false;
	if(0==fwrite(&m_nBezierType,sizeof(int),1,pFile)) return false;
	...
	if (iLen) fwrite((const char*)m_strActionName, 1, iLen, pFile);

	//iLen = m_strController.GetLength();
	//fwrite(&iLen, 1, sizeof (int), pFile);
	//if (iLen) fwrite((const char*)m_strController, 1, iLen, pFile);
	...
	while(pos)
	{
		...
		float fa = ptemp->GetSpeed();
		if(0==fwrite(&fa,sizeof(float),1,pFile)) return false;
		fa = ptemp->GetRatio();
		if(0==fwrite(&fa,sizeof(float),1,pFile)) return false;
		fa = ptemp->GetFov();
		if(0==fwrite(&fa,sizeof(float),1,pFile)) return false;
...
bool CEditerBezier::ExportSevBezier(FILE *pFile, const A3DVECTOR3& vOffset)
{
	...
	if(0==fwrite(&m_nLoopType,sizeof(int),1,pFile)) return false;
	if(0==fwrite(&m_nBezierType,sizeof(int),1,pFile)) return false;
	//int iLen = m_strController.GetLength();
	//fwrite(&iLen, 1, sizeof (int), pFile);
	//if (iLen) fwrite((const char*)m_strController, 1, iLen, pFile);
	//	写点的数据
	int Num;
...
void CEditerBezier::GeneralECBezier(CECBezier *pECBezier)
{
	...
	while(pos)
	{
		CEditerBezierSegment *ptemp1 = (CEditerBezierSegment *)m_list_segments.GetNext(pos);
		ptemp1->ReCalculateLength();
		CECBezierSeg* seg = new CECBezierSeg;
