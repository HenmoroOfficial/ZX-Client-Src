...
CMapBlockView::CMapBlockView()
{
	...
	m_dwColorArray[BLOCK_MAX_TEXTURE_LEVEL] = RGB(0,0,255);
}
...
bool CMapBlockView::DrawBackBuffer(RECT* lprc)
{
	...
	int iBlkNumPerMaskArea = g_Configs.iTExpMaskTile/ g_Configs.iTExpBlkGrid;
	...
	for (y=m_iOffsetY; y < iMaxRow; y++)
	{
		...
		for (x=m_iOffsetX; x < iMaxCol; x++, iCount++)
		{
			...
			{
				for(int n = 0; n<iBlkNumPerMaskArea; n++)
				{
					unsigned long long temp;
					for( int xx = 0; xx < iBlkNumPerMaskArea; ++xx)
					{
						int idx = (y*iBlkNumPerMaskArea+n) * m_iNumCol * iBlkNumPerMaskArea + (x*iBlkNumPerMaskArea + xx);
			...
			int nColorLevel = nLevel;
			if(nColorLevel > BLOCK_MAX_TEXTURE_LEVEL) nColorLevel = BLOCK_MAX_TEXTURE_LEVEL + 1;
			hBrush = AUX_CreateBrush(BS_SOLID, m_dwColorArray[nColorLevel-1], 0);
...
int CMapBlockView::GetLevelNum(unsigned long long llFlag)
{
	int n = 0;
	for(int i = 0; i < 64; i++)
		if(((llFlag>>i)&1) == 1) n++;
...
BEGIN_MESSAGE_MAP(CTerrainBlockViewDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_DIG_HOLE, OnDigHole)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CTerrainBlockViewDlg::OnOK()
{
	...
	if(g_nCurrentTerrainBlock!=-1)
	{
		...
		{
			h = g_nCurrentTerrainBlock / (pTR->GetBlockColNum()/iBlkNumPerMaskArea);
			c = g_nCurrentTerrainBlock % (pTR->GetBlockColNum()/iBlkNumPerMaskArea);
			block = pTR->GetBlock(h*iBlkNumPerMaskArea,c*iBlkNumPerMaskArea);
		}
	...
	if(pMap)
	{
		...
		if(m_bShowSmallBlock)
		{
			...
			{//大块变小块
				...
				int h = g_nCurrentTerrainBlock / (pTR->GetBlockColNum()/iBlkNumPerMaskArea);
				int c = g_nCurrentTerrainBlock % (pTR->GetBlockColNum()/iBlkNumPerMaskArea);
				m_nCurrentBlock = h*iBlkNumPerMaskArea*pTR->GetBlockColNum() + c*iBlkNumPerMaskArea;
				g_nCurrentTerrainBlock = m_nCurrentBlock;
				m_MapView.SetTileAndRow(pTR->GetBlockRowNum()*pTR->GetBlockColNum(),pTR->GetBlockColNum());
				SetWindowText("Block区域");
			}
		}else 
		{
			if(bOldState)
			{//小块变大块
				...
				m_nCurrentBlock = (h/iBlkNumPerMaskArea) * iBlkNumPerMaskArea + c/iBlkNumPerMaskArea;
				g_nCurrentTerrainBlock = m_nCurrentBlock;
				m_MapView.SetTileAndRow(pTR->GetBlockRowNum()*pTR->GetBlockColNum()/(iBlkNumPerMaskArea*iBlkNumPerMaskArea),pTR->GetBlockColNum()/iBlkNumPerMaskArea);
				SetWindowText("Mask区域");
			}
		}
		//m_nCurrentBlock = -1;
...
void CTerrainBlockViewDlg::FreshLayBox()
{
	...
	{
		h = m_MapView.GetSelectedBlock() / (pTR->GetBlockColNum()/iBlkNumPerMaskArea);
		c = m_MapView.GetSelectedBlock() % (pTR->GetBlockColNum()/iBlkNumPerMaskArea);
	...
	}else
	{
		...
		for(n = 0; n<iBlkNumPerMaskArea; n++)
		{
			A3DTerrain2Block* block;

			for (int xx = 0; xx < iBlkNumPerMaskArea; xx++)
				llFlags |= pMap->GetTerrain()->GetMaskModifier()->GetBlockFlags(h*iBlkNumPerMaskArea+n,c*iBlkNumPerMaskArea + xx);
...
void CTerrainBlockViewDlg::UpdateDlg()
{
	...
	if(pMap)
	{
		...
		if(m_bShowSmallBlock)
			m_MapView.SetTileAndRow(pTR->GetBlockRowNum()*pTR->GetBlockColNum(),pTR->GetBlockColNum());
		else m_MapView.SetTileAndRow(pTR->GetBlockRowNum()*pTR->GetBlockColNum()/(iBlkNumPerMaskArea*iBlkNumPerMaskArea),pTR->GetBlockColNum()/iBlkNumPerMaskArea);
		m_MapView.SetSelectedBlock(g_nCurrentTerrainBlock);
		FreshLayBox();
		
		if(m_bShowSmallBlock)
			SetWindowText("Block区域");
		else SetWindowText("Mask区域");
	...
	if(pLayer)
	{
		...
		}else
		{
			...
			for(n = 0; n < iBlkNumPerMaskArea; n++)
			{
				...
				for( int k = 0; k < iBlkNumPerMaskArea; ++k)
				{
					block = pTR->GetBlock(h*iBlkNumPerMaskArea + n,c*iBlkNumPerMaskArea + k);
...
void CTerrainBlockViewDlg::OnDigHole() 
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap == NULL) return;
	if ( g_nCurrentTerrainBlock==-1 )
	...
	int r = g_nCurrentTerrainBlock/16;
	int c = g_nCurrentTerrainBlock%16;
	pMap->ChangeTerrainHoleState(r,c);

	CTerrainRender* pRender = pMap->GetTerrain()->GetRender();
	A3DTerrain2Block* block = pRender->GetBlock(r, c);

	bool bState = pMap->GetTerrainHoleState(r, c);
	if(bState)
