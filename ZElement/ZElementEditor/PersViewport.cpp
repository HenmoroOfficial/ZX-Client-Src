...
bool CPersViewport::Init(CViewFrame* pViewFrame, int iWidth, int iHeight)
{
	...
	AMSoundEngine* pAMSoundEngine = g_Render.GetA3DEngine()->GetAMSoundEngine();
	m_pA3DCamera->SetAM3DSoundDevice(pAMSoundEngine->GetAM3DSoundDevice());
...
bool CPersViewport::FrameMove(DWORD dwDeltaTime)
{
	...
	if (m_pViewFrame->HasFocus() && !g_bLockCam && !g_Configs.bHostMode)
	{
		...
		if ( (GetAsyncKeyState('Q') & 0x8000) || (GetAsyncKeyState(VK_SPACE) & 0x8000) )
...
bool CPersViewport::OnWTPacket(int x, int y, DWORD press, DWORD param)
{
	//Send event to operation \xqf
	if(m_pViewFrame->GetCurrentOperation())
	{
		if(!(m_pViewFrame->GetCurrentOperation())->OnWTPacket(x,y,press,param))
			return false;
	}
	return true;
}
...
void CPersViewport::OnKeyDown(DWORD dwChar, DWORD dwRepCnt, DWORD dwFlags)
{
	if(m_pViewFrame->GetCurrentOperation())
	{
		m_pViewFrame->GetCurrentOperation()->OnKeyDown(dwChar, dwRepCnt,dwFlags);
	}
}
...
void CPersViewport::OnKeyUp(DWORD dwChar, DWORD dwRepCnt, DWORD dwFlags)
{
	if(m_pViewFrame->GetCurrentOperation())
	{
		m_pViewFrame->GetCurrentOperation()->OnKeyUp(dwChar, dwRepCnt,dwFlags);
	}
}

bool CPersViewport::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_pViewFrame->GetMap())
	{
		CHostPlayer *pHost = m_pViewFrame->GetMap()->GetHostPlayer();
		if (zDelta>0)
		{
			pHost->DeltaCamDist(3.0f);
		}
		else
			pHost->DeltaCamDist(-3.0f);
