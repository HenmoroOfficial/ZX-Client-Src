...
BOOL CTerrainPage::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
	...
	m_TerrainModifyColorMapPanel.Create(IDD_TERRAIN_MODIFY_COLORMAP, this);
	if(CreatePanel("颜色图设置", 0, &m_TerrainModifyColorMapPanel) < 0)
	{
		g_Log.Log("CTerrainPage::Create, Failed to create ColorMap Panel");
		return FALSE;
	}

	m_TerrainModifyColorSmoothPanel.Create(IDD_TERRAIN_MODIFY_COLOR_SMOOTH, this);
	if(CreatePanel("颜色图柔化", 0, &m_TerrainModifyColorSmoothPanel) < 0)
	{
		g_Log.Log("CTerrainPage::Create, Failed to create ColorMap Panel");
		return FALSE;
	}
