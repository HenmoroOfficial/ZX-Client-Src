...
void CDlgSelectFile::OnButtonCheckOut() 
{
	...
	CString dst,src;
	if(Aux_IsAllFilesCheckIn(m_strRelativePath.GetBuffer(0)))
	{
		dst.Format("%s%s\\%s",g_szWorkDir,g_szEditMapDir,m_strRelativePath);
		src.Format("%s\\MapsData\\%s\\%s",g_Configs.szServerPath,g_Configs.szCurProjectName,m_strRelativePath);
		if(!AUX_CopyFolder(dst,src)) return;
	...
	{
		dst.Format("地图部分文件已经取出! %s",g_VSS.GetLastCommandResult());
		MessageBox(dst);
	}
}
