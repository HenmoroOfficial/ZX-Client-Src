...
void CExpGameDataDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_EXP_HOLE, m_bExpHole);
	//}}AFX_DATA_MAP
}
...
void CExpGameDataDlg::OnOK() 
{
	...
	if(m_bLightMapTrans && m_bExpTerrain)
	{
		...
		trans.DoTrans();
/* 老地图用下面这块，计算相邻地图灯光阴影信息，新地图用新的光照算法，这块需要修改
...
*/	}
	...
	*/
	...
	dlge.m_strExpPath.Format("%s\\scenelights.dat",szPath);
	...
	if(m_bExpHole)
	{
		...
		{
			CString str;
			str.Format("%s\\%s.hole",szPath, szName);
			pMap->ExportTerrainHole(str);
		}
	}
...
BOOL CExpGameDataDlg::OnInitDialog() 
{
	...
	{
		if (g_Configs.iTExpMaskTile == m_aMaskTiles[i])
		{
