...
bool CLightMapMerger::DoExport()
{
	...
	else if (g_iTrnExport == 1)
	{
		...
		if (!ExpImp.DoExport(this, CLightMapMergerB::LM_DAY))
		{
			AUX_ProgressLog("CLightMapMerger::DoExport, Failed to export lightmap data");
			return false;
		}
		...
		if (!ExpImp.DoExport(this, CLightMapMergerB::LM_NIGHT))
		{
			AUX_ProgressLog("CLightMapMerger::DoExport, Failed to export lightmap data");
			return false;
		}

		AUX_ProgressLog("-- Export color map ...");
		if (!ExpImp.DoExport(this, CLightMapMergerB::LM_MANUAL))
		{
			AUX_ProgressLog("CLightMapMerger::DoExport, Failed to export manual light map data");
			return false;
		}
	}
	else
...
bool CLightMapMerger::ReadLightmapData(CELBitmap* pBmp, FILE* fp, int iMaskNum, int iMaskLMSize, int iPixelBit)
{
	ASSERT(pBmp);

	int iMaskLMDataLen = iMaskLMSize * iMaskLMSize * iPixelBit;
	...
	int iLineDataLen = iMaskLMSize * iPixelBit;
	...
	for (r=0; r < iMaskRow; r++)
	{
		...
		for (c=0; c < iMaskCol; c++, iMaskArea++)
		{
			...
			for (int i=0; i < LockInfo.iHeight; i++)
			{
				...
				for( int j = 0; j < LockInfo.iWidth; ++j)
				{
					*(pDst + j*3 + 0) = *(pSrc + j*iPixelBit + 0);
					*(pDst + j*3 + 1) = *(pSrc + j*iPixelBit + 1);
					*(pDst + j*3 + 2) = *(pSrc + j*iPixelBit + 2);
				}
				pSrc += iLineDataLen;
