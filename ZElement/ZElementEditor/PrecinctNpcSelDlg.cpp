...
BEGIN_MESSAGE_MAP(CPrecinctNpcSelDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_PRECINCT_MOVE_UP, OnPrecinctMoveUp)
	ON_BN_CLICKED(IDC_PRECINCT_MOVE_DOWN, OnPrecinctMoveDown)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CPrecinctNpcSelDlg::OnPrecinctMoveUp() 
{
	// TODO: Add your control notification handler code here
	int index = m_listPrecinctNpc.GetCurSel();
	if(m_pPrecinct==NULL || index == -1) return;
	if(m_pPrecinct->MoveUp(index))
	{
		FreshPrecinctList();
		m_listPrecinctNpc.SetCurSel(index-1);
	}
}

void CPrecinctNpcSelDlg::OnPrecinctMoveDown() 
{
	// TODO: Add your control notification handler code here
	int index = m_listPrecinctNpc.GetCurSel();
	if(m_pPrecinct==NULL || index == -1) return;
	if(m_pPrecinct->MoveDown(index))
	{
		FreshPrecinctList();
		m_listPrecinctNpc.SetCurSel(index+1);
	}
