...
CELBitmap* CLightMapTransition::LoadLightMap(int iProject,CString strProject,bool bNight)
{
	...
	else
	{
		...
		{
			AUX_ProgressLog("CLightMapTransition::LoadLightMap, project (%d) lightmap file is corrupt", iProject);
			goto failed;
		}

		if (IdVer.dwIdentify != T2LMFILE_IDENTIFY || IdVer.dwVersion > T2LMFILE_VERSION)
		{
			AUX_ProgressLog("CLightMapTransition::LoadLightMap, project (%d) lightmap file identity or version error", iProject);
			goto failed;
		}
		...
		else	//	IdVer.dwVersion >= 2
		{
			...
			{
				AUX_ProgressLog("CLightMapTransition::LoadLightMap, project (%d) lightmap is DDS format", iProject);
				goto failed;
			}
		...
		{
			AUX_ProgressLog("CLightMapTransition::ReadLightmapData, Read data error !");
			goto failed;
		}
...
bool CLightMapTransition::ReadLightmapData(FILE* fp, int iMaskNum, int iMaskLMSize,CELBitmap *pBitmap)
{
	...
	if((pBitmap->GetWidth() != iMaskCol * iMaskLMSize)  ||  (pBitmap->GetHeight() != iMaskRow * iMaskLMSize)) return false;
	...
	//ASSERT(pBitmap && pBitmap->GetWidth() == iMaskCol * iMaskLMSize && pBitmap->GetHeight() == iMaskRow * iMaskLMSize);
		...
		for (c=0; c < iMaskCol; c++)
		{
			...
			if(LockInfo2.pData)
			{
				...
				for( int i = 0; i < LockInfo1.iHeight; i ++)
				{
					...
					//memset(pDst + iLineDataLen - 3, 255, 3);
...
bool CLightMapTransition::WriteLightMap(CELBitmap *pBitmap, CString strProject,bool bNight)
{
	...
	BYTE* pTempRowData = new BYTE[pBitmap->GetWidth()*3];
	for (r=0; r < iMaskRow-1; r++)
	{
		rcLock.left		= 0;
		rcLock.right	= pBitmap->GetWidth();
		rcLock.top		= iMaskLMSize * (r + 1);
		rcLock.bottom	= rcLock.top + 1;
		pBitmap->LockRect(rcLock, &LockInfo);
		BYTE* pData = LockInfo.pData;
		memcpy(pTempRowData, pData, pBitmap->GetWidth()*3);
		pData += LockInfo.iPitch;
		for (int j=0; j<pBitmap->GetWidth()*3; ++j)
		{
			pTempRowData[j] = BYTE(((int)pData[j]+(int)pTempRowData[j])/2);
		}

		pData -= LockInfo.iPitch * (LockInfo.iHeight-1);

		for (int i=0; i < LockInfo.iHeight; i++)
		{
			memcpy(pData, pTempRowData, pBitmap->GetWidth()*3);
			pData += LockInfo.iPitch;
		}
	}
	delete []pTempRowData;

	for (c=0; c < iMaskCol-1; c++)
	{
		rcLock.left		= iMaskLMSize * (c + 1);;
		rcLock.right	= rcLock.left + 1;
		rcLock.top		= 0;
		rcLock.bottom	= pBitmap->GetHeight();
		pBitmap->LockRect(rcLock, &LockInfo);
		...
		for (int i=0; i < LockInfo.iHeight; i++)
		{
			for (int j=0; j<3; j++)
			{
				pData[j] = (BYTE)(((int)pData[j]+(int)pData[j+3])/2);
				pData[j+3] = pData[j];
			}
			pData += LockInfo.iPitch;
		}
