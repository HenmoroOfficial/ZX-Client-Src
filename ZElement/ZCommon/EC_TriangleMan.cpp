...
{
	// if CELBrushBuilding is for collided only, we don't need it to be added to triangle man
	if( pProvider->GetCollideOnly() || pProvider->GetBrushCount() == 0 )
		return;
...
{
	// if CELBrushBuilding is for collided only, we did not add it to triangle man
	if( pProvider->GetCollideOnly() || pProvider->GetBrushCount() == 0 )
		return true;
