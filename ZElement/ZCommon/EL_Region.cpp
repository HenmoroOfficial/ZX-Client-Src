...
///////////////////////////////////////////////////////////////////////////
//	
//	Implement CELBuffRegion
//	
///////////////////////////////////////////////////////////////////////////

CELBuffRegion::CELBuffRegion()
{
	m_nRegionTemplID = -1;
	m_nMonsterID = -1;
}
...
//	Load data from file
bool CELBuffRegion::Load(FILE* fp, int iVersion)
{
	int i, iNumPoint;
	float v[3];

	fread(&m_nRegionTemplID, sizeof (int), 1, fp);
	fread(&m_nMonsterID, sizeof (int), 1, fp);
...
bool CELTransportBox::Load(AWScriptFile* pFile, int iVersion)
{
	...
	if( iVersion >= 6)
		m_iLine = pFile->GetNextTokenAsInt(false);
	else m_iLine = 0;

	if( iVersion >= 7)
		m_iCtrlID = pFile->GetNextTokenAsInt(false);
	else m_iCtrlID = 0;
...
bool CELTransportBox::Load(FILE* fp, int iVersion)
{
	...
	if (iVersion >= 6)
		fread(&m_iLine, sizeof (int), 1, fp);
	else
		m_iLine = 0;

	if (iVersion >= 7)
		fread(&m_iCtrlID, sizeof (int), 1, fp);
	else
		m_iCtrlID = 0;
...
#ifdef _ELEMENTCLIENT
bool CELGuildBuilding::Load(AWScriptFile* pFile, int iVersion)
{
	if (iVersion >= 2)
	{
		if (!pFile->MatchToken(L"[GuildBuilding]", true))
			return false;
	}

	m_iTmplId = pFile->GetNextTokenAsInt(true);
	m_iWidht = pFile->GetNextTokenAsInt(false);
	m_iHeight = pFile->GetNextTokenAsInt(false);

	m_vPos.x = pFile->GetNextTokenAsFloat(true);
	m_vPos.y = pFile->GetNextTokenAsFloat(false);
	m_vPos.z = pFile->GetNextTokenAsFloat(false);

	m_vDir.x = pFile->GetNextTokenAsFloat(true);
	m_vDir.y = pFile->GetNextTokenAsFloat(false);
	m_vDir.z = pFile->GetNextTokenAsFloat(false);

	m_vUp.x = pFile->GetNextTokenAsFloat(true);
	m_vUp.y = pFile->GetNextTokenAsFloat(false);
	m_vUp.z = pFile->GetNextTokenAsFloat(false);
...
CELRegionSet::~CELRegionSet()
{
	...
	for (i=0;i < (int)m_aBuffRegions.size(); i++)
		delete m_aBuffRegions[i];

	for (i=0;i < (int)m_aGuildBuildings.size(); i++)
		delete m_aGuildBuildings[i];
}
...
CELBuffRegion* CELRegionSet::IsPointInBuffRegion(float x, float z)
{
	for (int i=0; i < (int)m_aBuffRegions.size(); i++)
	{
		CELBuffRegion* p = m_aBuffRegions[i];
		if (p->IsPointIn(x, z))
			return p;
	}
...
bool CELRegionSet::Load(const char* szFileName)
{
	...
	while (File.PeekNextToken(true))
	{
		...
		else if (!wcsicmp(File.m_szToken, L"[GuildBuilding]"))
		{
			CELGuildBuilding* pGuildBuilding = new CELGuildBuilding();
			if (!pGuildBuilding->Load(&File, iVersion))
				return false;
			m_aGuildBuildings.push_back(pGuildBuilding);
		}
		else
			return false;
	}
...
bool CELRegionSet::Load(const char* szFileName)
{
	...
	int iRegionCnt=0, iTransCnt=0, iBuffRegionCnt=0, iDataType;

	//	Read data
	while (iRegionCnt < Header.iNumRegion || iTransCnt < Header.iNumTrans || iBuffRegionCnt < Header.iNumBuffRegion)
	{
		...
		*/
		...
		{
			CELBuffRegion* pBuffRegion = new CELBuffRegion;
			if (!pBuffRegion->Load(fp, (int)dwVersion))
				return false;

			m_aBuffRegions.push_back(pBuffRegion);
