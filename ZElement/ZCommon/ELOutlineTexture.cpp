...
// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
int EXTERN_OUTLINE_TEX_LEVEL[OUTLINE_MAX_LEVEL*2] ={
	8,11,    //level 0
	4,6,     //level 1
	2,3,     //level 2
	1,2,     //level 3
...
bool CELOutlineTexture::Init(CELTerrainOutline2 * pOutline, int iLevel, int iSubIndex)
{
	...
	const int* OUTLINE_TEX_LEVEL = m_pOutline->GetOUTLINE_TEX_LEVEL();// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
	...
	case 0:
		...
		break;
	case 1:
		CELOutlineTextureCache::GetChildTex(1, m_iSubIndex, m_iTiles[0], m_pOutline->GetOUTLINE_TEX_LEVEL());// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
		break;
	case 2:
		{
			int child[4];
			CELOutlineTextureCache::GetChildTex(2, m_iSubIndex, child, m_pOutline->GetOUTLINE_TEX_LEVEL());// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
			for (int i = 0; i < 4; i++) {
				CELOutlineTextureCache::GetChildTex(1, child[i], m_iTiles[i], m_pOutline->GetOUTLINE_TEX_LEVEL());// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
			}
		}
		break;
	default:
...
CELOutlineTextureCache::CELOutlineTextureCache()
{
	...
	//const int* OUTLINE_TEX_LEVEL = m_pOutline->GetOUTLINE_TEX_LEVEL();
	//for (i = 0; i < OUTLINE_TEX_LEVEL[0] * OUTLINE_TEX_LEVEL[1]; i++) {
	//	m_pTiles[i] = NULL;
	//}
	memset(m_pTiles, 0, sizeof(CELOutlineQuadTree *)*88); // 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
}
...
void CELOutlineTextureCache::Release()
{
	...
	if (m_pOutline) // 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
	{
		const int* OUTLINE_TEX_LEVEL = m_pOutline->GetOUTLINE_TEX_LEVEL();
		for (i = 0; i < OUTLINE_TEX_LEVEL[0] * OUTLINE_TEX_LEVEL[1]; i++) {
			if (m_pTiles[i]) {
	...
	for (ih = iTopH; ih <= iBottomH; ih++) {
		for (iw = iLeftW; iw <= iRightW; iw++) {
			...
			if (RcOverlapRc(rcTileWorld, rcViewLevel0)) {
				...
				iLevel = 1;
				iTexSubIndex = GetParentTex(0, iSubIndex, m_pOutline->GetOUTLINE_TEX_LEVEL());// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
			}
			else if (RcOverlapRc(rcTileWorld, rcViewLevel2)) {
				iLevel = 2;
				iTexSubIndex = GetParentTex(0, iSubIndex, m_pOutline->GetOUTLINE_TEX_LEVEL());// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
				iTexSubIndex = GetParentTex(1, iTexSubIndex, m_pOutline->GetOUTLINE_TEX_LEVEL());// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
			}
			else{
				assert(0);
 ...
 *    ------------ 
 */
int CELOutlineTextureCache::GetChildTex(int iLevel, int iSubIndex, int iChild[], const int OUTLINE_TEX_LEVEL[])// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
{
...
int CELOutlineTextureCache::GetParentTex(int iLevel, int iSubIndex, const int OUTLINE_TEX_LEVEL[])// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
{
	if (iLevel < 0 || iSubIndex <= 0) {
		return 0;
	}
...
bool CELOutlineTextureCache::ActivateTile(CELOutlineTexture * pTex, int iSubIndex)
{
	...
	if (pTile == NULL) {
		...
		pTile->Init(m_pOutline->m_pA3DDevice, iSubIndex, m_pOutline->m_x, m_pOutline->m_z, m_pOutline->GetOUTLINE_TEX_LEVEL());// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
...
#ifdef _ANGELICA22
		hval = pDevice->StretchRect(pSurface2, &rect, pSurface1, &rect, A3DTEXF_POINT);
#else
