...
CELTerrainOutline2::CELTerrainOutline2()
{
	...
	memset(m_OUTLINE_TEX_LEVEL, 0, sizeof(int)*OUTLINE_MAX_LEVEL*2);// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
}
...
bool CELTerrainOutline2::Init(A3DDevice * pA3DDevice,  const char * szTexDir, const char * szTileDir, const char * szDetailTex, A3DTerrain2* pTerrain2, unsigned int nWTileCount, unsigned int nHTileCount)
{
	...
	m_OUTLINE_TEX_LEVEL[0] = nWTileCount;   //level0
	m_OUTLINE_TEX_LEVEL[1] = nHTileCount;
	m_OUTLINE_TEX_LEVEL[2] = (int)ceil(m_OUTLINE_TEX_LEVEL[0] *0.5f); //level1
	m_OUTLINE_TEX_LEVEL[3] = (int)ceil(m_OUTLINE_TEX_LEVEL[1] *0.5f);
	m_OUTLINE_TEX_LEVEL[4] = (int)ceil(m_OUTLINE_TEX_LEVEL[2] *0.5f);//level2
	m_OUTLINE_TEX_LEVEL[5] = (int)ceil(m_OUTLINE_TEX_LEVEL[3] *0.5f);
	m_OUTLINE_TEX_LEVEL[6] = (int)ceil(m_OUTLINE_TEX_LEVEL[4] *0.5f);//level3
	m_OUTLINE_TEX_LEVEL[7] = (int)ceil(m_OUTLINE_TEX_LEVEL[5] *0.5f);
	m_OUTLINE_TEX_LEVEL[8] = (int)ceil(m_OUTLINE_TEX_LEVEL[6] *0.5f);//level4
	m_OUTLINE_TEX_LEVEL[9] = (int)ceil(m_OUTLINE_TEX_LEVEL[7] *0.5f);
	...
	m_nWorldWidth = OUTLINE_TILE_WIDTH * m_OUTLINE_TEX_LEVEL[0];// 把全局数组OUTLINE_TEX_LEVEL修改为成员属性, 2013.4.26 wuweixin
	m_nWorldHeight = OUTLINE_TILE_HEIGHT * m_OUTLINE_TEX_LEVEL[1];
...
void CELTerrainOutline2::_RenderReplaceTerrain(A3DViewport* pViewport)
{
	...
	if( !bNeedDetailPass )
	{
		...
		m_pA3DDevice->SetTextureMatrix(iLayer, Scaling(220.0f, 220.0f, 220.0f));
	...
	else
	{
		...
		for (int i = 0; i < nMesh; i++) {
			...
			m_pA3DDevice->SetTextureTransformFlags(0, A3DTTFF_COUNT3);
			m_pA3DDevice->SetTextureMatrix(0, Scaling(220.0f, 220.0f, 220.0f));
