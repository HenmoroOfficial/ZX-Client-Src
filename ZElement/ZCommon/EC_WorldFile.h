...
//	Block information when 9 <= ECWDFILE_VERSION < 16
...
//	Box area information in BSD file. when ECBSDFILE_VERSION >= 17
struct ECBSDFILEBOXAREA17
{
	...
	float   fFogStart;		//	Fog start distance in daylight		// 距离开始 13.7 by fuchonggang
	float   fFogEnd;		//	Fog end distance in daylight		// 距离结束
	float   fFogDensity;	//	Fog density in daylight				// 垂直雾浓度
	DWORD   dwFogColor;		//	Fog color in daylight				// 水平雾上半球颜色
	float   fUWFogStart;	//	Under water fog start in daylight
	float   fUWFogEnd;		//	Under water fog end in daylight
	float   fUWFogDensity;	//	Under water fog density in daylight
	DWORD   dwUWFogCol;		//	Under water fog color in daylight
	...
	float   fGlbFogHeightStart;			// 垂直雾高度开始
	float   fGlbFogHeightEnd;			// 垂直雾高度结束
	float   fGlbFogDensityStart;		// 上半球浓度
	float   fGlbFogDensityEnd;			// 下半球浓度
	float   fGlbFogHeightStartNight;	// 垂直雾高度开始-夜晚
	float   fGlbFogHeightEndNight;		// 垂直雾高度结束-夜晚
	float	fGlbFogDensityStartNight;	// 上半球浓度-夜晚
	float   fGlbFogDensityEndNight;		// 下半球浓度-夜晚

	DWORD   dwFogColor2;				// 水平雾下半球颜色
	float   fFogColorDensityStart;		// 上半球不透明度
	float   fFogColorDensityEnd;		// 下半球不透明度
	float   fFogHorzeDensity;			// 水平雾浓度
	float   fFogHorzePower;				// 上下半球过渡

	DWORD   dwFogColor2Night;			// 水平雾下半球颜色-夜晚
	float   fFogColorDensityStartNight;	// 上半球不透明度-夜晚
	float   fFogColorDensityEndNight;	// 下半球不透明度-夜晚
	float   fFogHorzeDensityNight;		// 水平雾浓度-夜晚
	float   fFogHorzePowerNight;		// 上下半球过渡-夜晚

	DWORD	dwAmbient2;		//	垂直的环境光
	DWORD	dwAmbient2_n;	//	夜晚垂直的环境光
};
