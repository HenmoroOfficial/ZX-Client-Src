...
public:		//	Operations
	...
	int GetFeishengCnt() { return m_iFeishengCnt; }
	int GetAllowLevelMax() { return m_iAllowLevelMax; }
	int GetAllowLevelMin() { return m_iAllowLevelMin; }
	int GetKickCityID() { return m_idKickCity; }
	const VECTOR3& GetKickCityPos() { return m_vKickCityPos; }
	...
	const char* GetMiniMap(){return m_strMiniMap;}
	void GetBoundBox(float& fLeft,float& fTop,float& fRight,float& fBottom)
	{
	   fLeft   = m_fLeft;
	   fTop    = m_fTop;
	   fRight  = m_fRight;
...
#ifdef _ELEMENTCLIENT
	CELPrecinct* IsPointInWithMusic(float x, float z, int idSrcInst);
#endif
	//	Get time stamp
