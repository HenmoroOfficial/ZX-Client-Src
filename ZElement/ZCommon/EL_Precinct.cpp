...
CELPrecinct::CELPrecinct()
{
	...
	m_vKickCityPos.x = 0.0f;
	m_vKickCityPos.y = 0.0f;
	m_vKickCityPos.z = 0.0f;
...
bool CELPrecinct::Load(AWScriptFile* pFile, int iVersion)
{
	...
	if(iVersion >= 7)
	{
		...
		m_idKickCity = pFile->GetNextTokenAsInt(false);

		//	Read kick city position
	...
	for (i=0; i < iNumMark; i++)
	{
		...
		pMark->vPos.y = pFile->GetNextTokenAsFloat(false) + 1.8f;
	...
	for (i=0; i < iNumMusic; i++)
	{
		...
		*pstr = WC2AS(936, pFile->m_szToken);
	...
	{
		pFile->GetNextToken(true);
		m_strMiniMap = AString("Surfaces\\") + WC2AS(pFile->m_szToken);
	}
	//	Build bound box
	BuildBoundBox();
...
bool CELPrecinct::Load(FILE* fp, int iVersion)
{
	...
	if( iVersion >= 7)
	{
		fread(&m_iFeishengCnt, sizeof (int), 1, fp);
		fread(&m_iAllowLevelMax, sizeof (int), 1, fp);
		fread(&m_iAllowLevelMin, sizeof (int), 1, fp);
		fread(&m_idKickCity, sizeof (int), 1, fp);

		//	Read city position
		fread(v, sizeof (v), 1, fp);
		m_vKickCityPos.x = v[0];
		m_vKickCityPos.y = v[1];
		m_vKickCityPos.z = v[2];
	}
...
//	Get the precinct specified position is in with music
CELPrecinct* CELPrecinctSet::IsPointInWithMusic(float x, float z, int idSrcInst)
{
	CELPrecinct* pPrecinct = NULL;
