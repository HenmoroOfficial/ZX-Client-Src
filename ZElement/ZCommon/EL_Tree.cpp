...
bool CELTree::Init(A3DDevice * pA3DDevice, CELForest * pForest, DWORD dwTypeID, const char * szTreeFile, const char * szCompositeMap, ALog * pLogFile)
{
	...
	if( m_pForest->GetBranchPixelShader() )
	{
		...
		strcat(m_szBranchNormalMap, "normal.dds");
		if( !m_pA3DDevice->GetA3DEngine()->GetA3DTextureMan()->LoadTextureFromFile(/*"textures\\dev\\flatnormal.dds"*/m_szBranchNormalMap, &m_pTextureBranchNormal) )
		{
...
#ifdef TREE_BUMP_ENABLE
	if( m_pForest->GetBranchPixelShader() && m_pForest->GetBranchVertexShader() && !(GetKeyState(VK_F7) & 0x8000) )
	{
