	...
	while( pChildItem )
	{
		...
		else if( a_stricmp(pChildItem->GetName(), _AL("Resource")) == 0)
		{
			...
			while( pResouceChildItem )
			{
				...
				else if( a_stricmp(pResouceChildItem->GetName(), _AL("GfxChecked")) == 0 )
				{
					AString strFileName;
					XMLGET_STRING_VALUE(pResouceChildItem, _AL("FileName"), strFileName)
					SetGfxChecked(strFileName);
				}
...
bool AUIRadioButton::Release(void)
{
	...
	if (m_pGfxChecked)
	{
		AfxGetGFXExMan()->CacheReleasedGfx(m_pGfxChecked);
		m_pGfxChecked = NULL;
	}
...
bool AUIRadioButton::SaveXML(AXMLItem *pXMLItemDialog)
{
	...
	if( m_strGfxChecked != "" )
	{
		...
		pXMLItemGfx->SetName(_AL("GfxChecked"));
		XMLSET_STRING_VALUE(pXMLItemGfx, _AL("FileName"), AS2AC(m_strGfxChecked), _AL(""))
	}
...
bool AUIRadioButton::Tick(void)
{
	...
	{
		if( IsChecked() && m_pA2DSpriteCheckedOnHover || !IsChecked() && (m_pA2DSpriteOnHover || m_pGfxOnHover) )
			SetHover(true);
		else
			SetHover(false);
	}
	...
	if (m_pGfxChecked && m_bCheck )
	{
		DWORD dwTick = GetTickCount();
		POINT ptPos = GetPos();
		ptPos.x += m_nOffX + m_nWidth / 2;
		ptPos.y += m_nOffY + m_nHeight / 2;
		m_pGfxChecked->SetParentTM(a3d_Translate(ptPos.x, ptPos.y, 0));
		m_pGfxChecked->TickAnimation(m_pParent->GetAUIManager()->GetTickTime());
	}
	else if (m_pGfxOnHover && m_bHover)
	{
		DWORD dwTick = GetTickCount();
		POINT ptPos = GetPos();
		ptPos.x += m_nOffX + m_nWidth / 2;
...
bool AUIRadioButton::Render(void)
{
	...
	{
		if( !m_pA2DSpriteNormal && !m_pA2DSpriteChecked && !m_pGfxChecked )
			return true;
		...
		if( m_bEnabled )
		{
			...
			if( pCurGfx )
			{
				...
				pCurGfx->SetScale(fWindowScale);
				pCurGfx->SetAlpha(float(m_nAlpha) / 255.0f);
...
bool AUIRadioButton::GetProperty(const	char *pszPropertyName, PAUIOBJECT_SETPROPERTY Property)
{
	...
	else if( 0 == strcmpi(pszPropertyName, "Checked Gfx File") )
		strncpy(Property->fn, m_strGfxChecked, AFILE_LINEMAXLEN - 1);
...
bool AUIRadioButton::SetProperty(const	char *pszPropertyName, PAUIOBJECT_SETPROPERTY Property)
{
	if( 0 == strcmpi(pszPropertyName, "Align") )
		...
		return SetGfxChecked(Property->fn);
	else if( 0 == strcmpi(pszPropertyName, "OnHover Gfx File") )
...
bool AUIRadioButton::SetGfxChecked(const char* szGfx)
{
	if (m_pGfxChecked)
	{
		AfxGetGFXExMan()->CacheReleasedGfx(m_pGfxChecked);
	...
	{
		m_strGfxChecked = szGfx;

#ifdef _DX9
		m_pGfxChecked = AfxGetGFXExMan()->LoadGfx(m_pA3DDevice, szGfx, true, false);
#else
		m_pGfxChecked = AfxGetGFXExMan()->LoadGfx(m_pA3DDevice, szGfx);
#endif
		return m_pGfxChecked != NULL;
	}

	return true;
}

bool AUIRadioButton::SetGfxOnHover(const char* szGfx)
{
	if (m_pGfxOnHover)
	{
		AfxGetGFXExMan()->CacheReleasedGfx(m_pGfxOnHover);
...
A3DGFXEx* AUIRadioButton::GetGfxChecked()
{
	return m_pGfxChecked;
}

A3DGFXEx* AUIRadioButton::GetGfxOnHover()
{
	return m_pGfxOnHover;
}

void AUIRadioButton::StartOnHoverGfx()
{
	if( m_pGfxOnHover )
		m_pGfxOnHover->Start();
}
