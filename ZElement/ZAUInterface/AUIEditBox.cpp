		...
		if( m_pAUIFrame )
		{
			if( m_bMultiline && m_pA2DSprite[AUIEDITBOX_RECT_SCROLL] )
			{
				...
				bval = m_pAUIFrame->Render(ptPos.x, ptPos.y, m_nWidth - rcScroll.Width(), m_nHeight,
					NULL, 0, _AL(""), AUIFRAME_ALIGN_LEFT, 0, 0, nAlpha);
			}
			else
			{
			...
			if (m_nLanguage != KOREAN)
			{
				if (m_nLanguage == JAPANESE)
					...
					else
					{
						...
						{
							m_xCaretPos++;
							while(	m_szBuffer[m_xCaretPos] == L'\r' &&
...
void AUIEditBox::RenderImeWnd()
{
	...
	int nWndHeight = (nFontHeight + nOff) * ((m_ime_var.get_com_str() ? 2 : 1) + m_ime_var.get_size());
	int nMaxWidth = m_ime_var.get_com_str() ? GetWidthOfString(m_ime_var.get_com_str(), NULL, pFont) : 0;
	int nLen2 = GetWidthOfString(_ime_name, NULL, pFont);
	if (nLen2 > nMaxWidth) nMaxWidth = nLen2;
	int i;

	for (i = 0; i < m_ime_var.get_size(); i++)
	{
		int n = GetWidthOfString(m_ime_var.get_can_str(i), NULL, pFont);
		n += nSequWidth;
		if (n > nMaxWidth)
			nMaxWidth = n;
	}
	...
	if (m_ime_var.get_com_str())
		pFont->TextOut(nX + nOff, nY, m_ime_var.get_com_str(), A3DCOLORRGB(255, 255, 255));
	else
		nY -= nFontHeight + nOff;
	...
	{
		nY += nFontHeight + nOff;
		pFont->TextOut(nX + nOff, nY, ACString().Format(L"%d.", i+1 > 9 ? 0 : i+1) + m_ime_var.get_can_str(i), A3DCOLORRGB(255, 255, 255));
	}
