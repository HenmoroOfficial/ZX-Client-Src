...
bool AUIStillImageButton::Tick(void)
{
	...
	if (m_pGfxDown && (m_nState == AUISTILLIMAGEBUTTON_STATE_CLICK || !m_bPushLike && m_pGfxDown->GetState() == ST_PLAY) )
	{
...
bool AUIStillImageButton::Render(void)
{
		...
		m_pParent->GetRenderState() == AUIDIALOG_RENDERSTATE_DYNAMIC )
	{
		...
		if( !m_bPushLike && m_pGfxDown && m_pGfxDown->GetState() == ST_PLAY )
			pCurGfx = m_pGfxDown;
		...
		m_pParent->GetRenderState() == AUIDIALOG_RENDERSTATE_DYNAMIC && NeedDynamicRender() )
	{
		...
		if( pAUIFrame )
		{
			bval = pAUIFrame->Render(ptPos.x, ptPos.y, m_nWidth, m_nHeight,
				...
				? _AL("") : m_szText, m_nAlign, xOff + m_nTextOffsetX * fWindowScale, yOff + m_nTextOffsetY * fWindowScale, nAlpha, false, 0, m_bVerticalText);
