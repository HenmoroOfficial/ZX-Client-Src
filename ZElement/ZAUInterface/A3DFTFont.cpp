...
//	m_iFontHei += 2;
	...
	WORD r = A3DCOLOR_GETRED(clOutline) >> 4;
	WORD g = A3DCOLOR_GETGREEN(clOutline) >> 4;
	WORD b = A3DCOLOR_GETBLUE(clOutline) >> 4;
	m_clOutline = ((r << 8) | (g << 4) | b);
	r = A3DCOLOR_GETRED(clInnerText) >> 4;
	g = A3DCOLOR_GETGREEN(clInnerText) >> 4;
	b = A3DCOLOR_GETBLUE(clInnerText) >> 4;
	m_clInner = ((r << 8) | (g << 4) | b);

	m_bInterpColor = bInterpColor;
	m_clUpper[0] = A3DCOLOR_GETRED(clUpper) >> 4;
	m_clUpper[1] = A3DCOLOR_GETGREEN(clUpper) >> 4;
	m_clUpper[2] = A3DCOLOR_GETBLUE(clUpper) >> 4;
	m_clLower[0] = A3DCOLOR_GETRED(clLower) >> 4;
	m_clLower[1] = A3DCOLOR_GETGREEN(clLower) >> 4;
	m_clLower[2] = A3DCOLOR_GETBLUE(clLower) >> 4;
		...
		if (m_iOutline)
		{
					...
					else
					{
						if (bTestEdge)
						{
							...
							if (j != pInfo->y)
							{
								pCur = pBuf - size.cx - 1;
								if(i != pInfo->x)
								{sa = ((*pCur) >> 4); a = sa > a ? sa : a;}	//	Up Left
								sa = ((*(pCur+1)) >> 4); a = sa > a ? sa : a;	//	Up
								if(i != (nXEnd-1))
								{sa = ((*(pCur+2)) >> 4);  a = sa > a ? sa : a;}	//	Right
							}

							if(i != pInfo->x)
							{sa = ((*(pBuf-1)) >> 4);  a = sa > a ? sa : a;}	//	Left
							if(i != (nXEnd-1))
							{sa = ((*(pBuf+1)) >> 4);  a = sa > a ? sa : a;}	//	Right

							if (j != (nYEnd-1))
							{
								pCur = pBuf + size.cx - 1;
								if(i != pInfo->x)
								{sa = ((*pCur) >> 4);  a = sa > a ? sa : a;}	//	Bottom Left
								sa = ((*(pCur+1)) >> 4);  a = sa > a ? sa : a;	// Bottom
								if(i != (nXEnd-1))
								{sa = ((*(pCur+2)) >> 4);  a = sa > a ? sa : a;}	// Bottom Right
							}

							if (a)
							{
								pTexBuf[nOff] = _COLOR16(a, m_clOutline);
							}
							else
								pTexBuf[nOff] = 0;
						}
			...
			for ( j = pInfo->y; j < nYGridEnd; j++)
			{
				...
				if (m_bInterpColor)
				{
					...
					A3DCOLOR rgb;
					a3d_HSVToRGB(clhsv, &rgb);
					WORD r = (rgb >> 20) & 0xf;//WORD(m_clUpper[0] * nco + m_clLower[0] * co);
					WORD g = (rgb >> 12) & 0xf;//WORD(m_clUpper[1] * nco + m_clLower[1] * co);
					WORD b = (rgb >> 4)	 & 0xf;//WORD(m_clUpper[2] * nco + m_clLower[2] * co);
					clInner = ((r << 8) | (g << 4) | b);
				}
				else
					clInner = m_clInner;
				...
				for (int i = pInfo->x; i < nXGridEnd; i++)
				{
					...
					else
					{
						...
						{
							WORD a = (*pBuf >> 4);
							WORD sa = (pTexBuf[nOff]>>12);
							WORD suma = (a+sa)>15?15:(a+sa);
							float inv = a/15.0f;
							float invs = (1.0f - inv)*sa/15.0f;
							if (suma)
							{
								WORD r = ((clInner>>8)&0xF)*inv+((pTexBuf[nOff]>>8)&0xF)*invs;
								WORD g = ((clInner>>4)&0xF)*inv+((pTexBuf[nOff]>>4)&0xF)*invs;
								WORD b = (clInner&0xF)*inv+(pTexBuf[nOff]&0xF)*invs;

						     		WORD clInnerotl = ((r << 8) | (g << 4) | b);
								pTexBuf[nOff] = _COLOR16(suma, clInnerotl);
							}		
						}
		...
		else
		{
			...
			for (int j = pInfo->y; j < nYGridEnd; j++)
			{
				...
				for (int i = pInfo->x; i < nXGridEnd; i++)
				{
					...
					{
						WORD a = (*pBuf >> 4);
						pTexBuf[nOff] = _COLOR16(a, clInner);
						pBuf++;
					}
				}
			}
