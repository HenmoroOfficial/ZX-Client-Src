...
bool AUIManager::Tick(DWORD dwDeltaTime)
{
	...
	if( pDlg != m_pDlgMouseOn || pObj != m_pObjMouseOn )
	{
		...
		if( m_pObjMouseOn )
		{
			...
			else if( m_pObjMouseOn->GetType() == AUIOBJECT_TYPE_RADIOBUTTON )
				((PAUIRADIOBUTTON)m_pObjMouseOn)->StartOnHoverGfx();
		}
		m_nHintDelay = GetTickCount();
	}
	count ++;
...
PAUIDIALOG AUIManager::GetActiveDialog(void)
{
	return m_pDlgActive;
}

PAUIDIALOG AUIManager::GetHintDialog(void)
{
	return m_pDlgHint;
}
...
A3DFTFont * AUIManager::GetDefaultFont()
{
	return m_pFont;
}
...
A3DFTFontMan * AUIManager::GetA3DFTFontMan()
{
	return m_pFontMan;
}
...
void AUIManager::SetHWnd(HWND hWnd)
{
	m_hWnd = hWnd;
}

HWND AUIManager::GetHWnd()
{
	return m_hWnd;
}
