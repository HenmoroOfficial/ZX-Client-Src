...
// Gameversion
DWORD GAME_VERSION = ((0 << 24) | (3 << 16) | (0 << 8) | 5);
DWORD GAME_BUILD = 1322;
...
//	m_pGameSession->Update(m_dwRealTickTime);
	...
	if( g_Animator.GetState() == CAnimator::STATE_PLAYING )
		g_AnmSystem.m_fPlayedTime += 0.001f * m_dwTickTime;
...
// 			if( vecs.size() >= 2 )
// 			{	
// 				AString strHost = AC2AS(vecs[1]);
// 				AString strName = AC2AS(str);
// 				strncpy(info.szServerName, strName, 256);
// 				strncpy(info.szIP, strHost, 256);
// 				info.iPort = AWString(vecs[0]).ToInt();
...
// 	AWScriptFile s;
// 	ACString str;
// 	if (s.Open("configs\\serverlist.txt"))
// 	{
// 		while(s.PeekNextToken(true))
// 		{
// 			s.GetNextToken(true);
