...
#include "EC_Scene.h"
#include "EC_World.h"
#include "..\\ZElementClient\\EC_SceneObject.h"
...
{
	//	Scene loading will occupy about 45% loading progress, calculate
...
bool CECScene::AdjustEnvironment(DWORD dwDeltaTime, A3DCameraBase* pCamera, bool bInit)
{
	...
	if (bInit)
	{
		...
		m_Env.Ambient2_n.Reset(pCurArea->GetAmbient_n());
	...
	else if (m_Env.dwAreaID != pCurArea->GetAreaID())
	{
		...
		m_Env.Ambient2_n.SetTarget(pCurArea->GetAmbient_n(), fInvTime);
...
#ifndef BACK_VERSION
	g_pGame->GetGameRun()->GetElementRenderParam()->crClear = fogColor;
	
	float d = df;
	float n = nf;
	
	A3DEnvironment::FOG fogParams = *pA3DDevice->GetA3DEngine()->GetA3DEnvironment()->GetFogParamUnderWater();
	
	fogParams.bGlobalFogEnable      = g_pGame->GetConfigs()->GetGlobalFog(); //true;//g_Configs.bEnableFog;
	
	fogParams.crFog1                = (m_Env.UWFogCol.cur * df + m_Env.UWFogCol_n.cur * nf).ToRGBAColor();
	fogParams.crFog2                = fogParams.crFog1;
	fogParams.fFogStart             = m_Env.UWFogStart.cur * df + m_Env.UWFogStart_n.cur * nf;
	fogParams.fFogEnd               = m_Env.UWFogEnd.cur * df + m_Env.UWFogEnd_n.cur * nf;
	fogParams.fHeightDensity        = 0.0f;
	fogParams.fHeightStart          = -1000;
	fogParams.fHeightEnd            = -900;
	fogParams.fDensity1             = 0.001f;
	fogParams.fDensity2             = 0.001f;
	fogParams.fColorIntensity1      = 1000.0f;
	fogParams.fColorIntensity2      = 1000.0f;
	fogParams.fHorzDensity          = fogParams.fHeightDensity;
	fogParams.fHorzPower            = 0;
	
	pA3DDevice->GetA3DEngine()->GetA3DEnvironment()->SetFogParamUnderWater(&fogParams);
	
	fogParams.crFog1                = (m_Env.FogCol.cur * df + m_Env.FogCol_n.cur * nf).ToRGBAColor();
	fogParams.crFog2                = fogParams.crFog1;
	fogParams.fFogStart             = (m_Env.FogStart.cur * df + m_Env.FogStart_n.cur * nf) * (CECSceneBlock::GetFogTimes() > 1.0f ? 1.0f : CECSceneBlock::GetFogTimes());
	fogParams.fFogEnd               = (m_Env.FogEnd.cur * df + m_Env.FogEnd_n.cur * nf) * CECSceneBlock::GetFogTimes();
	fogParams.fHeightDensity        = 0.0f;
	fogParams.fHeightStart          = -1000;
	fogParams.fHeightEnd            = -900;
	fogParams.fDensity1             = 0.001f;
	...
	pA3DDevice->GetA3DEngine()->GetA3DEnvironment()->SetFogParamAboveWater(&fogParams);
	
	pA3DDevice->GetA3DEngine()->GetA3DEnvironment()->SetIsUnderWater(bUnderWater);
#endif
