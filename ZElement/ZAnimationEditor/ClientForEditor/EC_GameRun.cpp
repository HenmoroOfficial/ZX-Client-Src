...
// 		m_pUIManager->Tick(dwDeltaTime);
	...
	{												
		...
		else if ((m_pHostPlayer && m_pHostPlayer->IsDead()))			// Disconnect in game state
		{
	...
	else	// Now focused on
	{
		...
		if (!( 
			...
			(m_pHostPlayer && m_pHostPlayer->IsDead()) 		// Disconnect in game state
			))
		{
			m_bIsFirstFlash = true;
		}
...
bool CECGameRun::Render(bool bPresent)
{
	...
	A3DVECTOR3 vCameraPos = pViewport->GetA3DCamera()->GetPos();
	A3DVECTOR3 vCameraDir = pViewport->GetA3DCamera()->GetDir();
...
// 		m_pUIManager->Render();
	...
	g_Animator.RenderWire( pViewport->GetA3DViewport() );
	if( ! g_Animator.IsDisabledCamera() )
		g_Animator.RenderMask( pViewport->GetA3DViewport() );
	g_Animator.RenderText( pViewport->GetA3DViewport() );
...
bool CECGameRun::LoadConfigsFromServer(const void* pDataBuf, int iDataSize)
{
	...
	ASSERT(m_pWorld && m_pWorld->GetHostPlayer());
...
bool CECGameRun::JumpToInstance(int idInst, const A3DVECTOR3& vPos)
{
	...
	if(!CreateHostPlayer())
	{
		a_LogOutput(1, "CECGameRun::JumpToInstance, failed to create hostplayer");
		return false;
	}
...
// 		{
// 			pGameUIMan->GetDialog("Win_CharHead")->GetDlgItem("Btn_Accout")->Show((m_iPlatformMask&0x3)>0);
