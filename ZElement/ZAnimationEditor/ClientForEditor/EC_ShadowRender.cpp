...
bool CECShadowRender::AllocResource()
{
	...
	{
		g_Log.Log("CECShadowRender::Create load simple shadow map failed!");
		return false;
	}
	A3DRECT rect(0, 0, pSurface->GetWidth(), pSurface->GetHeight());
	int nWidth, nHeight;
	m_pSimpleShadowMap = new A3DTexture();
	if( !m_pSimpleShadowMap->CreateFromSurface(pA3DDevice, pSurface, rect, &nWidth, &nHeight) )
	{
		g_Log.Log("CECShadowRender::Create create simple shadow map texture failed!");
...
bool CECShadowRender::AddShadower(const A3DVECTOR3& vecCenter, const A3DAABB& aabb, SHADOW_RECEIVER shadowReceiver, LPRENDERFORSHADOW pfnRenderForShadow, LPVOID pArg, bool bSimpleShadow)
{
	...
	s.pArg					= pArg;
	s.bSimpleShadow			= bSimpleShadow;

	if( bSimpleShadow )
		s.vecCenter = s.vecCenter - A3DVECTOR3(0.0f, s.aabb.Extents.y * 0.6f, 0.0f);
...
bool CECShadowRender::Render(A3DViewport * pViewport, const A3DVECTOR3& vecLightDir)
{
	...
	for(int i=0; i<nNumShadow; i++)
	{
		...
		m_pShadowMapCamera->SetProjectionParam(-size * 1.5f, size * 1.5f, -size * 1.5f, size * 1.5f, -100.0f, 100.0f); //size * 3.0f不需要如此大，因为AddShadow时传进来得aabb修改了 Modified 2012-03-30.
		m_pShadowMapCamera->SetPos(s.vecCenter);
		...
		IDirect3DTexture8 * pShadowMap = s.bSimpleShadow ? m_pSimpleShadowMap->GetD3DTexture() : m_pShadowMapTarget->GetTargetTexture(); 
		// 2. render shadow in the world
		pA3DDevice->GetD3DDevice()->SetTexture(0, pShadowMap);
		pA3DDevice->GetD3DDevice()->SetTexture(1, pShadowMap);
		pA3DDevice->GetD3DDevice()->SetTexture(2, pShadowMap);
		pA3DDevice->GetD3DDevice()->SetTexture(3, pShadowMap);
		pA3DDevice->GetD3DDevice()->SetTexture(4, pShadowMap);
