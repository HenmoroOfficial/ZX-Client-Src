...
{
	EditBoxItemsSet set;
	ACString str = UnmarshalEditBoxText(szText, set);
	int nStart = 0;
	ACString strResult;
	int nEmotionCount = set.GetItemCountByType(enumEIEmotion);

	while (true)
	{
		if (nEmotionCount >= 3)
		{
			strResult += str.Mid(nStart);
			break;
		}

		int nEnd = str.Find(_AL("#"), nStart);
		...
		if (nEnd > nStart)
			strResult += str.Mid(nStart, nEnd-nStart);

		nStart = nEnd + 1;
		int nNumPos = nStart;

		while (nNumPos < str.GetLength())
		{
			ACHAR c = str[nNumPos];
		...
		{
			nEmotionCount++;
			int nIndex = _wtoi(str.Mid(nStart, nNumPos-nStart));
			nStart = nNumPos;
			ACHAR buf[32];
			wsprintf(buf, L"%d", nIndex);
			ACHAR ch = set.AppendItem(enumEIEmotion, 0, L"W", buf);
	...
	return MarshalEditBoxText(strResult, set);
}
...
/*		if(pTex->GetClassID() == A3D_CID_HLSLSHADER)
			continue;
*/
		if (szTexMap && szTexMap[0])
		{
			...
			else
			{
				...
				if( strstr(szFileTitle, "a_") == szFileTitle )
				{
					...
					pEffect->SetHLSL("shaders\\2.2\\HLSL\\Materials\\default_old.hlsl");
					pEffect->SetAlphaTexture(true);
					continue;
				}
/*				
...
void AfxSetA3DSkinModelSceneLightInfo(A3DSkinModel* pA3DSkinModel)
{
	...
	if( pSceneLights )
	{
		...
		{
			LightInfo.bPtLight = false; //true;升级2.2引擎以后这个会让模型暴光，美术来不及改所有特效的属性
