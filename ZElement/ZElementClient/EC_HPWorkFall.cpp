...
extern const int fly_low_fall_act[3] = 
{
...
extern const int fly_high_fall_act[3] = 
{
...
//		m_pHost->m_pCDRInfo->fYVel = 0.0f;
	if (!m_pHost->m_pCDRInfo->bOnSurface)
#ifdef CDR_HL2_MOVE
...
//	A3DVECTOR3 vCurVel = m_pHost->m_vVelocity;
	...
	{
		bWorkEnd = true;
		m_pHost->PlayAction(CECPlayer::ACT_LANDON, 1.0f, false);
	}
	else
	{
		...
		else
		{
			...
			if (fHei > 45.f)
			{
				if (m_nCurStage != enumStageUpperAir)
				{
					m_nCurStage = enumStageUpperAir;
					m_pHost->PlayAction(fly_high_fall_act[fly_mode], 1.0f, false);
				}
			}
			else if (fHei > 3.0f)
			{
				if (m_nCurStage != enumStageLowerAir)
				{
					m_nCurStage = enumStageLowerAir;
					m_pHost->PlayAction(fly_low_fall_act[fly_mode], 1.0f, false);
				}
			}
			else
			{
...
bool CECHPWorkFall::Fall_Water(float fDeltaTime)
{
	...
	{
		m_bEnterWater = true;
		vCurVel = m_pHost->m_pCDRInfo->vAbsVelocity;
	...
	if (fSpeed2)
	{
		...
		{
			if (vCurVel.y > -9.0f)
				m_pHost->PlayAction(CECPlayer::ACT_HANGINWATER, 1.0f, false);
		}
...
//	Finish
void CECHPWorkFall::Finish(A3DVECTOR3& vCurPos)
{
	m_bFinished = true;
	m_pHost->m_pCDRInfo->vAbsVelocity.Clear();
//	m_pHost->m_pCDRInfo->fYVel = 0.0f;
...
#ifdef CDR_HL2_MOVE
	//m_pHost->m_pCDRInfo->vClipVel.Clear();
	m_pHost->m_pCDRInfo->vClipVel.x = 0.0f;
	m_pHost->m_pCDRInfo->vClipVel.z = 0.0f;
#endif

	int state = gr.get_state();
	...
	{
		A3DVECTOR3& v = vCurPos;
		ECMSG msg;
		msg.dwMsg	= MSG_HST_FALL_RECORD;
