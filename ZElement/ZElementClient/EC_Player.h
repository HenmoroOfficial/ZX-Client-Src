...
//	
///////////////////////////////////////////////////////////////////////////
extern int g_nLoginPlayerDarkLightState;	//JUST USED FOR LOGIN PLAYER!
...
enum ProfRace
{
	...
	PROFRACE_TIANHUA	= 10,
	PROFRACE_CHENHUANG	= 11,
	PROFRACE_TAIHAO		= 12,
	PROFRACE_QIANJI     = 13,
	PROFRACE_YINGZHAO   = 14,
	PROFRACE_HUMAN		= 100,
	PROFRACE_DEITY		= 200,
	PROFRACE_TIANMAI	= 300,
};
...
{
	PUPPET_STATE_NONE   = 0,
	PUPPET_STATE_FIGHT = 1,
	PUPPET_STATE_NORMAL  = 2,
	PUPPET_STATE_ANIM   = 3,
};
...
public:		//	Types
	...
	struct EquipsLoadResult
	{
		...
		CECModel*		pNewBackWeapon;	//背部新武器，非战斗状态下 Added 2012-07-26.
		...
		CECModel*		pWing;
		bool			bTuneChanged;
		CECModel*		pTune;			//??????NO NEED????
		int				nRuneQuality;	//Added 2012-06-06.
		BYTE			stoneWeapon;
		BYTE			stoneHead;
		BYTE			stoneBody;
		BYTE			stoneFoot;
	...
	enum PLAYER_ACTION_TYPE
	{
        ...
        ACT_BATH,                   // 搓澡
		ACT_PROPOSE,				// 求婚 Added 2013-09-04

		// 145
		ACT_PROPOSE_LOOP,			// 求婚循环 Added 2013-09-04
	...
	enum AttachMode
	{
		...
		enumAttachHugPlayer,
		enumAttachBathPlayer,
		enumAttachInteractiveObj,
		enumAttachRideOnQilin,
		enumAttachActiveEmote,
	};
	...
	enum 
	{
		...
		SPECIAL_STATE_XUANFENG  = 0x00000010,   // 旋风斩技能
	};	
	//	GM flags
	enum
	{
	...
	enum
	{
		POSE_ON_PET_NORMAL = 0,
		POSE_ON_PET_SIDEWAYS,
		POSE_ON_PET_KNEELING,
		POSE_ON_PET_LAY,
		POSE_ON_PET_CROSS,
	};
	...
	struct TRANSFIGUREINFO
	{
		...
		bool	bCollisionRaid;		// 标示当前是否为碰撞副本中变身 Added 2012-08-16.
	};
	...
	struct INTERACTORINFO
	{
		int		id;
		int		pos;
		int		state;	// 互动状态：0 互动开始阶段，1互动中，2互动结束阶段。
		int		attachTime;
		int		detachTime;
	};

	//	Active emote info.
	struct ACTIVEEMOTEINFO
	{
		int		id;
		int		type;
	};
...
//	typedef abase::vector<CECElsePlayer*> MirrorModelTable;
...
public:		//	Operations
	...
	bool SetMirrorEquipsLoadedResult(EC_PLAYERLOADRESULT& Ret, bool bUpdateAtOnce);
	bool SetMirrorEquips(EquipsLoadResult& ret, bool bUpdateAtOnce);
	virtual bool SetPetLoadResult(CECModel* pPetModel);
	virtual bool SetChangedModelLoadResult(CECModel* pChangedModel, bool bMultiThreadLoaded = false);
	virtual bool SetMirrorModelLoadResult( EC_PLAYERLOADRESULT& Ret);
	...
	virtual bool SetPuppetModelLoadResult(CECModel* pPuppetModel);
	...
	void SetCurSkill(CECSkill* pSkill) { m_pCurSkill = pSkill; }
	//	Set last said words
	void SetLastSaidWords(const ACHAR* szWords, int nEmotionSet, CECIvtrItem* pItem, sAchievement* pAchiv = NULL);
	...
	// On start Qilin binding buddy
	virtual void OnStartQiLinBinding(int idInvitor, int idInvitee);
	void SetQilinConnectBuddy(int idBuddy){m_idQilinConnectBuddy = idBuddy;}
	int	GetQilinConnectBuddy(){return m_idQilinConnectBuddy;}
	void SetQilinConnectHangedOn(bool bHangerOn){m_bQilinConnectHangerOn = bHangerOn;}
	bool GetQilinConnectHangedOn(){return m_bQilinConnectHangerOn;}
	//	Get buddy state
	int GetBuddyState()
	{
	...
	//	Set item ID for open booth
	void SetBoothItemID(int iBoothItemID) {m_idBoothItem = iBoothItemID;}
	//	Set booth item slot
	void SetBoothItemSlot(int iSlot) {m_iBoothItemSlot = iSlot;}
	...
	bool IsSkillFlying() { return (IsFlying() && m_bSkillFly);	}
	...
	void SetExtraEquipEffect(char iLv) { m_iExtraEquipEffect = iLv; }
...
//	float GetGroundSpeed() { return m_bWalkRun ? m_ExtProps.mv.run_speed : m_ExtProps.mv.walk_speed; }
	...
	void SetPlayerModel(CECModel* pModel) { m_pPlayerModel = pModel; m_pBackupModel = pModel;}	// 镜像模型加载，需要用到此接口
	CECModel *	GetBackupModel()		{ return m_pBackupModel; }
	CECModel *  GetCacheModel()			{ return m_pCacheModel; }
	...
	CECModel *  GetPuppetModel()		{ return m_pPuppetModel;}
	...
	CECModel *	GetInteractorModel()	{ return m_pInteractorModel; }
	...
	//	Load player model
	static bool LoadPlayerModel(int iRoleID, int iPhysique, int iProfession, int iLevel, int iEarid, int iTailid, int iFaceid, int iHairid, int iDefaultSkin,
		int iGender, const int* pEquips, const char* szPetPath, const char* szChangedPath, EC_PLAYERLOADRESULT& Ret);
	//	Load player equips
	static bool LoadPlayerEquips(int iRoleID, int iPhysique, int iProfession, int iDefaultSkin, int iGender, unsigned int uMask, const int* pEquips, EquipsLoadResult& Ret);
	//	Load pet
	...
	// 加载四组体型变成人形的模型
	static bool LoadCharacterModel(const char* szPath, EC_PLAYERLOADRESULT& Ret);
	//	Load mirror model
	static bool LoadMirrorModel(int iRoleID, int iPhysique, int iProfession, int iLevel, int iEarid, int iTailid, int iFaceid, int iHairid, int iDefaultSkin,
		int iGender, const int* pEquips, const char* szPetPath, const char* szChangedPath, EC_PLAYERLOADRESULT& Ret);
	...
	//	Load puppet model
	static bool LoadPuppetModel(const char* szPath, EC_PLAYERLOADRESULT& Ret);
	...
	static const char* GetTransfiguredModelFileName(int id, char cType = 0);
	//	Get puppet model file name
	static const char* GetPuppetModelFileName(int iRoleid, int id);
	//	Get sect info
	...
	bool IsWeaponHide()	{ return (m_dwHideEquipMask & (1<<EQUIPIVTR_WEAPON)) != 0; }
	bool IsMaskHide()	{ return (m_dwHideEquipMask & (1<<EQUIPIVTR_FASHION_EYE)) != 0; }
	...
	int GetBathType(){return m_iBathType;} 
	...
	char IsHideVip() { return m_cIsHideVip; }

	bool IsRuneEquipOpen()  { return m_bEquipRuneOpen; }
	void UpdateRuneStateGFX( int nRuneQuality );		//更新装备的元婴特效 Added 2012-06-06.
	bool TraceWithBrush(BrushTraceInfo * pInfo);		//人物之间碰撞检测 Added 2012-06-13.
	bool IsSpecialMoving() const { return m_bSpecialMoving; }//get the current player special moving flag. Added 2012-07-20.
	int GetBlackAndLightValue() const { return m_iBlackAndLightValue; }
	void SetBlackAndLightValue( int iBlackAndLightValue ) { m_iBlackAndLightValue = iBlackAndLightValue; }
	DWORD GetForceChangeMask() const { return m_dwForceChangeMask; }
	void SetForceChangeMask( DWORD dwForceChangeMask ) { m_dwForceChangeMask = dwForceChangeMask; }
	char GetDarkLightState() const { return m_cXuanYuanDarkLightState; }
	void SetDarkLightState( char cDarkLightState ) { m_cXuanYuanDarkLightState = cDarkLightState; }
	void SetCollisionState( bool bCollision ) { m_bCollisionState = bCollision; }
	bool GetCollisionState() const { return m_bCollisionState; }
	unsigned char GetWingChangeColor() const { return m_ucWingChangeColor; }
	void SetWingChangeColor( unsigned char ucWingChangeColor ) { m_ucWingChangeColor = ucWingChangeColor; }
	int GetMass() const { return ( m_iCollisionSkillState & COLLISION_STATE_ASHILL ) ? COLLISION_PLAYER_MAX_MASS : m_iMass; }
	int GetRealMass() const { return m_iMass; }
	void SetMass( int iMass ) { m_iMass = iMass; }
	int GetCollisionSkillState() const { return m_iCollisionSkillState; }
	void SetCollisionSkillState( int iCollisionSkillState ) { m_iCollisionSkillState = iCollisionSkillState; } 
	bool IsCollisionRooting() const { return ( m_iCollisionSkillState & (COLLISION_STATE_ASHILL | COLLISION_STATE_ROOT | COLLISION_STATE_VOID) ) ? true : false;  }

	void SetMirrorModelCnt(int iMirrorImageCnt);
	int GetMirrorModelCnt() const { return m_iMirrorImageCnt;}
	
	char GetDarkLightSpirit(int index) { return m_cDarkLightSpirits[index];	}
	void SetDarkLightSpirit(int index, char value);	// 修改灵，并更新灵的gfx

	CECLoginPlayer* GetSpiritModel(int index) { return m_SpiritModels[index];}
	CECLoginPlayer* CreateSpiritModel(int index);
		
	void UpdateMirrorAndSpiritModels(DWORD dwDeltaTime);	// 更新镜像和灵的玩家模型

    bool GetLoadFlag() { return m_bLoadFlag;}

	//	Set the current Dir-Visible-States Added 2012-09-05.
	void SetDirVisibleStates( int iVisibleStateID, int iDir, bool bOn );
	
	//	Update the multiline skill gfx. Added 2012-09-11.
	void UpdateMultilineSkillGfx( const int* pTargets, int nTargetNum );
	
	void SetFlowBattle(int iFlowBattle);
	void ClearFlowBattle();

	void SetBloodPool(bool bBloodPool, int iCurBloodPool, int iMaxBloodPool);
	bool GetBloodPoolState() { return m_bBloodPool; }
	int  GetCurBloodPool() { return m_iCurBloodPool; }
	int  GetMaxBloodPool() { return m_iMaxBloodPool; }
	bool LoadBoothSkinModel();
	bool UpdateBoothModel();

protected:

	static void ChangeDefaultUpper(A3DSkin* aSkins[3], int iPhysique, int iProfession, int nGender, int nSkin, int nXYDarkLightState = 0);
	static void ChangeDefaultFoot(A3DSkin* aSkins[3], int iPhysique, int iProfession, int nGender, int nSkin, int nXYDarkLightState = 0);
	static void ChangeDefaultFashionUpper(A3DSkin* aSkins[3], int iPhysique, int iProfession, int nGender, int nSkin, int nXYDarkLightState = 0);
	static void ChangeDefaultFashionLower(A3DSkin* aSkins[3], int iPhysique, int iProfession, int nGender, int nSkin, int nXYDarkLightState = 0);
	static void ChangeDefaultFashionFoot(A3DSkin* aSkins[3], int iPhysique, int iProfession, int nGender, int nSkin, int nXYDarkLightState = 0);
...
//    static DWORD			m_HideEquipMaskInArena;		// 竞技场中的装备隐藏掩码
	...
	static const int        m_EquipsInBath[][14];
	...
	CECModel*		m_pCacheModel;		//  用于存储四组体型变为人形时 Player 的模型
	...
	CECModel*		m_pInteractorModel;	//	Interactor Model.
	CECModel*		m_pPuppetModel;		//	Puppet model
	A3DSkinModel*	m_pPortraitModel;	//	Portrait model
	A3DSkinModel*   m_pHeadPortraitModel; // Head Portrait for change face
	A3DSkinModel*	m_pBoothModel;					//	Booth model
	...
	int				m_idBoothItem;			//  摆摊道具ID
	int				m_iBoothItemSlot;		//	摆摊道具在包裹中的位置
	CECModel*       m_pSpeBoothModel;       //  VIP摆摊形象
	...
	INTERACTORINFO	m_CandInteractorInfo;
	INTERACTORINFO	m_InteractorInfo;	//	Interactor information
	...
	int				m_iFaceid;			//	Face id 除了怀光，都是0开始，怀光7开始
	int				m_iHairid;			//	Hair id 除了怀光，都是0开始，怀光7开始
	int				m_iEarid;			//  Ear id
	...
	CECSpriteDecal*	m_pTitleDecal;      //  Title icon	
	ACString		m_strTeamReq;		//	Team requirement string
	ACString		m_strBoothName;		//	Booth name string
	CECPateText*	m_pKillNum;			//	连杀值显示 2013--4-23 add ba zy
	// stones effects.
	...
	unsigned int	m_uAttackTypeBackUp;
	// 飞行
	int				m_nFlyMode;
	bool			m_bQilinFlashMove; //上麒麟冲锋
	...
	int             m_idCandBathBuddy;  //  ID of candidate bath buddy
	int				m_idCandQilinBuddy; //  ID of candidata Qilin buddy
	int				m_idQilinConnectBuddy; 
	bool			m_bQilinConnectHangerOn;
	bool			m_bAddQilinLinkGfx;
	bool			m_bCandHangerOn;
	RIDINGPET		m_CandPet;
	TRANSFIGUREINFO	m_CandTransfigure;
	float			m_fPetHeight;
	// 是否是互动主体
	bool			m_bInteractiveSubject;
	bool			m_bInteractiveRenderSubject;
	bool			m_bAttachedInteractor;

	// 是否是互动表情的主体
	ACTIVEEMOTEINFO	m_CandEmoteInfo;
	int				m_iEmoteType;
	int				m_iEmoteActionType;
	...
	bool			m_bPlayerModelIsChangedModel;

	//牵机职业的傀儡
	int				m_iPupPetState;
	int				m_iPuppetId;
	int				m_iCandPupPetId;
	CECCounter		m_PuppetStateCount;
	CECCounter		m_PuppetNormalStandCnt;
	CECCounter		m_PuppetAnimCnt;
	...
	DWORD			m_dwTetrapodFashionMask;
	
	int             m_iBathType;         //泡澡地图类型 （-1 ，没有在泡澡地图中）
	...
	int				m_nRuneQuality;			// 用于记录当前元婴品质等级，暂仅用于显示元婴特效

	//Added 2012-07-20.
	bool			m_bSpecialMoving;		//	It is a flag that means wheather the special move work is running.
	//Added end.

	//Added 2012-08-01.
	int				m_iCollisionSkillState;	// 用于记录处于碰撞副本中，角色中技能效果后的状态 Added 2012-08-01.
	//Added end.

	//Added 2012-08-03.
	int				m_iBlackAndLightValue;	//	用于记录轩辕职业当前的光系和暗系值 Added 2012-08-03.
	//Added end.

	//Added 2012-08-03.
	DWORD			m_dwForceChangeMask;	//	用于记录需要强制执行装备变换的掩码 用于处理：一个装备ID，对应多套装备的情况Added 2012-08-06.
	//Added end.

	//Added 2012-08-16.
	char			m_cXuanYuanDarkLightState;//用于记录轩辕当前光暗状态， Added 2012-08-16.
	//Added end.

	//Added 2012-08-17.
	bool			m_bCollisionState;			//用于记录当前角色的碰撞状态，务必注意状态同步问题 Added 2012-08-17.
	//Added end.
	
	char				m_cDarkLightSpirits[3];		// 轩辕职业的灵
	A3DGFXEx*			m_pSpiritsGFX[3];			// 轩辕职业灵特效
	float				m_fSpiritsAngle;			// 轩辕灵效果

	int					m_iMirrorImageCnt;			// 当前镜像的数量
	CECLoginPlayer*		m_MirrorModels[6];			// 当前镜像列表

	int					m_iSpiritModelCnt;			// 当前灵模型个数
	CECLoginPlayer*		m_SpiritModels[3];			// 当前灵模型

	//Added 2012-08-20.
	unsigned char		m_ucWingChangeColor;		//当前飞剑变色索引值 Added 2012-08-20.
	//Added end.

	//Added 2012-08-22.
	int					m_iMass;					//进入碰撞副本，变身后的玩家质量 Added 2012-08-22.
	//Added end.

	//Added 2012-09-05.
	A3DGFXEx*			m_pDirVisibleStateGFX;		//十字技能状态效果特效 Added 2012-09-05.
	//Added end.

	bool				m_bSkillFly;				// 由于使用技能而导致的飞行效果
	
	char				m_iExtraEquipEffect;		//  武器炼器等级显示，覆盖真实炼器等级

	short				m_iFlowBattle;				// 流水席状态
	int					m_iBattleKill;				
	//Added 2012-09-11
	A3DGFXEx*			m_pMultiLineSkillGfx[MAX_MULTILINE_SKILL_GFX_NUM];	//多线性技能攻击的特效 Added 2012-09-11.

	
	int m_nScore;			// 2013-4-8 add by zy 分数

	AString				m_sPlayerInteractiveAction;		//	交互时人物的动作	Added 2013-08-01.
	AString				m_sPlayerInteractiveMoveAction;	//	交互中移动时人物的动作 Added 2013-08-01
	AString				m_sObjInteractiveAction;		//	交互时互动物品的动作	Added 2013-08-01.
	AString				m_sObjInteractiveMoveAction;	//	交互中移动时互动物品的动作	Added 2013-08-01.

	DWORD				m_dwPoseOnPet;	// 骑乘动作 Added 2013-08-14
	int					m_iTravalVehicleId;	// 自由飞行骑乘物品id
	//Added end.
...
protected:	//	Operations
	...
	bool QueueLoadPuppetModel(const char* szPath, bool bAtOnce);
	...
	virtual	void OnMsgHstFlowBattlePlayerInfo(const ECMSG& Msg);	// 战场流水席角色信息 2013-4-19 by add zy
...
public:
	...
	//	Print bubble text
	void BubbleText(int iIndex, __int64 dwNum, DWORD dwWaitTime=0, int p1=0, bool bLastOne=true, bool bRandPos=false);
    //
	void BubbleGetSpecialItem(int itemid,int itemcount);
	//	Play an attack effect
	void PlayAttackEffect(int idTarget, int idSkill, int skillLevel, unsigned char cSerialId, int nDamage, DWORD dwModifier, int* piAttackTime=NULL, int spirit_index = -1, A3DVECTOR3* pCastSkillPos = NULL);
	
	//召唤傀儡
	void SummonPupPet(int idPuppet);
	int GetPupPetId(){return m_iPuppetId;}
	int GetPuppetState(){return m_iPupPetState;}
	virtual void UpdatePupPetState(DWORD dwDeltaTime);
	virtual void PupPetShow();
	void PuppetEnterFightState(){ m_PuppetStateCount.Reset(false); }

	// 变身
	bool IsShapeChanged() const { return m_iChangeShape != 0; }
	int GetShapeChangedId(){return m_iChangeShape;}
	void TransformShape(int form, bool bPlayerGfx = false);
	void Transfigure(int id, bool bPlayGfx = false);
	void NewTransfigure( int id, bool bPlayGfx = false ); //Added 2012-08-06.

	// 依附关系
	virtual void AttachBuddy(int iBuddy,bool isBath = false);
	virtual void DetachBuddy(CECPlayer* pBuddy = NULL);
	AttachMode GetAttachMode() const { return m_AttachMode; }

	virtual void AttachBathBuddy(int iBuddy);
	virtual void DetachBathBuddy(CECPlayer* pBuddy = NULL);
	virtual void ReAttachQilin();
	void AddQilinLinkGfx();

	virtual void AttachQilinBuddy(int iBuddy);
	virtual void DetachQilinBuddy(CECPlayer* pBuddy = NULL);

	virtual void AttachMultiBuddy(int iBuddy, int pos);
	virtual void DetachMultiBuddy(int pos, CECPlayer* pBuddy = NULL);

	virtual void AttachInteractorModel();
	virtual void DetachInteractorModel();
	...
	INTERACTORINFO	GetInteractorInfo()	{ return m_InteractorInfo; }

	virtual bool PlayActiveEmote(ACTIVEEMOTEINFO info);
	virtual void StopActiveEmote();
	...
	virtual bool IsSpellingMagic(){return false;}
	...
	// static funcs
	static bool ChangeWeapon	(EquipsLoadResult* pResult, const char* szLeft, const char* szRight, const char* szBack = "", const char* szNewBack = ""); //添加对背部武器的处理接口Modified 2011-08-09.
	...
	static bool ChangeWing		(EquipsLoadResult* pResult, const char* szModeFile, int iProfession = 0, int iChangeShape = 0);
	...
	static bool ChangeArmor		(const char* strSkinFile, A3DSkin* aSkins[3], int nLocation, bool bDefault, int iPhysique, int iProfession, int nGender, int iDefaultSkin, int nXYDarkLightState = 0, bool bHideXYMask = false);
	static void ChangeArmor		(const EQUIPMENT_ESSENCE* pArmor, DWORD& dwShowMask, DWORD& dwSkinChangeMask, int iPhysique, int iProfession, int nGender, int iSkin, A3DSkin* pSkins[][3], int nXYDarkLightState = 0, bool bHideXYMask = false);
	static bool ChangeFashion	(const char* strSkinFile, A3DSkin* aSkins[3], int nLocation, bool bDefault, int iPhysique, int iProfession, int nGender, A3DCOLOR color, int iDefaultSkin, int nXYDarkLightState = 0);
	static void ChangeFashion	(const EQUIPMENT_ESSENCE* pFashion, DWORD& dwShowMask, DWORD& dwSkinChangeMask, DWORD& dwTetrapodFashionMask, int iPhysique, int iProfession, int nGender, int iDefaultSkin, int iDefaultHair, A3DSkin* pSkins[][3], A3DCOLOR color, int nXYDarkLightState = 0);
	static void ShowEquipments	(int iRoleID, int iPhysique, int iProfession, int iDefaultSkin, int nGender, bool bHasDummy, const int* pEquipmentID, unsigned int uChangeMask, EquipsLoadResult* pResult);
	static int GetEquipPathId	(int nEquipId, int nGender, int nPhysique);
	static void GetUpdatedWingFile( const char* szModelFile, AString& strNewWingFile, unsigned char ucWingChangeColor );	//Added 2012-08-21.
};
