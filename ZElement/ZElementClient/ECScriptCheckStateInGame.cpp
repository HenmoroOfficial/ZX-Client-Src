...
bool CECScriptCheckStateInGame::GetIsTaskFinished(int nTaskID, const CIntArray & vecExcludedID) const
{
	if( nTaskID == -1)
	{
		...
		return (pTaskList->m_ulTaskCount > uExcludedFinsihedTaskCount);
	}
	else
	{
...
{
	return GetHostPlayer()->GetSkill(nSkillID)->GetSkillLevel();
}
...
{
	return 0; // GNET::ElementSkill::GetAbility(nSkillID);
}
