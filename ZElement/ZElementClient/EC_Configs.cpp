...
CECConfigs::CECConfigs()
{
	...
	for (int i=0;i<7;++i)
	{
	  m_ls.local_chat.bShowChannel[i] = true;
	}
}
...
bool CECConfigs::LoadConfigs(const char* szFile)
{
	...
	m_bConsumeScoreDisable	= IniFile.GetValueAsInt(strSect, "consumescore_disable", m_bConsumeScoreDisable? 1:0 ) ? true:false;
		...
		EC_VIDEO_SETTING *pvs, EC_GAME_SETTING *pgs)
{
	if( pss )
	{
		...
		ss.iDist_tree = 2000.0f;
		ss.iDist_Water = 2000.0f;
		ss.iDist_Buiding = 2000.0f;
		ss.iDist_BoxArea = 2000.0f;
		ss.iDist_Grass = 2000.0f;
		ss.iDist_Effect = 2000.0f;
		ss.iDist_Ecmodel = 2000.0f;
		ss.iDist_Critter = 2000.0f;
		ss.iDist_Bezzier = 2000.0f;
		ss.iDist_Sound  = 2000.0f;

		ss.bLimitedCameraMode = true;
		ss.fCameraModeWalkSpeed = 1.0f;
		*pss = ss;
	}
	if (pvs || pgs)
	{
...
bool CECConfigs::LoadSystemSettings(const char* szFile)
{
	...
	m_bSkipMovie		= IniFile.GetValueAsInt(strSect, "SkipMovie", 0) ? true : false;
	...
	//scene dist
	strSect = "scenedist";
	m_ss.iDist_tree = IniFile.GetValueAsFloat(strSect,"tree",2000.0f);
	m_ss.iDist_Water = IniFile.GetValueAsFloat(strSect,"water",2000.0f);
	m_ss.iDist_Buiding = IniFile.GetValueAsFloat(strSect,"building",2000.0f);
	m_ss.iDist_BoxArea = IniFile.GetValueAsFloat(strSect,"boxarea",2000.0f);
	m_ss.iDist_Grass = IniFile.GetValueAsFloat(strSect,"grass",2000.0f);
	m_ss.iDist_Effect = IniFile.GetValueAsFloat(strSect,"effect",2000.0f);
	m_ss.iDist_Ecmodel = IniFile.GetValueAsFloat(strSect,"ecmodel",2000.0f);
	m_ss.iDist_Critter = IniFile.GetValueAsFloat(strSect,"critter",2000.0f);
	m_ss.iDist_Bezzier = IniFile.GetValueAsFloat(strSect,"bezzier",2000.0f);
	m_ss.iDist_Sound = IniFile.GetValueAsFloat(strSect,"sound",2000.0f);

	strSect = "cameramode";
	m_ss.bLimitedCameraMode = IniFile.GetValueAsBoolean(strSect,"limitdis",true);
	m_ss.fCameraModeWalkSpeed = IniFile.GetValueAsFloat(strSect,"walkspeed",1.0f);
...
bool CECConfigs::SaveSystemSettings()
{
	...
	IniFile.WriteIntValue(strSect, "SkipMovie", m_bSkipMovie);
	//	Audio section
	strSect = "Audio";
	...
	/*
	strSect = "scenedist";
	IniFile.WriteFloatValue(strSect,"tree",m_ss.iDist_tree);
	IniFile.WriteFloatValue(strSect,"water",m_ss.iDist_Water);
	IniFile.WriteFloatValue(strSect,"building",m_ss.iDist_Buiding);
	IniFile.WriteFloatValue(strSect,"boxarea",m_ss.iDist_BoxArea);
	IniFile.WriteFloatValue(strSect,"grass",m_ss.iDist_Grass);
	IniFile.WriteFloatValue(strSect,"effect",m_ss.iDist_Effect);
	IniFile.WriteFloatValue(strSect,"ecmodel",m_ss.iDist_Ecmodel);
	IniFile.WriteFloatValue(strSect,"critter",m_ss.iDist_Critter);
	IniFile.WriteFloatValue(strSect,"bezzier",m_ss.iDist_Bezzier);
	IniFile.WriteFloatValue(strSect,"sound",m_ss.iDist_Sound);

	strSect = "cameramode";
	IniFile.WriteIntValue(strSect,"limitdis",m_ss.bLimitedCameraMode);
	IniFile.WriteFloatValue(strSect,"walkspeed",m_ss.fCameraModeWalkSpeed);
	*/
...
bool CECConfigs::SaveUserConfigData(void* pDataBuf, int* piSize)
{
	...
	if (pDataBuf)
	{
		AString strUserName = g_pGame->GetGameSession()->GetRawUserName();
		strUserName.MakeLower();
		// 保证新建角色播动画
		if (strUserName=="system"||strUserName=="system2" || strUserName=="system3")
		{
	...
	iTotalSize += sizeof(EC_FRIENDSREMARKS);
	if(pDataBuf)
	{
		*((EC_FRIENDSREMARKS*)pDatas) = m_ConfigsData.m_frs;
		pDatas += sizeof (EC_FRIENDSREMARKS);
	}
...
bool CECConfigs::LoadUserConfigData(const void* pDataBuf, int iDataSize)
{
	...
	try
	{
		...
		g_pGame->GetGameRun()->GetSecretary()->SetHelpGuideEnable(!m_ConfigsData.m_gs.bHideSecretary);
...
void CECConfigs::SetIsSkipMovie(bool bFlag)
{
	m_bSkipMovie=bFlag; 
	AIniFile IniFile;
	if (!IniFile.Open("userdata\\SystemSettings.ini"))
	{
		a_LogOutput(1, "CECConfigs::LoadConfigs, Failed to open file userdata\\SystemSettings.ini");
		return;
	}
	
	
	IniFile.WriteIntValue("Video", "SkipMovie", bFlag);
...
void CECConfigs::SaveLocalSetting()
{
	...
	AFile afile;
	if (afile.Open(szPath, AFILE_OPENEXIST | AFILE_TEMPMEMORY))
	{
		afile.Close();
		DeleteFileA(szPath);
	}
	...
	IniFile.WriteIntValue(strSect, "nFont", m_ls.local_chat.nFont);
	int iShowChannel = 0;
	for (int i=0; i<9; i++)
	{
		if (!m_ls.local_chat.bShowChannel[i])
			iShowChannel |= (1<<i);
	}
	IniFile.WriteIntValue(strSect, "iShowChannel",			iShowChannel);
	...
	IniFile.WriteIntValue(strSect, "bDisableShowQuitReason", m_ls.bDisableShowQuitReason ? 1 : 0);
	IniFile.WriteIntValue(strSect, "bAutoShowSkillXp", m_ls.bAutoShowSkillXp ? 1 : 0);
	IniFile.WriteIntValue(strSect, "bAutoShowEquipRune", m_ls.bAutoShowEquipRune ? 1 : 0);
	IniFile.WriteIntValue(strSect, "bFuWenPop1Showed", m_ls.bFuWenPop1Showed ? 1 : 0);
	IniFile.WriteIntValue(strSect, "bFuWenPop2Showed", m_ls.bFuWenPop2Showed ? 1 : 0);
	IniFile.WriteIntValue(strSect, "bFuWenPop3Showed", m_ls.bFuWenPop3Showed ? 1 : 0);
	IniFile.WriteIntValue(strSect, "bFuWenPop4Showed", m_ls.bFuWenPop4Showed ? 1 : 0);
	IniFile.WriteIntValue(strSect, "nRecordMountType", m_ls.nRecordMountType);
	IniFile.WriteIntValue(strSect, "nRecordMountID", m_ls.nRecordMountID);
	IniFile.WriteIntValue(strSect, "nRecordMountItemID", m_ls.nRecordMountItemID);
	IniFile.WriteIntValue(strSect, "bStopPromptConsumeCash", m_ls.bStopPromptConsumeCash ? 1 : 0);
	IniFile.Save(szPath);
	IniFile.Close();
}
...
void CECConfigs::LoadLocalSetting()
{
	...
	if (afile.Open(szPath, AFILE_OPENEXIST|AFILE_BINARY|AFILE_NOHEAD|AFILE_TEMPMEMORY))
	{
	...
	else
	{
		...
		m_ls.local_chat.nFont               = IniFile.GetValueAsInt(strSect, "nFont", 0);
		int iDefaultShowChannel = 0;
		for (int i=0;i<7;i++)
		{
			iDefaultShowChannel |= (1<<i);
		}
		int iShowChannel					= IniFile.GetValueAsInt(strSect, "iShowChannel", iDefaultShowChannel);
		for (int i=0; i<9; i++)
		{
			if (iShowChannel & (1<<i))
				m_ls.local_chat.bShowChannel[i] = false;
			else
				m_ls.local_chat.bShowChannel[i] = true;
		}
		...
		m_ls.bDisableShowQuitReason			= IniFile.GetValueAsInt(strSect, "bDisableShowQuitReason", 0) > 0;
		m_ls.bAutoShowSkillXp				= IniFile.GetValueAsInt(strSect, "bAutoShowSkillXp", 1) > 0;
		m_ls.bAutoShowEquipRune				= IniFile.GetValueAsInt(strSect, "bAutoShowEquipRune", 1) > 0;
		m_ls.bFuWenPop1Showed				= IniFile.GetValueAsInt(strSect, "bFuWenPop1Showed", 0) > 0;
		m_ls.bFuWenPop2Showed				= IniFile.GetValueAsInt(strSect, "bFuWenPop2Showed", 0) > 0;
		m_ls.bFuWenPop3Showed				= IniFile.GetValueAsInt(strSect, "bFuWenPop3Showed", 0) > 0;
		m_ls.bFuWenPop4Showed				= IniFile.GetValueAsInt(strSect, "bFuWenPop4Showed", 0) > 0;
		m_ls.nRecordMountType				= IniFile.GetValueAsInt(strSect, "nRecordMountType", 0);
		m_ls.nRecordMountID				= IniFile.GetValueAsInt(strSect, "nRecordMountID", 0) ;
		m_ls.nRecordMountItemID				= IniFile.GetValueAsInt(strSect, "nRecordMountItemID", 0);
		m_ls.bStopPromptConsumeCash		= IniFile.GetValueAsInt(strSect, "bStopPromptConsumeCash", 0) > 0;
