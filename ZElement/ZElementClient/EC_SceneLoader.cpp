...
inline void TerrLoaderModule::LoadOrnament(bool bInLoaderThread)
{
	...
	while (true)
	{
		...
		if (pHost && bInLoaderThread)
		{
			...
			{
				vHost = g_pGame->GetGameRun()->GetAnmSystem()->GetCenterPos();
			}
		...
		if (m_aToLoad.GetCount())
		{
			...
			*/
			...
			float fBlockComp = 50.0f / (m_aToLoad.GetCount());
			float fProgCnt = 0.0f;
...
inline void ECModelLoaderModule::LoadAndReleaseECModels(bool bInLoaderThread)
{
	...
	if (m_aTemplLoad.GetSize() > 0)
	{
		...
		for (i = 0; i < m_aTemplLoad.GetSize(); i++)
		{
			...
			case MTL_ECM_MIRROR_PLAYER_EQUIP:

				pPlayerMan->ThreadLoadMirrorPlayerEquips(Info.idObject, Info.dwBornStamp, Info.p1, Info.p5, Info.p9, Info.p2, Info.p4, Info.p3);
				break;
			...
			case MTL_ECM_MIRROR_MODEL:

				pPlayerMan->ThreadLoadMirrorModel(Info.idObject, Info.dwBornStamp, Info.p3 >> 16, Info.p8, Info.p5, Info.p6, Info.p7, Info.p1, Info.p2, 
					Info.p9, Info.p3 & 0xffff, Info.p4, Info.strFile, Info.strFile2);
				break;
			...
			case MTL_ECM_PUPPET:
				...
				pPlayerMan->ThreadLoadPuppetModel(Info.idObject, Info.dwBornStamp, Info.strFile);
				break;
			case MTL_ECM_NPC:
...
void LoadInMainThread(bool bForceLoad)
{
	...
	if (pTerrain && pTerrain->IsMultiThreadLoad() && pTerrain->GetDataLoadFlag())
	{
		...
		//	Terrain laoding will occupy about 35% loading progress, calculate
		//	each block's component
		float fBlockComp = pTerrain->GetCandidateBlockNum() ? 35.0f / pTerrain->GetCandidateBlockNum() : 0.17f;
		float fProgCnt = 0.0f;
