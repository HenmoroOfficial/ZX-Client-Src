...
const int FontSize[MaxFontNum]={10,12,14,16,20,24,32};
...
AString FormatParm(ACHAR *src)
{
	char c[300];
	WideCharToMultiByte(936,0,src,-1,c,300,NULL,NULL);
	AString des="";
	int len=strlen(c);
	for(int i=0;i<len;i++)
	{
...
UINT _DownloadPageImpl(LPVOID pParam)
{
	...
	if(threaddata->pageinfo->needrelease)
	{
		delete []threaddata->pageinfo->parm;
		threaddata->pageinfo->parm=NULL;
		delete threaddata->pageinfo;
		threaddata->pageinfo=NULL;
	}
	else
...
UINT _ParsePageImpl(LPVOID pParam)
{
...
CECBBSPage::CECBBSPage(CECBBSPage *hParentPage)
{
	...
	{
		n = iniFile.GetValueAsInt("Login", "CodePage", -1);
		iniFile.Close();
	}
	if( n != -1 )
		m_iCodePage=n;
				...
				if(hLabel->LabelType!="input"||hLabel->LabelType!="textarea")
				{
					...
					if(type==AUIOBJECT_TYPE_EDITBOX)
					{
						...
						if(aStr!=NULL)
						{
							...
							ACHAR *uCode=new ACHAR[l+1];
							MultiByteToWideChar(m_iCodePage,0,*aStr,-1,uCode,l);
							uCode[l]='\0';
							((PAUIEDITBOX)node->obj)->SetText(uCode);
							delete uCode;
						}
						else
							((PAUIEDITBOX)node->obj)->SetText(TEXT(""));
					}
					else if(type==AUIOBJECT_TYPE_CHECKBOX)
					{
...
bool CECBBSPage::CreatePageScroll()
{
	...
	scroll->SetSize(int(15*fWindowScale),m_aPostion.Height());
	strcpy(p.fn,"ListBox\\DragSlotD.tga");
	if(!scroll->SetProperty("Scroll Area Image",&p))
	{
		A3DRELEASE(scroll);
		return false;
	}
	strcpy(p.fn,"ListBox\\DragButtonC.tga");
	if(!scroll->SetProperty("Scroll Bar Image",&p))
	{
		A3DRELEASE(scroll);
		return false;
	}
	strcpy(p.fn,"ListBox\\DragUpC.tga");
	if(!scroll->SetProperty("Up Button Image",&p))
	{
		A3DRELEASE(scroll);
		return false;
	}
	strcpy(p.fn,"ListBox\\DragDownC.tga");
	if(!scroll->SetProperty("Down Button Image",&p))
	{
		A3DRELEASE(scroll);
		return false;
	}
	p.i=1;
	scroll->SetProperty("Frame Mode", &p);
	scroll->SetScrollRange(0,m_iScrollHeight);
	scroll->SetBarLength(-1);
...
bool CECBBSPage::CreateInput(HTMLLabel *hLabel,A3DRECT &lRect)
{
	...
	if(a=="text"||a=="password"||a=="file")
	{
		...
		if(m_pGlobalInfo->bDrawObject)
		{
			...
			AUIOBJECT_SETPROPERTY p;
			strcpy(p.fn,"InBBS/对话框_浏览器输入边框.bmp");
			if(!editBox->SetProperty("Frame Image",&p))
			{
				A3DRELEASE(editBox);
				return false;
			}
	...
	else if(a=="submit"||a=="reset")
	{
		...
		if(m_pGlobalInfo->bDrawObject)
		{
			...
			if(!button->SetProperty("Up Frame File",&p))
			{
				A3DRELEASE(button);
				return false;
			}
			...
			if(!button->SetProperty("Down Frame File",&p))
			{
				A3DRELEASE(button);
				return false;
			}
	...
	else if(a=="checkbox")
	{
		...
		if(m_pGlobalInfo->bDrawObject)
		{
			...
			AUIOBJECT_SETPROPERTY p;
			strcpy(p.fn,"InBBS/复选框_标准.tga");
			if(!checkbox->SetProperty("Image File",&p))
			{
				A3DRELEASE(checkbox);
				return false;
			}
	...
	else if(a=="radio")
	{
		...
		if(m_pGlobalInfo->bDrawObject)
		{
			...
			AUIOBJECT_SETPROPERTY p;
			strcpy(p.fn,"InBBS/单选框_标准.tga");
			if(!radio->SetProperty("Image File",&p))
			{
				A3DRELEASE(radio);
				return false;
			}
...
bool CECBBSPage::CreateSelect(HTMLLabel *hLabel,A3DRECT &lRect)
{
	...
	if(aStr==NULL||aStr->ToInt()==1)
	{
		...
		if(m_pGlobalInfo->bDrawObject)
		{
			...
			if(!combo->SetProperty("Frame Image",&p))
			{
				A3DRELEASE(combo);
				return false;
			}
			strcpy(p.fn,"InBBS/ComboHilight.bmp");
			if(!combo->SetProperty("Hilight Image",&p))
			{
				A3DRELEASE(combo);
				return false;
			}
	...
	else
	{
		...
		if(m_pGlobalInfo->bDrawObject)
		{
			...
			AUIOBJECT_SETPROPERTY p;
			strcpy(p.fn,"InBBS/ListFrame.bmp");
			if(!list->SetProperty("Frame Image",&p))
			{
				A3DRELEASE(list);
				return false;
			}
			strcpy(p.fn,"InBBS/ListHilight.bmp");
			if(!list->SetProperty("Hilight Image",&p))
			{
				A3DRELEASE(list);
				return false;
			}
			strcpy(p.fn,"InBBS/列表框_上移.bmp");
			if(!list->SetProperty("Up Button Image",&p))
			{
				A3DRELEASE(list);
				return false;
			}
			strcpy(p.fn,"InBBS/列表框_下移.bmp");
			if(!list->SetProperty("Down Button Image",&p))
			{
				A3DRELEASE(list);
				return false;
			}
			strcpy(p.fn,"InBBS/列表框_滚动槽1.bmp");
			if(!list->SetProperty("Scroll Area Image",&p))
			{
				A3DRELEASE(list);
				return false;
			}
			strcpy(p.fn,"InBBS/列表框_拖动按钮1.bmp");
			if(!list->SetProperty("Scroll Bar Image",&p))
			{
				A3DRELEASE(list);
				return false;
			}
	...
	for(int i=0;i<m_nTotal;i++)
		if(m_DownloadedPages[i]->downloaded)
		{
			if(m_DownloadedPages[i]->parm!=NULL)
			{
				...
				m_DownloadedPages[i]->parm=NULL;
			}
			delete m_DownloadedPages[i];
			m_DownloadedPages[i]=NULL;
		}
		else
			m_DownloadedPages[i]->needrelease=true;
...
int DownloadedPagesManager::AddPage(AString url,CECBBSPage *page)
{
	...
	m_DownloadedPages[m_nTotal]=new DownloadedPagesInfo;
	char s[100];
	...
	m_DownloadedPages[m_nTotal]->needrelease=false;
