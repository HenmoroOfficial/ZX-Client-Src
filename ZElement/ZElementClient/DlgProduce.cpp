...
bool CDlgProduce::OnInitDialog()
{
	...
	m_pChk_AutoSell = (PAUICHECKBOX)GetDlgItem("Chk_AutoSell");
	m_pChk_AutoSell->Enable(false);
	if( m_pCombo_ProList )
	{
		...
		for(i = 0; i < 10; i++)
		{
			a_sprintf(szText, GetStringFromTable(5302), i + 1);
...
void CDlgProduce::OnTick()
{
	CECHostPlayer *pHost = GetHostPlayer();
	ACHAR szText[100];
	a_sprintf(szText, _AL("%d"), pHost->GetProduceSkillLevel());
	m_pTxt_Level->SetText(szText);
	if( pHost->GetProduceSkillLevel() > 0 && 
		pHost->GetProduceSkillLevel() < 10 )
	{
		int nNow = pHost->GetProduceSkillExp();
		int nMax = pHost->GetProduceUpExp(pHost->GetProduceSkillLevel() + 1);
		a_sprintf(szText, _AL("%d/%d"), nNow, nMax);
		m_pTxt_Exp->SetText(szText);
	}
	else
		m_pTxt_Exp->SetText(_AL("0/0"));
...
void CDlgProduce::OnShowDialog()
{
	GetGameUIMan()->GetDialog("Win_PetFactory")->Show(false);
...
void CDlgProduce::OnCommand_stop(const char * szCommand)
{
...
void CDlgProduce::OnCommand_prolist(const char * szCommand)
{
	int i;
	for(i = 0; i < CDLGPRODUCE_MAX_MATERIALS; i++ )
	{
...
void CDlgProduce::OnCommand_help(const char * szCommand)
{
...
void CDlgProduce::UpdateSelectItem()
{
...
void CDlgProduce::UpdateMaxAmount()
{
		...
		if( m_pTv_Recipe->GetItemData(pItem) != 0 )
		{
			...
			if( datatype == DT_RECIPE_ESSENCE )
			{
				...
				else
				{
					...
					int nMax, nOne;
					nMax = 9999;
					for(int i = 0; i < CDLGPRODUCE_MAX_MATERIALS; i++ )
...
void CDlgProduce::AddOneRecipe(int idRecipe)
{
	const RECIPE_ESSENCE *pRecipe;
	DATA_TYPE datatype;
	elementdataman *pDataMan = g_pGame->GetElementDataMan();
	...
	if( datatype == DT_RECIPE_ESSENCE && pRecipe->recipe_class == 0 && 
		(m_pCombo_ProList->GetCurSel() <= 1 || pRecipe->level == m_pCombo_ProList->GetCurSel() - 1) )
	{
...
void CDlgProduce::UpdateRecipe(int idRecipe)
{
	const RECIPE_ESSENCE *pRecipe;
	DATA_TYPE datatype;
	elementdataman *pDataMan = g_pGame->GetElementDataMan();
	...
	if( datatype == DT_RECIPE_ESSENCE )
	{
		...
		AUICTranslate trans;
		if( szDesc )
			m_pItem_out->SetHint(trans.Translate(szDesc) + GetGameUIMan()->GetCompareEquipDesc(pItem));
		else
		...
		for(j = k; j < CDLGPRODUCE_MAX_MATERIALS; j++ )
		{
			m_pTxt_name[j]->Show(false);
			m_pImgItem[j]->Show(false);
			m_pTxt_no[j]->Show(false);
...
void CDlgProduce::CleanUpRecipe()
{
	DATA_TYPE datatype;
	elementdataman *pDataMan = g_pGame->GetElementDataMan();
	CECHostPlayer *pHost = GetHostPlayer();
...
void CDlgProduce::ProduceEnd()
{
	m_pPrgs_1->SetData(0);
	m_pPrgs_1->SetAutoProgress(0, 0, 0);
}

void CDlgProduce::SetCanAutoSell(bool bAutoSell)
{
	m_pChk_AutoSell->Enable(bAutoSell);
	
	if(bAutoSell)
		m_pChk_AutoSell->Check(false);
}

bool CDlgProduce::GetAutoSellOpt()
{
	if(m_pChk_AutoSell->IsEnabled())
	{
		return m_pChk_AutoSell->IsChecked();
	}

	return false;
}

void CDlgProduce::ShowAutoSell(bool bAutoSell)
{
	m_pChk_AutoSell->Show(bAutoSell);
}