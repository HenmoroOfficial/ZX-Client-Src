...
enum
{
	...
	IVTRTYPE_MOUNTWING,		//  飞剑、坐骑包裹
	IVTRTYPE_GIFT,			// 充值礼包包裹
	IVTRTYPE_FUWEN,			// 符文2013
#ifdef LUA_UI
	//define here not elsewhere to avoid name conflict.   By kuiwu. [18/5/2006]
	IVTRTYPE_EPEQUIPPACK	= 100,	//	Else player's equipment
#endif
	IVTRTYPE_EPPETPACK		= 101,		//	Else player's pet pack
	IVTRTYPE_EPPETEQUIPPACK,//	Else player's pet equipment
	
	IVTRTYPE_CLIENT_BUYBACK	=	1000,	// 回购包裹
};
...
enum
{
	...
	EQUIP_MASK_FASHION_WEAPON	= 0X4000000,
	EQUIP_MASK_ALL				= 0x7ffffff,
};
...
enum
{
	...
	IVTRSIZE_SELLBACKPACK	= 18,	//	Sell back pack
	...
	IVTRSIZE_RIDEPACK	= 18,   //  Mount Wing pack
	IVTRSIZE_GIFTPACK	= 12,
	IUTRSIZE_RUNE2013PACK = 6,
...
//	IVTR_ESSENCE_EQUIPMENT_NEW old_data;	//根据客户端的原有代码状况，决定不通过组合方式增加字段，而通过继承
	...
	int		reserved1;						// 已用(用于染色时装 . 颜色)
	int		reserved2;						// 已用(用于染色时装 . 品级)
	int		reserved3;						// 已用(用于判断是否是时装 .类型)
};
...
struct IVTR_ESSENCE_TALISMAN_MAIN
{
	...
	enum Param_Type
	{
		TYPE_REFINE_LEVEL	= 0,	// 法宝技能等级。对应脚本中96，以下顺延	无用
		TYPE_REFINE_0	,			// 技能id，等于m_Essence.addons[0]		无用
...
struct IVTR_ESSENCE_EQUIPMENT_NEW_FOR_ASTROLOGY : public IVTR_ESSENCE_EQUIPMENT_NEW_FOR_GEM
{
	int curr_hole;	// 当前孔数
	int reserverd1;
	int reserverd2;
...
struct IVTR_ESSENCE_BOTTLE_ESSENCE //2012-10-17 add 添加药剂合并消息结构 by zy
{
	int capacity;
};

struct IVTR_ESSENCE_GIFTBAT_LOTTERYDELIVER
{
	int item_id;
	int item_cnt;
	bool bind;
...
struct IVTR_ESSENCE_LOTTERY_TANGYUAN
{
	int exp;
	char ticket;
};

struct IVTR_SALE_PROMOTION_ITEM_ESSENCE // 台历道具
{
	int usedcount[20];		// 台历中各项已领取次数
	int extra_count;		// 完成的次数
	int is_get_extra_award; // 是否领取了额外奖励
};

struct IVTR_GIFT_PACK_ESSENCE
{
	int apoint;
};

struct IVTR_FUWEN_ESSENCE // 符文2013属性
{
	int exp;
	int reserver1;
	int reserver2;
