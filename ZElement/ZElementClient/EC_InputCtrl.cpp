...
#define KEYISPRESSED(k)	((GetKeyState(k) & 0x8000) ? true : false)
...
bool CECInputCtrl::Tick()
{
	...
	if (m_pComKBFilter)
		m_pComKBFilter->TranslateInput(CECInputFilter::INPUT_KEYBOARD);

	if (m_pComMouFilter)
		m_pComMouFilter->TranslateInput(CECInputFilter::INPUT_MOUSE);

	CECInputFilter* pFilter = PeekKBFilter();
	if (pFilter)
		pFilter->TranslateInput(CECInputFilter::INPUT_KEYBOARD);

	pFilter = PeekMouFilter();
	if (pFilter)
		pFilter->TranslateInput(CECInputFilter::INPUT_MOUSE);
...
//	return (GetKeyState(iKey) & 0x8000) ? true : false;
	...
	return m_aKeyStates[g_pGame->GetInverseMappingKeyCode(iKey)];
}
bool  CECInputCtrl::KeyIsBeingToggled(int iVirtualKey)
{
 	return ISKEYTOGGLED(iVirtualKey);
}
DWORD  CECInputCtrl::GetCtrlKeyStates()
{
	DWORD dwFlags = 0;

	if (ISKEYPRESSED(VK_SHIFT))
		dwFlags |= EC_KSF_SHIFT;

	if (ISKEYPRESSED(VK_CONTROL))
		dwFlags |= EC_KSF_CONTROL;
...
bool CECInputCtrl::DealKeyMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (bGenMsg)
	{
		DWORD dwFlags = m_dwCtrlFlags;
		if (bFirstPress)
			dwFlags |= EC_KSF_FIRSTPRESS;

		WINMSG Msg;
		Msg.uMsg		= message;
		Msg.wParam		= wParam;
		Msg.lParam		= lParam;
		Msg.dwCtrlkeys	= dwFlags;
