...
public:		//	Operations
	...
	// restore the game world after animation world release
	void RestoreGameWorld();

	//	Tick routine
	bool Tick(DWORD dwDeltaTime, CECViewport* pViewport);
	bool TickForAnmSystem(DWORD dwDeltaTime, CECViewport* pViewport);
...
#endif
	//login render
	bool RenderLogin(ECViewport* pViewport);
	...
	//	Load world from file for animation
	bool LoadWorldForAnm(const char* szFile, const char * szDataPath, const A3DVECTOR3& vInitLoadPos);
	//	Set load center
...
// 	}
	...
	const DWORD GetPostEffectFlag() const { return m_dwPostEffectFlag; }
	...
	const A3DPOST_EFFECT_CC_PARAM& GetPostEffectCCParam() const { return m_EffectCC; }
...
protected:	//	Attributes
	...
	CELPrecinct*		m_pCurPrecinctMusic;		//	Current precinct host player is with music
...
// 	AutoMove::CMoveMap   * m_pAutoMoveMap;          //  auto move map
	...
	A3DPOST_EFFECT_CC_PARAM			m_EffectCC;
#endif
...
protected:	//	Operations
	...
	bool IsPrecinctMusicChange(CELPrecinct* pPrecinct1, CELPrecinct* pPrecinct2);
};
#ifdef BACK_VERSION
