...
//	Interval of sending special move command
#define SPECIAL_MOVE_CMD_INTERVAL	100
...
//	m_CmdTimeCnt.SetPeriod(100);
	m_SpecialMoveCmdCnt.SetPeriod( SPECIAL_MOVE_CMD_INTERVAL );
...
void CECHostMove::Tick(DWORD dwDeltaTime)
{
	...
	m_SpecialMoveCmdCnt.IncCounter(dwRealTime);
...
//-----------------------------------------------------------------------------
void CECHostMove::SendStartSpecialMoveCmd( const A3DVECTOR3& velocity, const A3DVECTOR3& acceleration, const A3DVECTOR3& vCurPos, char collisionState )
{
	g_pGame->GetGameSession()->c2s_CmdStartSpecialMove( velocity, acceleration, vCurPos, collisionState );
	m_fMoveTime = 0.0f;
}

void CECHostMove::SendSpecialMoveCmd( const A3DVECTOR3& velocity, const A3DVECTOR3& acceleration, const A3DVECTOR3& curPos, bool bNoDelay, char collisionState )
{
	if (m_pHost->m_CameraCtrl.SceneryMode_Get())
	{
		m_pHost->m_CameraCtrl.SceneryMode_Set(false);
	...
	int iTime = (int)(m_fMoveTime * 1000);
	if ( !bNoDelay )	//有延迟，隔时发协议
	{
		if (!m_SpecialMoveCmdCnt.IsFull())
		{
			return;
		}
		...
		if ( iTime < 50 )	//!!!!!!!!!!暂时写死
		{
			m_SpecialMoveCmdCnt.SetCounter(iTime);
			return;
		}
	}

	m_SpecialMoveCmdCnt.Reset();
	m_fMoveTime = 0.0f;

	g_pGame->GetGameSession()->c2s_CmdSpecialMove( velocity, acceleration, curPos, iTime, m_wMoveStamp++, collisionState );
}

void CECHostMove::SendStopSpecialMoveCmd()
{
	A3DVECTOR3 vDir = m_pHost->GetDir();
	BYTE byDir = glb_CompressDirH(vDir.x, vDir.z);
	m_fMoveTime = 0.0f;
	
	g_pGame->GetGameSession()->c2s_CmdStopSpecialMove( byDir, m_pHost->GetPos(), m_wMoveStamp++ );	
}
