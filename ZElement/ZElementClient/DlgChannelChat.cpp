...
bool CDlgChannelChat::OnInitDialog()
{
	...
	GetGameUIMan()->AttachEmotion(m_pTxt_Content);
	GetGameUIMan()->AttachEmotion(m_pTxt_Chat);
...
void CDlgChannelChat::OnCommandMinimize(const char *szCommand)
{
	GetGameUIMan()->m_pDlgMinimizeBar[0]->MinimizeDialog(this, 
		m_pTxt_Subject->GetText(), CDlgMinimizeBar::MINIMIZEBAR_TYPE_CHATROOM);
}
...
void CDlgChannelChat::OnCommandInvite(const char* szCommand)
{
	...
	pDlg->SetDataPtr((void*)m_nRoomId, "int");
	pDlg->SetData(CDlgInputName::INPUTNAME_CHANNEL_INVITE);
	pDlg->Show(true);
}
...
void CDlgChannelChat::OnEventRButtonUp_ChatMember(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	...
	if( nSel > 0 && nSel < m_pList_ChatMember->GetCount() && idSel != idHostPlayer )
	{
		...
		if( idHostPlayer == idCreator )
			pMenu->EnableMenuItem("Kick", true);
		else
			pMenu->EnableMenuItem("Kick", false);
...
void CDlgChannelChat::OpenNewChatRoom()
{
	...
	m_pList_ChatMember->AddString(GetStringFromTable(774));
	m_pList_ChatMember->SetItemData(0, 0);
	m_pTxt_Content->SetText(_AL(""));
	m_pTxt_Chat->SetText(_AL(""));
	m_idSpeakTo = 0;
	m_pTxt_SpeakTo->SetText(GetStringFromTable(774));
	m_pTxt_SpeakTo->SetAlign(AUIFRAME_ALIGN_CENTER);
...
void CDlgChannelChat::ChannelAction(int idAction, void *pData)
{
	...
	else if( idAction == CHANNEL_ACTION_SPEAK )
	{
		...
		if( !IsShow() )
			GetGameUIMan()->m_pDlgMinimizeBar[0]->FlashDialog(this);
	}
	*/
}
...
{
	ACHAR strText[512];
	AUI_ConvertChatString(szText, strText, false);
	if( idType )
