...
CExplorerMan *g_pExplorerMan = NULL;
CPWBrowser    *g_pBrowser = NULL;
...
BOOL InitBrowser()
{
	...
	g_pExplorerMan = new CExplorerMan();
	g_pExplorerMan->CreateBrowser("win_explorer");
	g_pExplorerMan->CreateBrowser("win_explorer_xf");
	g_pExplorerMan->CreateBrowser("Win_Radio");
	return TRUE;
}
BOOL FreeBrowser()
{
	if(g_pExplorerMan)
	{
		if(g_pExplorerMan->ReleaseUI())
		{
			delete g_pExplorerMan;
...
void CDlgExplorer::NavigateUrl(const WCHAR* Url)
{
	if (NULL == g_pExplorerMan)
	{
		return;
	}
	if(g_pExplorerMan->GetBrowser())
	{
		g_pExplorerMan->GetBrowser()->NavigateUrl(Url);
	}
}

void CDlgExplorer::NavigateUrl(const CHAR* Url)
{
	if (NULL == g_pExplorerMan)
	{
		return;
...
bool CDlgExplorer::OnInitDialog()
{
	...
	g_pExplorerMan->CreateBrowser(m_szName);
...
void CDlgExplorer::OnHideDialog()
{
	if (NULL != g_pExplorerMan)
	{
		g_pExplorerMan->ReleaseUI();
		g_pExplorerMan->HideBrowser();
		g_pExplorerMan->Stop();
		
	}	
}
void CDlgExplorer::OnCommandCancel(const char *szCommand)
{
	Show(false);
	OnHideDialog();
}
...
void CDlgExplorer::OnShowDialog()
{
	...
	{
		//创建浏览器与程序UI同一线程（有问题，假如浏览器窗口死掉，回造成程序死掉）
		if(g_pExplorerMan == NULL)
		{
			//Create
			g_pExplorerMan = new CExplorerMan();
			g_pExplorerMan->BindUI(this);			
			g_pExplorerMan->CreateBrowser(m_szName);
			g_pExplorerMan->ShowBrowser();
			
			m_pExploreMan = g_pExplorerMan;
		}
		else if(g_pExplorerMan->GetDlgExplorer()!=this)
		{
			//Bind,指定消息的CALLBACK 对象
			g_pExplorerMan->BindUI(this);
			g_pExplorerMan->CreateBrowser(m_szName);
			g_pExplorerMan->ShowBrowser();
	...
	else
	{
		...
		m_pExploreMan->CreateBrowser(m_szName);
...
// ExplorerMan
//-----------------------------------------------------------------------------
CExplorerMan::CExplorerMan() : m_mapBrowsers(8)
{
...
CExplorerMan::~CExplorerMan()
{
	...
	//MSDN documentation says "A thread cannot use DestroyWindow to destroy a window created by a different thread.
	for (BrowserTable::iterator it = m_mapBrowsers.begin(); it != m_mapBrowsers.end(); ++it)
	{
		m_pBrowser = it->second;
		if(m_pBrowser!=NULL)
		{
...
BOOL CExplorerMan::CreateBrowser(AString strName)
{	
	if (m_mapBrowsers.find(strName)==m_mapBrowsers.end())
	{
		...
		if(m_pDlg)
		{
			if(GetWindowThreadProcessId(g_pGame->GetGameInit().hWnd,NULL) ==  GetCurrentThreadId())
			{
				...
				m_mapBrowsers[strName] = m_pBrowser;
...
//			if(tid1 ==  tid2)
	...
	{
		m_pBrowser = m_mapBrowsers[strName];
		if(m_pDlg)
		{
			PAUIWINDOWPICTURE m_pWPic = m_pDlg->GetWPic();
			m_pWPic->SetHWnd(m_pBrowser->m_hWnd);
...
BOOL CExplorerMan::BindUI(CDlgExplorer *pDlg)
{
	...
	{
		m_pDlg = pDlg;
		m_pBrowser = m_mapBrowsers[pDlg->GetName()];
	}
	return TRUE;
}
