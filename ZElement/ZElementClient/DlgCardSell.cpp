...
extern const MAX_AUCTION_PRICE = 2000000000;
...
void CDlgCardSell::OnCommandConfirm(const char * szCommand)
{
	...
	if( nPrice > 0 )
		...
		else if( GetGameSession()->GetAccountTime() >= nTime + 135000 + m_nSellPoint )
		{
			ACString strText;
			ACHAR szPrice[256];
			CECIvtrItem::GetPriceText(szPrice, nPrice);
			strText.Format(GetStringFromTable(753), szPrice, nTime / 60);
			PAUIDIALOG pMsgBox;
...
void CDlgCardSell::OnTick()
{
	...
	if( m_nSellPoint != 2000000000)
		...
		a_strcpy(szText, GetStringFromTable(773) );
...
void CDlgCardSell::CardAction(int idAction, void *pData)
{
	if( idAction == CARD_ACTION_GETSELFSELLLIST_RE )
	{
		...
		{
			ACHAR szText[256];
			a_sprintf(szText, GetStringFromTable(772), 
				p->list[i].sellid, p->list[i].point / 60, p->list[i].price);
	...
	else if( idAction == CARD_ACTION_SELLPOINT_RE )
	{
		...
		if( p->retcode == ERR_SUCCESS )
		{
			ACHAR szText[256];
			a_sprintf(szText, GetStringFromTable(772), 
				p->info.sellid, p->info.point / 60, p->info.price);
			m_pLst_Item->AddString(szText);
	...
	else if( idAction == CARD_ACTION_ANNOUNCESELLRESULT )
	{
		...
		if( GetHostPlayer()->GetCharacterID() == p->seller )
		{
			...
			{
				ACHAR szPrice[256];
				CECIvtrItem::GetPriceText(szPrice, p->price);
				szText.Format(GetStringFromTable(742), szPrice, p->sellid, p->point / 60);
		...
		else 
		{
			if( p->retcode== ERR_SUCCESS )
			{
				ACHAR szPrice[256];
				CECIvtrItem::GetPriceText(szPrice, p->price);
				szText.Format(GetStringFromTable(744), szPrice, p->sellid, p->point / 60);
				GetGameUIMan()->AddInformation(CDlgInfo::INFO_SYSTEM, "Game_MsgBox", 
					szText, 0xFFFFFFF, 0, 0, 0);
			}
			else
