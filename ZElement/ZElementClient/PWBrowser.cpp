...
CPWBrowser::CPWBrowser(CExplorer* pParent)
{
...
#ifdef BACK_VERSION
			res = CWindowImplSafe<CPWBrowser>::Create(hWndParent, rcPos, NULL,WS_POPUP, 0, 0, NULL);
#else
...
{
#ifdef BACK_VERSION
	CWindowImplSafe<CPWBrowser>::Create(hWndParent, rcPos, NULL,WS_POPUP|WS_DISABLED|SW_SHOWNORMAL, WS_EX_NOACTIVATE|WS_EX_STATICEDGE, 0, NULL);
#else
...
LRESULT CPWBrowser::OnMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	case WM_CREATE:
		{	
			...
			m_hwndIE = CreateWindow(CAxWindow::GetWndClassName(), _T("Shell.Explorer.2"), WS_CHILD|WS_VISIBLE, 10, 10, 20, 20, 
				m_hWnd, (HMENU)0, GetModuleHandle(NULL), NULL);
			m_hwndBackIE = CreateWindow(CAxWindow::GetWndClassName(), _T("Shell.Explorer.2"), WS_CHILD, 0, 0, 10, 10, 
	...
	case WM_TIMER:
		...
		if(::IsWindow(m_hwndIE))
		{
			...
			if(!hChild) //找不到如子窗口，说明已经被脚本关闭
			{
				...
				HWND hWndtmp = CreateWindow(CAxWindow::GetWndClassName(), _T("Shell.Explorer.2"), WS_CHILD|WS_VISIBLE, 10, 10, rc.right-rc.left, rc.bottom-rc.top, m_hWnd, (HMENU)0, GetModuleHandle(NULL), NULL);
...
{	
	...
	strUrl = bstrUrl;
#else
	strUrl = AS2WC(bstrUrl);
#endif		
    *Cancel = VARIANT_TRUE;
	if(-1 == strUrl.Find(L"embedded=true"))
	{
		AWString strTokenUrl;
#ifndef OLE2ANSI
		strTokenUrl = bstrUrlContext;
#else
		strTokenUrl = AS2WC(bstrUrlContext);
#endif
		int indexToken = strTokenUrl.Find(L"?t=");
		if(-1 == indexToken)
		{
		   indexToken = strTokenUrl.Find(L"&t=");
		}
		if(-1 != indexToken)
		{
			strTokenUrl.CutLeft(indexToken+1);
			if(-1 != strUrl.Find(L"?"))
			{
			    strTokenUrl = L"&" + strTokenUrl;
			}
			else
			{
			    strTokenUrl = L"?" + strTokenUrl;
		...
		CECGameUIMan::OpenUrl(WC2AS(strUrl+strTokenUrl));
	}
    else
	{
...
{	
	if ((GetTickCount()-m_dLastErrorTick)<1000)
		*Cancel = VARIANT_TRUE;
