...
#include "..\\zelementdata\\vector_string.h"
...
void CDlgQuestion::OnCommandMinimize(const char *szCommand)
{
	GetGameUIMan()->m_pDlgMinimizeBar[0]->MinimizeDialog(this, 
		GetStringFromTable(905), CDlgMinimizeBar::MINIMIZEBAR_TYPE_QUESTION);
}
...
void CDlgQuestion::OnTick()
{
	...
	int nRestTime = endtime - GetTickCount();
	if( nRestTime <= 0 )
		Show(false);
	ACString strText;
	if( nRestTime >= 60000 )
...
bool CDlgQuestion::Render()
{
	...
	if( type == 3 )
	{
		...
		{
			A3DVECTOR3 *verts = new A3DVECTOR3[m_vecPath[j].size()];
			WORD *indices = new WORD[m_vecPath[j].size() * 2];
			for(int l = 0; l < (int)m_vecPath[j].size(); l++)
			{
				verts[l].x = pt.x + fWindowScale * m_vecPath[j][l].x;
				verts[l].y = pt.y + fWindowScale * m_vecPath[j][l].y;
				verts[l].z = 0.0f;
		...
		for(int i = 0; i < 6; i++ )
			if( m_pAnswer[i] )
			{
				...
				for(int j = 0; j < vstring.GetCharNum(); j++)
				{
					...
					int px = int(16 * fWindowScale + 2) * j;
					for(int k = 0; k < c->m_nPathNum; k++)
					{
						VECTOR_PATH *p = c->m_pPathData[k];
						A3DVECTOR3 *verts = new A3DVECTOR3[p->m_nPtNum];
						WORD *indices = new WORD[p->m_nPtNum * 2];
						for(int l = 0; l < p->m_nPtNum; l++)
						{
							verts[l].x = px + pt.x + fWindowScale * (p->m_pPtData[l] >> 4 & 15);
							verts[l].y = pt.y + fWindowScale * (15 - p->m_pPtData[l] & 15);
							verts[l].z = 0.0f;
							if( l < p->m_nPtNum - 1 )
							{
								indices[l * 2] = l;
...
void CDlgQuestion::SetQuestion(void *pData)
{
	...
	if( type == 3 )
	{
		char *pPath = new char[p->question[0].size()];
