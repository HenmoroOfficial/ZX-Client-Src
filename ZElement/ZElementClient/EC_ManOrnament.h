...
#else
class CECOrnamentMan : public ECManager
#endif
{
public:		//	Types
...
public:		//	Operations
	...
	DWORD LoadEffect(CECScene* pScene, AFile* pFile, float fOffX, float fOffZ);
	...
	DWORD LoadECModel(AFile* pFile, float fOffX, float fOffZ);
	...
	DWORD LoadSoundObject(CECScene* pScene, AFile* pFile, float fOffX, float fOffZ);
	...
	void AddLandBuild(int tId, int nTempId,int nStatus,int nLevel);
	void DelLandBuild(int tId);
	void ChangeLandBuild(int tId, int nTempId,int nLevel,int nStatus);
	void ClearrLandBuild();
protected:	//	Attributes
	...
	APtrArray<EC_LandBuild*> m_aLandBuilds;
