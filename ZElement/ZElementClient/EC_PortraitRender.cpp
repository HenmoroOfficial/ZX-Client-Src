...
bool CECPortraitRender::Init(int nPortraitMapSize)
{
	...
	if( !m_pPortraitMapCamera->Init(pA3DDevice, DEG2RAD(20.0f), 0.05f, 40.0f, 1.0f) )
	{
...
bool CECPortraitRender::RenderPortraitToSprite(A3DViewport * pViewport, A2DSprite * pSprite, LPRENDERFORPORTRAIT pfnRenderForPortrait, LPVOID pArg)
{
	...
	// first render into portrait's map
	//pA3DDevice->SetRenderTarget(m_pPortraitMapTarget);
	m_pPortraitMapTarget->ApplyToDevice();
	...
	//pA3DDevice->RestoreRenderTarget();
	m_pPortraitMapTarget->WithdrawFromDevice();
	pViewport->Active();
	...
	m_pSysBackTarget->GetD3DTexture()->GetSurfaceLevel(0, &pD3DSurface);
	hval = pA3DDevice->StretchRect(m_pPortraitMapTarget->GetTargetSurface()->m_pD3DSurface, NULL, pD3DSurface, NULL, A3DTEXF_LINEAR);
	if( D3D_OK != hval )
		return false;
	...
	int nSrcPitch;
	//D3DLOCKED_RECT lock;
	if(!m_pSysBackTarget->LockRect(NULL, (void**)&pSrc, &nSrcPitch, 0, 0) )
		return false;
	...
	{
		memcpy(pDest + lockSprite.Pitch * i,
			pSrc + nSrcPitch * i, 4 * m_nPortraitMapSize);
	}
...
bool CECPortraitRender::AfterDeviceReset()
{
	...
	// 	hval = g_pGame->GetA3DDevice()->GetD3DDevice()->CreateRenderTarget(m_nPortraitMapSize, m_nPortraitMapSize, 
	// 		(D3DFORMAT)m_fmtTexture, D3DMULTISAMPLE_NONE, TRUE, &m_pSysBackTarget);
	m_pSysBackTarget = new A3DTexture;
	//m_pSysBackTarget->Create(g_pGame->GetA3DDevice(), m_nPortraitMapSize, m_nPortraitMapSize, m_fmtTexture);
	
	if(!m_pSysBackTarget->Create(g_pGame->GetA3DDevice(), m_nPortraitMapSize, m_nPortraitMapSize, m_fmtTexture))
	{
		g_Log.Log("CECPortraitRender::Create sys back render target fail");
		return false;
	}
