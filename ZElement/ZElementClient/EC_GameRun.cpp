...
//	Black the screen using dwBlackColor when HostPlayer is in special state
//	Alpha from nMaxAlpha/2 -> nMaxAlpha, then from nMaxAlpha -> nMaxAlpha/2
//	Default half-cycle time is 5000ms
void BlackScreen(A3DDevice* pA3DDevice, bool bBlink, DWORD dwTick, const int nMaxAlpha, A3DCOLOR dwBlackColor)
{
	if (bBlink)
	{
		static DWORD dwFadeCounter = 0;
		...
		if (dwFadeCounter >= 5000)
		{
			dwFadeCounter = 0;
		}
		float a;
		if (dwFadeCounter <= 1000)
			a = 0.1f*(float)dwFadeCounter / 1000.0f + 0.85f;
		else if (dwFadeCounter > 1000 && dwFadeCounter <= 4000)
			a = 0.95f;
		else
			a = 0.95f - 0.1f*(float)(dwFadeCounter - 4000)/1000.0f;

		int b = (int)(a*nMaxAlpha);
		a_ClampRoof(b, nMaxAlpha);

		DrawFadeColor(pA3DDevice, b, dwBlackColor);
	}
}
...
#ifndef BACK_VERSION
	m_pManLoginUIMan = NULL;
	m_pManAnmSystem = NULL;
#endif
	m_bGTLogin = false;
	m_strLoginIP = "";
	m_iLoginIPTime = INT_MAX;
}
...
// 		return false;
// 	}
	if (!(m_pECHelper = new CECAutoHelper))
	{
		glb_ErrorOutput(ECERR_NOTENOUGHMEMORY, "CECGameRun::Init", __LINE__);
		return false;
	}
	...
	if(!(m_pNewHelpGuide = new CECNewHelpGuide))
	{
		glb_ErrorOutput(ECERR_NOTENOUGHMEMORY, "CECGameRun::Init", __LINE__);
		return false;
	}
	if(!m_pNewHelpGuide->Init())
	{
		a_LogOutput(1, "CECGameRun::Init, Failed to initialize ECNewHelpGuide.");
		delete m_pNewHelpGuide;
		m_pNewHelpGuide = NULL;
	}
	...
	if(!(m_pHelpGraphGuide = new CECHelpGraphGuide))
	{
		glb_ErrorOutput(ECERR_NOTENOUGHMEMORY, "CECGameRun::Init", __LINE__);
		return false;
	}
	...
	// animation system
	if (!(m_pAnmSystem = new CAnmSystem))
	{
		glb_ErrorOutput(ECERR_NOTENOUGHMEMORY, "CECGameRun::Init", __LINE__);
		return false;
	}
	// init animation system and load animation config
	if (!m_pAnmSystem->Init(g_pGame->GetGameInit().hInstance, g_pGame->GetGameInit().hWnd))
	{
		a_LogOutput(1, "CECGameRun::Init, Failed to initialize AnmSystem.");
		return false;
	}
	...
	gameObjs.pImgRes = g_pGame->GetImageRes();
	gameObjs.pUIManager = m_pUIManager;
	// gameObjs.pShadowRender = g_pGame->GetShadowRender();
	...
	m_pRenderParam->fShadowDistance = 15.0f; 
	m_pRenderParam->eShadowMapSize = A3D_SHADOW_MAP_SIZE_LARGE;
	m_pRenderParam->eShadowSampleNum = A3D_SHADOW_MAP_SAMPLE_NUM_HIGH;

	m_pElementRender->SetAlphaModelWriteZ(true);

	m_pManLoginUIMan = new EC_ManLoginUIMan(this);
	m_pManAnmSystem = new CManagerWrapperForAnmSystem(m_pAnmSystem);
#endif
	return true;
}
...
void CECGameRun::Release()
{
	...
	if (m_pHelpGraphGuide)
	{
		delete m_pHelpGraphGuide;
		m_pHelpGraphGuide = NULL;
	}
	...
	A3DRELEASE(m_pManLoginUIMan);
	A3DRELEASE(m_pManAnmSystem);
#endif
}
...
void CECGameRun::LoadSpecialIDConfig()
{
	...
	MONSTER_ESSENCE *pEss1 = (MONSTER_ESSENCE*)g_pGame->GetElementDataMan()->get_data_ptr(41858, ID_SPACE_ESSENCE, dt);
	TALISMAN_MAINPART_ESSENCE *pEss3 = (TALISMAN_MAINPART_ESSENCE*)g_pGame->GetElementDataMan()->get_data_ptr(40468, ID_SPACE_ESSENCE, dt);
	FIREWORKS_ESSENCE *pEss2 = (FIREWORKS_ESSENCE*)g_pGame->GetElementDataMan()->get_data_ptr(13943, ID_SPACE_ESSENCE, dt);

	while (idEssence)
	{
		if (dt == DT_SPECIAL_ID_CONFIG)
		{
			if (idEssence==SPECIAL_ID_CONFIG_TEMPLID)
		...
		{
			m_vecDropInternalConfigs.push_back(*(DROP_INTERNAL_CONFIG*)g_pGame->GetElementDataMan()->get_data_ptr(idEssence, ID_SPACE_CONFIG, dt));
		}
		else if (dt == DT_COLLISION_RAID_TRANSFIGURE_CONFIG)
		{
			m_CollisionRaidTransfigureConfig = *(COLLISION_RAID_TRANSFIGURE_CONFIG*)g_pGame->GetElementDataMan()->get_data_ptr(idEssence, ID_SPACE_CONFIG, dt);
		}
		else if (dt == DT_COLLISION_RAID_AWARD_CONFIG)
		{
			m_CollisionRaidAwardConfig = *(COLLISION_RAID_AWARD_CONFIG*)g_pGame->GetElementDataMan()->get_data_ptr(idEssence, ID_SPACE_CONFIG, dt);
		}
...
void CECGameRun::DropSpeaking(int itemId, int remainNum, int roleId)
{
	unsigned int i;
	for (i=0; i<m_vecDropInternalConfigs.size(); ++i)
	{
		if (itemId==m_vecDropInternalConfigs[i].item_id)
		{
			ACString strText;
			CECIvtrItem *pItem = NULL;
			if (roleId==0)
			{
				if (m_vecDropInternalConfigs[i].speak_words[0]==0 || remainNum==0)
					break;
				else
				{
					ACString strItem;
					EditBoxItemBase item(enumEIIvtrlItem);
					strItem = (ACHAR)AUICOMMON_ITEM_CODE_START + item.Serialize();
					strText.Format(m_vecDropInternalConfigs[i].remain_words, remainNum, strItem);
					pItem = CECIvtrItem::CreateItem(itemId, 0, 1);
			...
			else
			{
				...
				EditBoxItemBase item(enumEIIvtrlItem);
				strItem = (ACHAR)AUICOMMON_ITEM_CODE_START + item.Serialize();
				strText.Format(m_vecDropInternalConfigs[i].speak_words, strItem);
				pItem = CECIvtrItem::CreateItem(itemId, 0, 1);
				pItem->GetDetailDataFromLocal();
			}

			AddChatMessage(strText, GP_CHAT_SYSTEM, -1, NULL, 0, 0, pItem);
			break;
		}
	}
...
bool CECGameRun::StartGame(int idInst, const A3DVECTOR3& vHostPos)
{
	...
	if (!pInst)
	{
		a_LogOutput(1, "CECGameRun::StartGame, wrong instance id: %d", idInst);
		return false;
	}
	...
	else if (pInst->GetID() == 401)
	{
		int index = pInst->GetIndexByCoord(vHostPos.x, vHostPos.z);
		if (index != -1)
		{
			char szNum[10] = {0};
			itoa(index+1, szNum, 10);
			strPath += "-";
			strPath += szNum;
		}			
	}
	...
	//	Begin load progress
	if (!gr.is_seeking())
		BeginLoadProgress(CECLoadProgress::LOAD_ENTERGAME, szBackPic);
...
bool CECGameRun::StartLogin(bool bFirst)
{
	...
	//	Begin load progress
	BeginLoadProgress(CECLoadProgress::LOAD_LOGIN, NULL);
...
void CECGameRun::EndGameState(bool bReset/* true */)
{
	...
	//FIXME!! 这里必须去掉Reset. Reset会导致A3DTextureMan中所有的纹理资源被释放掉! 带来的结果是之前所有用A3DTextureMan创建出的纹理指针都会失效
	// 但去掉这个东西的副作用是更换World之后也不会释放老的资源了.. 得重构一下这里的实现.
...
bool CECGameRun::CreateAnmWorld(int id, const A3DVECTOR3& vCameraPos)
{
	CECInstance* pInst = GetInstance(id);
	if (!pInst)
	{
		a_LogOutput(1, "CECGameRun::CreateAnmWorld, wrong instance id: %d", id);
		return false;
	}

	if (!(m_pAnmWorld = new CECWorld(this)))
	{
		glb_ErrorOutput(ECERR_NOTENOUGHMEMORY, "CECGameRun::CreateAnmWorld", __LINE__);
		return false;
	}

	if (!m_pAnmWorld->Init(id))
	{
		a_LogOutput(1, "CECGameRun::CreateAnmWorld, Failed to initialize world.");
		return false;
	}
	...
	AString strPath = pInst->GetPath();
	if (pInst->GetID() == 12 && vCameraPos.x > 355 && vCameraPos.x < 512 && vCameraPos.z > 353 && vCameraPos.z < 512 )
	{
		strPath += "-1";
	}
	else if (pInst->GetID() == 401)
	{
		int index = pInst->GetIndexByCoord(vCameraPos.x, vCameraPos.z);
		if (index != -1)
		{
			char szNum[10] = {0};
			itoa(index+1, szNum, 10);
	...
	//	Begin load progress
	BeginLoadProgress(CECLoadProgress::LOAD_JUMPINSTANCE, szBackPic);
	...
	if (!m_pAnmWorld->LoadWorldForAnm(szFile, szDataPath, vCameraPos))
	{
		a_LogOutput(1, "CECGameRun::CreateAnWorld, Failed to load world.");
		return false;
	}
...
void CECGameRun::ReleaseAnmWorld()
{
	...
	if (CECWorld* pAnmWorld = m_pAnmWorld)
	{
		m_pAnmWorld = NULL;
		pAnmWorld->ReleaseForAnm();
		delete pAnmWorld;
		pAnmWorld = NULL;
	}
}
...
#else
	g_pGame->GetViewport()->GetA3DCameraBase()->SetPos(vCameraPos);
#endif 
...
//	m_pTeamSCS->SetShortcut(2, pSC);
	...
	// Active pose command shortcut set
	m_pActivePoseSCS = new CECShortcutSet;
	m_pActivePoseSCS->Init(NUM_ROLEEXP_AP);

	for (int i=0; i < NUM_ROLEEXP_AP; ++i)
	{
		pSC = new CECSCCommand(CECSCCommand::CMD_PLAYACTIVEPOSE);
		pSC->SetParam(i);
		m_pActivePoseSCS->SetShortcut(i, pSC);
	}
...
//	Get active pose command shortcut by pose
CECSCCommand* CECGameRun::GetActivePoseCmdShortcuts(int iPose)
{
	ASSERT(m_pActivePoseSCS);
	for (int i = 0; i < m_pActivePoseSCS->GetShortcutNum(); ++i)
	{
		CECSCCommand* pCmdSC = (CECSCCommand*)m_pActivePoseSCS->GetShortcut(i);
		if (pCmdSC && pCmdSC->GetParam() == (DWORD)iPose)
			return pCmdSC;
	}
...
void CECGameRun::ToggleHideElsePlayer()
{
	if (!m_pWorld)
		return ;
	if (!m_pHostPlayer)
...
bool CECGameRun::DealWindowsMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	//{
		...
		//m_bHideElsePlayer = !m_bHideElsePlayer;
		//LuaEvent::FirstHideElsePlayer();
		//ToggleHideElsePlayer();
		//return true;
	//}
	
	if (GetHelpGraphGuide()->DealWndProc(message, wParam, lParam))
	{
		g_pGame->ShowCursor(true);
		g_pGame->ChangeCursor(RES_CUR_NORMAL);
		return false;
...
//			m_bUIHasCursor = false;
		...
		case WM_SYSKEYUP:
			...
			else
			{
				...
				else if (m_pInputCtrl->DealKeyMessage(message, wParam, lParam))
					return true;
			}
...
GNET::GamedataSend gen_server_time()
{
	...
	add_to_octets<int>(o, g_pGame->GetTimeZoneBias());
...
void restore_automove_data(GNET::Octets& o)
{
	CECGameRun* pGameRun = g_pGame->GetGameRun();
	CECHostPlayer* pHost = pGameRun->GetHostPlayer();
	try
	{
		...
		bool bAjust = (iTemp==1)? true:false;
		g_ECAutoMove.SearchPath(vStart, vEnd, iInst, bAjust);	
	}
	catch(Marshal::Exception &)
	{
	}
...
void CECGameRun::StartRecordGame()
{
	...
	gr.set_host_info(
		...
		m_pHostPlayer->GetEarid(),
		m_pHostPlayer->GetTailid(),
		m_pHostPlayer->GetDefaultSkinid(),
		m_pHostPlayer->GetProfession(),
...
void CECGameRun::RecordSaveKeyPoint()
{
	...
	fr->push_event(new event_record_protocol(&m_pHostPlayer->gen_reborn_info()));
	...
	fr->push_event(new event_record_protocol(&m_pHostPlayer->GetPetPack()->gen_item_detail_data(IVTRTYPE_PET)));
	fr->push_event(new event_record_protocol(&m_pHostPlayer->GetPetEquipPack()->gen_item_detail_data(IVTRTYPE_PET_EQUIP)));
...
{
	// 录像机部分之录像回放
	if (gr.get_start_replay_flag())
	{
		gr.clear_start_replay_flag();

		if (!_start_replay(gr.get_file_to_open()))
		{
			CECLoginUIMan* pUIMan = GetUIManager()->GetLoginUIMan();

			if (pUIMan)
				pUIMan->MessageBox("", pUIMan->GetStringFromTable(220), MB_OK, A3DCOLORRGBA(255, 255, 255, 160));
		}
	...
	// 录像录制
	if (gr.get_end_record_flag())
	{
		gr.clear_end_record_flag();
		...
		if (gr.get_state() == game_record::enum_gr_record)
		{
			...
			gr.release();
			gr.set_state(game_record::enum_gr_disable);
			AddFixedMessage(FIXMSG_RECORD_END);
		}
	}
	else if (gr.get_start_record_flag())
	{
		gr.clear_start_record_flag();
	...
	// 录像回放
	if (gr.get_state() == game_record::enum_gr_replay
	 && GetGameState() == CECGameRun::GS_GAME
	 && gr.is_paused()
	 && (!gr.get_seek_flag() || gr.get_fade_state() == game_record::enum_fade_out))
	{
		if (gr.get_fade_state() == game_record::enum_fade_out)
		{
			if (l_RecordFadeCnt.IncCounter(dwDeltaTime))
			{
				gr.set_paused(false);
				return true;
			}
		...
		if (bHostReady)
		{
			m_pHostPlayer->UpdateFollowCamera(false, dwDeltaTime);
#ifdef BACK_VERSION
	...
	{
		if (g_pGame->GetGameRun()->GetWallowInfo().anti_wallow_active!=0)
		{
			...
			for (i=1; i<48; ++i)
			{
				if ( nTime>i*3600 && m_nWallowStage==i-1)
				{
					ACString strText;
					strText.Format(pGameUI->GetStringFromTable(9314), i);
					pGameUI->AddChatMessage(strText, GP_CHAT_BROADCAST);
					pGameUI->CommonMessage("Win_Kr_AntiWallow", "show", i, 0);
					m_nWallowStage = i;
					break;
				}
			}
	...
	{
		if (gr.get_total_time() >= gr.get_check_time() + CHECK_RECORD_INTERVAL)
		{
			gr.set_check_time(gr.get_total_time());
			int item_pos = get_record_item_pos();
			...
			else
			{
				...
				CECGameUIMan* pUIMan = GetUIManager()->GetInGameUIMan();
				pUIMan->GetDialog("Win_Record")->Show(false);
				pUIMan->GetDialog("Win_RecordEnd")->Show(false);
				AddFixedMessage(FIXMSG_RECORD_END);
			}
		}

		if (gr.get_total_time() >= gr.get_kp_save_time() + RECORD_KEY_POINT_INTERVAL)
		{
			gr.set_kp_save_time(gr.get_total_time());
			RecordSaveKeyPoint();
		}

		gr.inc_frame()->set_delta_time((unsigned char)dwDeltaTime);
	}
	...
	if (gr.get_seek_flag())
	{
		...
		if (gr.seek())
		{
			GetUIManager()->GetInGameUIMan()->EnableUI(false);
			GetUIManager()->GetInGameUIMan()->Tick(dwDeltaTime);
			Render();
			StartGame(gr.get_instance(), gr.get_host_pos());

			if (gr.get_fade_state() == game_record::enum_fade_out)
			{
				gr.set_fade_state(game_record::enum_fade_in);
				l_RecordFadeCnt.Reset();
			}
	...
	// replay
	if (gr.get_state() == game_record::enum_gr_replay && GetGameState() == CECGameRun::GS_GAME && !gr.get_end_replay_flag())
	{
		if (!g_pGame->IsActive() && !gr.is_paused())
		{
			CECGameUIMan* pUIMan = GetUIManager()->GetInGameUIMan();
			CDlgReplay* pDlg = (CDlgReplay*)pUIMan->GetDialog("Win_Replay");

			if (pDlg && m_pHostPlayer && m_pHostPlayer->HostIsReady())
				pDlg->OnCommand("pause");

			return true;
		}
		// if host player out of work, restore host player work.
		if (gr.get_seek_adjust() != game_record::enum_seek_none)
		{
			if(m_pHostPlayer->GetHPWorkMan() && m_pHostPlayer->GetHPWorkMan()->GetCurWorkID() != CECHPWork::WORK_FREEFALL)
			{	
				m_pHostPlayer->GetHPWorkMan()->AdjustHPWork();
			}
		}
		// force to stop auto move at the end of auto moving.
		if(m_pHostPlayer->GetHPWorkMan() && m_pHostPlayer->GetHPWorkMan()->GetCurWorkID() == CECHPWork::WORK_AUTOMOVE)
		{
			m_pHostPlayer->GetHPWorkMan()->StopAutoMove();
		}

		if (gr.get_record_time() >= gr.get_check_time() + CHECK_RECORD_DELAY)
		{
			if (!gr.replay_is_valid())
			{
				::MessageBoxA(NULL, "Replay is invalid!", "Error", MB_OK);
				Logout(0);
				return true;
			}
			else
			...
			gr.set_check_time(gr.get_check_time() + CHECK_RECORD_INTERVAL);
		}
		...
		if (gr.get_fade_state() == game_record::enum_fade_in)
		{
			...
			{
				l_RecordFadeCnt.Reset();
				gr.set_fade_state(game_record::enum_fade_none);
			}
		}

		if (gr.get_seek_remain_time())
		{
			dwDeltaTime = gr.get_seek_remain_time();
			gr.reset_seek_remain_time();
		}
		else if (dwDeltaTime)
		{
			dwDeltaTime = DWORD(dwDeltaTime * gr.get_play_speed());

			if (dwDeltaTime == 0)
				dwDeltaTime = 1;

			DWORD dwRealTickTime = DWORD(g_pGame->GetRealTickTime() * gr.get_play_speed());

			if (dwRealTickTime == 0)
				dwRealTickTime = 1;

			g_pGame->SetRealTickTime(dwRealTickTime);
		}

		gr.inc_real_time(dwDeltaTime);

		while (gr.get_real_time() > gr.get_record_time())
		{
			frame_record* fr = gr.read_one_frame(false);

			if (gr.is_replay_ended())
			{
				CECGameUIMan* pUIMan = GetUIManager()->GetInGameUIMan();
				pUIMan->MessageBox("Game_ReplayEnd", pUIMan->GetStringFromTable(6680), MB_YESNO,
					A3DCOLORRGBA(255, 255, 255, 160));
				gr.set_end_replay_flag();
				break;
			}
			...
			event_vec& recs = fr->get_events();

			for (size_t i = 0; i < recs.size(); i++)
			{
				event_record* event = recs[i];

				if (event->get_type() == EVENT_TYPE_PROTOCOL)
				{
					GNET::Protocol* p = static_cast<event_record_protocol*>(event)->get_protocol();

					if (p)
						pSession->AddNewProtocol(p->Clone());
				}
				else if (event->get_type() == EVENT_TYPE_MSG)
				{
					ECMSG& msg = static_cast<event_record_msg*>(event)->get_msg();
					PostMessage(msg);
				}
				else if (event->get_type() == EVENT_TYPE_DATA)
				{
					int data_type = static_cast<event_record_data*>(event)->get_data_type();
					GNET::Octets& o = static_cast<event_record_data*>(event)->get_data();

					if (data_type == RECORD_DATA_PLAYER_CACHE)
						m_pWorld->GetPlayerMan()->create_cache_from_record(o);
					else if (data_type == RECORD_DATA_TEAM)
					{
						CECTeam* pTeam = m_pTeamMan->create_team_from_record(GNET::Marshal::OctetsStream(o));

						if (pTeam)
							m_pHostPlayer->SetTeam(pTeam);
					}
					else if (data_type == RECORD_DATA_SKILL_ADDON)
						m_pHostPlayer->set_skill_addon(o);
					else if (data_type == RECORD_DATA_FACTION)
						g_pGame->GetFactionMan()->unmarshal(GNET::Marshal::OctetsStream(o));
					else if (data_type == RECORD_DATA_EXTRA_DATA)
						apply_record_extra_data(o);
					else if (data_type == RECORD_DATA_CHAT_MSG)
						record_add_chat_msg(this, o);
					else if (data_type == RECORD_DATA_AUTOMOVE) 
						restore_automove_data(o);
				}
			}

			gr.inc_record_time(fr->get_delta_time());
...
#else
			g_pGame->GetViewport()->GetA3DCameraBase()->UpdateEar();
#endif
	}
...
// 		m_bTrayFlash = false;
	if (m_pAnmSystem->IsPlaying())
	{
		m_pAnmSystem->Tick(dwDeltaTime);
	}

	if (m_iLoginIPTime!=INT_MAX)
	{
		m_iLoginIPTime -= dwDeltaTime;
		if (m_iLoginIPTime<0)
		{
			m_iLoginIPTime = INT_MAX;
			...
			// do something
			char szpost[100];
			sprintf(szpost, "type=antifs&time=%dgame=zhuxian&srv=%s", g_pGame->GetServerGMTTime(), m_strLoginIP);
			int iPostLen = strlen(szpost);
			char *bufEncode = new char[(iPostLen+2)/3*4+32];
			int iOutLen = base64_encode((unsigned char*)szpost, iPostLen, bufEncode);
			m_strRecordUrl.Format("http://analytics.wanmei.com/__mtx_zx.gif?%s", bufEncode);
			
			DWORD dwThreadID;
			CreateThread(NULL, 0, CDlgBaseExplorer::OpenURL, m_strRecordUrl.GetBuffer(0), 0, &dwThreadID);
			
			//a_LogOutput(1, "PostInfoOrg:%s", szpost);
			//a_LogOutput(1, "PostInfoBase64:%s", bufEncode);
			delete [] bufEncode;
		}
	}
	return true;
...
void CECGameRun::CheckDefenceCode()
{
	while (true)
	{
		...
		g_bHostInGame = (m_pHostPlayer && m_pHostPlayer->HostIsReady() 
			&& g_pGame->GetGameSession()->GetNetManager()->IsInStateKeeping());
...
bool CECGameRun::TickGameWorld(DWORD dwDeltaTime, CECViewport* pViewport)
{
	...
	if (!m_pAnmSystem->IsPlaying() && bHostReady && !m_pHostPlayer->IsChangingFace())
	{
	...
	if(bHostReady && m_pSecretary && m_pHelpGraphGuide)
	{
		m_pSecretary->Tick(dwDeltaTime);
		m_pHelpGraphGuide->Tick(dwDeltaTime);
	}

	if(bHostReady && m_pNewHelpGuide)
	{
	    m_pNewHelpGuide->Tick(dwDeltaTime);
	}

	return true;
}
// Login Render
bool CECGameRun::RenderLogin(bool bPresent)
{
	if (!m_pWorld)
		return true;
	...
	if (m_pWorld)
	{
		...
		m_pWorld->RenderLogin(pViewport);
	}
	...
	if (m_pHostPlayer)
	{
		BlackScreen(pA3DDevice, m_pHostPlayer->IsInDarkState(), g_pGame->GetTickTime(), 255, A3DCOLORRGB(0,5,5));
		if(m_pHostPlayer->IsInDarkState())
		{
			if( m_pUIManager->GetCurrentUIManPtr() )
				m_pUIManager->GetCurrentUIManPtr()->GetA3DFTFontMan()->Flush();
			if (m_pUIManager)
...
bool CECGameRun::Render(bool bPresent)
{
	...
	if(GetGameState() == CECGameRun::GS_LOGIN && !m_pAnmSystem->IsPlaying() && !gr.is_seeking())
	{
	    return RenderLogin(bPresent);
	}
#ifdef BACK_VERSION
	if (m_pAnmSystem->IsPlaying())
	{
		m_pAnmSystem->Render(bPresent);
		return true;
	}
	...
	if (m_pAnmSystem->IsPlaying())
	{
		m_pWorld->UnRegisterManagers();
		GetElementRender()->UnRegisterManager(m_pManLoginUIMan);

		GetElementRender()->RegisterManager(m_pManAnmSystem);

		pA3DEngine->BeginRender();

		pViewport->Activate(false);
		pA3DDevice->Clear( D3DCLEAR_ZBUFFER, 0xff000000, 1, 0 );
		m_pRenderParam->vHostPos = m_pAnmSystem->GetCenterPos();
		m_pRenderParam->paramPostEffect.dwPostEffectMask = 0;
		m_pRenderParam->bRenderPostProcess = true;
		m_pRenderParam->bRenderShadow = true;
		m_pRenderParam->bRenderWater = true;
		m_pRenderParam->bRenderUI = false;
		GetElementRender()->Render(pViewport, m_pRenderParam);
	...
	{
		m_pWorld->RegisterManagers();
		GetElementRender()->UnRegisterManager(m_pManAnmSystem);
	}

	m_pRenderParam->vHostPos = GetSafeHostPos(pViewport);
	...
	if (m_pWorld)
	{
		...
		else if ( GetGameState() == CECGameRun::GS_GAME )
		{
		...
		{
			CECLoginUIMan* pLoginUI = m_pUIManager->GetLoginUIMan();
			m_pManLoginUIMan->SetLoginUIMan(pLoginUI);
			m_pElementRender->RegisterManager(m_pManLoginUIMan);
		}
		else
			m_pElementRender->UnRegisterManager(m_pManLoginUIMan);
...
{
	GetSecretary()->SaveStatesToServer();
	if (!m_pWorld || 
...
bool CECGameRun::SaveConfigsToLocal()
{
	if (!m_pWorld || !m_pWorld->GetHostPlayer() || 
		!m_pWorld->GetHostPlayer()->HostIsReady() || 
		!m_pUIManager->GetInGameUIMan())
		return false;
...
bool CECGameRun::LoadConfigsFromServer(const void* pDataBuf, int iDataSize)
{
	...
	if ( (GetAsyncKeyState('7') & 0x8000) && (GetAsyncKeyState('8') & 0x8000) )
	{
		return true;
	}
	...
	if (dwVer >= 3)
	{
		...
		if (m_bLocalNew)
		{
			pData = m_pCfgDataBuf + sizeof (DWORD);
			dwRealLen = m_iCfgDataSize - sizeof (DWORD);
			m_bLocalNew = false;
		}
	}
...
void CECGameRun::AddChatMessage(const ACHAR *pszMsg, char cChannel, int idPlayer/* -1 */, 
								const ACHAR* szName/* NULL */, BYTE byFlag, char cEmotion, CECIvtrItem* pItem, sAchievement* pAchievement)
{
	CECGameUIMan* pGameUI = m_pUIManager->GetInGameUIMan();
	if (pGameUI)
		pGameUI->AddChatMessage(pszMsg, cChannel, idPlayer, szName, byFlag, cEmotion, pItem, pAchievement);
	else
	{
...
//	Begin load progress
bool CECGameRun::BeginLoadProgress(int iLoadID, const char* szBackPic)
{
...
#else
		return pViewport->GetA3DCameraBase()->GetPos();
#endif
	}
}
...
const ACHAR* CECGameRun::GetPlayerName(int cid, bool bAutoGen)
{
	...
	{
		CECPlayer * pPlayer = GetWorld()->GetPlayerMan()->GetPlayer(cid);
		if (!pPlayer)
			return NULL;
		cid = pPlayer->GetSummonerID();
	}
...
bool CECGameRun::IsIn6v6Battle()
{
	return GetWorld()->GetInstanceID()==99999;		// 屏蔽
}

void CECGameRun::AddLstMsgType(int cid, int type)
{
	if (!ISPLAYERID(cid))
		return;
	if (ISCLONEPLAYERID(cid))
	...
	ID2LastMsgTypeTable::pair_type Pair = m_ID2MsgType.get(cid);
	if (Pair.second)
	{
		m_ID2MsgType.erase(cid);
	}
	m_ID2MsgType.put(cid, type);
}

void CECGameRun::EraseLstMsgType(int cid)
{
	ID2LastMsgTypeTable::pair_type Pair = m_ID2MsgType.get(cid);
	if (Pair.second)
	{
...
int CECGameRun::PlayerLstMsgType(int cid)
{
	ID2LastMsgTypeTable::pair_type Pair = m_ID2MsgType.get(cid);
	if (Pair.second)
		return *Pair.first;
...
bool CECGameRun::JumpToInstance(int idInst, const A3DVECTOR3& vPos)
{
	...
	if (m_pWorld)
	{
		if (m_pWorld->GetInstanceID() == pInst->GetID())
		{
			...
			return true;
			/*
			bool bTerrainLoaded = false, bSceneLoaded = false;
			int i, iNumBlock;
			iNumBlock = m_pWorld->GetTerrain()->GetActiveBlocks()->aBlocks.GetSize();
			for(i = 0; i < iNumBlock; i++)
			{
				A3DTerrain2Block* pBlock = m_pWorld->GetTerrain()->GetActiveBlocks()->aBlocks[i];
				if(pBlock)
				{
					A3DAABB aabb = pBlock->GetBlockAABB();
					A3DVECTOR3 vPosition = vPos;
					vPosition.y = aabb.Center.y;
					if(aabb.IsPointIn(vPosition))
					{
						bTerrainLoaded = true;
						break;
					}
			...
			iNumBlock = m_pWorld->GetScene()->GetActiveBlocks()->aBlocks.GetSize();
			for (i=0; i < iNumBlock; i++)
			{
				CECSceneBlock* pBlock = m_pWorld->GetScene()->GetActiveBlocks()->aBlocks[i];
				if (pBlock)
				{
					const ARectF& rc = pBlock->GetBlockArea();
					ARectF rcArea;
					rcArea.left = min(rc.left, rc.right);
					rcArea.top = min(rc.top, rc.bottom);
					rcArea.right = max(rc.left, rc.right);
					rcArea.bottom = max(rc.top, rc.bottom);
					if(rcArea.PtInRect(vPos.x, vPos.z))
					{
						bSceneLoaded = true;
						break;
					}
			...
			if(!bTerrainLoaded || !bSceneLoaded)
				m_pWorld->SetLoadCenter(vPos);
			return true;
			*/
		}
	...
	bool bOnlyJumpInstance = false;

	if( !m_pLoadProgress )
	{
		bOnlyJumpInstance = true;
		char szBackPic[MAX_PATH];

		AString strPath = pInst->GetPath();
		if (pInst->GetID() == 12 && vPos.x > 355 && vPos.x < 512 && vPos.z > 353 && vPos.z < 512 )
		{
			strPath += "-1";
		}
		else if (pInst->GetID() == 401)
		{
			int index = pInst->GetIndexByCoord(vPos.x, vPos.z);
			if (index != -1)
			{
				char szNum[10] = {0};
				itoa(index+1, szNum, 10);
	...
	if(bOnlyJumpInstance)
		EndLoadProgress();

	return true;
}

bool CECGameRun::JumpToInstanceForAnm(int idInst, const A3DVECTOR3& vPos)
{
	CECInstance* pInst = GetInstance(idInst);
	if (!pInst)
	{
		a_LogOutput(1, "CECGameRun::JumpToInstanceForAnm, wrong instance id: %d", idInst);
		return false;
	}

	if (m_pAnmWorld) // 已在动画中, 要跳出
	{
		char szBackPic[MAX_PATH];
		...
		// Stop the loader thread for animation
		//ExitLoaderThread();
		...
		//g_pGame->GetConfigs()->SetSceneLoadRadius(m_fSceneLoadRadius);

		m_pWorld->RestoreGameWorld();
		m_pWorld->SetLoadCenter(vPos);
	...
	{
		// Stop the loader thread for game
		ExitLoaderThread();

		//m_fSceneLoadRadius = g_pGame->GetConfigs()->GetSceneLoadRadius();
		//g_pGame->GetConfigs()->SetSceneLoadRadius(500.0f);

		//	Create new world
		if (!CreateAnmWorld(idInst, vPos))
		{
			a_LogOutput(1, "CECGameRun::JumpToInstanceForAnm, failed to create world %d", idInst);
			return false;
		}
...
bool CECGameRun::ProcessMessage(const ECMSG& Msg)
{
	...
	case MSG_DOUBLETIME:
	{
		...
		else if(Msg.dwParam2 == S2C::MULTI_EXP_MODE)
		{
			const S2C::multi_exp_mode* pCmd = (const S2C::multi_exp_mode*)Msg.dwParam1;
			m_iDExpEndTime = pCmd->timestamp;
			m_iDblExpMode  = pCmd->multi_exp_factor;
		}
	...
	{
		CECGameUIMan* pGameUIMan = m_pUIManager->GetInGameUIMan();
		pGameUIMan->CommonMessage("Win_QShop", "set_time", Msg.dwParam1, 0);
		
		break;
	}
	case MSG_VIP_AWARD_INFO:	//Added 2012-01-04.
	{
		CECGameUIMan* pGameUIMan = m_pUIManager->GetInGameUIMan();
		pGameUIMan->CommonMessage("Win_QShop_vip", "award_info", Msg.dwParam1, 0);

		break;
	}
	case MSG_PLATFORM_INFO:
	{
		const S2C::fill_platform_mask* pCmd = (const S2C::fill_platform_mask*)Msg.dwParam1;
		m_iPlatformMask = pCmd->mask;
		CECGameUIMan* pGameUIMan = m_pUIManager->GetInGameUIMan();
		if (pGameUIMan->GetDialog("Win_CharHead")->GetDlgItem("Btn_Accout"))
		{
			bool bShow = false;
			...
			if(pTaskInterafce)
			{
				...
				if((pTaskFinished->SearchTask(26560) == -1 && (m_iPlatformMask&0x10) > 0) || //还没领奖
					(pTaskFinished->SearchTask(26561) == -1 && (m_iPlatformMask&0x20) > 0) || 
					((m_iPlatformMask&0x3)>0) )
					bShow = true;
			}

			pGameUIMan->GetDialog("Win_CharHead")->GetDlgItem("Btn_Accout")->Show(bShow);
			pGameUIMan->GetDialog("Win_CharHead")->GetDlgItem("Btn_Accout")->SetFlash(true);
		}
		break;
	}
...
bool CECGameRun::CaptureScreen(int N)
{
	...
	if( N == 1 )
	{
		...
		{		
			...
			g_pGame->GetA3DDevice()->ExportColorToFile(szJPGFile, true);
			/*
			int nQuality;
	...
	else
	{
		...
		for(int h=0; h<N; h++)
		{
			for(int w=0; w<N; w++)
			{
				...
				//hval = g_pGame->GetA3DDevice()->StretchRect(g_pGame->GetA3DDevice()->GetBackBuffer()->m_pD3DSurface, NULL, pTempSurface->GetDXSurface(), NULL, A3DTEXF_LINEAR);
				if( hval != D3D_OK )
					continue;
#endif
...
void CECGameRun::SetWallowInfo(const S2C::player_wallow_info& info) 
{
	m_WallowInfo = info;  
	
	int stime = g_pGame->GetServerGMTTime();
	int nTime = stime - g_pGame->GetGameRun()->GetWallowInfo().play_time;
	int nWallowStage = m_WallowInfo.anti_wallow_active ? nTime/3600 : 0;
	
	const char *szCountry = g_pGame->GetConfigs()->GetCountryRegion();
	if ( (0==stricmp(szCountry, "kr")) &&
		nWallowStage!=m_nWallowStage && 
		nTime<(nTime/3600*3600+60) )
	{
		ACString strText;
		CECGameUIMan* pGameUI = (CECGameUIMan*)m_pUIManager->GetInGameUIMan();
		strText.Format(pGameUI->GetStringFromTable(9314), nWallowStage);
		pGameUI->AddChatMessage(strText, GP_CHAT_BROADCAST);
		pGameUI->CommonMessage("Win_Kr_AntiWallow", "show", nWallowStage, 0);
	}
	m_nWallowStage = nWallowStage;
}

bool CECGameRun::IsAnmPlaying()
{
	return m_pAnmSystem->IsPlaying();
}

void CECGameRun::RegisterIP(AString strIP)
{
	m_strLoginIP = strIP;
	m_iLoginIPTime = a_Random(30*60000, 60*60000);	// 30-60分钟发一次
}

#ifndef BACK_VERSION

//  Update Post Effect
void CECGameRun::UpdatePostEffect()
{
	CECConfigs* pConfigs = g_pGame->GetConfigs();
	...
	// 智能摄像机
	else if (m_pSmartCameraControler->IsPlaying())
	{
		m_pRenderParam->paramPostEffect = m_pSmartCameraControler->GetPostEffectParam();
	}
	else if (test)
	{
		if (GetGameState() == CECGameRun::GS_GAME)
		{
			CDlgSettingPostEffect *pDlg = (CDlgSettingPostEffect*)m_pUIManager->GetInGameUIMan()->GetDialog("Win_SettingPostEffect");
			if (pDlg)
			{
				m_pRenderParam->paramPostEffect.dwPostEffectMask = pDlg->GetMask();
			}
			m_pRenderParam->paramPostEffect.dwPostEffectMask = A3DPOST_EFFECT_CC | A3DPOST_EFFECT_Bloom | A3DPOST_EFFECT_AA | A3DPOST_EFFECT_SunMoon;
			m_pRenderParam->paramPostEffect.ccParam.bAfterUI = true;
			m_pRenderParam->paramPostEffect.ccParam.bUseManualParam1 = false;
			m_pRenderParam->paramPostEffect.ccParam.bUseManualParam2 = false;
			m_pRenderParam->paramPostEffect.ccParam.fFactor = .0f;
			//m_pRenderParam->paramPostEffect.ccParam.szTex1 = "textures\\die.dds";
			CDlgSettingEffect *pDlg = (CDlgSettingEffect*)m_pUIManager->GetInGameUIMan()->GetDialog("Panel_SetEffect");
			m_pRenderParam->paramPostEffect = *(pDlg->GetParam());
		}
		else
			m_pRenderParam->paramPostEffect.dwPostEffectMask = A3DPOST_EFFECT_Bloom | A3DPOST_EFFECT_AA | A3DPOST_EFFECT_SunMoon;

	}
	系统设置*/
	if(pConfigs->GetPostEffectType()==1)
	{
		CDlgSettingEffect *pDlg = (CDlgSettingEffect*)m_pUIManager->GetInGameUIMan()->GetDialog("Panel_SetEffect");
		m_pRenderParam->paramPostEffect = *(pDlg->GetParam());
	}
	...
	{
		m_pRenderParam->paramPostEffect = pConfigs->GetPostEffectParam();
		if (m_pWorld)
		{
			m_pRenderParam->paramPostEffect.dwPostEffectMask = m_pWorld->GetPostEffectFlag();
			m_pRenderParam->paramPostEffect.ccParam = m_pWorld->GetPostEffectCCParam();
			m_pRenderParam->paramPostEffect.dofParam = m_pWorld->GetPostEffectDOFParam();
			m_pRenderParam->paramPostEffect.bloomParam = m_pWorld->GetPostEffectBloomParam();		
			m_pRenderParam->paramPostEffect.fullGlowParam = m_pWorld->GetPostEffectFullGlowParam();
		}

		if (0 && m_pHostPlayer)
		{
			// 加速跑特殊处理
			const A3DPOST_EFFECT_RADIUSBLUR_PARAM &accelerateEffect = m_pHostPlayer->GetAccelerateRunEffect();
			if (accelerateEffect.fBlurLength > 0.0f)
			{
				m_pRenderParam->paramPostEffect.dwPostEffectMask |= A3DPOST_EFFECT_RadialBlur;
				m_pRenderParam->paramPostEffect.radiusBlurParam = accelerateEffect;
			}

			// 水下特殊处理
			const A3DPOST_EFFECT_WARP_PARAM &underWaterEffect = m_pHostPlayer->GetUnderWaterEffect();
			if (underWaterEffect.fScale > 0.0f)
			{
				m_pRenderParam->paramPostEffect.dwPostEffectMask |= A3DPOST_EFFECT_Warp;
				m_pRenderParam->paramPostEffect.warpParam = underWaterEffect;
			}

			// 死亡特殊处理
			const A3DPOST_EFFECT_CC_PARAM &deathEffect = m_pHostPlayer->GetDeathEffect();
			if (deathEffect.fFactor > 0.0f)
			{
				m_pRenderParam->paramPostEffect.dwPostEffectMask |= A3DPOST_EFFECT_CC;
				m_pRenderParam->paramPostEffect.ccParam = deathEffect;
			}

			// 景深特殊处理
			const A3DPOST_EFFECT_DOF_PARAM &dofEffect = m_pHostPlayer->GetCameraEffect();
			if (dofEffect.fClampBlurFar > 0.0f)
			{
				m_pRenderParam->paramPostEffect.dwPostEffectMask |= A3DPOST_EFFECT_Dof;
				m_pRenderParam->paramPostEffect.dofParam = dofEffect;
			}

			pConfigs->AdjustPostEffectBySystemSetting(m_pRenderParam->paramPostEffect);
		}
	}

	pConfigs->AdjustPostEffectGlobal(m_pRenderParam->paramPostEffect);
}

ElementRender* CECGameRun::GetElementRender()
{
	return m_pElementRender;
}

ELEMENT_RENDER_PARAM* CECGameRun::GetElementRenderParam()
{
	return m_pRenderParam;
}
