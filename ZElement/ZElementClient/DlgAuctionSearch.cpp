...
bool CDlgAuctionSearch::OnInitDialog()
{
	...
	AFileImage fList;
	if( fList.Open("surfaces\\ingame\\auctiontree.txt",AFILE_OPENEXIST | AFILE_TYPE_BINARY) )
	{
		
		ACHAR *szData = new ACHAR[fList.GetFileLength() / sizeof(ACHAR) + 1];
		DWORD dwLen;
...
void CDlgAuctionSearch::OnTick()
{
	...
	if( pItem && !bOnIcon && m_pTv_Item->GetItemData(pItem) != 0 )
	{
		CECIvtrItem *pIvtr = CECIvtrItem::CreateItem(m_pTv_Item->GetItemData(pItem), 0, 1);
		pIvtr->GetDetailDataFromLocal();
		pIvtr->SetPriceScale(CECIvtrItem::SCALE_BUY, 1.0f);
		const wchar_t* szDesc = pIvtr->GetDesc();
		AUICTranslate trans;
		if( szDesc && pIvtr->GetClassID() != CECIvtrItem::ICID_ERRORITEM )
			m_pTv_Item->SetHint(trans.Translate(szDesc));
		else
			m_pTv_Item->SetHint(_AL(""));
		delete pIvtr;
	}
	else
		m_pTv_Item->SetHint(_AL(""));
...
void CDlgAuctionSearch::OnCommandSearchItem(const char* szCommand)
{
	...
	if( pItem && m_pTv_Item->GetItemData(pItem) != 0 )
	{
		...
		GetGameUIMan()->m_pDlgAuctionBuyList->m_pTxt_Search->
			SetText(m_pTv_Item->GetItemText(pItem));
		ACString szText;
	...
	else
	{
		...
		if( nSel >= 0 && nSel < m_pLst_Item->GetCount() )
		{
			GetGameUIMan()->m_pDlgAuctionBuyList->m_nIDNext = m_pLst_Item->GetItemData(nSel);
			GetGameUIMan()->m_pDlgAuctionBuyList->m_nPageNext = 0;
			GetGameUIMan()->m_pDlgAuctionBuyList->m_pTxt_Search->
				SetText(m_pLst_Item->GetText(nSel));
			ACString szText;
