...
bool CECOrnamentMan::Tick(DWORD dwDeltaTime)
{
	...
	for (int i=0; i < m_aLandBuilds.GetSize(); i++)
	{
		EC_LandBuild* pLandBuild = m_aLandBuilds[i];
		if (a3d_Magnitude(pLandBuild->GetModelPos() - vHostPos) > 100)
		{
			pLandBuild->SetModelShow(false);
		}
		else
		{
			pLandBuild->SetModelShow(true);
		}
		pLandBuild->Tick(dwDeltaTime);
	}
	return true; 
}
...
DWORD CECOrnamentMan::LoadECModel(AFile* pFile, float fOffX, float fOffZ)
{
	...
	if (IsLoadThreadReady())
	{
		...
		QueueECModelForLoad(MTL_ECM_SCNMODEL, (int)Data.dwExportID, 0, vPos, strModelFile, NULL);
	}
	else
	{
...
bool CECOrnamentMan::TraceWithBrush(BrushTraceInfo * pInfo)
{
	...
	for (int nIndex = 0; nIndex < (int)m_aLandBuilds.GetSize(); nIndex++)
	{
		EC_LandBuild* pLandBuild = m_aLandBuilds[nIndex];
		if ((fFraction > 0.0f) && pLandBuild->TraceWithBrush(pInfo) 
			&& (pInfo->fFraction < fFraction ) ) 
		{
			fFraction = pInfo->fFraction;
			bAllSolid = pInfo->bAllSolid;
...
void CECOrnamentMan::ClearrLandBuild()
{
	for (int i=0; i < m_aLandBuilds.GetSize(); i++)
	{
		EC_LandBuild* pLandBuild = m_aLandBuilds[i];
	...
	m_aLandBuilds.RemoveAll();
}
void CECOrnamentMan::AddLandBuild(int tId, int nTempId,int nStatus,int nLevel)
{
	if(g_pGame&&g_pGame->GetGuildEnter() != true)
	{
		return;
	}
	EC_LandBuild* pBuild = new EC_LandBuild();

	if(pBuild)
	{
		pBuild->LoadGuildBuildingObject(nTempId,nStatus,nLevel - 1);
		pBuild->SetFieldIndex(tId);
		m_aLandBuilds.Add(pBuild);
	}
}

void CECOrnamentMan::DelLandBuild(int tId)
{
	if(g_pGame&&g_pGame->GetGuildEnter() != true)
	{
		return;
	}
	for(int nIndex = 0; nIndex < m_aLandBuilds.GetSize();nIndex++)
	{
		EC_LandBuild* pBuild = m_aLandBuilds[nIndex];
		if(pBuild->GetFieldIndex() == tId)
		{
			m_aLandBuilds.RemoveAt(nIndex);
		}
	}
}
void CECOrnamentMan::ChangeLandBuild(int tId, int nTempId,int nLevel,int nStatus)
{
	if(g_pGame&&g_pGame->GetGuildEnter() != true)
	{
		return;
	...
	for(int nIndex = 0; nIndex < m_aLandBuilds.GetSize();nIndex++)
	{
		...
		if(pBuild->GetFieldIndex() == tId)
		{
			pBuild->LandBuildComplete(nTempId,nLevel,nStatus);
		}
	}
}