...
class ECViewport;
class ElementRender;
class EC_ManLoginUIMan;
class CManagerWrapperForAnmSystem;
...
//	MOD_MAGIC_ATTACK_RUNE	= 0x02,		// 法术攻击优化符生效
	...
	MOD_DEITY_DAMAGE		= 0x40000000,// 神圣攻击
...
public:		//	Types
	...
	// wallow stage for korea, useless from 2012.7.5
	enum
	{
		WALLOW_STAGE0	= 0,
		WALLOW_STAGE1,
	...
	typedef abase::hashtab<int, int, abase::_hash_function> ID2LastMsgTypeTable;	// 0-未收到	1-game	2-GT
...
public:		//	Operations
	...
	// Login Render
	bool RenderLogin(bool bPresent=true);
	//	Capture screen
	...
	//	Get game world
	CECWorld* GetWorld() { return m_pAnmWorld ? m_pAnmWorld : m_pWorld; }
	//  Get real game world
	CECWorld* GetRealGameWorld() { return m_pWorld; }
	//	Get input control
	...
	// 将数据保存到本地，换线的时候如果本地有数据则丢弃服务器数据
	bool SaveConfigsToLocal();
	//	Set logout flag
	...
	CAnmSystem* GetAnmSystem() { return m_pAnmSystem; }

	//	Begin load progress
	bool BeginLoadProgress(int iLoadID, const char* szBackPic);
	//	End load progress
	...
	void AddChatMessage(const ACHAR *pszMsg, char cChannel, int idPlayer=-1, const ACHAR* szName=NULL, BYTE byFlag = 0, char cEmotion = 0, CECIvtrItem* pItem = NULL, sAchievement* pAchievement = NULL);
	...
	bool IsIn6v6Battle();	// 当前在碰撞副本中

	void AddLstMsgType(int cid, int type);
	void EraseLstMsgType(int cid);
	int PlayerLstMsgType(int cid);
	...
	bool JumpToInstanceForAnm(int idInst, const A3DVECTOR3& vPos);
	...
	CECShortcutSet*	GetActivePoseCmdShortcuts() { return m_pActivePoseSCS; }
	...
	CECSCCommand* GetActivePoseCmdShortcuts(int iPose);
	...
	COLLISION_RAID_TRANSFIGURE_CONFIG& GetCollisionRaidTransfigureConfig() { return m_CollisionRaidTransfigureConfig; }
	COLLISION_RAID_AWARD_CONFIG& GetCollisionRaidAwardConfig() { return m_CollisionRaidAwardConfig; }
	...
	int GetLastLoginId() { return m_iLastLoginId; }
	void SetLastLoginId(int id) { m_bShowWallowIpInfo=(m_iLastLoginId!=id); m_iLastLoginId=id; }
	int GetLastLoginIp() { return m_iLastLoginIp; }
	void SetLastLoginIp(int id) { m_iLastLoginIp=id; }
	int GetPlatformMask() { return m_iPlatformMask; }
	bool ShowLoginWallowIpInfo() { return m_bShowWallowIpInfo; }
	abase::vector<GNET::RoleInfo>& GetVecRoleInfo() { return m_vecRoleInfo;}
	void DropSpeaking(int itemId, int remainNum, int roleId=0);		// roleId=0表示产出喊话，roleId!=0表示玩家拾取喊话
	//	当前窗口是否被托盘化
...
//	CECHelper * GetHelper() {return m_pECHelper;}
	CECAutoHelper* GetHelper() { return m_pECHelper; }
	CECSecretary * GetSecretary() { return m_pSecretary; }
	CECHelpGraphGuide * GetHelpGraphGuide() { return m_pHelpGraphGuide; }
	CECNewHelpGuide* GetNewHelpGuide(){return m_pNewHelpGuide;}
	bool IsHideElsePlayer() { return m_bHideElsePlayer; } 
	void ToggleHideElsePlayer();
	...
	void GTLogin(bool b) { m_bGTLogin=b; }
	bool IsGTLogin() { return m_bGTLogin; }
	void RegisterIP(AString strIP);
protected:	//	Attributes
	...
	ID2LastMsgTypeTable	m_ID2MsgType;		//  Last Msg From ?
	...
	CECShortcutSet*		m_pActivePoseSCS;	//	Active pose shortcut set
	...
	bool		m_bLocalNew;		//  Config data local is valid than server
...
// 	bool		m_bTrayFlash;
	...
	int			m_iLastLoginId;
	int			m_iLastLoginIp;
	bool		m_bShowWallowIpInfo;
	int			m_iPlatformMask;

	unsigned int m_nGShopTimeStamp;
	SPECIAL_ID_CONFIG m_SpecialIDConfig;
	abase::vector<DROP_INTERNAL_CONFIG>	m_vecDropInternalConfigs;
	COLLISION_RAID_TRANSFIGURE_CONFIG	m_CollisionRaidTransfigureConfig;
	COLLISION_RAID_AWARD_CONFIG			m_CollisionRaidAwardConfig;
	
//	CECHelper	* m_pECHelper;      //the helper baby
	CECAutoHelper* m_pECHelper;
	CECSecretary * m_pSecretary;	// the Secretary 
	CECHelpGraphGuide * m_pHelpGraphGuide;

	CECNewHelpGuide* m_pNewHelpGuide;

	S2C::player_wallow_info m_WallowInfo;
	abase::vector<GNET::RoleInfo> m_vecRoleInfo;	// save roleinfo when loginuiman is released

	CAnmSystem*     m_pAnmSystem;
	float			m_fSceneLoadRadius; // save the origin radius when playing animation

	bool			m_bGTLogin;

	AString			m_strLoginIP;
	int				m_iLoginIPTime;
	AString			m_strRecordUrl;

#ifndef BACK_VERSION
	ElementRender*			m_pElementRender;
	ELEMENT_RENDER_PARAM*	m_pRenderParam;
	//PLATFORM_RENDERINFO*	m_pRenderInfo;

	EC_ManLoginUIMan* m_pManLoginUIMan;
	CManagerWrapperForAnmSystem* m_pManAnmSystem;
#endif
...
public:
	...
	void ResetWallowInfo() { memset(&m_WallowInfo, 0, sizeof(m_WallowInfo)); m_WallowInfo.wallow_level = 0xff; /* 服务器没有发防沉迷的标志*/;  m_nWallowStage=0;}
	const S2C::player_wallow_info& GetWallowInfo() { return m_WallowInfo; }
	void SetWallowInfo(const S2C::player_wallow_info& info);

#ifndef BACK_VERSION
	ElementRender*			GetElementRender();
	ELEMENT_RENDER_PARAM*	GetElementRenderParam();
	//PLATFORM_RENDERINFO*	GetPlatformRenderInfo();
#endif
...
protected:	//	Operations
	...
	bool CreateAnmWorld(int id, const A3DVECTOR3& vCameraPos);
