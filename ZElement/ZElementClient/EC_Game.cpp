...
#include "ExplorerMan.h" //Added 2012-05-30.
...
// Gameversion
DWORD GAME_VERSION = ((0 << 24) | (4 << 16) | (0 << 8) | 0);
DWORD GAME_BUILD = 1324;
...
extern CECConfigs g_GameCfgs;
extern DWORD g_dwLastRenderTime;
...
m_PoseOnPetTab(256)
{
	...
	memset(m_KeyCodeMap, 0, sizeof(m_KeyCodeMap));
	memset(m_KeyCodeInverseMap, 0, sizeof(m_KeyCodeInverseMap));
...
bool CECGame::Init(const GAMEINIT& GameInit)
{
...
#ifdef BACK_VERSION
	m_pSkinRender1_W = new A3DSkinRender;
	m_pSkinRender1_W->Init(m_pA3DEngine);
	m_pSkinRender2 = new A3DSkinRender;
	m_pSkinRender2->Init(m_pA3DEngine);
#else
	m_pSkinRender1_W = m_pA3DEngine->CreateSkinRender(true);
	...
	if (!(m_pImageRes = new CECImageRes(m_pA3DDevice)))
	{
		glb_ErrorOutput(ECERR_NOTENOUGHMEMORY, "CECGame::Init", __LINE__);
		return false;
	}
	...
	{
		MessageBoxA(NULL, "无法加载游戏数据文件：data\\hometowndata!\n此错误可能是更新不完全所致，请重新进行更新或进行版本验证！", "错误", MB_OK | MB_ICONSTOP);
		glb_ErrorOutput(ECERR_FAILEDTOCALL, "CECGame::Init", __LINE__);
		return false;
	}
	...
	//任务数据加载需要判断石否成功，如果任务数据没有加载成功，则客户端不应该起来。Added 2011-05-17
	if ( !m_pTaskMan->LoadTasksFromPack("data\\tasks.data", "data\\forbidden_task.txt", true) )
	{
		MessageBoxA(NULL, "无法加载游戏数据文件：data\\tasks.data!\n此错误可能是更新不完全所致，请重新进行更新或进行版本验证！", "错误", MB_OK | MB_ICONSTOP);
		glb_ErrorOutput(ECERR_NOTENOUGHMEMORY, "CECGame::Init", __LINE__);
		return false;
	}
...
{
	overlay::GTOverlay::Instance().LeaveDestroyGameWindow();
	overlay::GTOverlay::Instance().LeaveMain();
	...
	for (int i = 0; i < m_DungeonStepInfoArray.GetSize(); ++ i)
	{
		if( m_DungeonStepInfoArray[i].iStepNum == 0 )
			continue;

		delete [] m_DungeonStepInfoArray[i].aStepMonsterIDs;
	}
	...
	A3DRELEASE(m_pA3DGFXExMan);//根据引擎的规则，应该是先释放gfx再释放Model。Modified the call place 2011-05-16.
	...
	//A3DRELEASE(m_pA3DGFXExMan);//canceled 2011-05-16.
	A3DRELEASE(m_pSkinRender2);
	A3DRELEASE(m_pSkinRender1_W);
#ifndef BACK_VERSION
	...
	CECMenology::GetInstance()->Release();
	m_pConfigs = NULL;
	if (m_pConfigFromLua)
	{
...
bool CECGame::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
			m_iTaskbarRestart = RegisterWindowMessage(_AL("TaskbarCreated"));
			overlay::GTOverlay::Instance().EnterCreateGameWindow(hWnd);
	...
	case WM_ACTIVATE:
		...
		// Sometimes system will inform us the game windows was activated via only WM_ACTIVATE, but not WM_ACTIVATEAPP message.
		if (LOWORD(wParam) != WA_INACTIVE)
		{
			m_bActive = true;
		}

		return false;	// 继续交给系统处理
...
// 
	...
	case WM_ENTERSIZEMOVE:
		{
		    ...
		    m_SizeMoveParam.bSizeMoving = true;
			m_SizeMoveParam.iOldWidth  = param.Width;
			m_SizeMoveParam.iOldHeight = param.Height;
			m_SizeMoveParam.iNewWidth  = param.Width;
			m_SizeMoveParam.iNewHeight = param.Height;
		}
		break;
	case WM_EXITSIZEMOVE:
		{
			if(m_SizeMoveParam.bSizeMoving)
			{
				if(m_SizeMoveParam.iOldWidth != m_SizeMoveParam.iNewWidth || m_SizeMoveParam.iOldHeight != m_SizeMoveParam.iNewHeight)
				{
					if( GetConfigs() && !GetConfigs()->IsAdjusting() && m_pA3DDevice )
					{
					   OnWindowResize(m_SizeMoveParam.iNewWidth,m_SizeMoveParam.iNewHeight);
					}
				}
				m_SizeMoveParam.bSizeMoving = false;
			}
		}
		break;
	case WM_SIZE:
		if( GetConfigs() && !GetConfigs()->IsAdjusting() && m_pA3DDevice )
		{
			...
			if(m_SizeMoveParam.bSizeMoving)
			{
               m_SizeMoveParam.iNewWidth  = w;
			   m_SizeMoveParam.iNewHeight = h;
			}
			else
			{
			   OnWindowResize(w,h);
	...
	default:
		if(m_iTaskbarRestart==message)
			CECBossKey::GetInstance()->AddTaskbarIcon();
		else if (m_pGameRun)
...
void CECGame::OnWindowResize(int width,int height)
{
	EC_SYSTEM_SETTING ss = GetConfigs()->GetSystemSettings();
	ss.iRndWidth = width;
	ss.iRndHeight = height;
...
bool CECGame::IsWindowSizeMoving()
{
  return m_SizeMoveParam.bSizeMoving && (m_SizeMoveParam.iOldWidth != m_SizeMoveParam.iNewWidth || m_SizeMoveParam.iOldHeight != m_SizeMoveParam.iNewHeight);
}
...
bool CECGame::OnceRun()
{
	...
	g_dwLastRenderTime = dwLastFrame;
...
#else
	ECViewport::INITPARAMS ViewParams;
#endif
...
bool CECGame::Reset()
{
	...
	if (m_pSkinRender1_W)
	{
		m_pSkinRender1_W->ResetRenderInfo(true);
		m_pSkinRender1_W->ResetRenderInfo(false);
	}
...
//				ASSERT(0);
				a_LogOutput(1, ("CECGame::LoadItemColor(), Property id collision, shouldn't happen: %d\n"), idItem);
			}
		}
...
//	Load pose on pet table.
bool CECGame::LoadPoseOnPetTab()
{
	AScriptFile sf;

	if (sf.Open("Configs\\pose_on_pet.txt"))
	{
		while(sf.GetNextToken(true))
		{
			AString keyword = sf.m_szToken;
			DWORD value = sf.GetNextTokenAsInt(true);
			m_PoseOnPetTab.put(keyword, value);
		}
		sf.Close();
		return true;
	}
	...
	{
		a_LogOutput(1, "CECGame::LoadPoseOnPet, failed to open file pose_on_pet.txt");
		return false;
	}
}

// Load dungeon step table.
bool CECGame::LoadDungeonStepTab()
{
	AIniFile IniFile;

	if (!IniFile.Open("Configs\\dungeonstep.txt"))
	{
		a_LogOutput(1, "CECGame::LoadDungeonStepTab, failed to open file dungeonstep.txt");
		return false;
	}

	int iNum = IniFile.GetValueAsInt("dungeon", "Num", 0);
	if (iNum == 0)
		return true;

	int *dungeonIDs = new int[iNum];
	IniFile.GetValueAsIntArray("dungeon", "ID", iNum, dungeonIDs);

	AString szKey, szSect;
	for(int i = 0; i < iNum; ++ i)
	{
		DUNGEONSTEPINFO info;
		info.iDungeonID = dungeonIDs[i];

		szSect.Format("dungeon_%d", dungeonIDs[i]);
		info.szSubWndName = IniFile.GetValueAsString(szSect, "UI", 0);
		info.iStartMsgID = IniFile.GetValueAsInt(szSect, "StringID", 0);
		info.iStepNum = IniFile.GetValueAsInt(szSect, "StepNum", 0);
		info.aStepMonsterIDs = info.iStepNum > 0 ? new abase::vector<DWORD>[info.iStepNum] : NULL;

		for (int j = 0; j < info.iStepNum; ++ j)
		{
			szKey.Format("Step%d", j + 1);

			int monsterNum = 0;
			AString strPos;
			strPos = IniFile.GetValueAsString(szSect, szKey);
			if (!strPos.IsEmpty())
			{
				int idx = -1, iStart = 0;
				do
			...
			int *monsterIDs = new int[monsterNum];
			IniFile.GetValueAsIntArray(szSect, szKey, monsterNum, monsterIDs);

			for (int k = 0; k < monsterNum; ++ k)
			{
				DWORD mid = monsterIDs[k];
				info.aStepMonsterIDs[j].push_back(mid);
			}

			delete[] monsterIDs;
		}

		m_DungeonStepInfoArray.Add(info);
	}
	...
	if (sf.Open("Configs\\dungeonstep.txt"))
	{
		while( sf.GetNextToken(true))
		{
			DUNGEONSTEPINFO info;
			info.iDungeonID = atoi(sf.m_szToken);
			
			sf.GetNextToken(true);
			info.szSubWndName = sf.m_szToken;

			info.iStartMsgID = sf.GetNextTokenAsInt(true);
			info.iStepNum = sf.GetNextTokenAsInt(true);

			for (int i = 0; i < info.iStepNum; ++ i)
				info.aMonsterIDs.Add(sf.GetNextTokenAsInt(true));
...
#ifndef BACK_VERSION
class HighLightRenderModifier : public A3DSkinModelRenderModifier
{
public:
	//在ForwardRender之前对该Model做一些修改
	virtual bool BeforeForwardRender(A3DSkinModel* pModel)
	{
		m_mapModelEmissiveColor[pModel] = pModel->GetExtraEmissive();
		pModel->SetExtraEmissive(A3DCOLORVALUE(0.5f));
		return true;
	}
	//在ForwardRender之后对该Model再做一些修改
	virtual bool AfterForwardRender(A3DSkinModel* pModel)
	{
		A3DCOLORVALUE c = m_mapModelEmissiveColor[pModel];
		pModel->SetExtraEmissive(c);
		return true;
	}
	//在一帧渲染完之后做一些清理工作.
	virtual bool ClearAfterAllRender()
	{
		m_mapModelEmissiveColor.clear();
		return true;
	}
	
private:
	abase::hash_map<A3DSkinModel*, A3DCOLORVALUE> m_mapModelEmissiveColor;
};

static HighLightRenderModifier s_highLightRenderModifier;
#endif
...
#else
bool CECGame::RenderHighLightModel(ECViewport* pView, CECModel* pModel, bool bRenderSkinModel)
{
	if (pView && pModel)
	{
		pModel->Render(pView->GetA3DViewport(), false, bRenderSkinModel, 0, &s_highLightRenderModifier);
		return true;
	}
	return false;
}

bool CECGame::RenderHighLightModel(ECViewport* pView, A3DSkinModel* pModel)
{
	if (!pModel || !pView)
		return false;
	
	return pModel->Render(pView->GetA3DViewport(), true, &s_highLightRenderModifier);
}
#endif
...
bool CECGame::LoadKeyCodeMapping()
{
	for(int i = 0; i<256; ++i)
	{
		m_KeyCodeMap[i] = i;
		m_KeyCodeInverseMap[i] = i;
	}
	AIniFile iniFile;
	if(iniFile.Open("configs\\KeyCodeMapping.ini"))
	{
		int nNum = 0;
		int i = 0;
		nNum = iniFile.GetValueAsInt("KeyCodeMap", "Number", nNum);
		
		for(i = 1; i <= nNum; i++)
		{
			AString strChannelNameKey;
			AString strChannelURLKey;
			int keyCode =0, mapCode = 0;
			strChannelNameKey.Format("keyCode%d", i);
			strChannelURLKey.Format("mapCode%d", i);

			keyCode = iniFile.GetValueAsInt( "KeyCodeMap", strChannelNameKey, keyCode);
			mapCode = iniFile.GetValueAsInt( "KeyCodeMap", strChannelURLKey, mapCode);
			if (keyCode!=0 && mapCode != 0)
			{
				m_KeyCodeMap[keyCode] = mapCode;
				m_KeyCodeInverseMap[mapCode] = keyCode;
			}
		}
	}
	return true;
...
int CECGame::GetMappingKeyCode(int key)
{
	ASSERT(key>=0 && key< 256);
	return m_KeyCodeMap[key];
}

int CECGame::GetInverseMappingKeyCode(int key)
{
	ASSERT(key>=0 && key< 256);
	return m_KeyCodeInverseMap[key];
}
...
bool CECGame::LoadCrossServerInfo()
{
	AWScriptFile s;
	ACString str;
	if (s.Open("configs\\crossserverlist.txt"))
	{
		while(s.PeekNextToken(true))
		{
			...
			SERVERINFO info;
			CSplit ss(s.m_szToken);
			CSplit::VectorAWString vecs = ss.Split(_AL(":"));
			if( vecs.size() >= 2 )
			{	
				AString strName = AC2AS(str);
				strncpy(info.szServerName, strName, 256);
				CSplit sAddr(AC2AS(vecs[1]));
				CSplit::VectorAString vecAddrs = sAddr.Split("*");
				unsigned int k;
				for (k=0; k<vecAddrs.size(); ++k)
				{
					info.vecAddress.push_back(vecAddrs[k]);
				}
				CSplit sPort(vecs[0]);
				CSplit::VectorAWString vecPorts = sPort.Split(_AL("*"));
				for (k=0; k<vecPorts.size(); ++k)
				{
					info.vecPorts.push_back(vecPorts[k].ToInt());
				}
				if (s.PeekNextToken(false))
				{
					info.iServerID = s.GetNextTokenAsInt(false);
					m_CrossServerInfo.push_back(info);
				}
			}
		}
		s.Close();
...
bool CECGame::LoadSrcServerInfo()
{
	// serverlist改为patch更新，将其复制到element\userdata\server\newserver目录下
	static const AString filesToCopy[] = {
		"serverlist.txt",
		"servergroup_new.txt",
	};
	CreateDirectoryA("userdata\\server\\newserver", NULL);
	int iSize = sizeof(filesToCopy)/sizeof(AString);
	int i;
	for (i=0; i<iSize; ++i)
	{
		if (glb_FileExist(AString("..\\launcher\\")+filesToCopy[i]))
		{
			CopyFileA(AString("..\\launcher\\")+filesToCopy[i], 
				AString("userdata\\server\\newserver\\")+filesToCopy[i],
				false);
		}
		else
			CopyFileA(AString("userdata\\server\\")+filesToCopy[i], 
			AString("userdata\\server\\newserver\\")+filesToCopy[i],
				false);
	}

	m_SrcServerInfo.clear();
	ACString strCurGroup = m_pGameRun->GetCurGroup();
	ACString strGroup;
	
	AWScriptFile s;
	ACString str;
	if (s.Open("userdata\\server\\newserver\\serverlist.txt"))
	{
		while(s.PeekNextToken(true))
		{
			s.GetNextToken(true);
			...
			if(s.PeekNextToken(false))
			{
...

				{
					...
					{	
						...
						strncpy(info.szServerGroup, AC2AS(strGroup), 256);	// 2012-9-5 add zy 添加服务器名称
						CSplit sPort(vecs[0]);
						CSplit::VectorAWString vecPorts = sPort.Split(_AL("*"));
						for (k=0; k<vecPorts.size(); ++k)
						{
						...
						info.iServerID = s.GetNextTokenAsInt(false);
						m_SrcServerInfo.push_back(info);					
					}
				}
			}
			else
...
// 返回跨服服务器的地址信息
CECGame::SERVERINFO* CECGame::GetCrossServerInfo(int id)
{
     for(size_t i = 0; i< m_CrossServerInfo.size();++i)
	 {
		 if (m_CrossServerInfo[i].iServerID == id)
			 return &(m_CrossServerInfo[i]);
	 }
	 return NULL;
}
// 返回源服务器的地址信息

CECGame::SERVERINFO* CECGame::GetSrcServerInfo(int id)
{
     for(size_t i = 0; i< m_SrcServerInfo.size();++i)
	 {
		 if (m_SrcServerInfo[i].iServerID == id)
			 return &(m_SrcServerInfo[i]);
	 }
	 return NULL;
}

bool CECGame::OpenURL(const char* szURL)
{
	if( szURL && szURL != "" )
	{
		const char *szCountry = g_pGame->GetConfigs()->GetCountryRegion();
		if( stricmp(szCountry, "us") == 0 ||
			stricmp(szCountry, "fr") == 0 ||
			stricmp(szCountry, "de") == 0 )
		{
			if(l_hDll > 0 && l_hSDK > 0)
			{
				typedef HRESULT (*CCGOTOURL)(const CC_HANDLE hSDK, TCHAR* pszUrl);
				
				AWString url = AS2WC(szURL);
				
				CCGOTOURL ccurlproc;
				ccurlproc = (CCGOTOURL)GetProcAddress(l_hDll, "CC_GotoUrlInOverlay");
				if(ccurlproc != NULL)
				{
					HRESULT hRes = ccurlproc(l_hSDK, url.GetBuffer(0));
					if(hRes == S_OK)
						return true;
				}
			}
...
DWORD CECGame::GetPoseOnPet(AString petModelName)
{
	DWORD pose = 0;
	abase::hashtab<DWORD, AString, abase::_hash_function>::pair_type Pair = m_PoseOnPetTab.get(petModelName);
	if (Pair.second)
		pose = *Pair.first;
...
AString CECGame::LoadMid()
{
	AIniFile IniFile;
	char szIniFile[MAX_PATH];
	sprintf(szIniFile, "userData\\mid.ini");
	IniFile.Open(szIniFile);
	AString szTemp = IniFile.GetValueAsString("mid", "mid","");
	bool isRevamp = false;
	if(szTemp.IsEmpty())
	{
		isRevamp = true;
	...
	if(szTemp.GetLength() > 10)
	{
		isRevamp = true;
	}
	char* szBuff = szTemp.GetBuffer(0);
	for(int nIndex = szTemp.GetLength() - 1 ; nIndex >= 0; nIndex--)
	{
		if((szBuff[nIndex] >= 'a' && szBuff[nIndex] <= 'z')
			|| (szBuff[nIndex] >= 'A' && szBuff[nIndex] <= 'Z')
			|| (szBuff[nIndex] >= '1' && szBuff[nIndex] <= '9' ))
		{
	...
	if(isRevamp)
	{
		IniFile.WriteStringValue("mid", "mid", "wanmei");
