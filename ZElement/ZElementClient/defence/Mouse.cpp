	...
	if( HC_ACTION == nCode )
	{
		if( WM_MOUSEMOVE == wParam )
		{
			...
			if( 0 != GetInstance().g_ptLast.x && 0 != GetInstance().g_ptLast.y )
				GetInstance().g_mousemovedistance += (float)sqrtf( (pinfo->pt.x - GetInstance().g_ptLast.x)*(pinfo->pt.x - GetInstance().g_ptLast.x) + (pinfo->pt.y - GetInstance().g_ptLast.y)*(pinfo->pt.y - GetInstance().g_ptLast.y) );
				...
				if( ::PtInRect(&(GetInstance().g_rects[i]), pinfo->pt) )
				{
					...
					if( i!=GetInstance().g_regionlast )
					{
						if( GetInstance().g_regionlast >= 0 && GetInstance().g_regionlast < RegionCount )
						{
							...
							GetInstance().g_times[GetInstance().g_regionlast] += (dwTick-GetInstance().g_ticklast);
			...
			{
				RECT rect;
				HWND hwnd = FindWindowA( "ZElementClient Window", "Element Client" );
				if( NULL != hwnd )
				{
