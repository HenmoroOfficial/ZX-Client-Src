...
void CECHPWorkDead::OnFirstTick()
{
	...
	if (m_bBeDead)
	{
		m_pHost->PlayAction(CECHostPlayer::ACT_GROUNDDIE_LOOP, 1.0f);
	}
	else
	{
		m_pHost->PlayAction(CECHostPlayer::ACT_GROUNDDIE, 1.0f);
		m_pHost->PlayAction(CECHostPlayer::ACT_GROUNDDIE_LOOP, 1.0f, true, 200, true);
	}
...
void CECHPWorkRevive::OnFirstTick()
{
	...
	m_pHost->PlayAction(CECHostPlayer::ACT_REVIVE, 1.0f);
}
