...
public:		//	Operations
	...
	bool IsInteractor() { return (DistinguishID(m_NPCInfo.tid) == NPC_INTERACTOR) ? true : false; }
	...
	bool IsTeleport(){return m_bTeleport;}
	void SetTeleportNum(int iTeleportNum) { m_iTeleportNum = iTeleportNum; }
	int  GetTeleportNum(){ return m_iTeleportNum; }
...
protected:	//	Attributes
	...
	bool			m_bPhase;			//  NPC是否被相位标记
	short			m_iPhaseId;			//  NPC相位
	...
	bool			m_bDim;				//  朦胧状态
	bool			m_bInvisible;		//  隐身状态
	bool			m_bTeleport;		//  是否有传送功能
	int				m_iSpecialState;    // 由buff附加的状态，以后统一放在这里面进行处理
	bool			m_bScope;
	bool			m_bIsRepuNPC;
	NPC_Reputation*	m_NPCReputation;
	A3DGFXEx*		m_pDirVisibleStateGFX;
...
protected:	//	Operations
	...
	virtual void StartWork_BlewUp(DWORD dwParam);
	...
	virtual void TickWork_BlewUp(DWORD dwDeltaTime);
	...
	void OnMsgNPCDirVisibleState( const ECMSG& Msg );
	void OnMsgNPCBeCharge(const ECMSG& Msg);
	void OnMsgNPCBlewUp(const ECMSG& Msg);
