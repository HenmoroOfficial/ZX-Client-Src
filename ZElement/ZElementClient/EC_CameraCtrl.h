...
#define CECCAMERA_MAX_DISTANCE		24.0f
...
//	void Fix_Set(bool bFix);
	bool FixCamPitch_Get();
	void FixCamPitch_Set(bool bFix);
	bool SceneryMode_Get();
	void SceneryMode_Set(bool bSet);
	bool MovieMode_Get();
	void MovieMode_Set(bool bSet);
...
//	void CameraPos_Set(int nPos);
	...
	inline float GetCurDistance() { return m_fDistance; }

protected:
//	bool m_bFixed;
	bool m_bCamPitchFixed;
	bool m_bSliding;
//	int m_nCameraPos;
	int m_nCameraMode;
	bool m_bSceneryMode;
...
//	float m_fDistanceUser[CECCAMERA_MAX_USERDEFINE];
	...
	DWORD m_dwMovieTickTime;	// time tick in movie mode, for test

	bool UpdateDirAndUp();
	bool Action_CamPitchFix(P_ACTION_DATA pActionData);
	float InterpCamPitch_ByDistance();
};
