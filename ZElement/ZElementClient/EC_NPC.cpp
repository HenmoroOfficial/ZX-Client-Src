...
int CECNPC::DistinguishID(int tid)
{
	...
	case DT_INTERACTION_OBJECT_ESSENCE: iNPCType = NPC_INTERACTOR;	break;
	default:
		ASSERT(0);
		break;
	}
...
bool CECNPC::LoadNPCModel(int tid, const char* szFile, EC_NPCLOADRESULT& Ret)
{
	...
	if (iNPCType == NPC_MONSTER || iNPCType == NPC_PET ||iNPCType == NPC_INTERACTOR)
	{
...
bool CECNPC::Init(int tid, const S2C::info_npc& Info)
{
	...
	m_idMaster		= Info.id_master;
	m_cPetStar		= Info.pet_star;
	m_strName		= Info.name;
	if (m_dwStates & GP_STATE_NPC_SUMMON)
		m_idSummoner	= Info.id_summoner;
	if (m_dwStates & GP_STATE_NPC_OWNER)
	{
	...
	if (m_dwStates & GP_STATE_NPC_DIM)
		m_bDim = true;
	if(m_dwStates & GP_STATE_NPC_INVISBLE)
		m_bInvisible = true;
	if(m_dwStates & GP_STATE_NPC_TELEPORT1)
		m_bTeleport = true;
	if (m_dwStates & GP_STATE_NPC_FROZEN)
		m_iSpecialState |= CECPlayer::SPECIAL_STATE_FROZEN;

	if ( m_dwStates & GP_STATE_NPC_DIR_VISIBLE )
	{
		SetDirVisibleStates( Info.visible_state_id, Info.visible_state_dir, Info.visible_state_on );
	}

	if(m_dwStates & GP_STATE_NPC_PHASE)
	{
		m_bPhase = true;
		m_iPhaseId = Info.phase_id;
	}

	if(m_dwStates & GP_STATE_NPC_TELEPORT1 || m_dwStates & GP_STATE_NPC_TELEPORT2)
		m_iTeleportNum = Info.teleportnum;

	if (m_pPateName)
		m_pPateName->SetText(m_strName, false);
//	if (m_pPateDesc)
// 		m_pPateDesc->SetText(m_strName, false);

	m_pcdr->fStepHeight = 0.4f;
	...
	abase::hash_map<int, NPC_Reputation>* npcreputation;
	npcreputation = m_pNPCMan->GetNPCReputation();
	abase::hash_map<int, NPC_Reputation>::iterator it = npcreputation->find(m_NPCInfo.tid);

	if(it != npcreputation->end())
	{
		m_NPCReputation = &(it->second);
		m_bIsRepuNPC = true;
	}
	else
	{
		m_NPCReputation = NULL;
		m_bIsRepuNPC = false;
	}
...
GNET::Octets CECNPC::gen_npc_info()
{
	GNET::Octets o;
	o.reserve(sizeof(S2C::info_npc) + 64);

	add_to_octets<int>(o, m_NPCInfo.nid);
	add_to_octets<int>(o, m_NPCInfo.tid);
	...
	add_to_octets<int>(o, m_dwStates);

	if (m_dwStates & GP_STATE_EXTEND_PROPERTY)
		add_to_octets<__int64>(o, m_dwExtStates);

	if (m_dwStates & GP_STATE_NPC_PET)
		add_to_octets<int>(o, m_idMaster);

	if (m_dwStates & GP_STATE_NPC_NAME)
	{
		char len = char((m_strName.GetLength() + 1) * sizeof(wchar_t));
		add_to_octets<char>(o, len);
		o.insert(o.end(), (const wchar_t*)m_strName, len);
	}
...
bool CECNPC::Tick(DWORD dwDeltaTime)
{
	...
	else if (m_pNPCSkin && IsInteractor())
	{
		const MONSTER_ESSENCE *monster_essence = ((CECInteractor *)this)->GetMonstDBEssence();
		float vScale = monster_essence ? monster_essence->size * 0.08f : 0.08f;
	...
	if(m_NPCReputation && m_bIsRepuNPC)
	{
		if(m_fDistToHost < 10)
		{
			if(m_pNPCModel && !m_bScope)
			{
				if(m_NPCReputation->type == 1)
				{
					if(m_NPCReputation->reputationvalue2 != 0 && g_pGame->GetGameRun()->GetHostPlayer()->GetRegionReputation(25) > m_NPCReputation->reputationvalue2)
					{
						NPCTurnFaceTo(g_pGame->GetGameRun()->GetHostPlayer()->GetCharacterID(), 100);
						
						if(m_NPCReputation->action2 != "" && m_iCurWork == WORK_STAND)
							m_pNPCModel->PlayActionByName(m_NPCReputation->action2, 1);

						if(m_NPCReputation->words2)
						{
							ACString szName = m_strName;
							szName += _AL(":  ");
							szName += g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan()->GetStringFromTable(m_NPCReputation->words2);		
							g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan()->AddChatMessage(szName, 0);		
						}
							
					}
					else if(m_NPCReputation->reputationvalue1 != 0 && g_pGame->GetGameRun()->GetHostPlayer()->GetRegionReputation(25) > m_NPCReputation->reputationvalue1)
					{
						NPCTurnFaceTo(g_pGame->GetGameRun()->GetHostPlayer()->GetCharacterID(), 100);
						
						if(m_NPCReputation->action1 != "" && m_iCurWork == WORK_STAND)
							m_pNPCModel->PlayActionByName(m_NPCReputation->action1, 1);

						if(m_NPCReputation->words1)
						{
							ACString szName = m_strName;
							szName += _AL(":  ");
							szName += g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan()->GetStringFromTable(m_NPCReputation->words1);				
							g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan()->AddChatMessage(szName, 0);	
						}
				...
				else if(m_NPCReputation->type == 2)
				{
					if(m_NPCReputation->reputationvalue2 != 0 && g_pGame->GetGameRun()->GetHostPlayer()->GetRegionReputation(26) > m_NPCReputation->reputationvalue2)
					{
						NPCTurnFaceTo(g_pGame->GetGameRun()->GetHostPlayer()->GetCharacterID(), 100);
					...
					else if(m_NPCReputation->reputationvalue1 != 0 && g_pGame->GetGameRun()->GetHostPlayer()->GetRegionReputation(26) > m_NPCReputation->reputationvalue1)
					{
						NPCTurnFaceTo(g_pGame->GetGameRun()->GetHostPlayer()->GetCharacterID(), 100);
				...
				else if(m_NPCReputation->type == 3)
				{
					if(m_NPCReputation->reputationvalue2 != 0 && g_pGame->GetGameRun()->GetHostPlayer()->GetRegionReputation(27) > m_NPCReputation->reputationvalue2)
					{
						NPCTurnFaceTo(g_pGame->GetGameRun()->GetHostPlayer()->GetCharacterID(), 100);
					...
					else if(m_NPCReputation->reputationvalue1 != 0 && g_pGame->GetGameRun()->GetHostPlayer()->GetRegionReputation(27) > m_NPCReputation->reputationvalue1)
					{
						NPCTurnFaceTo(g_pGame->GetGameRun()->GetHostPlayer()->GetCharacterID(), 100);
	...
	{
		if(m_pNPCModel)
			m_pNPCModel->GetA3DSkinModel()->SetTransparent(0.5);
	}
...
bool CECNPC::InternalRender(CECViewport* pViewport, int iRenderFlag, bool bHighLight)
{
	...
	CECHostPlayer* pHost = g_pGame->GetGameRun()->GetHostPlayer(); 
	if(m_bInvisible &&  m_idSummoner != pHost->GetCharacterID() && !pHost->IsTeamMember(m_idSummoner))
		return true;
...
bool CECNPC::ProcessMessage(const ECMSG& Msg)
{
	...
	case MSG_NM_DIR_VISIBLE_STATE: OnMsgNPCDirVisibleState(Msg); break;
	case MSG_NM_BECHARGE:		OnMsgNPCBeCharge(Msg);		break;
	case MSG_NM_BLEWOFF:		OnMsgNPCBlewUp(Msg);		break;
							 ...
							 int iTransTime/* 200 */)
{
	...
	if (m_pNPCModel)
	{
		...
		if (IsInteractor())
		{
			CECInteractor *pInteractor = ((CECInteractor *) this);
			bool isObjPlay = false;
			AString actionName;

			for (int i = 0; i < CECInteractor::MAX_INTERACTIVED_PLAYER_NUM; ++ i)
			{
				CECInteractor::InteractiveData data = pInteractor->GetInteractiveData(i);
				if (data.m_sState == CECInteractor::NO_INTERACTIVE)
					continue;

				if (m_iAction != ACT_WALK && m_iAction != ACT_RUN && m_iAction != ACT_NPC_WALK && m_iAction != ACT_NPC_RUN)
				{
					//	Play player's action.
					if (data.m_pPlayerModel)
						data.m_pPlayerModel->PlayActionByName(ACT_CHANNEL_BODY, data.m_sPlayerAction, fWeight, bRestart, iTransTime, true);

					//	Play npc's action.
					if (!data.m_sObjAction.IsEmpty())
					{
						actionName = data.m_sObjAction;
						//m_pNPCModel->PlayActionByName(data.m_sObjAction, fWeight, bRestart, iTransTime);
						isObjPlay = true;

						// If someone starts or ends interaction, play action exclusively.
						if (data.m_sState == CECInteractor::START_INTERACT || data.m_sState == CECInteractor::END_INTERACT)
							break;
					}
				}
				else
				{
					//	Play player's action.
					if (data.m_pPlayerModel)
						data.m_pPlayerModel->PlayActionByName(ACT_CHANNEL_BODY, data.m_sPlayerMoveAction, fWeight, bRestart, iTransTime, true);

					//	Play npc's action.
					if (!data.m_sObjMoveAction.IsEmpty())
					{
						actionName = data.m_sObjMoveAction;
						//m_pNPCModel->PlayActionByName(data.m_sObjMoveAction, fWeight, bRestart, iTransTime);
						isObjPlay = true;
					}
				}
			}
			...
			if (isObjPlay)
			{
				m_pNPCModel->PlayActionByName(actionName, fWeight, bRestart, iTransTime);
				return true;
			}
		}
...
void CECNPC::StartWork_BlewUp(DWORD dwParam)
{
}
...
void CECNPC::TickWork_BlewUp(DWORD dwDeltaTime)
{
...
void CECNPC::OnMsgNPCAtkResult(const ECMSG& Msg)
{
	...
	{
		AfxSkillGfxShowDamage(m_NPCInfo.nid, pCmd->target_id, nDamage, 1, pCmd->attack_flag);
		AfxSkillGfxShowCaster(m_NPCInfo.nid, pCmd->attack_flag);
	}	
	else if (m_pNPCModel)
		m_pNPCModel->AddOneAttackDamageData(m_NPCInfo.nid, pCmd->target_id, pCmd->attack_stamp, pCmd->attack_flag, nDamage);
}
...
void CECNPC::OnMsgNPCStartAttack(const ECMSG& Msg)
{
	...
	{
		//	Face to target
		NPCTurnFaceTo(pCmd->idTarget);
		m_idAttackTarget = pCmd->idTarget;
		...
		if (IsMonsterNPC())
		{
			...
			StartWork(WT_NORMAL, WORK_FIGHT, MONSTER_ATTACK_SPEED);
		}
...
void CECNPC::OnMsgNPCSkillResult(const ECMSG& Msg)
{
	...
	if (pCmd->attack_flag & MOD_RETORT)
	{
		AfxSkillGfxShowDamage(m_NPCInfo.nid, pCmd->target_id, nDamage, 1, pCmd->attack_flag | MOD_SKILL);
		AfxSkillGfxShowDamage(m_NPCInfo.nid, pCmd->target_id, pCmd->dt_damage, 1, pCmd->attack_flag | MOD_DEITY_DAMAGE);
		AfxSkillGfxShowCaster(m_NPCInfo.nid, pCmd->attack_flag | MOD_SKILL);
	}
	else if (m_pNPCModel)
	{
		m_pNPCModel->AddOneAttackDamageData(m_NPCInfo.nid, pCmd->target_id, pCmd->attack_stamp, pCmd->attack_flag | MOD_SKILL, nDamage);
		m_pNPCModel->AddOneAttackDamageData(m_NPCInfo.nid, pCmd->target_id, pCmd->attack_stamp, pCmd->attack_flag | MOD_DEITY_DAMAGE, pCmd->dt_damage);
	}
}
...
void CECNPC::OnMsgNPCDirVisibleState( const ECMSG& Msg )
{
	using namespace S2C;

	if ( Msg.dwParam2 == DIR_VISIBLE_STATE )
	{
		cmd_dir_visible_state* pCmd = (cmd_dir_visible_state*)Msg.dwParam1;
		ASSERT(pCmd && pCmd->role_id == m_NPCInfo.nid);
		SetDirVisibleStates( pCmd->state_id, pCmd->dir, pCmd->on );
	}
}

void CECNPC::OnMsgNPCBeCharge(const ECMSG& Msg)
{
	using namespace S2C;
	
	if(Msg.dwParam2 == PLAYER_CHARGE)
	{
		
	}
}

void CECNPC::OnMsgNPCBlewUp(const ECMSG& Msg)
{
	using namespace S2C;
	if(Msg.dwParam2 == BLOW_OFF)
	{
		blow_off* pCmd = (blow_off*)Msg.dwParam1;
		if(!pCmd)
			return;

		PrepareBlewUp(800, 15.0f);
		StartWork(WT_NORMAL, WORK_BLEWUP);
	}
}
...
void CECNPC::OnMsgNPCCastSkill(const ECMSG& Msg)
{
	...
	if( Msg.dwParam2 == OBJECT_CAST_SKILL )
	{
		...
		PlayAttackEffect(pCmd->targets[0], pCmd->skill, pCmd->level, pCmd->attack_stamp, -1, 0);
	}
	else if( Msg.dwParam2 == SKILL_INTERRUPTED )
	{
		if( m_pNPCModel )
			m_pNPCModel->ClearComActFlag(false);
...
//	if( pCmd->target != g_pGame->GetGameRun()->GetWorld()->GetPlayerMan()->GetHostPlayer()->GetCharacterID() )
//		mask &= MOD_PHYSIC_ATTACK_RUNE | MOD_MAGIC_ATTACK_RUNE | MOD_CRITICAL_STRIKE;

	if (m_pNPCModel)
		m_pNPCModel->AddOneAttackDamageData(
			pCmd->caster,
			pCmd->target,
			pCmd->attack_stamp,
...
bool CECNPC::RenderName(CECViewport* pViewport, DWORD dwFlags)
{
	...
	else if ((IsServerNPC() && !(dwFlags & RNF_NPCNAME)) ||
		(IsMonsterNPC() && !(dwFlags & RNF_MONSTERNAME)) ||
		IsCarrierNPC())
		bDrawName = false;
	...
	if( m_pPateOwner && bDrawName )
	{
		...
		m_pPateOwner->RegisterRender(x, y, dwNameCol, m_PateContent.z, m_PateContent.rhw);
	}

	if( m_pPateDesc && bDrawName && !m_strName.IsEmpty() )
	{
		m_pPateDesc->GetExtent(&cx, &cy);
		cx = int(cx * fScale);
		cy = int(cy * fScale);
		x = m_PateContent.iBaseX - (cx >> 1);
		y -= cy + 2;
		...
		DWORD dwNameCol = GetNameColor();
		m_pPateDesc->RegisterRender(x, y, dwNameCol, m_PateContent.z, m_PateContent.rhw);
	}
	if (g_pGame->GetConfigs()->GetShowIDFlag())
	{
		ACString strID;
		strID.Format(_AL("%d"), GetTemplateID());
		m_pPateName->SetText(strID, false);
	}
	else
		m_pPateName->SetText(m_strName, false);

	if ( m_pPateName && bDrawName && !m_strName.IsEmpty() )
	{
	...
	if (dwFlags & RNF_WORDS)
	{
		...
		if (m_pPateLastWords2 && m_pPateLastWords2->GetItemNum())
		{
			...
			y -= cy + 2;
			x = m_PateContent.iBaseX - (cx >> 1);
			m_pPateLastWords2->RegisterRender(x, y, dwCol, m_PateContent.z, m_PateContent.rhw);
		}
		...
		if (m_pPateLastWords1 && m_pPateLastWords1->GetItemNum())
		{
			...
			y -= cy + 2;
			x = m_PateContent.iBaseX - (cx >> 1);
			m_pPateLastWords1->RegisterRender(x, y, dwCol, m_PateContent.z, m_PateContent.rhw);
		}
	}
...
void CECNPC::SetNewExtendStates(__int64 dwNewStates)
{
	...
	if (m_pNPCModel)
	{
		for (int i=0; i < 64; i++)
		{
			...
			const GNET::VisibleState* pvs = GNET::VisibleState::Query(127, i);
			if (!pvs)
				continue;
			...
			{
				//	Remove old state
				m_pNPCModel->RemoveGfx(strGFXFile, pvs->GetHH());
			}
			else
			{
				//	Add new state
				m_pNPCModel->PlayGfx(strGFXFile, pvs->GetHH());
			}
		}
	}
...
{
	// monsters, pets and interactors need not to do trace with brush, so don't build the brushes here.
	if (IsMonsterNPC())
	{
		return true;
	}
...
void CECNPC::BubbleText(int iIndex, DWORD dwNum, int p1/* 0 */, bool bLastOne/* true */, bool bRandPos/* false*/)
{
	...
	if (bRandPos)
		vPos += a3d_RandDirH() * aabb.Extents.x;
	...
	float fScale = fScale2; // medium font
	...
	case BUBBLE_DAMAGE:
		if (p1 & 0x0001)
		{
			pDecal->AddIcon(CECImageRes::IMG_DEADLYSTRIKE, 0, A3DCOLORRGB(255, 255, 255));
			fScale *= 1.5f; // large font
			pBubbleDecal->SetJumpOut(true);
		}
		else if (p1 & 0x0002)
		{
			pDecal->AddIcon(CECImageRes::IMG_RETORT, 0, A3DCOLORRGB(255, 255, 255));
			pBubbleDecal->SetJumpOut(true);
		}

		pDecal->AddNumIcons(CECImageRes::IMG_POPUPNUM, dwNum, A3DCOLORRGB(255, 255, 255));
		break;
	...
	case BUBBLE_SKILL_DAMAGE:
		if (p1 & 0x0001)
		{
			pDecal->AddIcon(CECImageRes::IMG_DEADLYSTRIKE, 0, A3DCOLORRGB(255, 205, 75));
			fScale *= 1.5f; // large font
			pBubbleDecal->SetJumpOut(true);
		}
		else if (p1 & 0x0002)
		{
			pDecal->AddIcon(CECImageRes::IMG_RETORT, 0, A3DCOLORRGB(255, 205, 75));
			pBubbleDecal->SetJumpOut(true);
		}

		pDecal->AddNumIcons(CECImageRes::IMG_POPUPNUM, dwNum, A3DCOLORRGB(255, 205, 75));
		break;
	...
	case BUBBLE_DEITY_DAMAGE:
		...
		if (p1 & 0x0004)
		{
			pDecal->AddIcon(CECImageRes::IMG_DT_DEADLYSTRIKE, 0, A3DCOLORRGB(0, 0, 225));
			fScale *= 1.5f; // large font
			pBubbleDecal->SetJumpOut(true);
		}
		else if (p1 & 0x0002)
		{
			pDecal->AddIcon(CECImageRes::IMG_RETORT, 0, A3DCOLORRGB(0, 0, 225));
			pBubbleDecal->SetJumpOut(true);
		}
		// 颜色待调整
		pDecal->AddNumIcons(CECImageRes::IMG_POPUPNUM, dwNum, A3DCOLORRGB(0, 0, 255));
		break;
	default:
		return;
	}
...
bool CECNPC::IsHide()
{
	...
	if (!pHostPlayer || !pHostPlayer->GetCutscene() || !pHostPlayer->GetCutscene()->IsPlayingCutscene())
	{
		return false;
	}
	CECCutscene* pCutscene = pHostPlayer->GetCutscene();
	if (IsPetNPC())
	{
		//pet.
		if (GetMasterID())
		{
			if (GetMasterID()==pHostPlayer->GetCharacterID())
			{
				//host player's pet.
				return pHostPlayer->GetCutscene()->IsSelfShow();
			}else
			{
				//else player's pet.
				return pHostPlayer->GetCutscene()->IsElsePlayerShow();
			}
		}
		ASSERT(0);
		return false;
	...
	{
		//monster
		return !pCutscene->IsMonsterShow();
	}else if (IsServerNPC())
	{
		//NPC.
		return !pCutscene->IsNPCShow();
	}
...
void CECNPC::SetDirVisibleStates( int iVisibleStateID, int iDir, bool bOn )
{
	if ( bOn )	//开启状态
	{
...
void CECNPC::PrepareBlewUp(DWORD DWDuration, float fSpeed)
{
	m_dwPastTime = 0;
