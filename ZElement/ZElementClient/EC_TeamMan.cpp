...
CECTeam* CECTeamMan::create_team_from_record(GNET::Marshal::OctetsStream& os)
{
	CECTeam* pTeam = new CECTeam(0);

	if (!pTeam->unmarshal(os))
	{
		delete pTeam;
		return NULL;
	}

	m_TeamTab.put(pTeam->GetLeaderID(), pTeam);
	return pTeam;
}
