...
#define CECCAMERA_FIX_PITCH_NEAR	(-15.0f)
#define CECCAMERA_FIX_PITCH_FAR		(-45.0f)
#define CECCAMERA_FIX_PITCH_DELTA	(CECCAMERA_FIX_PITCH_FAR - CECCAMERA_FIX_PITCH_NEAR)

#define CECCAMERA_MIN_ZOOM			0.5f
#define CECCAMERA_MAX_ZOOM			2.0f

#define CECCAMERA_FIX_MIN_DISTANCE  3.0f
#define CECCAMERA_FIX_MAX_DISTANCE	10.0f

#define PRESSING(vk)				(GetKeyState(vk) & 0x8000)
...
const float cameraHeight = 1.3f;
const float lockDistance = 20.0f;
const float lockAdjustRatio = 0.05f;

CECCamera::CECCamera()
{
//	m_bFixed = false;
	m_bCamPitchFixed = false;
	m_bSliding = false;
...
//	m_nCameraPos = CAMERA_POS_GROUND;
	...
	m_fDistance = m_fDistanceDest = CECCAMERA_MAX_DISTANCE * .5f;// * 2.0f / 3.0f;
...
bool CECCamera::Init(P_INIT_DATA pInitData)
{
	...
	m_bCamPitchFixed = ss.bLockCamera;

	if (m_bCamPitchFixed)
		m_fPitch = m_fPitchDest = InterpCamPitch_ByDistance();
	else
...
float CECCamera::InterpCamPitch_ByDistance()
{
	return -float(RAD2DEG(atan( (lockAdjustRatio*m_fDistance*m_fDistance + cameraHeight) / (lockDistance + m_fDistance) ))) - 28.0f;
//	return CECCAMERA_FIX_PITCH_NEAR + CECCAMERA_FIX_PITCH_DELTA * (m_fDistance - CECCAMERA_MIN_DISTANCE) / (CECCAMERA_MAX_DISTANCE - CECCAMERA_MIN_DISTANCE);
}

bool CECCamera::Action_CamPitchFix(P_ACTION_DATA pActionData)
{	
	...
	// Adjust camera pitch
	m_fPitch = InterpCamPitch_ByDistance();
	...
	if( pActionData->nDeltaZ < 0 )
	{
		...
		if( m_fDistanceDest < CECCAMERA_FIX_MIN_DISTANCE )
			m_fDistanceDest = CECCAMERA_FIX_MIN_DISTANCE;
	}
	else if( pActionData->nDeltaZ > 0 )
	{
		m_bSliding = true;
		m_fDistanceDest += gs.fCamZoomSpeed;
		if( m_fDistanceDest > CECCAMERA_FIX_MAX_DISTANCE )
			m_fDistanceDest = CECCAMERA_FIX_MAX_DISTANCE;
	}
...
//	if( m_bFixed ) return true;
	if( m_bCamPitchFixed && !m_bSceneryMode ) return Action_CamPitchFix(pActionData);
...
bool CECCamera::Tick(P_TICK_DATA pTickData, P_ADJUST_DATA pAdjustData)
{
	...
	m_dwMovieTickTime += pTickData->dwTime;
	...
	if(m_bCamPitchFixed)
	{
		fDegTick = fDegTick/5.0f;
		fZoomTick = fZoomTick/5.0f;
	}
	
	if( m_bSliding && !m_bCamPitchFixed)
	{
	...
	if( m_bSceneryMode )
	{
		...
		if(!ss.bLimitedCameraMode)
		{
              fDist = ss.fCameraModeWalkSpeed * pTickData->dwTime / 1000.0f;
		}
	...
	else if( m_bMovieMode )
	{
		...
		// for test, camera change for only 10 seconds
		if(m_dwMovieTickTime < 10000 )
		{
			//	Turn camera
			int nDeltaY = 4;
			ACTION_DATA ActData;
			memset(&ActData, 0, sizeof(CECCamera::ACTION_DATA));
			ActData.nDeltaX		= nDeltaY;
			ActData.nDeltaY		= 0;
			ActData.nDeltaZ		= 0;
			ActData.vecDirChar	= pCamera->GetDir();
			Action(&ActData);

			pCamera->TurnAroundAxis(g_pGame->GetGameRun()->GetHostPlayer()->GetPos(), m_vecAxisY, DEG2RAD(10.0f * nDeltaY / 80));
		}
		else if(m_dwMovieTickTime < 20000)
		{
			// move up
			A3DVECTOR3 vecMove;
			vecMove.x = fDist;
			...
			pCamera->Move(vecMove);	
		}
		else if(m_dwMovieTickTime < 30000)
		{
			A3DVECTOR3 vecMove;
			vecMove.x = 0;
			vecMove.y = fDist;
		...
		else if(m_dwMovieTickTime < 40000)	// 1
		{
			A3DVECTOR3 vecMove;
 			vecMove.x = 113.6f;
 			vecMove.y = 163.6f;
 			vecMove.z = 63.38f;
			pCamera->SetPos(vecMove);
		...
		else if(m_dwMovieTickTime < 44000)	// 2
		{
			A3DVECTOR3 vecMove;
 			vecMove.x = 137.47f;
 			vecMove.y = 163.771f;
		...
		else if(m_dwMovieTickTime < 48000)	// 3
		{
			A3DVECTOR3 vecMove;
 			vecMove.x = 163.678f;
 			vecMove.y = 165.051f;
		...
		else if(m_dwMovieTickTime < 52000)	// 4
		{
			A3DVECTOR3 vecMove;
 			vecMove.x = 188.0f;
 			vecMove.y = 186.0f;
		...
		else
		{
			...
			pCamera->TurnAroundAxis(vecMove, m_vecAxisY, DEG2RAD(10.0f * nDeltaY / 80));
		}
	...
	else if (m_bCamPitchFixed)
	{
		...
		pAdjustData->vecPosCam.y = pTickData->vecPosChar.y + lockAdjustRatio*m_fDistance*m_fDistance + cameraHeight;
	}
	else
		pAdjustData->vecPosCam = pTickData->vecPosChar - m_fDistance * m_vecDirCam;
...
bool CECCamera::FixCamPitch_Get()
{
	return m_bCamPitchFixed;
}

void CECCamera::FixCamPitch_Set(bool bFix)
{
	m_bCamPitchFixed = bFix;
	if (m_bCamPitchFixed)
		m_fPitchDest = InterpCamPitch_ByDistance();
}
...
bool CECCamera::MovieMode_Get()
{
	return m_bMovieMode;
}

void CECCamera::MovieMode_Set(bool bSet)
{
	m_bMovieMode = bSet;
	
	if(bSet)	// if first press, hide all panels
		g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan()->SetShowAllPanels(false);
	else
		g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan()->SetShowAllPanels(true);
