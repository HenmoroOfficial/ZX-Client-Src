...
float TaskExpAnalyser::Run(TaskInterface* pTask, const TaskExpressionArr& aExp, int nCalcType)
{
	float ret = 0.0f;
	...
	for (size_t i = 0; i < aExp.size(); i++)
	{
		...
		else if (exp.type == enumTaskExpVar)
		{
			...
			{
				if (nCalcType == enumTaskCalcExp)
				{
					ret = (float)pTask->GetPlayerLevel();
				}
				else if (nCalcType == enumTaskCalcGlobalVal)
				{
					if((long)exp.value == 0)
					{
						ret = 0;
