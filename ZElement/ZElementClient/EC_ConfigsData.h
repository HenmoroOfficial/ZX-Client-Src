...
enum
{
	...
	VER_NUM,
	VER_BASE = 23,
	EC_CONFIG_VERSION = VER_BASE+VER_26,
};
...
// 24-25 增加自定义1和自定义2两个聊天频道
class CECConfigsData25 : public CECConfigsDataBase
{
public:
	CECConfigsData25()
	{
	...
	~CECConfigsData25(){}
	bool LoadData(const void* pDataBuf, int iDataSize);
	CECConfigsDataBase& operator = (CECConfigsDataBase* data);
...
// 25-26  增加好友备注
class CECConfigsData26 : public CECConfigsData25
{
public:
	CECConfigsData26()
	{
		m_nVersion = 26; 
		memset(&m_frs,0, sizeof(m_frs));
	}
	~CECConfigsData26(){}
	bool LoadData(const void* pDataBuf, int iDataSize);
	CECConfigsDataBase& operator = (CECConfigsDataBase* data);
	...
	struct EC_FRIENDALIASES
	{
		int    iPlayerId;
		ACHAR  szName[EC_FRIENDNAME_LEN];
	};

	struct EC_FRIENDSREMARKS
	{
	     EC_FRIENDALIASES  friendsaliases[EC_FRIENDS_NUM];
	}m_frs;
...
typedef CECConfigsData26 CECConfigsCur;
