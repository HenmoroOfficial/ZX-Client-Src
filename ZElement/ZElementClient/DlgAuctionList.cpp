	...
	else if( idAction == AUCTION_ACTION_GETITEM_RE )
	{
		...
		for(i = 0; i < p->items.size(); i++ )
		{
			GetGameUIMan()->m_pDlgAuctionBuyList->m_mapAuctionHint[p->ids[i]]
				= p->items[i].data;
			GetGameUIMan()->m_pDlgAuctionSellList->UpdateHint(p->ids[i], p->items[i].id, p->items[i].expire_date, p->items[i].count);
			GetGameUIMan()->m_pDlgAuctionBuyList->UpdateHint(p->ids[i], p->items[i].id, p->items[i].expire_date, p->items[i].count);
			UpdateHint(p->ids[i], p->items[i].id, p->items[i].expire_date, p->items[i].count);
		}
	}
}
