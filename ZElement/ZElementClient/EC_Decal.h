...
public:		//	Operations
	...
	bool AddNumIcons(int iImageSlot, __int64 iNumber, A3DCOLOR col, bool bShowMinus = false);

	//	Render routine
	virtual bool Render(CECViewport* pViewport);
	bool Render(int x, int y, float z, float rhw);
...
public:		//	Operations
	...
	float GetScaleY() { return m_fScaleY; }
	//	Set / Get z position
	void SetZValue(float z, float rhw) { m_fz = z; m_rhw = rhw; }
...
protected:	//	Operations
	bool     TransformPosToScreen(CECViewport* pViewport);
...
public:		//	Operations
	...
	//	has more?
	bool IsLastOne() const { return m_bLastOne; }
	//	Set last one flag
	void SetLastOneFlag(bool b) { m_bLastOne = b; }
	//	Need jump out
	bool GetJumpOut() const { return m_bJumpOut; }
	//	Set jump out flag
	void SetJumpOut(bool flag) { m_bJumpOut = flag; }

	void SetGfx(const char* strName);
...
protected:	//	Attributes
	...
	bool		m_bLastOne;		//	Whether bubble more ones
	bool		m_bJumpOut;		//	Whether need jump out effects
...
public:		//	Operations
	...
	//	Add a bubble decal
	CECBubbleDecal* AddDecal(const A3DVECTOR3& vPos, int iDCID, bool bLastOne);
