...
const int MAX_AUCTION_PRICE = 2000000000;
...
void CDlgAuctionSellList::OnTick()
{
	...
	if( IsShow() )
	{
		...
		a_sprintf(szText, _AL("%d"), GetHostPlayer()->GetMoneyAmount() );
		m_pTxt_PackGold->SetText(szText);
...
void CDlgAuctionSellList::OnCommandCloseAuction(const char* szCommand)
{
	...
	{
//		if( m_pLst_Item->GetItemData(nSel, 4) == 0 )
		{
			PAUIDIALOG pMsgBox;
