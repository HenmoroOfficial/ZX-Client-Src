...
bool CECSceneBlock::Load(AFile* pWorldFile, AFile* pBSDFile, float sx, float sz, float fDist)
{
	...
	{
//		a_LogOutput(1, ("CECSceneBlock::Load, failed to load post effect in block"));
		//	return false;
	}
#endif
...
bool CECSceneBlock::LoadTrees(int iNumTree, bool bSkipData)
{
	...
	if (m_pScene->GetSceneFileVersion() < 5)
	{
		...
		for (int i=0; i < iNumTree; i++)
		{
			...
			vPos.x	= TreeInfo.vPos[0] + m_pScene->GetCenterX();
			vPos.z	= TreeInfo.vPos[1] + m_pScene->GetCenterZ();
	...
	else	//	Version >= 5
	{
		...
		for (int i=0; i < iNumTree; i++)
		{
			...
			vPos.z	= TreeInfo.vPos[2] + m_pScene->GetCenterZ();
...
bool CECSceneBlock::LoadECModels(int iNumModel, bool bSkipData)
{
	...
	for (int i=0; i < iNumModel; i++)
	{
		DWORD dwID = pOnmtMan->LoadECModel(m_pWorldFile, m_pScene->GetCenterX(), m_pScene->GetCenterZ());
		if (!dwID)
			continue;
...
bool CECSceneBlock::LoadPostEffect(int iOffset)
{
	...
	DWORD dwRead;
	m_pBSDFile->Read(&m_dwEffectFlag, sizeof(A3DPOST_EFFECT), &dwRead);
	m_dwEffectFlag = A3DPOST_EFFECT_CC;
	m_dwEffectFlag |= A3DPOST_EFFECT_Bloom;
	m_dwEffectFlag |= A3DPOST_EFFECT_FullGlow;
	m_dwEffectFlag |= A3DPOST_EFFECT_Dof;
	...
	m_pBSDFile->Read(&(m_EffectWarp.fScale), sizeof(float), &dwRead);
	m_pBSDFile->Read(&(m_EffectWarp.fSpeed), sizeof(float), &dwRead);
	m_pBSDFile->Read(&(m_EffectWarp.bEnableMask), sizeof(bool), &dwRead);
	m_pBSDFile->ReadString(m_EffectWarp.strMaskTextureName);
	m_pBSDFile->Read(&(m_EffectCCDay.fFactor), sizeof(float), &dwRead);
	m_pBSDFile->Read(&(m_EffectCCDay.bUseManualParam1), sizeof(bool), &dwRead);
	m_pBSDFile->Read(&(m_EffectCCDay.bUseManualParam2), sizeof(bool), &dwRead);
	m_pBSDFile->Read(&(m_EffectCCDay.bAfterUI), sizeof(bool), &dwRead);
	m_pBSDFile->Read(&(m_EffectCCNight.fFactor), sizeof(float), &dwRead);
	m_pBSDFile->Read(&(m_EffectCCNight.bUseManualParam1), sizeof(bool), &dwRead);
	m_pBSDFile->Read(&(m_EffectCCNight.bUseManualParam2), sizeof(bool), &dwRead);
	m_pBSDFile->Read(&(m_EffectCCNight.bAfterUI), sizeof(bool), &dwRead);
#pragma pack (push, 1)
	...
	m_pBSDFile->Read(&test, sizeof(EDITOR_DATA_POST_EFFECT_CC_MANUAL_PARAM), &dwRead);
	m_pBSDFile->Read(&test, sizeof(EDITOR_DATA_POST_EFFECT_CC_MANUAL_PARAM), &dwRead);

#pragma pack (pop)
	m_pBSDFile->ReadString(m_EffectCCDay.szTex1);
	m_pBSDFile->ReadString(m_EffectCCDay.szTex2);
	m_pBSDFile->ReadString(m_EffectCCNight.szTex1);
	m_pBSDFile->ReadString(m_EffectCCNight.szTex2);
...
//	fDist: distance between block center and world's loading center
bool CECSceneBlock::UpdateRes(float fDist,bool bNoDelete)
{
...
void CECSceneBlock::SetResLoadDists(int iScheme)
{
	...
	case 1:	//	In game scheme - near mode
		...
		m_aResLoadDist[SCNRES_WATER]	= 300.0f;
		m_aResLoadDist[SCNRES_BUILDING] = 30.0f;
	...
	case 4:	
		m_aResLoadDist[SCNRES_TREE]		= 150.0f;
		m_aResLoadDist[SCNRES_WATER]	= 600.0f;
		m_aResLoadDist[SCNRES_BUILDING] = 150.0f;
		m_aResLoadDist[SCNRES_BOXAREA]	= 300.0f;
		m_aResLoadDist[SCNRES_GRASS]	= 40.0f;
	...
	case 5:	//	In game scheme - middle mode
		...
		m_aResLoadDist[SCNRES_BUILDING] = 200.0f;
	...
	case 6:	//	In game scheme - new mode 2013.7.30

		m_aResLoadDist[SCNRES_TREE]		= 300.0f;
		m_aResLoadDist[SCNRES_WATER]	= 1000.0f;
		m_aResLoadDist[SCNRES_BUILDING] = 250.0f;
		m_aResLoadDist[SCNRES_BOXAREA]	= 500.0f;
		m_aResLoadDist[SCNRES_GRASS]	= 55.0f;
		m_aResLoadDist[SCNRES_EFFECT]	= 160.0f;
		m_aResLoadDist[SCNRES_ECMODEL]	= 160.0f;
	...
	case 51:	//	In game scheme - test mode
		const EC_SYSTEM_SETTING& ss = g_pGame->GetConfigs()->GetSystemSettings();
		m_aResLoadDist[SCNRES_TREE]		= ss.iDist_tree;
		m_aResLoadDist[SCNRES_WATER]	= ss.iDist_Water;
		m_aResLoadDist[SCNRES_BUILDING] = ss.iDist_Buiding;
		m_aResLoadDist[SCNRES_BOXAREA]	= ss.iDist_BoxArea;
		m_aResLoadDist[SCNRES_GRASS]	= ss.iDist_Grass;
		m_aResLoadDist[SCNRES_EFFECT]	= ss.iDist_Effect;
		m_aResLoadDist[SCNRES_ECMODEL]	= ss.iDist_Ecmodel;
		m_aResLoadDist[SCNRES_CRITTER]	= ss.iDist_Critter;
		m_aResLoadDist[SCNRES_BEZIER]	= ss.iDist_Bezzier;
		m_aResLoadDist[SCNRES_SOUND]	= ss.iDist_Sound;

		if( g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline() )
		{
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(0, 800.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(1, 1200.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(2, 2000.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetViewScheme(1);
		}
		//g_pGame->GetViewport()->GetA3DCamera()->SetProjectionParam(-1.0f, -1.0f, 1000.0f, -1.0f);
		...
		/*
	case 3:	//	In game scheme - far mode

		m_aResLoadDist[SCNRES_TREE]		= 500.0f;
		m_aResLoadDist[SCNRES_WATER]	= 1000.0f;
		m_aResLoadDist[SCNRES_BUILDING] = 400.0f;
		m_aResLoadDist[SCNRES_BOXAREA]	= 800.0f;
		m_aResLoadDist[SCNRES_GRASS]	= 70.0f;
		m_aResLoadDist[SCNRES_EFFECT]	= 200.0f;
		m_aResLoadDist[SCNRES_ECMODEL]	= 400.0f;
		m_aResLoadDist[SCNRES_CRITTER]	= 300.0f;
		m_aResLoadDist[SCNRES_BEZIER]	= 800.0f;
		m_aResLoadDist[SCNRES_SOUND]	= 100.0f;
		...
		if( g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline() )
		{
			...
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(1, 900.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(2, 1000.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetViewScheme(2);
		}
		g_pGame->GetViewport()->GetA3DCamera()->SetProjectionParam(-1.0f, -1.0f, 1000.0f, -1.0f);
	...
	case 4:	//	In game scheme - very far mode

		m_aResLoadDist[SCNRES_TREE]		= 500.0f;
		m_aResLoadDist[SCNRES_WATER]	= 1200.0f;
		m_aResLoadDist[SCNRES_BUILDING] = 450.0f;
		m_aResLoadDist[SCNRES_BOXAREA]	= 800.0f;
		m_aResLoadDist[SCNRES_GRASS]	= 70.0f;
		m_aResLoadDist[SCNRES_EFFECT]	= 200.0f;
		m_aResLoadDist[SCNRES_ECMODEL]	= 400.0f;
		...
		{
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(0, 800.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(1, 1000.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(2, 1200.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetViewScheme(3);
		}
		g_pGame->GetViewport()->GetA3DCamera()->SetProjectionParam(-1.0f, -1.0f, 1200.0f, -1.0f);

		m_fFogTimes = 1.2f;
		break;

	case 5:	//	In game scheme - extreme far mode

		m_aResLoadDist[SCNRES_TREE]		= 600.0f;
		m_aResLoadDist[SCNRES_WATER]	= 1400.0f;
		m_aResLoadDist[SCNRES_BUILDING] = 500.0f;
		m_aResLoadDist[SCNRES_BOXAREA]	= 900.0f;
		m_aResLoadDist[SCNRES_GRASS]	= 70.0f;
		m_aResLoadDist[SCNRES_EFFECT]	= 500.0f;
		m_aResLoadDist[SCNRES_ECMODEL]	= 400.0f;
		m_aResLoadDist[SCNRES_CRITTER]	= 300.0f;
		m_aResLoadDist[SCNRES_BEZIER]	= 900.0f;
		m_aResLoadDist[SCNRES_SOUND]	= 100.0f;
		...
		if( g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline() )
		{
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(0, 800.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(1, 1200.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetEndDist(2, 1400.0f);
			g_pGame->GetGameRun()->GetWorld()->GetTerrainOutline()->SetViewScheme(4);
		}

		g_pGame->GetViewport()->GetA3DCamera()->SetProjectionParam(-1.0f, -1.0f, 1400.0f, -1.0f);
		m_fFogTimes = 1.4f;
		break;
		*/
	}
