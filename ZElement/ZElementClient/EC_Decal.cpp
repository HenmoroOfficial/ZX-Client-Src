...
bool CECIconDecal::AddNumIcons(int iImageSlot, __int64 iNumber, A3DCOLOR col, bool bShowMinus)
{
	if (iNumber < 0)
	{
		bShowMinus = true;
...
bool  CECSpriteDecal::TransformPosToScreen(CECViewport* pViewport)
{
	A3DViewport* pA3DView = pViewport->GetA3DViewport();
	A3DVIEWPORTPARAM* pViewPara = pA3DView->GetParam();
	A3DMATRIX4 matVPS = pA3DView->GetCamera()->GetVPTM() * pA3DView->GetViewScale();
...
bool CECSpriteDecal::Render(CECViewport* pViewport)
{
	...
	if(!m_bScreenPos)
	{
		if(!TransformPosToScreen(pViewport))
			return true;
	}
...
//	m_pA2DSprite->SetColor(m_Color);
	...
	m_pA2DSprite->DrawToBack(m_ix, m_iy);
...
CECBubbleDecal::~CECBubbleDecal()
{
	...
	{
		m_pGfx->Stop();
		g_pGame->GetGFXCaster()->ReleaseGFXEx(m_pGfx);
		m_pGfx = NULL;
	}
}
...
bool CECBubbleDecal::Tick(DWORD dwDeltaTime)
{
	...
	if(m_pGfx)
	{
		if (m_pGfx->GetState() == ST_STOP
			|| m_pGfx->GetAlpha() < 1.0f)
		{
			m_pGfx->SetAlpha(1.0f);
			m_pGfx->Start(true);
		}
		m_pGfx->SetPos(GetPos());
...
void CECBubbleDecal::SetGfx(const char* strName)
{
    if(m_pGfx)
	{
	   m_pGfx->Stop();
    ...
    m_pGfx = g_pGame->GetGFXCaster()->LoadGFXEx(strName);
}
///////////////////////////////////////////////////////////////////////////
//	
...
CECBubbleDecal* CECBubbleDecalList::AddDecal(const A3DVECTOR3& vPos, int iDCID, bool bLastOne)
{
	...
	CECBubbleDecal* pDecal = pDecalMan->CreateBubbleDecal(iDCID);
	if (!pDecal)
		return NULL;
