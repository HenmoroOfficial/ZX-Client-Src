#ifndef __GNET_GET_ITEM_DESC_RESPONSE_HPP
#define __GNET_GET_ITEM_DESC_RESPONSE_HPP
...
namespace GNET
{
	...
	class GetItemDescResponse : public GNET::Protocol
	{
	public:
		byte		retCode;
		INT64		sn;
		...
		Octets		icon;
		Octets		detailInfo;
		enum { PROTOCOL_TYPE = PROTOCOL_GETITEMDESC_RE };//the type of the protocol.
	public:
		GetItemDescResponse() : retCode(0), sn(0), 
		timestamp(0), templateID(-1), level(-1)
		{
			type = PROTOCOL_GETITEMDESC_RE; 
		}
		GetItemDescResponse(void*) : Protocol(PROTOCOL_GETITEMDESC_RE),
		retCode(1), sn(0), timestamp(0), templateID(-1), level(-1){ }
		GetItemDescResponse( byte retcode, INT64 serialNum, INT64 timeStamp, int templateid, 
		...
		GetItemDescResponse(const GetItemDescResponse &rhs)
		...
		GNET::Protocol *Clone() const { return new GetItemDescResponse(*this); }
...
#endif //__GNET_GET_ITEM_DESC_RESPONSE_HPP
