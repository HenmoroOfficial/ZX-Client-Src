...
bool CConvexPolytope::Init(const CGiftWrap &gw)
{
	...
	for(int i=0;i<m_nOriginPatchNum+1;i++)
...
void CConvexPolytope::Render(A3DFlatCollector *pFC,A3DMATRIX4& tMatrix, DWORD dwColor,CPatch* pSpecialPatch)
{
	...
	//***************************************
	// Revised by wenfeng, 05-11-15
	// m_nCurVNum只是当前的有效顶点数量，即m_nCurVNum <= m_arrVertecies.GetSize()
	// 这就可能造成某些顶点未被添加到绘制顶点列表中
	// 用于渲染的顶点，要作一个变换
	//***************************************
	for(i=0;i<vNum /* m_nCurVNum */;i++)
	{
