...
{
	CCDBrush *pNewBrush = new CCDBrush();

	pNewBrush->m_aabb.Center = m_aabb.Center;
	pNewBrush->m_aabb.Extents = m_aabb.Extents;
	pNewBrush->m_aabb.Mins = m_aabb.Mins;
	pNewBrush->m_aabb.Maxs = m_aabb.Maxs;

	pNewBrush->m_dwReserved = m_dwReserved;
	pNewBrush->m_nSides = m_nSides;
	assert(m_nSides > 0);
	pNewBrush->m_pSides = new CCDSide[m_nSides];
	assert(pNewBrush->m_pSides);
	for (int i = 0; i < m_nSides; ++i)
	{
		pNewBrush->m_pSides[i].Init(m_pSides[i].plane.GetNormal(),
			m_pSides[i].plane.GetDist(),
			m_pSides[i].bevel);
	}
	return pNewBrush;
}
 ...
 *  @author: kuiwu
 *  @ref:   quake2 quick3 source code and  RayTraceBrush
 */
bool ClipBrush(BrushTraceInfo * pInfo, CCDSide * pSides, int nSides)
{
...
#ifdef BACK_VERSION
bool CQBrush::Trace(BrushTraceInfo * pInfo)
{
	if (!m_pCDBrush)
	{
		m_pCDBrush = new CCDBrush;
