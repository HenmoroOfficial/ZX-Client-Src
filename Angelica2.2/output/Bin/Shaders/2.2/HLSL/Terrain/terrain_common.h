...
struct VS_OUTPUT
{
	...
	float4 oDiffuse	: COLOR0;		// diffuse
	float4 oSpecular: COLOR1;		// specular
	float4 uv01		: TEXCOORD0;
	float4 uv23		: TEXCOORD1;
	float4 uv45		: TEXCOORD2;
	float2 uvMask	: TEXCOORD3;
	//Shadow
	float4	shadowProj					: TEXCOORD4;
...
void glb_vs_Common(VS_INPUT input, inout VS_OUTPUT Output)
{
	...
	float fHDotN = glb_CalcHDotN(input.Normal, -g_vecMainLightDir, vViewDir);
