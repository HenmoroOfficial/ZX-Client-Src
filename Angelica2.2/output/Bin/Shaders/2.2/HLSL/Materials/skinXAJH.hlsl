...
//SSS Warp Skin Color
	...
	colorToTune.a = 1.0f;
	colorToTune = mul(colorToTune, g_matColorTone);
	colSkinTex.rgb = colorToTune.rgb + colorDiff.rgb * g_matColorTone._14;	
#endif
...
//Rim Light
	float3 RimColor = g_RimColor.rgb;
#ifdef COLOR_TONE_ENABLE
	RimColor.rgb = (float3)mul(float4(RimColor, 1.0f), g_matColorTone);
#endif
#ifdef COLOR_MULTIPLY_ENABLE
	RimColor.rgb *= g_DestColorTone;
#endif

	RimLight.rgb = tex2D(g_RimLightTextureSampler, float2(fNdotV,0.0)).rgb*RimColor.rgb;
