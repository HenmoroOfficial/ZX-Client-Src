...
float4 g_fScaleFlags   : register(c3);
...
float4 ps_main(VS_OUTPUT Input): COLOR0
{
    ...
    if (g_fFlags.y && !g_fVertFlags.x)
    {
        ...
        //crLit = pow(crLit, 1 / g_fFlags.z);
        //crLit = crLit / max(0.00001, 1 - crLit);
        //crLit.rgb = crLit.rgb * crLit.a * 8;
        //float2 crRealLit;
        //crRealLit = tex2D(LightTable, float2(crLit.r, 0)).gr;
        //crLit.r = crRealLit.x * 8 + crRealLit.y;
        //crRealLit = tex2D(LightTable, float2(crLit.g, 0)).gr;
        //crLit.g = crRealLit.x * 8 + crRealLit.y;
        //crRealLit = tex2D(LightTable, float2(crLit.b, 0)).gr;
        //crLit.b = crRealLit.x * 8 + crRealLit.y;
        

        //crLit = - log(max(0.00001, 1 - crLit)) / 0.792;
        c.rgb *= crLit.rgb * g_fScaleFlags.x;
        //c = c / (1 + c);
    }
    if (g_fFlags.y && g_fVertFlags.x)
    {
