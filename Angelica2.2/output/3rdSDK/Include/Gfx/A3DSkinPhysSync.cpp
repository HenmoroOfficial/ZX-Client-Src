...
void A3DSkinPhysSync::InitAnimVertInfo()
{
	...
	for(int i=0; i<iClothNum; i++)
	{
		...
		pAvInfo->mSkinMesh = (A3DSkinMesh*)(m_pA3DSkin->GetSkinMeshRefByName(pAvInfo->mClothMesh->GetName())->pMesh);
...
bool A3DSkinPhysSync::InitAdhereMesh()
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		{
			AString strName = m_pA3DSkin->GetClothMeshImp(j)->GetName();
			if (strAdhereMesh == strName)
				pDstMesh = m_pA3DSkin->GetClothMeshImp(j);
			else if (strMeshName == strName)
				pSrcMesh = m_pA3DSkin->GetClothMeshImp(j);
		}
...
bool A3DSkinPhysSync::UpdateSkinByAnimation(int iDt, A3DSkinModel* pSkinModel, bool bUpdateNormals /*= true*/)
{
	...
	if(m_pCoreData)
	{
		...
		for(int i=0; i<iClothNum; i++)
		{
			...
			APhysXClothObject* pClothObject = m_pPhysSkin->GetClothObject(pClothMesh->GetName());
