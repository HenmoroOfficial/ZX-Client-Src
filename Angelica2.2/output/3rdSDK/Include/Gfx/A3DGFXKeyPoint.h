...
#define		ID_KPCTRL_JITTER_POSITION	10000
#define		ID_KPCTRL_JITTER_VELOCITY	10001
#define		ID_KPCTRL_JITTER_FACTOR		10002
...
struct PROC_DATA
{
	...
	m_fScaleNoise(0),
	m_vVelocity(0),
	m_bJitter(false) { m_KPProcess.Init(); }
};
...
// Jitter affector like the one in BigWorld. [3/21/2014 mage] 
class A3DGFXCtrlJitter : public A3DGFXKeyPointCtrlBase
{
public:
	A3DGFXCtrlJitter();
	virtual ~A3DGFXCtrlJitter() {}

private:
	bool	m_bJitterPos;
	bool	m_bJitterVelocity;
	float	m_fJitterFactor;
...
protected:
	...
	virtual A3DGFXKeyPointCtrlBase* Clone() const
	{
		A3DGFXCtrlJitter* p = new A3DGFXCtrlJitter;
		return &(*p = *this);
	}

	A3DGFXCtrlJitter& operator = (const A3DGFXCtrlJitter& src)
	{
		if (&src == this)
			return *this;
		...
		m_bJitterPos = src.m_bJitterPos;
		m_bJitterVelocity = src.m_bJitterVelocity;
		m_fJitterFactor = src.m_fJitterFactor;
...
protected:
	...
	typedef abase::vector<A3DVECTOR3> VecArray;
	VecArray		m_VertArray;
	BezierPt*		m_pBezier;
