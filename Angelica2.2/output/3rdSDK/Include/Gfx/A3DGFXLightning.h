...
protected:
	...
	GfxPerturbParam	m_perturbParam;
	float				m_fPastTime;
	A3DGFXCtrlCurveMove* m_pCurveCtrlForSpreadEffect;
	std::vector<float>	m_vecSegScale;
...
protected:
	...
	void	UpdatePath();
	void	_InitBufImpl(A3DGFXCtrlCurveMove* pCurveCtrl);
	void	_UpdateSpreadEffect(DWORD dwTickTime);
...
public:
	...
	virtual GfxPerturbParam* GetPerturbParam() { return &m_perturbParam; }
...
inline bool A3DGFXLightning::Play()
{
	...
	m_perturbParam.Reset();
	for (int i = 0; i <= m_nSegs; i++)
		m_perturbParam.AddSpreadDirection();
	...
	m_vecSegScale.clear();
	m_vecSegScale.resize(m_nVertPairNum);
	m_vecSegScale.assign(m_vecSegScale.size(), 1.0f);
