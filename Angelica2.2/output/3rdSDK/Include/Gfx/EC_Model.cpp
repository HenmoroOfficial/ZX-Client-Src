...
{
	DISABLE_COPY_AND_ASSIGNMENT(TrackBoneVelocity);
...
void CECModel::SetHideByHiddenHH(bool bSet, bool bAffectChild/* = true*/)
{
	bool bOldRealShow = m_bRealShown;
	...
	DWORD dwFlags/*=0*/, A3DSkinModelRenderModifier* pRenderModifier/* = NULL*/, bool bRenderGfx/* = true*/)
{
	...
	if (IsRenderSkinModel(bRenderSkinModel))
	{
	...
	DWORD dwBindFlag/* = 0*/)
{
	...
	{
		char log[1024];
		sprintf(log, "CECModel::AddChildModel child of szHangerName %s already exists!", szHangerName);
