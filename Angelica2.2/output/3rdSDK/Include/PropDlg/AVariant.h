...
public:
	...
	enum AVARTYPE
	{
		...
		AVT_APointF,		// 向量					m_avData.a2dvV;
		AVT_A3DVECTOR4,		// 向量					m_avData.a4dvV;
	};
...
//	AVariant(ASet * pSet);
	...
	DEFINEOPERATOR(APointF)
	DEFINEOPERATOR(A3DVECTOR3)
	DEFINEOPERATOR(A3DVECTOR4)
...
private:
	...
	union AVARIANT
	{
		...
		double				dV;			// 双精度浮点数
		AObject *			paV;		// 对象指针
		char				a2dvV[sizeof(APointF)];	// 向量2数据区
		char				a3dvV[sizeof(A3DVECTOR3)];	// 向量3数据区
		char				a4dvV[sizeof(A3DVECTOR4)];	// 向量4数据区
...
public:
	...
	{
		assert(lpsz);
		if (nLen < 0) nLen = (int)strlen(lpsz);
	...
	{
		assert(lpwsz);
		if (nLen < 0) nLen = (int)wcslen(lpwsz);
