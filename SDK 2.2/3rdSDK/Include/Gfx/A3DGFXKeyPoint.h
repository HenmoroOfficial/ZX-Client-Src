...
#define		ID_KPCTRL_MOVE_NOISE		113
...
struct PROC_DATA
{
	...
	A3DVECTOR3	m_vAccDir;			// 力场加速度方向
	bool		m_bHasForceField;	// 是否有力场作用
	...
	m_fScaleNoise(0),
	m_vAccDir(0),
	m_bHasForceField(false) { m_KPProcess.Init(); }
};
...
class A3DGFXCtrlMoveNoise : public A3DGFXCtrlNoiseBase
{
public:
	A3DGFXCtrlMoveNoise() 
	{ 
		m_nType		= ID_KPCTRL_MOVE_NOISE; 
	}
	virtual ~A3DGFXCtrlMoveNoise() {}
...
protected:
	...
	virtual A3DGFXKeyPointCtrlBase* Clone() const
	{
		A3DGFXCtrlMoveNoise* p = new A3DGFXCtrlMoveNoise;
		return &(*p = *this);
	}

	A3DGFXCtrlMoveNoise& operator = (const A3DGFXCtrlMoveNoise& src)
	{
		if (&src == this)
			return *this;
