...
public:
	...
	enum eTrailPerturbMode
	{
		...
		eTrailPerturbMode_Sine,
		eTrailPerturbMode_ComposeSine
	};
	...
	struct STrailPerturbParam 
	{
		STrailPerturbParam()
			...
			,m_fDisappearSpeed(0)
			,m_fSpreadSpeed(0)
			,m_fAmpAttenuFactor(1.0f)
			,m_fSpreadDelay(0)
			,m_iSegCntToShareDir(10)
			,m_nCurPerturbDirInc(m_iSegCntToShareDir)
			,m_fSpreadAccel(0)
			,m_vSpreadDirMinRange(0)
			,m_vSpreadDirMaxRange(0) {}

		eTrailPerturbMode m_iPerturbMode;
		A3DVECTOR3		m_vPerturbAxis;
		float			m_fPerturbFreq;
		float			m_fPerturbAmp;
		float			m_fPerturbFreq2;
		float			m_fPerturbAmp2;
		float			m_fTime;
		float			m_fTimeToLast;			// How long before it's disappeared, -1 means never 
		float			m_fDisappearSpeed;		// Control the disappear speed
		float			m_fSpreadSpeed;			// Control the spread speed
		float			m_fSpreadDelay;			// Delay time of spread
		float			m_fAmpAttenuFactor;		// Control attenuation ratio of the amplitude
		float			m_fSpreadAccel;			// Acceleration of spreading
		A3DVECTOR3		m_vSpreadDirMinRange;	// The direction of spreading's min range
		A3DVECTOR3		m_vSpreadDirMaxRange;	// The direction of spreading's max range
...
public:
	...
	STrailPerturbParam& GetPerturbParams() { return m_perturbParam; }
};
