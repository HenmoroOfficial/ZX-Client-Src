...
//*
	...
	output.PtLightDirection.x  = dot( blendTangent.xyz, vDeltaPtLight );
	output.PtLightDirection.y  = dot( blendBinormal.xyz, vDeltaPtLight );
	output.PtLightDirection.z  = dot( blendNormal.xyz, vDeltaPtLight );
	...
	//float fNDotL = dot(blendNormal.xyz, vDeltaPtLight);
	//float atten = 
	//output.diffuse = max(g_colPtDiffuse * fNDotL * atten, 0);
	//output.ambient = g_colPtAmbient;
	//output.diffuse = 0;
	//output.ambient = 0;
	float e = (d / g_vPtAtten.z) * g_vPtAtten.y;
	//float strength = g_vPtAtten.x / exp(e * e);
	output.PtLightDirection.w = g_vPtAtten.x / exp(e * e);
		// 1.0 / ( g_vPtAtten.x + g_vPtAtten.y * d + g_vPtAtten.z * d * d);
	output.uv = input.uv;
