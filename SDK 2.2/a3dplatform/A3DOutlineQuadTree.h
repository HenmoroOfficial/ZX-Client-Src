...
#ifndef   _A3D_OUTLINE_QUAD_TREE_H_
#define   _A3D_OUTLINE_QUAD_TREE_H_
...
public:
	A3DOutlineQuadTree();
	~A3DOutlineQuadTree();
	bool  Init(A3DDevice * pDevice, int iSubIndex, float wx, float wz, int iTexLevel);
	void  SetOwnerTex(A3DOutlineTexture * pELOTex);
	void  Release();
	...
	void ZPrePass(A3DCameraBase * pCamera);
...
private:
	...
	{
		A3DOutlineQuadNode * pBlkQNode;  ///< the block quad node 
	...
	void _StitchSingleBlkTop(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, 
					int nBlkEdge, int iBlkEdgeLen, int iLeft, int iTop,
					A3DLVERTEX * pVerts, WORD* pIndices);

	void _StitchSingleBlkBottom(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, 
					int nBlkEdge, int iBlkEdgeLen, int iLeft, int iTop,
					A3DLVERTEX * pVerts, WORD* pIndices);

	void _StitchSingleBlkRight(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, 
					int nBlkEdge, int iBlkEdgeLen, int iLeft, int iTop,
					A3DLVERTEX * pVerts, WORD* pIndices);
	void _StitchSingleBlkLeft(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, 
					int nBlkEdge, int iBlkEdgeLen, int iLeft, int iTop,
					A3DLVERTEX * pVerts, WORD* pIndices);

	void _AddSeam(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, 
					int nBlkEdge, int iBlkEdgeLen, int iEdge, int iLeft, int iTop, 
					A3DLVERTEX * pVerts, WORD* pIndices);
	void _AddSeam(A3DOutlineQuadNode * pQNode, const Seam_Block_Info& seamInfo,
					A3DLVERTEX * pVerts, WORD* pIndices);


	int  _GetSeamVertIndex(int ix, int iz, int iLeft, int iTop, const A3DVECTOR3& pos, A3DLVERTEX * pVerts);

	void _ExtractNodeEdge(A3DOutlineQuadNode * pNode);
	int   _ExtractNodeEdge(A3DOutlineQuadNode * pQNode, int EdgeFlag, A3DVECTOR3 * ppEdgeVert[]);
	void  _BuildStream();
	...
	void _StitchQNode(A3DOutlineQuadNode * pQNode, const Seam_Block_Info& seamInfo, A3DLVERTEX * pVerts, WORD * pIndices);
	...
	A3DOutlineQuadNode * _GetBlkQNode(float x, float z);
	A3DOutlineQuadNode * _GetBlkQNode(int ix, int iz);

	


	A3DOutlineTexture * m_pELOTexture;
    ...
    A3DLVERTEX *        m_pSeamVertRef;
    WORD *              m_pSeamIdxRef;
};



#endif // _A3D_OUTLINE_QUAD_TREE_H_
