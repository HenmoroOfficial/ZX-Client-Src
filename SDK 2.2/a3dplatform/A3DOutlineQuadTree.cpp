...
A3DOutlineQuadTree::A3DOutlineQuadTree()
{
	m_pELOTexture = NULL;
	m_pA3DDevice = NULL;
	m_RectWorld.Clear();
...
void A3DOutlineQuadTree::Release()
{
	if (m_pVertPos) {
		delete[] m_pVertPos;
		m_pVertPos = NULL;
    ...
    if (m_pSeamVertRef) {
        delete m_pSeamVertRef;
        m_pSeamVertRef = NULL;
    }

    if (m_pSeamIdxRef) {
        delete m_pSeamIdxRef;
        m_pSeamIdxRef = NULL;
    }

}

bool A3DOutlineQuadTree::Init(A3DDevice * pDevice , int iSubIndex, float wx, float wz, int iTexLevel)
{
	m_iSubIndex = iSubIndex;
	m_pA3DDevice = pDevice;
	float fw = (float)g_iOutlineTileWidth;
	float fh = (float)g_iOutlineTileHeight;
	int iw = (iSubIndex-1) % iTexLevel;
	int ih = (iSubIndex-1) / iTexLevel;  //level 0
	...
	m_Rect.left = iw * (g_iOutlineTileWidth / A3DOutlineTexture::GetTileGridLen());
	m_Rect.top  = ih * (g_iOutlineTileHeight / A3DOutlineTexture::GetTileGridLen());
	m_Rect.right = m_Rect.left + (g_iOutlineTileWidth / A3DOutlineTexture::GetTileGridLen());
	m_Rect.bottom = m_Rect.top + (g_iOutlineTileHeight / A3DOutlineTexture::GetTileGridLen());
	
	m_RectBlk.left = iw * (g_iOutlineTileWidth/ (A3DOutlineTexture::GetTileGridLen() * OUTLINE_BLK_LEN));
	m_RectBlk.top  = ih *(g_iOutlineTileHeight/ (A3DOutlineTexture::GetTileGridLen() * OUTLINE_BLK_LEN));
	m_RectBlk.right = m_RectBlk.left + (g_iOutlineTileWidth/ (A3DOutlineTexture::GetTileGridLen() * OUTLINE_BLK_LEN));
	m_RectBlk.bottom = m_RectBlk.top + (g_iOutlineTileHeight/ (A3DOutlineTexture::GetTileGridLen() * OUTLINE_BLK_LEN));
...
void A3DOutlineQuadTree::SetOwnerTex(A3DOutlineTexture * pELOTex)
{
	...
	int i;
	for (i =0; i < m_nVert; i++) {
        pVerts[i].x = m_pVertPos[i].x;
        pVerts[i].y = m_pVertPos[i].y;
        pVerts[i].z = m_pVertPos[i].z;
        pVerts[i].diffuse = OUTLINE_DIFFUSE_VIEW;
        pVerts[i].specular = OUTLINE_SPECULAR;
...
bool A3DOutlineQuadTree::Load(AString& szDir)
{
	Release();

	m_bLoadOk = false;
	char path[MAX_PATH];
	sprintf(path, _EA("%s%d.olm"), szDir, m_iSubIndex);
	...
	m_pQNodes = new A3DOutlineQuadNode[m_nQNodes];
	int i;
...
//		m_pQNodes[i].iActiveFlag = _head.iActiveFlag;
    ...
    m_pSeamVertRef = new A3DLVERTEX[m_nVert*2];
    m_pSeamIdxRef = new WORD[m_nMaxFace*3];
...
void A3DOutlineQuadTree::_BuildStream()
{
	if (!m_bStreamDirty) {
		return;
	}
...
void A3DOutlineQuadTree::CreateView(DWORD dwFrame, const ARectI& rcSeam, const ARectI& rcView)
{
	if ((!m_bLoadOk) ||(m_pELOTexture->m_dwFrameStamp != dwFrame) 
		|| (!A3DOutlineTextureCache::RcOverlapRc(m_Rect, rcView))){
		m_nFace = 0;
		return;
	}
	...
	for (int i =0; i < m_nQNodes; i++ ) {
		...
		bool bRender = A3DOutlineTextureCache::RcOverlapRc(rcNode, rcView);
	...
	if (A3DOutlineTextureCache::RcOverlapRc(rcSeam, m_Rect)) {
		//m_bStreamDirty = true;
		ARectI rc;
		//top edge
...
void A3DOutlineQuadTree::_SetQNodeRenderFlag(ARectI& rc, bool bRender)
{
	for (int i = 0; i < m_nQNodes; i++) {
 ...
 * \see 
 */
void A3DOutlineQuadTree::StitchAndCompleteTerrain(A3DTerrain2::ACTBLOCKS *pActBlocks, const ARectI& rcVisible,  A3DTerrain2LOD::GRID * pTerGrids)
{
	...
	if (!m_bLoadOk || !A3DOutlineTextureCache::RcOverlapRc(rcSeamBlk, m_RectBlk)) {
		return;
	}
    A3DLVERTEX * pVerts = m_pSeamVertRef;
    WORD * pIndices = m_pSeamIdxRef;
    ...
    if (!m_pSeamStream->LockVertexBufferDynamic(0, 0, (BYTE**)&pVerts, 0)) {
        return ;
    }
    memcpy(pVerts, m_pSeamVertRef, sizeof(A3DLVERTEX) * m_nVert * 2);
    m_pSeamStream->UnlockVertexBufferDynamic();

    if (!m_pSeamStream->LockIndexBufferDynamic(0,0, (BYTE**)&pIndices, 0)) {
        return ;
    }
    memcpy(pIndices, m_pSeamIdxRef, sizeof(WORD) * m_nMaxFace * 3);

    m_pSeamStream->UnlockIndexBufferDynamic();
	//////////////////////////////////////////////////////////////////////////
	//debug
//	char msg[100];
//	sprintf(msg, _EA("stream max vert %d , face %d  real vert %d face %d\n"), m_nVert, m_nMaxFace, m_nSeamVert, m_nSeamFace);
...
void A3DOutlineQuadTree::Render(A3DCameraBase * pCamera)
{
	...
	if ((m_nSeamFace > 0) && (m_pSeamStream != NULL)) {
		m_pSeamStream->Appear(0, false);
		m_pA3DDevice->DrawIndexedPrimitive(A3DPT_TRIANGLELIST, 0, m_nSeamVert, 0, m_nSeamFace);
	}

}
void A3DOutlineQuadTree::ZPrePass(A3DCameraBase * pCamera)
{
	if((!m_bLoadOk)|| (!pCamera->AABBInViewFrustum(m_vBBMins, m_vBBMaxs))) {
		return;
	}
...
void A3DOutlineQuadTree::_StitchSingleBlkTop(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, int nBlkEdge, int iBlkEdgeLen,  
...
#undef  SEAM
	if (!pNode->IsLeaf()) {
		for (int i= 0; i< 4; i++) {
			A3DOutlineQuadNode * pChild = &m_pQNodes[pNode->iChildren[i]];
...
void A3DOutlineQuadTree::_StitchSingleBlkBottom(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, int nBlkEdge, int iBlkEdgeLen,  
											 int iLeft, int iTop, A3DLVERTEX * pVerts, WORD* pIndices)
{
	int iBlkBottom = iTop ;
...
void A3DOutlineQuadTree::_StitchSingleBlkRight(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, int nBlkEdge, int iBlkEdgeLen,  
											 int iLeft, int iTop, A3DLVERTEX * pVerts, WORD* pIndices)
{
	int iBlkRight = iLeft ;
...
void A3DOutlineQuadTree::_StitchSingleBlkLeft(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, int nBlkEdge, int iBlkEdgeLen,  
											 int iLeft, int iTop, A3DLVERTEX * pVerts, WORD* pIndices)
{
...
void A3DOutlineQuadTree::_ExtractNodeEdge(A3DOutlineQuadNode * pNode)
{
	...
	for (i= 0; i < 4; i++ ) {
		...
		pNode->pEdgeVert[i] = new WORD[pNode->iLen / A3DOutlineTexture::GetTileGridLen() + 2];
	}
	
	A3DOutlineQuadNode * pChild;
	WORD * pEdgeVert;
	int    nEdgeVert;
	WORD  * pDest;
	int   * pDestCount;
 ...
 * \see 
 */
int A3DOutlineQuadTree::_GetSeamVertIndex(int ix, int iz, int iLeft, int iTop, const A3DVECTOR3& pos, A3DLVERTEX * pVerts)
{
	int i, j;
 ...
 */
void A3DOutlineQuadTree::_AddSeam(A3DOutlineQuadNode * pNode, A3DVECTOR3* pBlkEdge, int nBlkEdge, int iBlkEdgeLen, int iEdge, int iLeft, int iTop, A3DLVERTEX * pVerts, WORD* pIndices)
{
	...
	if ((iEdge == OLQ_EDGE_BOTTOM) && (pNode->iz == iTop )) {
		...
		for (i = 1; i < nNodeEdge; i++) {
			ix = (int)((m_pVertPos[pNodeEdge[i]].x - fLeft) / A3DOutlineTexture::GetTileGridLen() + 0.5);
			ix += pNode->ix;
	...
	if ((iEdge == OLQ_EDGE_LEFT) && (pNode->ix + pNode->iLen == iLeft + OUTLINE_BLK_LEN)) {
		...
		for (i = 1; i < nNodeEdge; i++) {
			iz = (int)((fTop - m_pVertPos[pNodeEdge[i]].z ) / A3DOutlineTexture::GetTileGridLen() + 0.5);
			iz += pNode->iz;
			v2 = _GetSeamVertIndex(ix, iz, iLeft, iTop, m_pVertPos[pNodeEdge[i]], pVerts);
			pIndices[m_nSeamFace*3] = v0;
			pIndices[m_nSeamFace*3+1] = v1;
...
void A3DOutlineQuadTree::_StitchTop(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	...
	for (int i = iStart; i < iEnd; i++) {
		A3DOutlineQuadNode * pQNode = _GetBlkQNode(iCurGrid, tlGrid);
...
void A3DOutlineQuadTree::_StitchBottom(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	int tlGrid = tblr * OUTLINE_BLK_LEN;
	int iCurGrid = iStart * OUTLINE_BLK_LEN;
	A3DTerrain2Block * pBlock;
...
void A3DOutlineQuadTree::_StitchRight(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	int tlGrid = tblr * OUTLINE_BLK_LEN;
	int iCurGrid = iStart * OUTLINE_BLK_LEN;
	A3DTerrain2Block * pBlock;
	...
	for (int i = iStart; i < iEnd; i++) {
		A3DOutlineQuadNode * pQNode = _GetBlkQNode(tlGrid, iCurGrid);
...
void A3DOutlineQuadTree::_StitchLeft(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	int tlGrid = tblr * OUTLINE_BLK_LEN;
	int iCurGrid = iStart * OUTLINE_BLK_LEN;
	A3DTerrain2Block * pBlock;
...
void A3DOutlineQuadTree::_Sticth(A3DTerrain2::ACTBLOCKS *pActBlocks, const ARectI& rcVisible, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	ARectI rcSeamBlk(rcVisible);
	rcSeamBlk.Inflate(1,1);
//   overlapping has been already checked.  By kuiwu. [14/2/2006]
//	if (!A3DOutlineTextureCache::RcOverlapRc(rcSeamBlk, m_RectBlk)) {
//		return;
//	}
...
void A3DOutlineQuadTree::_CompleteTerrain(A3DTerrain2::ACTBLOCKS *pActBlocks, const ARectI& rcVisible, A3DTerrain2LOD::GRID * pTerGrids, A3DLVERTEX*pVerts, WORD*pIndices)
{
	if (!A3DOutlineTextureCache::RcOverlapRc(m_RectBlk, rcVisible)) {
		return;
	}
...
float A3DOutlineQuadTree::_GetHeight(float x, float z)
{
	float y = -1.0f;
...
A3DOutlineQuadNode * A3DOutlineQuadTree::_GetBlkQNode(float x, float z)
{
	//world -> grid
	int ix, iz;
	ix = m_Rect.left + (int)(x - m_RectWorld.left + 0.1) / A3DOutlineTexture::GetTileGridLen();
	iz = m_Rect.top  + (int)(m_RectWorld.top - z + 0.1) / A3DOutlineTexture::GetTileGridLen();

	return _GetBlkQNode(ix, iz);
}

A3DOutlineQuadNode * A3DOutlineQuadTree::_GetBlkQNode(int ix, int iz)
{
	A3DOutlineQuadNode * pQNode = NULL;
	for (int i = 0; i < m_nQNodes; i++) {
 ...
 * \see 
 */
int A3DOutlineQuadTree::_ExtractBlkEdge(A3DTerrain2Block * pBlock, A3DTerrain2LOD::GRID * pTerGrids,int edge, int& nVert, A3DVECTOR3 * pVerts)
{
...
void A3DOutlineQuadTree::_CompleteTerrain(int r, int c, A3DTerrain2::ACTBLOCKS *pActBlocks, 
										  ...
										  A3DLVERTEX*pVerts, WORD*pIndices)
{
		...
		A3DOutlineQuadNode * pQNode = _GetBlkQNode(ix, iz);
...
//		char msg[100] ;
		...
		//////////////////////////////////////////////////////////////////////////
		//debug
//		sprintf(msg, _EA("fake face %d\n"), m_nSeamFace - facebegin);
//		OutputDebugStringA(msg);
//		for (i = facebegin; i < m_nSeamFace; i++) {
//			v0 = pIndices[i*3];
//			v1 = pIndices[i*3+1];
//			v2 = pIndices[i*3+2];
//			sprintf (msg, _EA("%d %3.1f, %3.1f %3.1f %d %3.1f, %3.1f %3.1f 	%d %3.1f, %3.1f %3.1f\n"), 
 ...
 * \see 
 */
 int A3DOutlineQuadTree::_ExtractNodeEdge(A3DOutlineQuadNode * pQNode, int EdgeFlag, A3DVECTOR3 * ppEdgeVert[])
 {
...
void A3DOutlineQuadTree::_CompleteTerrain2(int r, int c, A3DTerrain2::ACTBLOCKS *pActBlocks, 
 ...
 * \see 
 */
void A3DOutlineQuadTree::_StitchBlk(const Seam_Block_Info& seamInfo, A3DLVERTEX * pVerts, WORD* pIndices)
{
 ...
 * \see 
 */
void A3DOutlineQuadTree::_StitchQNode(A3DOutlineQuadNode * pQNode, const Seam_Block_Info& seamInfo, A3DLVERTEX * pVerts, WORD * pIndices)
{
 ...
 * \see 
 */
void A3DOutlineQuadTree::_AddSeam(A3DOutlineQuadNode * pQNode, const Seam_Block_Info& seamInfo, A3DLVERTEX * pVerts, WORD* pIndices)
{
	...
	while (pPos != NULL)
	{
		ix = (int)((pPos->x - fLeft) / A3DOutlineTexture::GetTileGridLen() + 0.5);
		ix += pQNode->ix;
	...
	while (pPos != NULL)
	{
		iz = (int)((fTop - pPos->z ) / A3DOutlineTexture::GetTileGridLen() + 0.5);
		iz += pQNode->iz;
		v2= _GetSeamVertIndex(ix, iz, seamInfo.pBlkQNode->ix, seamInfo.pBlkQNode->iz, *pPos, pVerts);
		pIndices[m_nSeamFace*3] = v0;
		pIndices[m_nSeamFace*3+1] = v1;
 ...
 * \see 
 */
void A3DOutlineQuadTree::_StitchTop2(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	int tlGrid = tblr * OUTLINE_BLK_LEN;
	int iCurGrid = iStart * OUTLINE_BLK_LEN;
	A3DTerrain2Block * pBlock;
 ...
 * \see 
 */
void A3DOutlineQuadTree::_StitchBottom2(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	int tlGrid = tblr * OUTLINE_BLK_LEN;
	int iCurGrid = iStart * OUTLINE_BLK_LEN;
	A3DTerrain2Block * pBlock;
 ...
 * \see 
 */
void A3DOutlineQuadTree::_StitchLeft2(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	int tlGrid = tblr * OUTLINE_BLK_LEN;
	int iCurGrid = iStart * OUTLINE_BLK_LEN;
	A3DTerrain2Block * pBlock;
 ...
 * \see 
 */
void A3DOutlineQuadTree::_StitchRight2(int tblr, int iStart, int iEnd, A3DTerrain2::ACTBLOCKS *pActBlocks, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	int tlGrid = tblr * OUTLINE_BLK_LEN;
	int iCurGrid = iStart * OUTLINE_BLK_LEN;
	A3DTerrain2Block * pBlock;
 ...
 * \see 
 */
void A3DOutlineQuadTree::_Sticth2(A3DTerrain2::ACTBLOCKS *pActBlocks, const ARectI& rcVisible, A3DTerrain2LOD::GRID * pTerGrid, A3DLVERTEX*pVerts, WORD*pIndices)
{
	ARectI rcSeamBlk(rcVisible);
	rcSeamBlk.Inflate(1,1);
//   overlapping has been already checked.  By kuiwu. [14/2/2006]
