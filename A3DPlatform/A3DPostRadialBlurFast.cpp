/*
 * FILE: A3DPostRadialBlurFast.cpp
 *
...
A3DPostRadialBlurFast::~A3DPostRadialBlurFast()
{
}

bool A3DPostRadialBlurFast::Init( A3DPostSystem* pSystem )
{
	m_pSystem = pSystem;

	const char* szMacro[1];
	szMacro[0] = "_FAST_";
	m_pHLSLRB = CreateA3DHLSL_Common(NULL, NULL, szPost_RadialBlur_hlsl, "ps_main", 1, szMacro,
...
void A3DPostRadialBlurFast::Release()
{
...
bool A3DPostRadialBlurFast::IsHardwareSupport(A3DDevice* pA3DDev)
{
...
bool A3DPostRadialBlurFast::Render( A3DViewport* pViewport )
{
...
void A3DPostRadialBlurFast::SetBlurCenterScreenSpace( float x, float y ) /* (0.5, 0.5)是屏?中间, (0,0)是左上角, (1,1)是右下角 */
{
	m_vBlurCenter.x = x;
	m_vBlurCenter.y = y;
}

void A3DPostRadialBlurFast::GetBlurCenterScreenSpace( float* pX, float* pY )
{
	*pX = m_vBlurCenter.x;
	*pY = m_vBlurCenter.y;
}

void A3DPostRadialBlurFast::SetBlurLength( float fBlurLength )
{
	fBlurLength = max(0.f, fBlurLength);
	m_fBlurLength = fBlurLength;
}

float A3DPostRadialBlurFast::GetBlurLength()
{
	return m_fBlurLength;
}

void A3DPostRadialBlurFast::SetFocusRadius( float fInFocusRadius, float fOutFocusRadius )
{
...
void A3DPostRadialBlurFast::GetFocusRadius( float* pfInFocusRadius, float* pfOutFocusRadius )
{
	*pfInFocusRadius = m_fInFocusRadius;
	*pfOutFocusRadius = m_fOutFocusRadius;
}

void A3DPostRadialBlurFast::SetBlurCenterWorldSpace( const A3DVECTOR3& vWorldPos, A3DViewport* pViewPort )
{
...
A3DPOST_EFFECT A3DPostRadialBlurFast::GetEffectType() const
{
	return A3DPOST_EFFECT_RadialBlurFast;
}

A3DRenderTarget* A3DPostRadialBlurFast::ApplyEffect( A3DViewport* pViewport, A3DRenderTarget* pSrcRT )
{
    A3DDevice* pDev = m_pSystem->GetA3DDevice();
