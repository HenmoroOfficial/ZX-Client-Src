...
#ifndef   _A3D_TERRAINOULINE2_H_
#define   _A3D_TERRAINOULINE2_H_
...
{
	friend class A3DOutlineTexture;
...
private:
    ...
    int                 m_OutlineTexLevel[OUTLINE_MAX_LEVEL*2];
	
	A3DOutlineTextureCache * m_pOutlineTexCache;
    ...
    A3DHLSL*                m_pHLSLOutline;
    A3DHLSL*                m_pHLSLReplace;
	A3DHLSL*				m_pHLSLReplaceShadow;
...
private:
    ...
    void _RenderReplaceTerrainHLSL(A3DViewport* pViewport);
...
public:
	 ...
	 *  @author kuiwu
	 */
	bool Init(A3DDevice * pA3DDevice, const char * szTexDir, const char * szTileDir, const char * szDetailTex,
		A3DTerrain2 * pTerrain2, unsigned int nWTileCount = 8, unsigned int nHTileCount = 11, int nTileGridLen = 1);
	void Release();
	void Render2(A3DViewport * pViewport, bool bShadowMap);
	void ZPrePass(A3DViewport * pViewport);
