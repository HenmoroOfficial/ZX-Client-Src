...
{
	if (m_pSrcMesh->GetMeshBase()->GetClassID() == A3D_CID_SKINMESH)
	{
		return ((A3DSkinMesh*)(pClothMesh->GetMeshBase()))->GetVertexNum();
	}
	else if (m_pSrcMesh->GetMeshBase()->GetClassID() == A3D_CID_RIGIDMESH)
	{
		return ((A3DRigidMesh*)(pClothMesh->GetMeshBase()))->GetVertexNum();
	}
...
{
	if (pClothMesh->GetMeshBase()->GetClassID() == A3D_CID_SKINMESH)
	{
		return ((A3DSkinMesh*)(m_pSrcMesh->GetMeshBase()))->GetIndexNum();
	}
	else if (pClothMesh->GetMeshBase()->GetClassID() == A3D_CID_RIGIDMESH)
	{
		return ((A3DRigidMesh*)(m_pSrcMesh->GetMeshBase()))->GetIndexNum();
	}
...
void A3DAdhereMesh::GetClothMeshOriVertsAndIdxs(A3DClothMeshImp* pClothMesh, abase::vector<A3DVECTOR3>& vs, abase::vector<DWORD>& Idxs)
{
	...
	if (pClothMesh->GetMeshBase()->GetClassID() == A3D_CID_SKINMESH)
	{
		A3DSkinMesh* pSkinMesh = (A3DSkinMesh*)(pClothMesh->GetMeshBase());
	...
	{
		A3DRigidMesh* pRigidMesh = (A3DRigidMesh*)(pClothMesh->GetMeshBase());
...
{
	A3DSkinMesh*  pSkinMesh = (A3DSkinMesh*)m_pSrcMesh->GetMeshBase();
...
void A3DAdhereMesh::SyncSrcMesh()
{
	...
	RIGID_VERTEX* pVert = m_pDstMesh->GetVertexBuf();
	...
	for (i = 0; i < nSrcVertNum; i++)
	{
		...
		RIGID_VERTEX* vertex = &(pVert[index[3*iTri] ]);
	...
	int nIndex = 0;
	index = ((A3DSkinMesh*)m_pSrcMesh->GetMeshBase())->GetOriginIndexBuf();//m_pSrcMesh->GetIndexBuf();	
	nIndex = ((A3DSkinMesh*)m_pSrcMesh->GetMeshBase())->GetIndexNum();//GetClothMeshIdxNum(m_pSrcMesh);	
